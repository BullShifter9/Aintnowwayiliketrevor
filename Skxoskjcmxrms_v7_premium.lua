...
-- ================================================================
-- PHANTOM ULTRA ENGINE — Premium Silent Aim v1.0
--
-- Improvements over the standard Fusion Engine (solveIntercept):
--
-- [1] 3-State Kalman per axis (pos + vel + ACCEL)
--     Standard: predicted = pos + vel·t
--     Premium:  predicted = pos + vel·t + ½·accel·t²
--     At 180ms ping with 20 studs/s² lateral accel → +0.32 studs
--     of correction the standard engine ignores completely.
--
-- [2] Ping percentile tracker — 2 Hz, 20-sample ring buffer,
--     75th-percentile + 33ms server-tick buffer.
--     Eliminates misses caused by ping spikes on the sample.
--
-- [3] Adaptive input bias factor — scales from 0.35 at 60ms
--     to 0.48 at 180ms so shots lead the target correctly
--     at any latency.
--
-- [4] Movement-state classifier — still / straight / strafing /
--     airborne. Selects optimal aim-point height per state:
--     head when still, torso-center when jumping, upper-chest
--     otherwise.  Wider Y-clamp (±4.0) for airborne targets.
--
-- [5] 4-frame burst with fresh prediction every frame — frame 0
--     fires synchronously before equip latency, frames 1-4 each
--     recompute solvePremiumIntercept against the live position.
--
-- [6] Combined mode — one toggle drives BOTH the floating button
--     (Silent Aim) AND left-click fire (SharpShooter).
-- ================================================================

-- ── Premium ping percentile tracker ───────────────────────────────
local _premPingBuf    = {}
local _premPingBufMax = 20
local _premPingBufPtr = 0
local _premPingP75    = 0.060   -- starts at 60ms, updated at 2Hz

task.spawn(function()
    while true do
        pcall(function()
            local ms = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
            ms = math.clamp(ms, 5, 500)
            _premPingBufPtr = (_premPingBufPtr % _premPingBufMax) + 1
            _premPingBuf[_premPingBufPtr] = ms
            if #_premPingBuf >= 4 then
                local s = {}
                for i, v in ipairs(_premPingBuf) do s[i] = v end
                table.sort(s)
                _premPingP75 = s[math.ceil(#s * 0.75)] / 1000
            end
        end)
        task.wait(0.5)
    end
...
local PREM_SERVER_TICK = 0.033  -- ~1 server tick at 30Hz

local function getPremPing()
    if predictionState.pingEnabled then
        return math.clamp(predictionState.pingValue, 5, 300) / 1000
    end
    -- Take whichever estimate is higher for safety, then add server tick
    return math.max(_premPingP75, _ping) + PREM_SERVER_TICK
end

-- ── 3-State Kalman filter per scalar axis ─────────────────────────
-- State vector: [p, v, a] — position, velocity, acceleration.
-- Transition (constant-acceleration model):
--   p' = p + v·dt + ½a·dt²
--   v' = v + a·dt
--   a' = a
-- Covariance stored as upper-triangle: Pp, Ppv, Ppa, Pvv, Pva, Paa

local PKQ_P, PKQ_V, PKQ_A = 0.002, 0.10, 4.0  -- process noise
local PKR_P, PKR_V        = 0.10,  0.55        -- measurement noise

local function pk3New(p, v)
    return { p=p, v=v, a=0,
             Pp=1, Ppv=0, Ppa=0, Pvv=1, Pva=0, Paa=1 }
end

local function pk3Step(k, mp, mv, dt)
    if dt <= 0 or dt > 0.5 then return end
    local dt2 = dt * dt

    -- ── Predict step ───────────────────────────────────────────
    local pp = k.p + k.v*dt + 0.5*k.a*dt2
    local vp = k.v + k.a*dt
    local ap = k.a

    -- Propagate covariance: P_pred = F·P·Fᵀ + Q
    -- F = [[1, dt, ½dt²], [0, 1, dt], [0, 0, 1]]
    -- F·P rows:
    local FP00 = k.Pp  + dt*k.Ppv + 0.5*dt2*k.Ppa
    local FP01 = k.Ppv + dt*k.Pvv + 0.5*dt2*k.Pva
    local FP02 = k.Ppa + dt*k.Pva + 0.5*dt2*k.Paa
    local FP11 = k.Pvv + dt*k.Pva       -- [0,1,dt]·P col 1
    local FP12 = k.Pva + dt*k.Paa       -- [0,1,dt]·P col 2

    -- (F·P)·Fᵀ elements (only upper-triangle needed, matrix is symmetric):
    local Pp00 = FP00 + FP01*dt + FP02*0.5*dt2  + PKQ_P
    local Pp01 = FP01 + FP02*dt
    local Pp02 = FP02
    local Pp11 = FP11 + FP12*dt                  + PKQ_V
    local Pp12 = FP12
    local Pp22 = k.Paa                            + PKQ_A

    -- ── Update step ────────────────────────────────────────────
    local yp = mp - pp           -- position innovation
    local yv = mv - vp           -- velocity innovation

    -- S = H·P_pred·Hᵀ + R  (H selects rows 0 and 1)
    local S00 = Pp00 + PKR_P
    local S01 = Pp01
    local S11 = Pp11 + PKR_V
    local det = S00*S11 - S01*S01
    if math.abs(det) < 1e-10 then
        -- Singular — skip update, keep predicted state
        k.p,k.v,k.a = pp,vp,ap
        k.Pp,k.Ppv,k.Ppa = Pp00,Pp01,Pp02
        k.Pvv,k.Pva,k.Paa = Pp11,Pp12,Pp22
        return
    end
    local id = 1 / det

    -- Kalman gain K (3×2):
    -- K[i][0] = (PH[i][0]·S11 - PH[i][1]·S01) / det
    -- K[i][1] = (-PH[i][0]·S01 + PH[i][1]·S00) / det
    -- PH cols: [Pp00,Pp01,Pp02] and [Pp01,Pp11,Pp12]
    local K00 = (Pp00*S11 - Pp01*S01)*id
    local K01 = (-Pp00*S01 + Pp01*S00)*id
    local K10 = (Pp01*S11 - Pp11*S01)*id
    local K11 = (-Pp01*S01 + Pp11*S00)*id
    local K20 = (Pp02*S11 - Pp12*S01)*id
    local K21 = (-Pp02*S01 + Pp12*S00)*id

    -- State update
    k.p = pp + K00*yp + K01*yv
    k.v = vp + K10*yp + K11*yv
    k.a = ap + K20*yp + K21*yv

    -- Covariance update: P = (I − K·H)·P_pred
    -- P[i][j] = P_pred[i][j] - K[i][0]·P_pred[0][j] - K[i][1]·P_pred[1][j]
    k.Pp  = Pp00 - K00*Pp00 - K01*Pp01
    k.Ppv = Pp01 - K00*Pp01 - K01*Pp11
    k.Ppa = Pp02 - K00*Pp02 - K01*Pp12
    k.Pvv = Pp11 - K10*Pp01 - K11*Pp11
    k.Pva = Pp12 - K10*Pp02 - K11*Pp12
    k.Paa = Pp22 - K20*Pp02 - K21*Pp12
end

-- ── Premium tracker pool ───────────────────────────────────────────
local premTrackers = {}

local function getPremTracker(player)
    if premTrackers[player] then return premTrackers[player] end
    local char = player.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    local pos  = root and root.Position or Vector3.new()
    local vel  = root and root.AssemblyLinearVelocity or Vector3.new()
    premTrackers[player] = {
        kx = pk3New(pos.X, vel.X),
        ky = pk3New(pos.Y, vel.Y),
        kz = pk3New(pos.Z, vel.Z),
        lastT = tick(),
    }
    return premTrackers[player]
end

local function premUpdate(player)
    local char = player.Character; if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart"); if not root then return end
    local t    = getPremTracker(player)
    local now  = tick()
    local dt   = now - t.lastT
    if dt <= 0 or dt > 0.5 then t.lastT = now; return end
    local pos  = root.Position
    local vel  = root.AssemblyLinearVelocity
    pk3Step(t.kx, pos.X, vel.X, dt)
    pk3Step(t.ky, pos.Y, vel.Y, dt)
    pk3Step(t.kz, pos.Z, vel.Z, dt)
    t.lastT = now
end

-- Returns Kalman-smoothed: position, velocity, acceleration (Vector3 each)
local function getPremState(player)
    local t = premTrackers[player]
    if not t then
        local char = player.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local pos  = root and root.Position or Vector3.new()
        local vel  = root and root.AssemblyLinearVelocity or Vector3.new()
        return pos, vel, Vector3.new()
    end
    return
        Vector3.new(t.kx.p, t.ky.p, t.kz.p),
        Vector3.new(t.kx.v, t.ky.v, t.kz.v),
        Vector3.new(t.kx.a, t.ky.a, t.kz.a)
end

-- ── Movement-state classifier ─────────────────────────────────────
-- Returns one of: "still" | "straight" | "strafing" | "airborne"
local function classifyMovement(vel, accel)
    local horizSpd  = Vector3.new(vel.X, 0, vel.Z).Magnitude
    local vertSpd   = math.abs(vel.Y)
    local horizAccel = Vector3.new(accel.X, 0, accel.Z).Magnitude
    if vertSpd > 8 then          return "airborne"
    elseif horizSpd < 1.5 then   return "still"
    elseif horizAccel > 28 then  return "strafing"
    else                         return "straight"
    end
end

-- ── Premium intercept solver ───────────────────────────────────────
local function solvePremiumIntercept(player)
    local char = player.Character; if not char then return nil end
    local root = char:FindFirstChild("HumanoidRootPart"); if not root then return nil end
    local hum  = char:FindFirstChild("Humanoid"); if not hum then return nil end

    premUpdate(player)  -- ensure tracker is fresh this frame

    local pos, vel, accel = getPremState(player)
    local t = getPremPing()    -- seconds (percentile-based + server tick)

    -- ── [1] Kinematic prediction — core improvement ──────────────
    --   Standard: offset = vel·t           (first-order)
    --   Premium:  offset = vel·t + ½·a·t²  (second-order)
    local kinematic = pos + vel*t + accel*(0.5*t*t)

    -- ── [2] Adaptive input bias ───────────────────────────────────
    --   Scales from 0.35@60ms → 0.48@180ms.
    --   At higher ping the server sees more of the player's input-driven
    --   movement, so MoveDirection contributes a larger displacement.
    local pingMs     = t * 1000
    local biasFactor = 0.35 + 0.13 * math.clamp((pingMs - 60) / 120, 0, 1)
    local inputBias  = hum.MoveDirection * (hum.WalkSpeed * t * biasFactor)

    -- ── [3] Seismic / overflow micro-correction (reduced weight) ──
    --   The kinematic term handles the bulk of the offset; these only
    --   cancel sub-frame jitter in AssemblyLinearVelocity.
    local smoothVelStd = getSmoothedVel(player)   -- re-use standard Kalman vel
    local pSE = seismicPredict(root, smoothVelStd)
    local pOV = overflowPredict(root, hum, smoothVelStd)
    local mc  = Vector3.new(
        (pOV.X - root.Position.X)*0.08 + (pSE.X - root.Position.X)*0.06,
        (pOV.Y - root.Position.Y)*0.08 + (pSE.Y - root.Position.Y)*0.06,
        (pOV.Z - root.Position.Z)*0.08 + (pSE.Z - root.Position.Z)*0.06
    )

    -- ── [4] Combine all terms ──────────────────────────────────────
    local rawOffset = (kinematic - root.Position) + inputBias + mc

    -- ── [5] Movement-aware aim height & Y clamp ───────────────────
    local moveState = classifyMovement(vel, accel)
    local headY, yClamp
    if moveState == "still" then
        headY = 2.6;  yClamp = 2.0  -- aim head, tight clamp (not moving)
    elseif moveState == "airborne" then
        headY = 0.6;  yClamp = 4.5  -- aim torso-center, wide clamp (vertical uncertainty)
    elseif moveState == "strafing" then
        headY = 1.8;  yClamp = 3.0  -- upper-chest (horizontal uncertainty)
    else -- straight
        headY = 2.1;  yClamp = 3.5  -- upper-chest/neck
    end

    local clampedY  = math.clamp(rawOffset.Y, -yClamp, yClamp)
    local predicted = root.Position + Vector3.new(rawOffset.X, clampedY, rawOffset.Z)
    predicted       = Vector3.new(predicted.X, predicted.Y + headY, predicted.Z)

    -- ── [6] Floor / ceiling safety raycast ────────────────────────
    local rp = RaycastParams.new()
    rp.FilterType = Enum.RaycastFilterType.Blacklist
    rp.FilterDescendantsInstances = {char}
    local fR = workspace:Raycast(predicted, Vector3.new(0, -8, 0), rp)
    local cR = workspace:Raycast(predicted, Vector3.new(0,  8, 0), rp)
    if fR then predicted = Vector3.new(predicted.X, math.max(predicted.Y, fR.Position.Y + 1.2), predicted.Z) end
    if cR then predicted = Vector3.new(predicted.X, math.min(predicted.Y, cR.Position.Y - 0.3), predicted.Z) end

    return predicted
end

-- ── Smart shoot dispatcher ────────────────────────────────────────
-- When Premium is enabled this replaces BOTH Silent Aim and SharpShooter.
-- premiumEnabled is declared in the Premium tab UI section below.
local premiumEnabled = false

local function doSmartShoot(targetPlayer)
    if premiumEnabled then
        doPremiumShoot(targetPlayer)
    else
        doFusionShoot(targetPlayer)
    end
end

-- ── Premium fire: 4-frame burst, per-frame prediction refresh ─────
function doPremiumShoot(targetPlayer)
    ...
    -- Pre-compute BEFORE equip — snapshot before any equip latency
    ...
    -- Frame 0: synchronous fire (best timing, pre-computed intercept)
    pcall(function() shootRemote:FireServer(getArg1(), CFrame.new(predicted)) end)

    -- Frames 1-4: burst, refreshing prediction each frame
    local burst = 0
    local burstConn
    burstConn = RunService.Heartbeat:Connect(function()
        burst = burst + 1
        if burst > 4 then burstConn:Disconnect(); return end

        local freshPredicted = solvePremiumIntercept(targetPlayer)
                            or (mHead and mHead.Position)
                            or mRoot.Position

        pcall(function() shootRemote:FireServer(getArg1(), CFrame.new(freshPredicted)) end)
    end)
end

-- Keep ALL player premium trackers warm every frame
RunService.RenderStepped:Connect(function()
...
Players.PlayerRemoving:Connect(function(p) premTrackers[p] = nil end)
...
-- ================================================================
-- PREMIUM TAB — Phantom Ultra Engine UI
-- ================================================================
Tabs.Premium:AddSection("⚡ Phantom Ultra Engine")

Tabs.Premium:AddParagraph({
    Title = "What is Phantom Ultra?",
    Content = "Premium aims more accurately than the standard engine, especially at high ping (100ms–300ms). It uses a 3-state Kalman filter tracking acceleration, a ping percentile estimator, a movement classifier, and a 4-frame burst fire — all in one click."
})

Tabs.Premium:AddParagraph({
    Title = "How to Use",
    Content = "1. Enable the toggle below.
2. Left-click to fire at the murderer (SharpShooter).
3. Or press the floating button (Silent Aim).

You do NOT need to enable Combat → Silent Aim or SharpShooter separately. Premium replaces both."
})

local PremiumToggle = Tabs.Premium:AddToggle("PremiumAimToggle", {
    Title = "⚡ Enable Premium Aim",
    Description = "Replaces standard Silent Aim + SharpShooter with the Phantom Ultra engine. Left-click = fire. Button = fire.",
    Default = false,
    Callback = function(toggle)
        premiumEnabled = toggle
        SilentAimButtonV2.Visible = toggle
        if toggle then
            SilentAimEnabled = false  -- disable standard silent aim UI state
            SharpShooterEnabled = false
            Fluent:Notify({
                Title = "⚡ Phantom Ultra ENABLED",
                Content = "Premium aim ON. Left-click or floating button fires precision shot.",
                Duration = 5
            })
        else
            SilentAimButtonV2.Visible = false
            Fluent:Notify({
                Title = "Phantom Ultra",
                Content = "Premium aim disabled. Standard modes restored.",
                Duration = 3
            })
        end
    end
...
Tabs.Premium:AddSection("Engine Stats (Live)")

Tabs.Premium:AddParagraph({
    Title = "Ping Estimate Method",
    Content = "Samples ping every 500ms. Takes the 75th-percentile of the last 20 samples, then adds 33ms for server-tick latency.

This is more stable than the single-sample method used by the standard engine."
})

Tabs.Premium:AddSection("Technical Details")

Tabs.Premium:AddParagraph({
    Title = "3-State Kalman Filter",
    Content = "Standard Phantom Engine tracks [position, velocity] per axis.

Phantom Ultra tracks [position, velocity, ACCELERATION].

The prediction formula becomes:
  pos' = pos + vel·t + ½·accel·t²

At 180ms ping with a player changing direction at ~20 studs/s², this adds 0.32 studs of correction the standard engine misses — enough to turn edge-hits into solid hits."
})

Tabs.Premium:AddParagraph({
    Title = "Movement Classifier",
    Content = "Detects 4 movement states and adjusts aim height:
• Still → aim at HEAD (2.6 studs above root)
• Straight run → upper-chest/neck (2.1 studs)
• Strafing → upper-chest (1.8 studs)
• Airborne/Jumping → torso-center (0.6 studs)

Also adjusts Y-clamp width per state so jumping targets get wider coverage."
})

Tabs.Premium:AddParagraph({
    Title = "4-Frame Burst Fire",
    Content = "Frame 0: fires synchronously with prediction pre-computed BEFORE equip wait — no extra frame delay.
Frames 1–4: each frame recomputes solvePremiumIntercept using the target's LATEST position.

This vs. the standard 2-frame burst: at 180ms, the target can move ~0.5 studs between frames. Fresh predictions each burst frame compensate for this."
})
