...
local _state = {
    ...
    hwid           = "UNKNOWN",
    hwid_status    = "PENDING",
    lastAnnouncement = "",  -- tracks last shown announcement so re-execute doesn't repeat it
}
...
-- pos: UDim2, absSize: Vector2 (Root.AbsoluteSize)
local function clampPosition(pos, absSize)
...
-- Notification layer — right-side column, avoids HorizontalAlignment on Frame entirely
local NotifLayer = new("Frame", {
    Name                 = "NotifLayer",
    Size                 = UDim2.new(0, 324, 1, 0),
    Position             = UDim2.new(1, -348, 0, 0),
...
-- Show an announcement only if it hasn't been shown yet this session
function UILib:NotifyAnnouncement(message)
    if not message or message == "" then return end
    if message == _state.lastAnnouncement then return end  -- already shown, skip
    _state.lastAnnouncement = message
    self:Notify({
        title    = "Announcement",
        message  = message,
        type     = "info",
...
function UILib:CreateWindow(opts)
    ...
    -- ── Dragging (RenderStepped lerp — smooth 60fps drag) ──
    -- Pattern: InputChanged stores the RAW target position.
    --          RenderStepped lerps the actual position toward it every frame.
    --          This decouples input poll rate from render rate = buttery smooth.
    local DRAG_LERP   = 0.35   -- 0 = instant snap, 1 = never moves; 0.3–0.4 feels best
    local dragging    = false
    local dragStart   = nil
    local dragOrigin  = nil
    local targetPos   = nil    -- where we WANT the window to be
    local dragConn    = nil    -- RenderStepped connection, cleaned up when not dragging
    ...
    TitleBar.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Start the render loop only while dragging
            dragConn = RunService.RenderStepped:Connect(function()
                if not targetPos then return end
                local cur = Root.Position
                local tx  = cur.X.Offset + (targetPos.X.Offset - cur.X.Offset) * (1 - DRAG_LERP)
                local ty  = cur.Y.Offset + (targetPos.Y.Offset - cur.Y.Offset) * (1 - DRAG_LERP)
                Root.Position = UDim2.new(0, tx, 0, ty)
            end)
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement
            ...
            local rawPos = UDim2.new(0,
                dragOrigin.X.Offset + delta.X,
                0,
                dragOrigin.Y.Offset + delta.Y
            )
            -- Clamp the TARGET, not the rendered position
            targetPos = clampPosition(rawPos, Root.AbsoluteSize)
        end
    end)
    ...
    UserInputService.InputEnded:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Snap to final clamped target so it doesn't drift
            if targetPos then
    ...
    local maximizeWindow  -- forward declaration so minimize() can call it
    ...
    local function minimize()
        ...
        -- Make floating button draggable (same smooth lerp approach)
        local fb_drag, fb_dragStart, fb_origin = false, nil, nil
        local fb_target, fb_conn = nil, nil

        MinBtn2.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1
            or inp.UserInputType == Enum.UserInputType.Touch then
                fb_drag      = true
                fb_dragStart = inp.Position
                fb_origin    = MinBtn2.Position
                fb_target    = MinBtn2.Position
                fb_conn = RunService.RenderStepped:Connect(function()
                    if not fb_target then return end
                    local cur = MinBtn2.Position
                    local tx  = cur.X.Offset + (fb_target.X.Offset - cur.X.Offset) * 0.65
                    local ty  = cur.Y.Offset + (fb_target.Y.Offset - cur.Y.Offset) * 0.65
                    MinBtn2.Position = UDim2.new(0, tx, 0, ty)
                end)
            end
        end)
        UserInputService.InputChanged:Connect(function(inp)
            if fb_drag and (inp.UserInputType == Enum.UserInputType.MouseMovement
                ...
                fb_target = UDim2.new(0, fb_origin.X.Offset + d.X, 0, fb_origin.Y.Offset + d.Y)
            end
        end)
        UserInputService.InputEnded:Connect(function(inp)
            ...
            or inp.UserInputType == Enum.UserInputType.Touch then
                ...
                if fb_target then MinBtn2.Position = fb_target end
            end
        end)
    ...
    -- Blur intentionally disabled — causes game lag especially on mobile
    playSound("open")
