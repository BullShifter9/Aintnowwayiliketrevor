...
-- ============ GUN REMOTE SPY ============
-- MM2 updates the gun remote path/args frequently so hardcoding it breaks.
-- Instead we hook __namecall, WATCH for the real ShootGun InvokeServer the
-- game sends when you fire normally, and store the remote + args.
-- "Shoot Murderer" then replays those exact args with only the aim
-- position (arg index 2) swapped to the predicted murderer position.
--
-- HOW TO ACTIVATE:
--   Pick up the gun → aim anywhere → shoot ONCE normally.
--   The spy captures the remote and all args automatically.
--   The Sheriff tab label will change from "⚠ Not captured yet"
--   to "✓ Gun remote captured!" once it's ready.

local GunSpy = {
    Remote = nil,   -- actual RemoteFunction object
    Args   = nil,   -- last full args table from a real shot
    Ready  = false, -- true after first real shot is captured
}

-- Deep search a gun tool for any RemoteFunction (works even if path changes)
...
-- Feed captured data into GunSpy from the hook below
local function OnGunShot(remote, args)
    GunSpy.Remote = remote
    GunSpy.Args   = args
    GunSpy.Ready  = true
end

-- Fire the gun at a specific position using captured real args
local function FireGunAt(aimPos)
    ...
    -- Position is always arg index 2 in MM2 ShootGun calls
    args[2] = aimPos
...
pcall(function()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "InvokeServer" then
            ...
            -- ── GUN SPY + SHERIFF SILENT AIM ──
            -- We detect any RemoteFunction inside a Gun tool being invoked.
            -- This works regardless of the remote being named "ShootGun" or
            -- anything else — we just check if the parent chain contains a
            -- Gun tool owned by us.
            local isGunRemote = false
            local obj = self
            for _ = 1, 4 do   -- walk up max 4 levels
                if not obj then break end
                if obj:IsA("Tool") and obj.Name == "Gun" then
                    -- confirm it's in our character or backpack
                    local owner = obj.Parent
                    if owner == LP.Character or owner == LP.Backpack then
                        isGunRemote = true
                        break
                    end
                end
            ...
            if isGunRemote then
                -- ① Always feed the spy so Shoot Murderer button works
                if not State.ShootingMurderer then
                    OnGunShot(self, { table.unpack(args) })
                end

                -- ② Apply silent aim if enabled and not our own programmatic shot
                if State.SheriffSilentAim and not State.ShootingMurderer then
                    ...
                    if murderer and murderer.Character then
                        ...
                        if root then
                            ...
                            -- arg index 1 = distance (set 0), index 2 = position
                            args[1] = 0
                            args[2] = aimPos
...
Tabs.Sheriff:AddParagraph({
    Title = "SETUP: Fire Gun Once First",
    Content = "Pick up the gun → aim anywhere → shoot ONCE normally.\nThe script auto-captures the real remote + args.\nThe status below updates when ready."
})

-- Live spy status — updates every second via coroutine
local SpyStatus = Tabs.Sheriff:AddParagraph({
    Title = "Gun Remote Status",
    Content = "Not captured yet — fire the gun once to activate"
})
coroutine.wrap(function()
    while true do
        task.wait(1)
            ...
            pcall(function()
                if GunSpy.Ready then
                    SpyStatus:SetContent(
                        "READY: " .. (GunSpy.Remote and GunSpy.Remote.Name or "?") ..
                        "  (" .. (GunSpy.Args and #GunSpy.Args or 0) .. " args)" ..
                        "\nShoot Murderer button is unlocked."
                    )
                else
                    SpyStatus:SetContent("Not captured yet — equip gun and shoot once at anything")
                end
            end)
        end
    end
...
Tabs.Sheriff:AddParagraph({
    ...
    Content = "Hooks your shot to auto-aim at murderer when you fire normally.\nDynamic = best all-round | Regular = still targets | Seismic = sprinting"
})
...
Tabs.Sheriff:AddButton({
    ...
    Description = "Uses the spied gun remote to fire at murderer. Requires firing gun once first.",
    Callback = function()
        if not GunSpy.Ready then
            Notify("Gun not captured yet!\nEquip gun → shoot once at anything → try again.")
            return
        end
        local murderer = Players:FindFirstChild(State.Murderer)
