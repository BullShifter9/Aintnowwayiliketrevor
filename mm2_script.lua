...
-- ============ GUN REMOTE SPY ============
-- The gun remote path changes with MM2 updates so we cannot hardcode it.
-- Instead we use LISTEN MODE: you press "Start Capture", shoot the gun once,
-- and the hook grabs the EXACT remote + args the game sent — regardless of
-- where the remote lives (inside the tool, in ReplicatedStorage, anywhere).
--
-- How it works:
--   GunSpy.Listening = true  → hook records the next InvokeServer that has
--                               a Vector3 as its second argument (that is
--                               always the aim position in MM2 ShootGun)
--   After capture → GunSpy.Ready = true, button unlocked, spy turns off.
--
-- FindShootRemote is kept for Break Gun (searches inside sheriff gun tool).

local GunSpy = {
    Remote    = nil,   -- captured RemoteFunction
    Args      = nil,   -- captured full args table
    Ready     = false, -- true after first successful capture
    Listening = false, -- true while waiting for the next shot to capture
}

-- Deep-search a tool for any RemoteFunction (used by Break Gun)
local function FindShootRemote(gunTool)
    if not gunTool then return nil end
    local ks = gunTool:FindFirstChild("KnifeServer")
    if ks then
        local rf = ks:FindFirstChildWhichIsA("RemoteFunction")
        if rf then return rf end
    end
    for _, d in ipairs(gunTool:GetDescendants()) do
        if d:IsA("RemoteFunction") then return d end
    end
    return nil
end

-- Fire the gun at aimPos using the captured remote + args
local function FireGunAt(aimPos)
    if not GunSpy.Ready then return false, "notready" end
    -- Rebuild args, replace position slot (index 2) with our target
    local args = {}
    for i, v in ipairs(GunSpy.Args) do args[i] = v end
    args[2] = aimPos
    local ok, err = pcall(function()
        GunSpy.Remote:InvokeServer(table.unpack(args))
    end)
...
-- ============ NAMECALL HOOK ============
-- Does THREE things in one hook:
--  1. GUN SPY      – captures real ShootGun remote + args when you fire normally
--  2. SILENT AIM   – redirects your shot / knife throw toward murderer
--  3. ANTI-KICK    – blocks Kick calls on our own player
pcall(function()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        -- Never tamper with calls made by our own script
        ...
        if method == "InvokeServer" then
            ...
            -- ── GUN SPY (LISTEN MODE) + SHERIFF SILENT AIM ──
            -- When GunSpy.Listening is true (user pressed "Start Capture" button),
            -- we treat the NEXT InvokeServer that has a Vector3 as arg[2] as the
            -- gun shot — regardless of where the remote lives. This catches it
            -- whether the remote is inside the Gun tool, in ReplicatedStorage, anywhere.
            if GunSpy.Listening and not State.ShootingMurderer then
                -- arg[2] being a Vector3 is the MM2 shoot signature
                if typeof(args[2]) == "Vector3" then
                    GunSpy.Remote    = self
                    GunSpy.Args      = { table.unpack(args) }
                    GunSpy.Ready     = true
                    GunSpy.Listening = false
                    warn("[MM2] Gun captured! Remote: " .. tostring(self:GetFullName())
                        .. " | Args: " .. #GunSpy.Args)
                end
            end

            -- ── SHERIFF SILENT AIM ──
            -- Once spy is ready, redirect shots when silent aim is on
            if GunSpy.Ready and self == GunSpy.Remote
               and State.SheriffSilentAim and not State.ShootingMurderer then
                local murderer = Players:FindFirstChild(State.Murderer)
                if murderer and murderer.Character then
                    local root = State.SharpShooter
                    ...
                    if root then
                        ...
                        if acc == "Seismic" then
                            ...
                            if vel.Magnitude < 0.1 then
                                ...
                                aimPos = root.Position + Vector3.new(v.X, vy, v.Z / 1.25)
                            end
                        elseif acc == "Dynamic" then
                            local hum = murderer.Character:FindFirstChildOfClass("Humanoid")
                            aimPos = root.Position + (hum and hum.MoveDirection or Vector3.zero)
                        else
                            aimPos = root.Position
                        end
                        args[1] = 0
...
Tabs.Sheriff:AddParagraph({
    Title = "Step 1 — Capture the Gun Remote",
    Content = "Press START CAPTURE below, then shoot the gun ONCE normally (aim anywhere).\nThe script will intercept the real remote automatically.\nYou only need to do this once per session."
})

-- Live status label
local SpyStatus = Tabs.Sheriff:AddParagraph({
    Title = "Status",
    Content = "Not captured — press Start Capture first"
})
...
coroutine.wrap(function()
    while true do
        ...
        if SpyStatus then
            pcall(function()
                if GunSpy.Ready then
                    SpyStatus:SetContent(
                        "READY  |  Remote: " .. (GunSpy.Remote and GunSpy.Remote.Name or "unknown") ..
                        "  |  Args: " .. tostring(GunSpy.Args and #GunSpy.Args or 0) ..
                        "\nShoot Murderer is now unlocked."
                    )
                elseif GunSpy.Listening then
                    SpyStatus:SetContent("LISTENING...  Shoot the gun now!")
                else
                    SpyStatus:SetContent("Not captured — press Start Capture, then shoot gun once")
                end
            end)
        end
    end
...
-- START CAPTURE button — enables listen mode, next gun shot gets captured
Tabs.Sheriff:AddButton({
    Title = "Start Capture (then shoot gun once)",
    Description = "Activates spy mode. Shoot the gun once after pressing this.",
    Callback = function()
        if GunSpy.Ready then
            -- Allow re-capture in case round reset / gun changed
            GunSpy.Ready     = false
            GunSpy.Remote    = nil
            GunSpy.Args      = nil
        end
        GunSpy.Listening = true
        Notify("LISTENING — shoot the gun once now!")
    end
})
...
Tabs.Sheriff:AddParagraph({
    Title = "Step 2 — Sheriff Silent Aim",
    Content = "Once captured, enable this to auto-aim at murderer every time you fire normally.\nDynamic = best all-round | Regular = still targets | Seismic = sprinting"
})
...
Tabs.Sheriff:AddButton({
    ...
    Description = "Fires at murderer using captured remote. Press Start Capture + shoot once if not ready.",
    Callback = function()
        if not GunSpy.Ready then
            Notify("Not captured yet!\n1. Press \'Start Capture\'\n2. Shoot gun once normally\n3. Then press this button")
            return
        end
        local murderer = Players:FindFirstChild(State.Murderer)
            ...
            if v then
                ContextActionService:BindAction("MM2_ShootMurd",
                    function(_, state)
                        ...
                        if not GunSpy.Ready then return end
                        local murderer = Players:FindFirstChild(State.Murderer)
...
Tabs.Gun:AddButton({
    ...
    Callback = function()
        ...
        pcall(function()
            remote:InvokeServer(0, Vector3.new(), "AH")
        end)
                        ...
                        if not roundActive or not IsAlive(LP) then
                            if not State.FarmSTOP then
                                ...
                                elseif State.MyRole == "Sheriff" and State.ShootMurdWhenDone then
                                    ...
                                    if murd and murd.Character and GunSpy.Ready then
...
Tabs.Farm:AddToggle("ShootMurdWhenDone", {
    ...
    Description = "Requires gun remote captured first",
    Default = false,
...
Tabs.Settings:AddParagraph({
    ...
    Content = "1. Go to Sheriff tab\n2. Press 'Start Capture'\n3. Pick up your gun and shoot once (aim anywhere)\n4. Status label turns green — all gun features now work\n5. Re-capture every time you rejoin or die"
})
Fluent:Notify({
    Title   = "MM2 Script",
