...
------------------------------------------------------------
-- LOADER RUNNER
-- Uses ONLY task.wait() — never tw.Completed:Wait()
-- so it works on every executor.
------------------------------------------------------------
local function RunLoader(title, subtitle, steps, done)
    LTitle.Text = title    or "OMNIHUB"
    LSub.Text   = subtitle or "By Azzakirms"

    -- ── animate progress bar with a plain loop (no tween blocking) ──
    local currentPct = 0  -- tracks where the fill currently is (0–1)

    local function animateFill(targetPct)
        local startPct = currentPct
        local diff     = targetPct - startPct
        if diff <= 0 then
            currentPct = targetPct
            PF.Size    = UDim2.new(targetPct, 0, 1, 0)
        ...
        for i = 1, STEPS do
            ...
            -- ease out quart
            local et  = 1 - (1 - t)^4
            local pct = startPct + diff * et
            PF.Size   = UDim2.new(pct, 0, 1, 0)
    ...
    -- ── slide card in ────────────────────────────────────────────────
    TwPlay(Card, {Position=UDim2.new(0.5,0,0.5,0)}, 0.5,
        Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    TwPlay(BGGlow, {ImageTransparency=0.82}, 0.5)
    task.wait(0.35)
    TwPlay(TopStrip, {Size=UDim2.new(1,0,0,2)}, 0.4)
    task.wait(0.45)

    -- ── icon pulse (fire and forget) ─────────────────────────────────
    local ip = true
    task.spawn(function()
        while ip do
    ...
    -- ── dots on status label (fire and forget) ───────────────────────
    ...
    task.spawn(function()
        while dotActive do
            dotIdx = (dotIdx % #dotPattern) + 1
            LStat.Text = dotBase .. dotPattern[dotIdx]
            task.wait(0.35)
        end
    end)
    ...
    local function setStep(pct, label)
        dotActive = false          -- pause dots
        task.wait(0.05)
        dotBase    = label
        dotIdx     = 0
        LStat.Text = label
        dotActive  = true          -- resume dots

        animateFill(pct)           -- blocking loop — no Completed:Wait needed
        task.wait(0.1)
    end
    ...
    for _, s in ipairs(steps) do
        setStep(s[1], s[2])
        task.wait(0.28 + math.random() * 0.2)
    end
    ...
    -- ── outro: slide card out, fade cover ─────────────────────────────
    TwPlay(Card, {
    ...
    -- manually fade out cover with a loop (no Completed:Wait)
    local FADE_STEPS    = 20
    local FADE_INTERVAL = 0.016
    for i = 1, FADE_STEPS do
        Cover.BackgroundTransparency = i / FADE_STEPS
        task.wait(FADE_INTERVAL)
    end
