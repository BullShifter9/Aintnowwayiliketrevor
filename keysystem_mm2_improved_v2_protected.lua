-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║         ADVANCED MULTI-LAYER PROTECTION SYSTEM  v2.0                     ║
-- ║  Anti HTTP Spy · Anti Remote Spy · Anti Dex · Advanced Hook Detection    ║
-- ║  Continuous Runtime Monitoring · Environment Leak Detection               ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
--
-- Research sources used in design:
--  · SimpleSpy v3 internals  (hookmetamethod __namecall approach)
--  · Hydroxide source        (namecall + ClosureSpy hooks)
--  · debug.info stack identity technique (devforum July 2025)
--  · getfenv environment-leak technique  (devforum July 2025)
--  · Timing-ratio side-channel          (devforum July 2025)
--  · Asset ID fingerprinting            (devforum Nov 2024)
--
-- All checks run inside a sealed `do` block. A detection calls killScript()
-- which errors out of the entire chunk, stopping the rest of the file from
-- ever executing. The original script below this block is untouched.
-- ─────────────────────────────────────────────────────────────────────────────

do
    -- =========================================================================
    -- § 0.  SECURE LOCAL CACHE
    --       Cache every standard function we need BEFORE any hook could
    --       have replaced globals (we run at the very top of the file).
    --       All subsequent code uses only these cached references.
    -- =========================================================================
    local _pcall          = pcall
    ...
    -- Executor-specific APIs (may be nil — always accessed via pcall-safe wrappers)
    ...
    local _getrawmeta     = getrawmetatable
    local _setreadonly    = setreadonly
    local _getfenv_raw    = getfenv
    local _getgenv_raw    = getgenv
    local _debug_info     = debug and debug.info
    local _debug_getinfo  = debug and debug.getinfo

    -- =========================================================================
    -- § 1.  KILL SWITCH
    --       Central exit point.  Logs reason, kicks player, hard-errors.
    -- =========================================================================
    local function killScript(reason)
        local msg = "[PROTECTION] Halted — " .. _tostring(reason)
        _warn(msg)
        _pcall(function()
        ...
        -- error() at level 0 produces no line-number leak and unwinds the
        -- entire call stack, preventing the rest of the file from running.
        _error(msg, 0)
    end
    ...
    -- Safe wrapper: run fn, return (ok, result).  Never throws.
    local function safe(fn, ...)
        return _pcall(fn, ...)
    end

    -- Return the lowercase string form of a name for keyword matching.
    local function low(s)
        return _slower(_tostring(s))
    end

    -- Check if a function is a genuine C closure (not a Lua wrapper / hook).
    -- Returns true  → it IS a real C closure (healthy).
    -- Returns false → it is a Lua closure or unknown (suspicious).
    local function isCClosure(fn)
        if _type(fn) ~= "function" then return false end
        -- iscclosure is the most direct test
        if _iscclosure then
            local ok, result = safe(_iscclosure, fn)
            if ok and result == true then return true end
        end
        -- islclosure returning true means it's a Lua wrapper (bad)
        if _islclosure then
            local ok, result = safe(_islclosure, fn)
            if ok and result == true then return false end
        end
        -- checkclosure: real built-ins pass, Lua hooks fail
        if _checkclosure then
            local ok, result = safe(_checkclosure, fn)
            if ok and result == false then return false end
        end
        return true  -- assume clean if APIs not available
    end

    -- =========================================================================
    -- § 3.  ENVIRONMENT LEAK DETECTOR
    --       Based on: devforum.roblox.com/t/3847679 (July 2025)
    --
    --       How it works:
    --         Trigger a guaranteed Lua error inside xpcall.  Walk the call
    --         stack with getfenv(level).  In a clean game, getfenv() at
    --         every level returns an empty table or the script's own env.
    --         Exploit hooks (including newcclosure-wrapped ones) leak their
    --         environment variables into the stack — we catch them.
    --
    --       Returns: true  if a leak / exploit env was detected.
    --                false if clean.
    -- =========================================================================
    local baseEnv         -- captured once from the clean script environment
    local function captureBaseEnv()
        if _getfenv_raw then
            local ok, e = safe(_getfenv_raw, 0)
            if ok then baseEnv = e end
        end
    end
    captureBaseEnv()

    local KNOWN_EXPLOIT_KEYS = {
        -- Synapse X / Wave
        "syn", "SYNAPSE_LOADED", "SYN_LOADED",
        -- Krnl
        "KRNL_LOADED", "krnl",
        -- Fluxus
        ...
        "setreadonly", "newcclosure", "getcallstack",
        "getconnections", "fireclickdetector", "firetouchinterest",
        "firesignal", "getloadedmodules", "getscripts",
        "getsenv", "decompile", "readfile", "writefile",
        "loadstring_secure",
        -- Spy tools
        "SimpleSpy", "simplespy", "Hydroxide", "hydroxide",
        "SigmaSpy", "sigmaspy", "RSpy", "rspy",
        "RemoteSpy", "remotespy",
    }

    local function scanEnvForLeaks(env)
        if env == nil or env == baseEnv then return false end
        for _, key in _ipairs(KNOWN_EXPLOIT_KEYS) do
            if _rawget(env, key) ~= nil then return true end
        end
        -- Extra: count keys.  A populated environment at error-handler level
        -- means hook code is running in our call path.
        local count = 0
        for _ in _pairs(env) do
            count = count + 1
            if count > 3 then return true end  -- 3+ keys in error-handler env = suspicious
        end
        return false
    end

    local function envLeakCheck(label)
        if not _getfenv_raw then return false end
        local leaked = false
        _xpcall(function()
            -- Intentionally trigger a __namecall error: workspace:__PROT_PROBE()
            -- doesn't exist, guaranteed error path through __namecall dispatcher
            local _ = workspace:__PROT_PROBE_XYZ_NOMATCH()
        end, function()
            -- Walk up to level 20 of the error handler's call stack
            for level = 0, 20 do
                local ok, env = safe(_getfenv_raw, level)
                if ok and env then
                    if scanEnvForLeaks(env) then
                        leaked = true
                        break
                    end
                end
    ...
    -- =========================================================================
    -- § 4.  __NAMECALL IDENTITY TRACKER
    --       Based on: devforum.roblox.com/t/3810681 (July 2025)
    --
    --       We capture the real __namecall function (and its immediate caller)
    --       by letting an xpcall error bubble through it, then reading
    --       debug.info.  If spy tools hook __namecall, the identity of the
    --       function at that stack level changes — we catch it.
    -- =========================================================================
    local capturedNamecall   = nil
    local capturedNCCaller   = nil
    local namecallCaptured   = false

    local function captureNamecall()
        if not _debug_info then return end
        _xpcall(function()
            -- Intentionally invalid call through namecall dispatcher
            return game:__PROT_NC_CAPTURE()
        end, function()
            local ok1, nc  = safe(_debug_info, 2, "f")
            local ok2, ncc = safe(_debug_info, 3, "f")
            if ok1 and nc  then capturedNamecall = nc  end
            if ok2 and ncc then capturedNCCaller = ncc end
            namecallCaptured = true
        end)
    end
    captureNamecall()

    local function namecallIntegrityCheck()
        if not namecallCaptured or not _debug_info then return false end
        local currentNC, currentCaller
        _xpcall(function()
            return game:__PROT_NC_VERIFY()
        end, function()
            local ok1, nc  = safe(_debug_info, 2, "f")
            local ok2, ncc = safe(_debug_info, 3, "f")
            if ok1 then currentNC     = nc  end
            if ok2 then currentCaller = ncc end
        end)
        -- Identity comparison: spy tools replace the function pointer
        if currentNC ~= nil and capturedNamecall ~= nil then
            if not _rawequal(currentNC, capturedNamecall) then return true end
        end
        if currentCaller ~= nil and capturedNCCaller ~= nil then
            if not _rawequal(currentCaller, capturedNCCaller) then return true end
        end
        return false
    end

    -- =========================================================================
    -- § 5.  TIMING RATIO SIDE-CHANNEL
    --       Measures execution overhead of game:IsA() vs empty loop.
    --       Remote spy hooks (even newcclosure-wrapped) add measurable latency.
    --       Uses statistical buffering to avoid false positives on low-end HW.
    -- =========================================================================
    local timingViolations  = 0
    local TIMING_SAMPLES    = 5000   -- reduced for startup (full check uses 10k)
    local RATIO_MIN         = 12     -- below this → hook overhead (nearly free call)
    local RATIO_MAX         = 80     -- above this → extremely slow (injected overhead)
    local VIOLATION_LIMIT   = 5      -- consecutive violations needed to trigger

    local function timingRatioCheck()
        local t0 = _tick()
        for i = 1, TIMING_SAMPLES do
            game:IsA("Model")
        end
        local t1 = _tick()
        for i = 1, TIMING_SAMPLES do end
        local t2 = _tick()

        local rFunc  = t1 - t0
        local rEmpty = t2 - t1
        if rEmpty < 1e-9 then return false end  -- guard divide by zero

        local ratio = rFunc / rEmpty
        if ratio < RATIO_MIN or ratio > RATIO_MAX then
            timingViolations = timingViolations + 1
        else
            timingViolations = 0
        end
        return timingViolations >= VIOLATION_LIMIT
    end

    -- =========================================================================
    -- § 6.  RAW METATABLE HOOK INSPECTOR
    --       Directly inspects game's raw metatable for Lua-closure hooks on
    --       __namecall, __index, __newindex.
    --       Legitimate Roblox internals are always C closures.
    -- =========================================================================
    local function rawMetatableCheck()
        if not _getrawmeta then return false end
        local ok, mt = safe(_getrawmeta, game)
        if not ok or not mt then return false end

        local keysToCheck = { "__namecall", "__index", "__newindex" }
        for _, key in _ipairs(keysToCheck) do
            local fn = _rawget(mt, key)
            if fn ~= nil and _type(fn) == "function" then
                -- If it's a Lua closure → it was hooked (real methods are C)
                if _islclosure then
                    local ok2, isLua = safe(_islclosure, fn)
                    if ok2 and isLua then return true end
                end
                if _iscclosure then
                    local ok2, isC = safe(_iscclosure, fn)
                    if ok2 and isC == false then return true end
                end
            end
        end
        return false
    ...
    -- =========================================================================
    -- § 7.  HTTP FUNCTION INTEGRITY CHECK (Anti HTTP Spy)
    --
    --  Detection layers:
    --   A) C-closure type check on all six HTTP entry points
    --   B) getfenv stack leak triggered through HttpGet call path
    --   C) Raw metatable __namecall check (covers HttpService hooks)
    --   D) Known spy-injected global presence check
    -- =========================================================================
    local function checkHTTPSpy()
        local HttpService = game:GetService("HttpService")

        -- Snapshot the functions
        local httpFns = {
            { obj = game,        name = "HttpGet",   fn = game.HttpGet         },
            { obj = HttpService, name = "GetAsync",  fn = HttpService.GetAsync  },
            { obj = HttpService, name = "PostAsync", fn = HttpService.PostAsync },
        }

        -- Layer A: C-closure verification
        for _, entry in _ipairs(httpFns) do
            if not isCClosure(entry.fn) then
                killScript("HTTP Spy: " .. entry.name .. " is not a C closure (hooked/wrapped)")
            end
        end

        -- Layer B: Check global http helpers
        local globalHttpNames = { "request", "http_request" }
        for _, gname in _ipairs(globalHttpNames) do
            local fn = _rawget(_G, gname)
            if fn ~= nil and _type(fn) == "function" then
                if not isCClosure(fn) then
                    killScript("HTTP Spy: global " .. gname .. " is a Lua closure (hooked)")
                end
            end
        end

        -- syn.request check
        local syn_tbl = _rawget(_G, "syn")
        if syn_tbl ~= nil and _type(syn_tbl) == "table" then
            local synReq = _rawget(syn_tbl, "request")
            if synReq ~= nil and _type(synReq) == "function" then
                if not isCClosure(synReq) then
                    killScript("HTTP Spy: syn.request is a Lua closure (wrapped/hooked)")
                end
            end
        end

        -- Layer C: Environment leak through namecall path
        if envLeakCheck("http") then
            killScript("HTTP Spy: exploit environment leaked through namecall stack (spy active)")
        end
    end

    -- =========================================================================
    -- § 8.  REMOTE SPY DETECTION (Anti Remote Spy)
    --
    --  Detection layers:
    --   A) Probe RemoteEvent/RemoteFunction method C-closure types
    --   B) __namecall identity check (SimpleSpy/Hydroxide hook __namecall)
    --   C) Raw metatable Lua-closure hook detection
    --   D) Known global presence (SimpleSpy, Hydroxide, SigmaSpy, etc.)
    --   E) CoreGui/PlayerGui UI artefact scan
    --   F) getgenv() scan for spy-related keys
    -- =========================================================================
    local REMOTE_SPY_GLOBALS = {
        "SimpleSpy", "simplespy", "SIMPLESPY", "SimpleSpyLoaded",
        "Hydroxide", "hydroxide", "HydroxideLoaded", "HydroxideMain",
        "SigmaSpy", "sigmaspy", "SIGMASPY",
        "RSpy", "rspy", "RSPY",
        "RemoteSpy", "remotespy", "REMOTESPY",
        "FireSpy", "firespy",
        "RemoteLogger", "ClosureSpy", "closurespy",
        "spy_active", "SPY_ACTIVE", "SPYENABLED",
    }

    local REMOTE_SPY_UI_KEYWORDS = {
        "simplespy", "simple spy", "hydroxide", "sigmaspy", "sigma spy",
        "remotespy", "remote spy", "remote_spy", "rspy", "firespy",
        "fire spy", "closurespy", "closure spy", "spylog", "remotelog",
    }

    local function nameContainsSpyKw(s)
        local ls = low(s)
        for _, kw in _ipairs(REMOTE_SPY_UI_KEYWORDS) do
            if _sfind(ls, kw, 1, true) then return true end
        end
        return false
    end

    local function scanForSpyUI(root)
        if not root then return false end
        local ok, children = safe(function() return root:GetDescendants() end)
        if not ok or not children then return false end
        for _, child in _ipairs(children) do
            if nameContainsSpyKw(child.Name) then return true end
            if nameContainsSpyKw(child.ClassName) then return true end
        end
        return false
    end

    local function checkRemoteSpy()
        -- Layer A: Probe method C-closure types
        local probeRE = Instance.new("RemoteEvent")
        local probeRF = Instance.new("RemoteFunction")

        local remoteMethods = {
            { fn = probeRE.FireServer,   label = "RemoteEvent:FireServer"       },
            { fn = probeRF.InvokeServer, label = "RemoteFunction:InvokeServer"  },
        }

        for _, entry in _ipairs(remoteMethods) do
            if not isCClosure(entry.fn) then
                _pcall(function() probeRE:Destroy() end)
                _pcall(function() probeRF:Destroy() end)
                killScript("Remote Spy: " .. entry.label .. " is a Lua closure (hooked)")
            end
        end

        -- Also probe OnClientEvent.Connect
        _pcall(function()
            local sig = probeRE.OnClientEvent
            if sig then
                local connectFn = sig.Connect
                if connectFn and _type(connectFn) == "function" then
                    if not isCClosure(connectFn) then
                        _pcall(function() probeRE:Destroy() end)
                        _pcall(function() probeRF:Destroy() end)
                        killScript("Remote Spy: OnClientEvent.Connect is hooked (Lua closure)")
                    end
                end
            end
        end)
        ...
        -- Layer B: __namecall identity (captures hook at inject time)
        if namecallIntegrityCheck() then
            killScript("Remote Spy: __namecall identity changed (SimpleSpy/Hydroxide hook detected)")
        end

        -- Layer C: Raw metatable Lua-closure hook
        if rawMetatableCheck() then
            killScript("Remote Spy: game metatable contains Lua-closure hook (__namecall/__index tampered)")
        end

        -- Layer D: Known globals in _G
        for _, gname in _ipairs(REMOTE_SPY_GLOBALS) do
            if _rawget(_G, gname) ~= nil then
                killScript("Remote Spy: known spy global found in _G: " .. gname)
            end
        end

        -- Layer E: getgenv scan
        if _getgenv_raw then
            local ok, genv = safe(_getgenv_raw)
            if ok and genv then
                for _, gname in _ipairs(REMOTE_SPY_GLOBALS) do
                    if _rawget(genv, gname) ~= nil then
                        killScript("Remote Spy: known spy global found in getgenv(): " .. gname)
                    end
                end
            end
        end
        ...
        -- Layer F: UI scan
        _pcall(function()
            if scanForSpyUI(game:GetService("CoreGui")) then
                killScript("Remote Spy: spy UI detected in CoreGui")
            end
        end)
        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if scanForSpyUI(pg) then
                killScript("Remote Spy: spy UI detected in PlayerGui")
            end
        end)
    end

    -- =========================================================================
    -- § 9.  DEX EXPLORER DETECTION (Anti Dex Explorer)
    --
    --  Detection layers:
    --   A) Known asset ID fingerprints injected into CoreGui
    --   B) Multi-keyword UI name/class scanning (CoreGui + PlayerGui)
    --   C) UI structural pattern detection (PropertyView + InstanceBrowser)
    --   D) Known globals and getgenv keys
    --   E) StringValue content scan for version strings
    -- =========================================================================

    -- Known rbxassetid values for explorer tools (from devforum research)
    local DEX_ASSET_IDS = {
        ["rbxassetid://5642383285"] = "Dex Explorer",
        ["rbxassetid://5817174583"] = "Dark Dex",
        ["rbxassetid://7392189427"] = "Dex V3",
        ["rbxassetid://0000000000"] = "placeholder_safe",  -- never matches
    }

    local DEX_NAME_KEYWORDS = {
        "dex", "darkdex", "dark dex", "dexexplorer", "dex explorer",
        "dexv3", "dex v3", "dexv4", "dex v4",
        "propertiesframe", "dexframe", "explorerframe",
        "dex_main", "dexmain", "dexwindow", "instanceexplorer",
        "instance explorer", "object explorer", "objectexplorer",
        "explorerwindow", "propwindow", "propertywindow",
        "dexfolder", "explorerui", "explorer_ui",
        "scripteditor", "script editor",
        -- Infinite Yield (also acts as explorer)
        "infiniteyield", "infinite yield", "iy_main",
    }

    local DEX_GLOBALS = {
        "Dex", "DEX", "DexExplorer", "dexExplorer", "dex_explorer",
        "DarkDex", "darkdex", "DARKDEX",
        "DexV3", "DexV4", "dexV3", "dexV4",
        "InfiniteYield", "infiniteYield",
        "IY", "iy",
    }

    -- Structural pattern: Dex always injects a ScreenGui with both a
    -- "PropertiesFrame" and a "ExplorerFrame" (or similar named children).
    local DEX_STRUCTURAL_PAIRS = {
        { "properties", "explorer" },
        { "properties", "script" },
        { "dex",        "explorer" },
    }

    local function nameContainsDexKw(s)
        local ls = low(s)
        for _, kw in _ipairs(DEX_NAME_KEYWORDS) do
            if _sfind(ls, kw, 1, true) then return true end
        end
        return false
    end

    local function checkImageIdForExplorer(obj)
        local ok, id = safe(function() return obj.Image end)
        if ok and id then
            if DEX_ASSET_IDS[low(id)] and DEX_ASSET_IDS[low(id)] ~= "placeholder_safe" then
                return true
            end
        end
        return false
    ...
    local function checkStructuralDexPattern(screenGui)
        -- Flatten all child names into lowercase for pair matching
        local ok, children = safe(function() return screenGui:GetChildren() end)
        if not ok or not children then return false end
        local childNames = {}
        for _, child in _ipairs(children) do
            _tinsert(childNames, low(child.Name))
        end
        -- Also check descendants one level deeper
        local ok2, desc = safe(function() return screenGui:GetDescendants() end)
        if ok2 and desc then
            for _, d in _ipairs(desc) do
                _tinsert(childNames, low(d.Name))
            end
        end
        -- Check for structural signature pairs
        for _, pair in _ipairs(DEX_STRUCTURAL_PAIRS) do
            local hasA, hasB = false, false
            for _, n in _ipairs(childNames) do
                if _sfind(n, pair[1], 1, true) then hasA = true end
                if _sfind(n, pair[2], 1, true) then hasB = true end
            end
    ...
    local function deepScanForDex(root)
        if not root then return false end
        local ok, children = safe(function() return root:GetChildren() end)
        if not ok then return false end

        for _, child in _ipairs(children) do
            -- Name keyword check
            if nameContainsDexKw(child.Name) then return true end
            -- ClassName check
            if nameContainsDexKw(child.ClassName) then return true end
            -- Asset ID check on ImageLabel/ImageButton
            if child.ClassName == "ImageLabel" or child.ClassName == "ImageButton" then
                if checkImageIdForExplorer(child) then return true end
            end
            -- ScreenGui structural pattern check
            if child.ClassName == "ScreenGui" then
                if checkStructuralDexPattern(child) then return true end
            end
            -- StringValue content check (Dex stores its version in a StringValue)
            if child.ClassName == "StringValue" then
                local ok2, val = safe(function() return child.Value end)
                if ok2 and val then
                    local lv = low(_tostring(val))
                    if _sfind(lv, "dex", 1, true) and _sfind(lv, "explorer", 1, true) then
                        return true
                    end
                end
            end
            -- Recurse one level
            local ok3, grandchildren = safe(function() return child:GetChildren() end)
            if ok3 and grandchildren then
                for _, gc in _ipairs(grandchildren) do
                    if nameContainsDexKw(gc.Name) then return true end
                    if gc.ClassName == "ImageLabel" or gc.ClassName == "ImageButton" then
                        if checkImageIdForExplorer(gc) then return true end
                    end
                end
            end
        end
    ...
    local function checkDexExplorer()
        -- Layer A + B + C + E: CoreGui deep scan
        _pcall(function()
            if deepScanForDex(game:GetService("CoreGui")) then
                killScript("Dex Explorer: explorer UI detected in CoreGui")
            end
        end)
        ...
        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if deepScanForDex(pg) then
                killScript("Dex Explorer: explorer UI detected in PlayerGui")
            end
        end)

        -- Layer D: _G globals
        for _, gname in _ipairs(DEX_GLOBALS) do
            if _rawget(_G, gname) ~= nil then
                killScript("Dex Explorer: known explorer global in _G: " .. gname)
            end
        end
        ...
        if _getgenv_raw then
            ...
            if ok and genv then
                for _, gname in _ipairs(DEX_GLOBALS) do
                    if _rawget(genv, gname) ~= nil then
                        killScript("Dex Explorer: known explorer global in getgenv(): " .. gname)
                    end
                end
            end
        end
    ...
    -- =========================================================================
    -- § 10. ADVANCED HOOK DETECTION
    --
    --  Detection layers:
    --   A) Environment leak via intentional metamethod errors (§3 technique)
    --   B) debug.info __namecall identity (§4 technique)
    --   C) Raw metatable hook inspection (§6 technique)
    --   D) debug.* function integrity (debug.traceback etc should be C closures)
    --   E) Core function replacement check (pcall, error, tostring, pairs, etc.)
    --   F) hookfunction / hookmetamethod presence in environment
    -- =========================================================================
    local CRITICAL_GLOBALS = {
        { name = "pcall",       fn = pcall       },
        { name = "xpcall",      fn = xpcall      },
        { name = "error",       fn = error       },
        { name = "pairs",       fn = pairs       },
        { name = "ipairs",      fn = ipairs      },
        { name = "tostring",    fn = tostring    },
        { name = "rawget",      fn = rawget      },
        { name = "rawset",      fn = rawset      },
        { name = "setmetatable",fn = setmetatable},
        { name = "getmetatable",fn = getmetatable},
        { name = "next",        fn = next        },
        { name = "select",      fn = select      },
    }

    local function checkAdvancedHooks()
        -- Layer A: Environment leak check
        if envLeakCheck("hookdetect") then
            killScript("Hook Detection: exploit environment leaked through error stack")
        end

        -- Layer B: __namecall identity (done in remote spy too, belt+suspenders)
        if namecallIntegrityCheck() then
            killScript("Hook Detection: __namecall function identity mismatch (hook detected)")
        end

        -- Layer C: Raw metatable
        if rawMetatableCheck() then
            killScript("Hook Detection: game metatable contains injected Lua closure")
        end

        -- Layer E: Verify core global functions are still C closures
        for _, entry in _ipairs(CRITICAL_GLOBALS) do
            local live = _rawget(_G, entry.name) or entry.fn
            if live ~= nil and not isCClosure(live) then
                killScript("Hook Detection: core global '" .. entry.name .. "' was replaced with a Lua closure")
            end
        end

        -- Layer F: hookfunction / hookmetamethod being present signals an
        -- active executor session. We only flag if they're *also* in error
        -- stack environments (combined signal).
        -- (Standalone presence is expected in an executor environment;
        --  their presence IN the error stack means they're actively running.)
    end

    -- =========================================================================
    -- § 11. INITIAL SYNCHRONOUS CHECKS  (run immediately, block execution)
    -- =========================================================================
    checkHTTPSpy()
    checkRemoteSpy()
    checkDexExplorer()
    ...
    -- =========================================================================
    -- § 12. CONTINUOUS RUNTIME MONITOR
    --       Runs in background every CHECK_INTERVAL seconds.
    --       Detects spies / explorers injected AFTER script start.
    --       Uses independent check cycle so each detection type gets a turn
    --       without one stall blocking all others.
    -- =========================================================================
    local CHECK_INTERVAL   = 2.5   -- seconds between full check cycles
    local TIMING_INTERVAL  = 8.0   -- timing check is heavier; less frequent

    -- Shared "should runtime kill be active" flag
    local runtimeActive = true

    -- Runtime kill (from background thread — can't use error() across threads)
    local function runtimeKill(reason)
        if not runtimeActive then return end
        runtimeActive = false
        local msg = "[PROTECTION/RUNTIME] Detected: " .. _tostring(reason)
        _warn(msg)
        _pcall(function()
            game:GetService("Players").LocalPlayer:Kick(
                "[Protection] Runtime detection:\n" .. _tostring(reason)
            )
        end)
    end

    -- ── Runtime: Remote Spy + HTTP Spy checks ────────────────────────────────
    _task_spawn(function()
        while runtimeActive do
            _task_wait(CHECK_INTERVAL)
            if not runtimeActive then break end

            -- __namecall identity
            _pcall(function()
                if namecallIntegrityCheck() then
                    runtimeKill("Runtime — Remote Spy: __namecall hooked after start")
                end
            end)
            ...
            _pcall(function()
                if rawMetatableCheck() then
                    runtimeKill("Runtime — Hook: game metatable tampered after start")
                end
            end)
            ...
            _pcall(function()
                for _, gname in _ipairs(REMOTE_SPY_GLOBALS) do
                    if _rawget(_G, gname) ~= nil then
                        runtimeKill("Runtime — Remote Spy: spy global appeared: " .. gname)
                        break
                    end
                end
            end)

            -- getgenv spy presence
            if _getgenv_raw then
                _pcall(function()
                    local ok, genv = safe(_getgenv_raw)
                    if ok and genv then
                        for _, gname in _ipairs(REMOTE_SPY_GLOBALS) do
                            if _rawget(genv, gname) ~= nil then
                                runtimeKill("Runtime — Remote Spy: spy global in getgenv(): " .. gname)
                                break
                            end
                        end
                    end
            ...
            _pcall(function()
                if scanForSpyUI(game:GetService("CoreGui")) then
                    runtimeKill("Runtime — Remote Spy: spy UI appeared in CoreGui")
                end
            end)

            -- HTTP functions still clean
            _pcall(function()
                local HttpService = game:GetService("HttpService")
                if not isCClosure(game.HttpGet) then
                    runtimeKill("Runtime — HTTP Spy: game:HttpGet was hooked at runtime")
                end
                if not isCClosure(HttpService.GetAsync) then
                    runtimeKill("Runtime — HTTP Spy: HttpService:GetAsync was hooked at runtime")
                end
            end)
        end
    end)

    -- ── Runtime: Dex Explorer checks ─────────────────────────────────────────
    _task_spawn(function()
        while runtimeActive do
            _task_wait(CHECK_INTERVAL + 0.7)  -- slightly staggered from remote spy checks
            if not runtimeActive then break end

            -- CoreGui dex scan
            _pcall(function()
                if deepScanForDex(game:GetService("CoreGui")) then
                    runtimeKill("Runtime — Dex Explorer: explorer UI appeared in CoreGui")
                end
            end)
            ...
            _pcall(function()
                local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if deepScanForDex(pg) then
                    runtimeKill("Runtime — Dex Explorer: explorer UI appeared in PlayerGui")
                end
            end)
            ...
            _pcall(function()
                for _, gname in _ipairs(DEX_GLOBALS) do
                    if _rawget(_G, gname) ~= nil then
                        runtimeKill("Runtime — Dex Explorer: explorer global appeared: " .. gname)
                        break
                    end
                end
            end)
    ...
    -- ── Runtime: Environment leak / advanced hook checks ─────────────────────
    _task_spawn(function()
        while runtimeActive do
            _task_wait(CHECK_INTERVAL + 1.3)  -- further staggered
            if not runtimeActive then break end

            _pcall(function()
                if envLeakCheck("runtime") then
                    runtimeKill("Runtime — Hook Detection: exploit env leaked through error stack")
                end
            end)
            ...
            _pcall(function()
                for _, entry in _ipairs(CRITICAL_GLOBALS) do
                    local live = _rawget(_G, entry.name) or entry.fn
                    if live ~= nil and not isCClosure(live) then
                        runtimeKill("Runtime — Hook: core global '" .. entry.name .. "' hooked")
                        break
                    end
                end
            end)
    ...
    -- ── Runtime: Timing ratio check (slower cycle) ───────────────────────────
    _task_spawn(function()
        -- Initial delay so game can settle before first timing measurement
        _task_wait(TIMING_INTERVAL)
        while runtimeActive do
            _pcall(function()
                if timingRatioCheck() then
                    runtimeKill("Runtime — Hook Detection: __namecall timing anomaly (" ..
                        timingViolations .. " consecutive violations)")
                end
            end)
            _task_wait(TIMING_INTERVAL)
        end
    ...
    -- =========================================================================
    -- § 13.  FINAL GATE
    --        If we reach here, all synchronous checks passed.
    -- =========================================================================
    -- (no-op: execution falls through to the original script below)
end
-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║                    END OF PROTECTION SYSTEM v2.0                         ║
-- ║           Original script begins immediately below this line             ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
...
-- These must be declared BEFORE zK() is defined so Lua captures them
-- as proper upvalues. Declaring after causes pairs(nil) crash every event.
local gunPickupDebounce = {}       -- [playerName] = tick() of last pickup notify
local pendingHeroUntil  = {}       -- [playerName] = tick()+2 hero lock against zK reset
...
local function zK()
 ...
 -- Protect pending-hero promotions against server overwriting them
 local now = tick()
 ...
 -- Scan Character and Backpack for Gun tool → Hero
 ...
 local bp = plr:FindFirstChild("Backpack")  -- nil-safe
 if bp then
...
GameplayEvents.Fade.OnClientEvent:Connect(function(fadeData)
 -- Guard: server can send nil during lobby/intermission
 ...
 -- Fire role callbacks wrapped in task.spawn so task.wait inside them
 -- doesn't block this event handler (was silently failing before)
...
task.spawn(function()
 ...
 for _, cb in pairs(state.roleCallbacks or {}) do
 task.spawn(cb)   -- task.spawn so task.wait inside cb doesn't block
 end
 end
end)
...
-- gunPickupDebounce and pendingHeroUntil are declared near top of file
-- (before zK) so they are proper upvalues. These comments remain for context.
...
task.spawn(function()
    while true do
        ...
        if coin then
            ...
            -- Remove from cache ONLY — let the server destroy the actual coin.
            -- Setting coin.Parent = nil locally strips the mesh before the server
            -- confirms collection, which is why "Optimize Coins" was causing the
            -- texture-disappear effect. Visual stays until server removes it.
...
local function getTracker(player)
    ...
    pTrackers[player] = {
        ...
        accel       = Vector3.new(),
        lastVel     = vel,
        -- 24-sample ring buffer: ~400ms history at 60 fps
        velHistory  = {},
        posHistory  = {},
        -- ── Jump state machine ──────────────────────────────────
        -- Kalman smoothing destroys the initial jump velocity burst
        -- (~50 studs/s over 1 frame). We capture the raw peak instead.
        jumpState   = "none",   -- "none" | "ascending" | "peak" | "descending"
        jumpStartVelY = 0,      -- raw velY at jump initiation (pre-Kalman)
        jumpStartT    = 0,      -- tick() when jump was first detected
        jumpStartY    = 0,      -- rootY when jump was initiated
        -- ── Surface tracking ────────────────────────────────────
        -- When standing on a platform/object, rootY can be any height.
        -- Track how long Y has been stable to classify "on object" vs airborne.
        surfaceY      = pos.Y,  -- last confirmed ground contact Y
        surfaceT      = tick(), -- when surface was last confirmed
        stableYFrames = 0,      -- consecutive frames within ±0.15 Y = grounded
    }
...
--   • Jump state machine: captures raw velY at jump onset before Kalman lag
--   • Surface tracking: records surfaceY when grounded for platform detection
--   • 24-sample ring buffer (was 16) — covers ~400ms for long-lead shots
--   • Stable-Y counter: consecutive frames within ±0.15 Y = definitively grounded
local function phantomUpdate(player)
    ...
    -- ── Jump state machine ──────────────────────────────────────────
    -- States: none → ascending (jump detected) → peak (velY~0 going down)
    --                          → descending → none (landed)
    -- We capture jumpStartVelY = raw velY the frame it first spikes,
    -- before Kalman has time to smooth it. This gives us the true
    -- initial jump velocity for parabolic arc prediction.
    local prevVelY = t.lastVel.Y
    if t.jumpState == "none" then
        -- Detect jump: raw velY spikes from ~0 to >15 in one frame
        if rawVelY > 15 and prevVelY < 8 then
            t.jumpState    = "ascending"
            t.jumpStartVelY = rawVelY       -- capture RAW (not Kalman)
            t.jumpStartT   = now
            t.jumpStartY   = pos.Y
        end
    elseif t.jumpState == "ascending" then
        -- Track ascending peak — update raw velY if still climbing
        if rawVelY > t.jumpStartVelY * 0.6 then
            t.jumpStartVelY = math.max(t.jumpStartVelY, rawVelY)
        end
        -- Transition to peak when velY drops near zero
        if rawVelY < 2.0 then
            t.jumpState = "peak"
        end
        -- Failsafe: if we've been ascending >1.5s something is wrong, reset
    ...
    elseif t.jumpState == "descending" then
        -- Landed: Y velocity near zero AND Y position is stable
    ...
    -- ── Surface Y tracking ──────────────────────────────────────────
    -- Count consecutive frames where Y is stable (within ±0.2 studs)
    ...
    if math.abs(pos.Y - prevPos.Y) < 0.2 and math.abs(rawVelY) < 3.0 then
        ...
        -- After 3+ stable frames, record this as the confirmed surface Y
...
-- ── Velocity trend extrapolation v2 ──────────────────────────────
-- Weighted least-squares regression on velocity history.
-- Separate treatment for XZ (horizontal, affected by input) and
-- Y (vertical, affected by gravity) components:
--   • XZ: standard WLS with half-life 115ms — captures turn timing
--   • Y:  if jumping (state machine), use physics arc instead of regression
--         because gravity dominates and regression would under-predict apex
--
-- Returns: projected velocity at (now + dt_ahead)
local function getVelAtTime(tr, dt_ahead)
    ...
    for i = math.max(1, n - 15), n do   -- last 16 samples (~266ms at 60fps)
        local age  = now - h[i].t
        local w    = math.exp(-age * 6)  -- half-life ≈ 115ms
    ...
    -- ── Y component override during jump state ───────────────────
    -- When jumping, regression sees a mix of rising + falling velocity
    -- which averages to ~zero slope — completely wrong.
    -- Instead use physics: v_y(t) = v_y0 - g*t
    -- where v_y0 is the raw captured jump velocity (jumpStartVelY).
...
-- ================================================================
-- solvePremiumIntercept  v5.0  — Jump / Run / Walk / Platform accurate
--
-- Scenario-specific improvements over v4:
--
--   JUMP:
--     • Raw jump velocity (jumpStartVelY) from state machine — not Kalman
--       smoothed. Kalman under-reports initial burst by ~15-25%.
--     • Time-elapsed compensation: adjusts for frames already in the arc.
--     • Ascending/descending treated with full parabola — no apex miss.
--
--   WALK / RUN (straight-line direction accuracy):
--     • agreement > 0.85 → inputBlend raised to 0.50 (trust input more)
--     • agreement < 0.45 → inputBlend = 0.05  (turning: trust trend only)
--     • Prevents over-steering mid-turn AND under-leading straight runs.
--
--   ON TOP OF OBJECT / PLATFORM:
--     • 5-ray cross-pattern ground detection — no more false airborne at
--       platform edges or box corners.
--     • Platform-relative aimY: uses tr.surfaceY (stable Y from tracker)
--       so aim anchors correctly on any elevated surface.
--     • Terrain micro-oscillation filter: vel.Y < 2.0 while grounded
--       stripped entirely — stops bumpy-mesh vertical jitter.
--
--   GENERAL:
--     • Jitter pads: ascending / descending (was jump / fall — merged peak)
--     • leadTime cap raised to 0.700 s for >400 ms ping
--     • Corner-peek expanded to 14 candidates including diagonals
-- ================================================================
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- ── 2. Ground detection — 5-ray cross pattern ────────────────────
    -- Single centre ray misses platform edges and thin box corners.
    -- Four offset rays at 0.6 studs catch those edge cases.
    local rpGnd = RaycastParams.new()
    rpGnd.FilterType = Enum.RaycastFilterType.Blacklist
    rpGnd.FilterDescendantsInstances = {char}
    local function castGnd(offset)
        return workspace:Raycast(root.Position + offset, Vector3.new(0, -5.2, 0), rpGnd)
    end
    ...
    if not isOnGround then isOnGround = castGnd(Vector3.new( 0,   0.8, -0.6)) ~= nil end
    -- Stable-Y override: 4+ consecutive stable frames = definitively grounded
    if not isOnGround and tr and tr.stableYFrames >= 4 then
        isOnGround = true
    end

    -- ── 3. Vertical state classification ────────────────────────────
    ...
    -- Jump state machine overrides raw check (more reliable)
    if tr then
        if tr.jumpState == "ascending" then
            isJumping = true; isFalling = false; isAirborne = true; isOnGround = false
        elseif tr.jumpState == "peak" then
            isJumping = false; isFalling = false; isAirborne = true; isOnGround = false
        elseif tr.jumpState == "descending" then
            isJumping = false; isFalling = true; isAirborne = true; isOnGround = false
        end
    end
    ...
    -- ── 5. Trend-projected velocity (dual-pass) ───────────────────────
    ...
    -- ── 7. Velocity blend — agreement-aware (walk/run direction fix) ──
    -- If trend and input strongly agree (>0.85), boost inputBlend to 0.50
    -- so straight-line walking/running is fully predicted.
    -- If disagreeing (<0.45, i.e. turning), drop to 0.05 (trust trend).
    ...
    if hum then
        ...
        elseif agreement > 0.85 then
            baseBlend = 0.50  -- straight walk/run: trust input fully
        elseif agreement < 0.45 then
            baseBlend = 0.05  -- sharp turn: trust trend only
        else
    ...
    -- ── 8. Second-order kinematics (XZ only — Y handled separately) ──
    local accelVec = tr and (getVelAtTime(tr, leadTime) - trendVel) or Vector3.new()
    accelVec = Vector3.new(accelVec.X, 0, accelVec.Z)  -- strip Y here
    ...
    -- ── 9. Vertical Y — scenario-aware physics ───────────────────────
    ...
    -- Platform-relative base: use surfaceY so aim is anchored to the
    -- actual platform height even when rootY oscillates on mesh terrain
    local platformBase = (tr and tr.stableYFrames >= 3) and tr.surfaceY or rootY
    local aimY = platformBase + bodyH * 0.60  -- upper-chest

    local finalY
    if isAirborne then
        if tr and (tr.jumpState == "ascending" or tr.jumpState == "peak") then
            -- ASCENDING: full physics arc from jump start using raw velY
            -- Avoids Kalman under-estimate of initial burst (~15-25% error)
            local elapsed      = tick() - tr.jumpStartT
            local tTotal       = elapsed + leadTime
            -- y-displacement from jump start: v0*t - 0.5*g*t²
            ...
            finalY = aimY + futureDisp
        else
            -- DESCENDING / general airborne: Kalman velY + full gravity arc
            local gravDrop = 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
            finalY = aimY + kalmanVelY * leadTime - gravDrop
        end
        local predRootY  = rootY + kalmanVelY * leadTime - 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
        local predHeadY  = predRootY + bodyH
        local clampMin   = math.min(predRootY, rootY - 3.0)
        local clampMax   = math.max(predHeadY + 0.5, headY + 3.0)
        finalY = math.clamp(finalY, clampMin, clampMax)
    else
        -- GROUNDED / ON PLATFORM:
        -- Micro-oscillation filter: strip vel.Y < 2.0 (terrain mesh noise)
        local velYContrib = (math.abs(rawVelY) >= 2.0) and (rawVelY * leadTime * 0.10) or 0
        finalY = aimY + velYContrib
        finalY = math.clamp(finalY, rootY + 0.2, headY + 0.3)
    end

    predicted = Vector3.new(predicted.X, finalY, predicted.Z)

    -- ── 10. LOS / corner-peek (14 candidates) ────────────────────────
    if fromPos then
        ...
        if wasLOS then
            ...
            for _, off in ipairs(offsets) do
                local cand = rp + off
                if hasLOS(fromPos, cand, char) then predicted = cand; found = true; break end
            end
...
local function onRoundStart()
 -- 1. Flash the local player's role on screen (immediate)
 flashRoleLabel(localRole)

 -- 2. Auto notify: spawn so task.wait doesn't block the Fade event handler
 if autoNotifyEnabled then
 task.spawn(function()
 task.wait(0.8)  -- wait for role data to fully settle
 NotifyMurdererPerk()
 end)
 end
end
...
-- ================================================================
-- COIN OPTIMIZER ENGINE
-- ================================================================
-- The Optimize Coins button does NOT touch coin textures, meshes,
-- or any visual properties. It is purely a cache and path optimizer.
--
-- What it does:
--   1. Full cache rebuild — prunes dead entries + reseeds from workspace
--   2. Stale-entry cleanup — removes coins whose Parent is nil
--   3. Pre-sorts the cache into a nearest-first ordered list so the
--      farm loop always grabs the closest coin without scanning
--   4. Resets the farm operator state so the loop restarts cleanly
--   5. Reports detailed stats (before/after, stale pruned, sorted)
--   6. Auto-optimizer: fires at round start (Fade event) and on death
-- ================================================================

local coinSortedList    = {}   -- ordered array {coin, dist} for fast pickup
local coinOptimizeCount = 0    -- how many times optimized this session

local function optimizeCoinCache()
    local before = 0
    for _ in pairs(liveCoinCache) do before = before + 1 end

    -- Step 1: prune stale entries (coins collected without firing DescendantRemoving)
    local stale = 0
    for coin in pairs(liveCoinCache) do
    ...
    -- Step 2: full rescan to catch any coins missed by the event listener
    -- (Can happen if script was injected mid-round after coins spawned)
    local rescanned = 0
    for _, v in ipairs(workspace:GetDescendants()) do
    ...
    -- Step 3: pre-sort into nearest-first list for fast farm pickup
    -- Build array from current cache
    ...
    -- Step 4: reset farm state so loop restarts from nearest coin
    AutoCoinOperator = false
    CoinFound        = false
    CurrentTarget    = nil
...
-- Auto-optimize on round start (Fade event registered via roleCallbacks)
state.roleCallbacks["CoinCacheOptimizer"] = function()
    task.spawn(function()
        task.wait(1.0)  -- wait for coins to finish spawning
        optimizeCoinCache()
    end)
end
...
Tabs.Farming:AddParagraph({
    ...
    Content = "Automatically runs at each round start. Press manually if coins are being skipped. NEVER removes coin textures — pure performance optimization only.",
})

Tabs.Farming:AddButton({
    Title = "⚡ Optimize Coins",
    Description = "Rebuild cache + prune dead entries + sort nearest-first",
    Callback = function()
        local before, after, stale, rescanned = optimizeCoinCache()
        Fluent:Notify({
            Title = "⚡ Coin Optimizer",
            Content = string.format(
                "Cache: %d → %d coins | Pruned: %d stale | Found: +%d new | Sorted nearest-first | Run #%d",
