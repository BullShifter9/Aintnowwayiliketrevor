...
local GameplayEvents = ReplicatedStorage:WaitForChild("Remotes", 10) and ReplicatedStorage.Remotes:WaitForChild("Gameplay", 10)
...
local function getHWID()
    ...
    if type(KRNL_LOADED) ~= "nil" and KRNL_LOADED then
        local ok, id = pcall(function()
            local raw_hwid = rawget and rawget(_G, "HWID") or nil
            return raw_hwid
        end)
...
-- Safety shim: if executor does not support Drawing library,
-- use a no-op stub so the script still loads without crashing.
if not Drawing then
    local _stub = {}
    local _stubMT = {
        __index = function() return function() end end,
        __newindex = function() end,
        __call = function() return _stub end,
    }
    setmetatable(_stub, _stubMT)
    ...
    warn("[OmniHub] Drawing library not found — ESP disabled (stub active)")
end
...
-- Designed for 180ms+ ping accuracy.
--
-- Core improvements over the standard engine:
--  1. ITERATIVE INTERCEPT SOLVER — repeatedly refines the predicted
--     position by simulating how the bullet travels, then re-solving
--     where the target will be at bullet-arrival time.  Converges in
--     3-5 iterations even at 300ms ping.
--  2. ACCELERATION EXTRAPOLATION — derives a per-axis acceleration
--     vector from recent Kalman filter state changes and adds it to
--     the trajectory prediction (targets don't instantly change speed).
--  3. HIGH-PING GRAVITY CORRECTION — at >100ms ping, vertical
--     trajectory naturally deviates; we pre-compensate so the aim
--     point stays at chest level despite the extra travel time.
--  4. MULTI-ENSEMBLE VOTE — runs 3 independent predictions with
--     slightly varied physics and returns the confidence-weighted
--     centroid, suppressing outliers.
--  5. FLOOR / CEILING RAYCAST — prevents the aim-point from clipping
--     through the floor or ceiling even after the full offset is added.
-- ================================================================

-- Approximate in-game gun projectile speed (studs / second).
-- The actual Roblox MM2 sheriff gun is hitscan, but the server applies
-- lag compensation equal to ping.  We model this as a "virtual bullet"
-- whose travel time == ping so our intercept maths stays unified.
local BULLET_SPEED = 350  -- studs/s  (effectively instantaneous hitscan on LAN)

-- Extract Kalman-derived acceleration for a player (smoothed delta of velocity)
local _prevSmoothedVel = {}
local _prevSmoothedTime = {}

local function getSmoothedAccel(player)
 local now = tick()
 local cur = getSmoothedVel(player)
 local prevV = _prevSmoothedVel[player]
 local prevT = _prevSmoothedTime[player]
 _prevSmoothedVel[player] = cur
 _prevSmoothedTime[player] = now
 if not prevV or not prevT then return Vector3.new() end
 local dt = now - prevT
 if dt <= 0 or dt > 0.5 then return Vector3.new() end
 return (cur - prevV) / dt
end

-- Single premium prediction sample
local function _premiumSample(player, pingOverride)
 local char = player.Character; if not char then return nil end
 local root = char:FindFirstChild("HumanoidRootPart"); if not root then return nil end
 local hum  = char:FindFirstChild("Humanoid"); if not hum then return nil end
 ...
 local pos     = root.Position
 local vel     = getSmoothedVel(player)
 local accel   = getSmoothedAccel(player)
 local moveDir = hum.MoveDirection
 local wSpeed  = hum.WalkSpeed

 -- ── Step 1: initial "where will they be at t=ping?" ─────────────
 -- pos(t) = pos + vel*t + 0.5*accel*t²  (kinematic)
 local t = ping
 local kinematic = pos + vel * t + accel * (0.5 * t * t)

 -- ── Step 2: input-bias (player is pressing WASD / joystick) ──────
 -- We weight the MoveDirection heavily because at high ping the player
 -- will keep walking the same direction for most of the ping window.
 local inputBias = moveDir * wSpeed * t * 0.45

 -- ── Step 3: iterative intercept solve (3 passes) ─────────────────
 -- Bullet travels from shooter to target.  We don't know the exact
 -- travel time until we know where the target will be, and we don't
 -- know where the target will be until we know the travel time.
 -- Iterate: guess tTarget = ping, compute target pos, compute bullet
 -- flight time to that pos, recompute target pos, repeat.
 local myChar = LocalPlayer.Character
 local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
 local shootOrigin = myRoot and myRoot.Position or pos

 local tTarget = ping
 local bestPos = kinematic + inputBias
 for _ = 1, 4 do
 local dist = (bestPos - shootOrigin).Magnitude
 -- bullet flight time (for hitscan == 0, but server lag adds ping)
 tTarget = dist / BULLET_SPEED + ping
 tTarget = math.clamp(tTarget, ping, ping + 0.25)
 bestPos = pos + vel * tTarget + accel * (0.5 * tTarget * tTarget) + moveDir * wSpeed * tTarget * 0.45
 end

 -- ── Step 4: seismic/overflow micro-correction ─────────────────────
 local pSE = seismicPredict(root, vel)
 local pOV = overflowPredict(root, hum, vel)
 local micro = Vector3.new(
 (pOV.X - pos.X) * 0.15 + (pSE.X - pos.X) * 0.10,
 (pOV.Y - pos.Y) * 0.15 + (pSE.Y - pos.Y) * 0.10,
 (pOV.Z - pos.Z) * 0.15 + (pSE.Z - pos.Z) * 0.10
 )

 -- ── Step 5: high-ping gravity droop correction ────────────────────
 -- At >80ms ping, the server lag window is long enough that gravity
 -- pulls aim down noticeably; compensate upward.
 local gravityDroop = Vector3.new(0, workspace.Gravity * tTarget * tTarget * 0.012, 0)

 -- ── Combine all terms ─────────────────────────────────────────────
 local rawOffset = (bestPos - pos) + micro + gravityDroop
 local clampedY  = math.clamp(rawOffset.Y, -3.5, 3.5)
 local predicted = pos + Vector3.new(rawOffset.X, clampedY, rawOffset.Z)
 ...
 rp.FilterType = Enum.RaycastFilterType.Exclude
 rp.FilterDescendantsInstances = {char}
 local fR = workspace:Raycast(predicted, Vector3.new(0, -7, 0), rp)
 local cR = workspace:Raycast(predicted, Vector3.new(0,  7, 0), rp)
 if fR then predicted = Vector3.new(predicted.X, math.max(predicted.Y, fR.Position.Y + 1.5), predicted.Z) end
 if cR then predicted = Vector3.new(predicted.X, math.min(predicted.Y, cR.Position.Y - 0.5), predicted.Z) end
...
-- Multi-ensemble premium solver — run 3 samples with slight variations,
-- return the weighted centroid (suppresses outlier predictions).
local function solveInterceptPremium(player)
 local ping = getPing()

 -- Sample A: nominal ping
 local sA = _premiumSample(player, ping)
 if not sA then return nil end

 -- Sample B: ping + 15ms  (upper bound estimate)
 local sB = _premiumSample(player, ping + 0.015) or sA

 -- Sample C: ping - 10ms  (lower bound estimate)
 local sC = _premiumSample(player, math.max(ping - 0.010, 0.005)) or sA

 -- Confidence: A=0.5, B=0.3, C=0.2  (weight nominal most)
 local total = 0.5 + 0.3 + 0.2
 local centroid = Vector3.new(
 (sA.X*0.5 + sB.X*0.3 + sC.X*0.2) / total,
 (sA.Y*0.5 + sB.Y*0.3 + sC.Y*0.2) / total,
 (sA.Z*0.5 + sB.Z*0.3 + sC.Z*0.2) / total
 )

 return centroid
end

-- Premium knife intercept: same ensemble but tuned for knife projectile speed (~110 studs/s)
local KNIFE_SPEED = 110  -- approximate MM2 knife throw speed

local function _premiumKnifeSample(player, pingOverride)
 local char = player.Character; if not char then return nil end
 local root = char:FindFirstChild("HumanoidRootPart"); if not root then return nil end
 local hum  = char:FindFirstChild("Humanoid"); if not hum then return nil end
 ...
 -- Iterative solve for knife (slower projectile = more travel time = more offset needed)
 local tTarget = ping
 local bestPos = pos + vel * ping + moveDir * wSpeed * ping * 0.5
 for _ = 1, 5 do
 local dist = (bestPos - shootOrigin).Magnitude
 tTarget = dist / KNIFE_SPEED + ping
 tTarget = math.clamp(tTarget, ping, ping + 0.5)
 bestPos = pos + vel * tTarget + accel * (0.5 * tTarget * tTarget) + moveDir * wSpeed * tTarget * 0.5
 end

 -- Gravity droop compensation (knife is a real projectile — gravity matters more)
 local gravityDroop = Vector3.new(0, workspace.Gravity * tTarget * tTarget * 0.018, 0)

 local rawOffset = (bestPos - pos) + gravityDroop
 local clampedY  = math.clamp(rawOffset.Y, -4, 4)
 local predicted = pos + Vector3.new(rawOffset.X, clampedY, rawOffset.Z)
 predicted = Vector3.new(predicted.X, predicted.Y + 1.5, predicted.Z)
...
local function solveKnifeInterceptPremium(player)
 local ping = getPing()
 local sA = _premiumKnifeSample(player, ping)           if not sA then return nil end
 local sB = _premiumKnifeSample(player, ping + 0.020)  or sA
 local sC = _premiumKnifeSample(player, math.max(ping - 0.012, 0.005)) or sA
 return Vector3.new(
 sA.X*0.5 + sB.X*0.3 + sC.X*0.2,
 sA.Y*0.5 + sB.Y*0.3 + sC.Y*0.2,
 sA.Z*0.5 + sB.Z*0.3 + sC.Z*0.2
 )
end

-- Premium mode state
local premiumSilentAimEnabled = false
...
local function doFusionShoot(targetPlayer)
 ...
 -- Premium mode uses the iterative multi-ensemble solver for high-ping accuracy.
 local predicted = (premiumSilentAimEnabled and solveInterceptPremium(targetPlayer))
 or solveIntercept(targetPlayer)
 ...
 local freshPredicted = (premiumSilentAimEnabled and solveInterceptPremium(targetPlayer))
 or solveIntercept(targetPlayer)
 or (mHead and mHead.Position)
 or mRoot.Position
...
local function doKnifeThrow(targetPlayer)
 ...
 -- KC8WAJ6 knife formula: vel/3 with Y/1.5
 -- Premium mode uses iterative physics solver for high-ping accuracy.
    phantomUpdate(targetPlayer)
    local predicted = (premiumSilentAimEnabled and solveKnifeInterceptPremium(targetPlayer))
                   or solveKnifeIntercept(targetPlayer)
 ...
 local freshPredicted = (premiumSilentAimEnabled and solveKnifeInterceptPremium(targetPlayer))
                            or solveKnifeIntercept(targetPlayer)
                            or (tHead and tHead.Position)
                            or tRoot.Position + Vector3.new(0, 1.5, 0)
        pcall(function()
...
-- Manual keybind: default T (PC only)
...
-- ================================================================
-- MOBILE KNIFE THROW BUTTON
-- A draggable on-screen button shown only on TouchEnabled devices.
-- Appears / disappears based on knifeThrowState.enabled toggle.
-- ================================================================
local isMobileDevice = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled

local MobileKnifeGui = Instance.new("ScreenGui")
MobileKnifeGui.Name = "MobileKnifeGui"
MobileKnifeGui.ResetOnSpawn = false
MobileKnifeGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
MobileKnifeGui.DisplayOrder = 99
MobileKnifeGui.Parent = game:GetService("CoreGui")

local MobileKnifeBtn = Instance.new("TextButton")
MobileKnifeBtn.Name = "KnifeThrowBtn"
MobileKnifeBtn.Size = UDim2.new(0, 90, 0, 90)
MobileKnifeBtn.Position = UDim2.new(0.5, -45, 0.85, -45)
MobileKnifeBtn.AnchorPoint = Vector2.new(0, 0)
MobileKnifeBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
MobileKnifeBtn.BackgroundTransparency = 0.2
MobileKnifeBtn.BorderSizePixel = 0
MobileKnifeBtn.Text = "🗡️\nTHROW"
MobileKnifeBtn.TextColor3 = Color3.fromRGB(255, 80, 80)
MobileKnifeBtn.TextSize = 16
MobileKnifeBtn.Font = Enum.Font.GothamBold
MobileKnifeBtn.Visible = false  -- hidden until toggle is on
MobileKnifeBtn.Active = true
MobileKnifeBtn.Selectable = false
MobileKnifeBtn.Parent = MobileKnifeGui

local MobileKnifeCorner = Instance.new("UICorner")
MobileKnifeCorner.CornerRadius = UDim.new(0, 18)
MobileKnifeCorner.Parent = MobileKnifeBtn

local MobileKnifeStroke = Instance.new("UIStroke")
MobileKnifeStroke.Color = Color3.fromRGB(255, 60, 60)
MobileKnifeStroke.Thickness = 2
MobileKnifeStroke.Parent = MobileKnifeBtn

-- Dragging logic for the mobile button
do
 local dragging, dragStart, startPos = false, nil, nil
 MobileKnifeBtn.InputBegan:Connect(function(input)
 if input.UserInputType == Enum.UserInputType.Touch then
 dragging = true
 dragStart = input.Position
 startPos = MobileKnifeBtn.Position
 ...
 MobileKnifeBtn.InputChanged:Connect(function(input)
 if dragging and input.UserInputType == Enum.UserInputType.Touch then
 local delta = input.Position - dragStart
 -- Only treat as drag if moved more than 8px; otherwise it's a tap
 if delta.Magnitude > 8 then
 MobileKnifeBtn.Position = UDim2.new(
...
-- Tap = throw
MobileKnifeBtn.Activated:Connect(function()
 if not knifeThrowState.enabled then return end
 if localRole ~= "Murderer" then
 Fluent:Notify({ Title = "Knife Throw", Content = "You must be the Murderer.", Duration = 3 })
 return
 end
 ...
 local orig = MobileKnifeBtn.BackgroundColor3
 MobileKnifeBtn.BackgroundColor3 = Color3.fromRGB(200, 30, 30)
 task.spawn(function() doKnifeThrow(target) end)
 task.delay(0.15, function()
 MobileKnifeBtn.BackgroundColor3 = orig
 end)
 else
 Fluent:Notify({
 Title = "Knife Throw",
 Content = string.format("No player within %d studs.", knifeThrowState.throwRange),
 Duration = 2
 })
 end
end)

-- Hook into the knife throw toggle so the mobile button appears/disappears
-- We patch the existing toggle callback via a wrapper that fires AFTER the original.
-- (The original callback is already registered above via AddToggle — we use
-- a RunService loop to keep Visible in sync rather than patching the closure.)
RunService.Heartbeat:Connect(function()
 if not MobileKnifeBtn then return end
 local shouldShow = knifeThrowState.enabled and isMobileDevice
 if MobileKnifeBtn.Visible ~= shouldShow then
 MobileKnifeBtn.Visible = shouldShow
 end
end)
...
-- ── Premium Silent Aim ───────────────────────────────────────────
Tabs.Combat:AddToggle("PremiumSilentAimToggle", {
 Title = "⭐ Premium Silent Aim (High-Ping Accurate)",
 Description = "Iterative physics solver with multi-ensemble voting. Stays accurate even at 180ms+ ping. Overrides standard Silent Aim prediction when enabled.",
 Default = false,
 Callback = function(toggle)
 premiumSilentAimEnabled = toggle
 Fluent:Notify({
 Title = "⭐ Premium Silent Aim",
 Content = toggle
 and string.format(
 "PREMIUM ON — iterative solver active. Current ping: ~%dms",
 math.floor(getPing() * 1000)
 )
 or "Premium Silent Aim DISABLED — back to standard engine.",
 Duration = 4
 })
 end
})
...
Tabs.Combat:AddParagraph({
 ...
 Content = "On mobile, a draggable [🗡️ THROW] button will appear on-screen when Knife Throw is enabled. Drag it to reposition, tap to throw."
})

Tabs.Combat:AddParagraph({
 Title = "Premium Engine",
 Content = "Enable ⭐ Premium Silent Aim above to use the iterative physics solver for knife throws — works great at 180ms+ ping."
})
...
-- ================================================================
-- VALUE CHECKER DATABASE — GitHub-Fetched
--
-- The full item database is hosted as a JSON file on GitHub.
-- URL is set in DB_RAW_URL below — update it once you upload the
-- mm2_sv_database.json file to your repo.
--
-- Fallback: if the fetch fails (no internet, wrong URL, executor
-- blocks HTTP), a minimal built-in table catches the most common
-- high-value items so the script still works offline.
-- ================================================================

local DB_RAW_URL = "https://raw.githubusercontent.com/BullShifter9/Aintnowwayiliketrevor/refs/heads/main/Xkosoeoficjd_database.json"
-- ↑ Replace YOUR_USERNAME and YOUR_REPO with your actual GitHub details
-- after uploading mm2_sv_full_database.json to your repository.

-- Minimal fallback database (top items only — used if GitHub is unreachable)
local SV_FALLBACK = {
 ...
 ["traveler's gun"]        = { name="Traveler's Gun",        value=4200,   demand=6, rarity="ancient" },
 ["corrupt"]                = { name="Corrupt",                value=800,    demand=4, rarity="unique" },
 ["zombie dog"]             = { name="Zombie Dog",             value=450,    demand=3, rarity="pet" },
}

local SV_DATABASE = SV_FALLBACK   -- will be replaced by fetched data

-- ValueChecker wrapper
local ValueChecker = {
 cache = SV_DATABASE,  -- starts as fallback; replaced on successful fetch
 loaded = false,       -- true once GitHub data is in cache
 loading = true,
 autoTradeVal = true,
}

-- GitHub JSON fetch (async, background)
-- When complete, ValueChecker.cache is hot-swapped to the full dataset.
task.spawn(function()
    local function tryFetch(url)
        local httpFn = (type(http_request)=="function" and http_request)
        ...
        local ok2, body = pcall(function() return game:HttpGet(url, true) end)
        if ok2 and type(body)=="string" and #body > 10 then return body end
        return nil
    end

    local body = tryFetch(DB_RAW_URL)

    if not body then
        ValueChecker.loading = false
        ValueChecker.loaded  = true
        Fluent:Notify({
            Title   = "Value DB",
            Content = "Could not reach GitHub. Using built-in fallback (limited items).\n\nSet DB_RAW_URL in the script to your raw GitHub JSON link.",
            Duration = 8
        })
        return
    end
    ...
    if not ok or type(parsed) ~= "table" then
        ...
        Fluent:Notify({
            Title   = "Value DB",
            Content = "GitHub JSON failed to parse. Using fallback.",
            Duration = 6
        })
        return
    end
    ...
    for key, entry in pairs(parsed) do
        if type(entry) == "table" then
            newCache[tostring(key):lower()] = {
                name   = tostring(entry.name   or key),
                value  = tonumber(entry.value  or 0),
                demand = tonumber(entry.demand or 0),
                rarity = tostring(entry.rarity or "misc"),
                trend  = tostring(entry.trend  or "stable"),
            }
            count = count + 1
        end
    end

    ValueChecker.cache   = newCache
    ValueChecker.loading = false
    ValueChecker.loaded  = true

    Fluent:Notify({
        Title   = "Value DB Loaded",
        Content = string.format("%d items loaded from GitHub!", count),
        Duration = 5
    })
end)

-- Ensure fallback entries have .trend field
...
do
 -- Count items in whatever cache is currently loaded (fallback or fetched)
 local n = 0
 for _ in pairs(ValueChecker.cache) do n = n + 1 end
 local statusLine = ValueChecker.loaded
  and string.format("%d items loaded | supremevalues.com", n)
  or  "Fetching from GitHub... (loading)"
 Tabs.Values:AddParagraph({
  Title = "Database Info",
  Content = statusLine .. "\nChromas • Ancients • Vintages • Godlies • Legendaries • Rares • Uncommons • Uniques • Pets"
 })
end
