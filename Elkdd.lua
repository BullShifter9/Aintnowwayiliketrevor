...
-- ── FIX: premiumSilentAim was never declared — caused "attempt to index nil with 'enabled'" ──
local premiumSilentAim = {
    enabled              = false,
    lastShot             = 0,
    cooldown             = 0.25,
    _clickLatencyOffset  = nil,
}

-- ═══════════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════════════
-- PHYSICS SIMULATION PREDICTOR v4 — High-Ping Accurate, Human-Coded
-- Exact Roblox character controller replica with improvements:
--
--  POSITION (Verlet — photo line ~1423):
--    SimPos.X += SimVel.X*dt + FrictionDecel * MoveDir.X * dt²/2
--    SimPos.Y += SimVel.Y*dt + (-Gravity)    * dt²/2
--    SimPos.Z += SimVel.Z*dt + FrictionDecel * MoveDir.Z * dt²/2
--
--  VELOCITY per-axis goal-seeking (photo ~1427-1460):
--    Goal = MoveDir[Axis] * WalkSpeed * (16.001/16)
--    curVel > Goal  →  vel -= FrictionDecel * |CalcMoveDir| * dt  (clamp ≥ Goal)
--    curVel < Goal  →  vel += FrictionDecel * |CalcMoveDir| * dt  (clamp ≤ Goal)
--    else           →  snap to Goal
--
--  IMPROVEMENTS over photo:
--    + Kalman-smoothed velocity seed (no raw network-noisy vel)
--    + Goal factor = 16.001/16 = 1.000625 (exact photo value)
--    + High-ping scaling: steps cover ping*(1 + pingBonus)
--    + Sub-step Interval = 1/120 when ping > 150ms
--    + Jump-state-aware Y from Kalman tracker
-- ═══════════════════════════════════════════════════════════════════

local WalkSpeedScale = 16.001 / 16  -- matches photo's MoveDir * 16.001

-- CalculateMoveDirection: mirrors photo helper exactly
local function CalculateMoveDirection(MoveDirection, Axis, SimVel, WalkSpeed)
    local axisGoal = (Axis == "X") and MoveDirection.X or MoveDirection.Z
    local axisVel  = (Axis == "X") and SimVel.X        or SimVel.Z
    if math.abs(axisGoal) < 0.001 then return 1.0 end
    local goalFull = axisGoal * WalkSpeed * WalkSpeedScale
    if (goalFull > 0 and axisVel > goalFull) or
       (goalFull < 0 and axisVel < goalFull) then
        return 1.0
    end
    return math.abs(axisGoal)
end

local function physicsSimPredict(char, root, hum, targetPlayer, fromPos, ping)
    if not char or not root or not hum then return nil end
    -- Guard all numeric inputs against nil
    ping = tonumber(ping) or 0.066
    local GRAVITY    = tonumber(workspace.Gravity) or 196.2
    local WalkSpeed  = tonumber(hum.WalkSpeed) or 16
    local MoveDirection = hum.MoveDirection or Vector3.new(0,0,0)


    -- FrictionDeceleration: Roblox char reaches WalkSpeed from rest in ~8 frames
    local FrictionDeceleration = WalkSpeed * 8.5

    -- Sub-step at 120Hz when ping > 150ms
    local basePing = math.max(ping, 0.016)
    local Interval = (basePing > 0.150) and (1 / 120) or (1 / 60)

    -- Seed SimVel from Kalman tracker (removes network jitter)
    local tr = pTrackers[targetPlayer]
    local SimVel
    if tr then
        SimVel = Vector3.new(tr.kx.v, tr.ky.v, tr.kz.v)
    else
        local rv = root.AssemblyLinearVelocity
        SimVel   = Vector3.new(rv.X, rv.Y, rv.Z)
    end
    local SimPos = Vector3.new(root.Position.X, root.Position.Y, root.Position.Z)

    local myChar = LocalPlayer.Character
    local rpGnd  = RaycastParams.new()
    rpGnd.FilterType = Enum.RaycastFilterType.Blacklist
    rpGnd.FilterDescendantsInstances = myChar and {char, myChar} or {char}

    local function castFloor(pos)
        return workspace:Raycast(pos + Vector3.new(0, 1.5, 0), Vector3.new(0, -8.0, 0), rpGnd)
    end

    local initFloor = castFloor(SimPos)
    local rootH     = initFloor
        and math.clamp(SimPos.Y - initFloor.Position.Y, 1.5, 5.5) or 2.9

    local jumpState  = tr and tr.jumpState or "none"
    local isGrounded = (initFloor ~= nil) and (SimVel.Y < 5.0) and (jumpState == "none")

    -- High-ping compensation: add 20% per 100ms ping above 60ms
    local pingMs    = ping * 1000
    local pingBonus = math.max(0, (pingMs - 60) / 100) * 0.20
    local totalLead = basePing * (1 + pingBonus)
    local steps     = math.clamp(math.ceil(totalLead / Interval), 1, 80)

    local Couple = {"X", "Z"}

    for _ = 1, steps do
        -- STEP 1: Verlet position (photo line ~1423)
        local halfDtSq = Interval * Interval * 0.5
        local accelX   = FrictionDeceleration * MoveDirection.X
        local accelZ   = FrictionDeceleration * MoveDirection.Z
        local accelY   = isGrounded and 0 or -GRAVITY
        SimPos = Vector3.new(
            SimPos.X + SimVel.X * Interval + accelX * halfDtSq,
            SimPos.Y + SimVel.Y * Interval + accelY * halfDtSq,
            SimPos.Z + SimVel.Z * Interval + accelZ * halfDtSq
        )

        -- STEP 2: Per-axis velocity goal-seeking (photo ~1427-1460)
        for _, Axis in ipairs(Couple) do
            local Goal   = ((Axis == "X") and MoveDirection.X or MoveDirection.Z)
                         * WalkSpeed * WalkSpeedScale
            local curVel = (Axis == "X") and SimVel.X or SimVel.Z
            local comp   = CalculateMoveDirection(MoveDirection, Axis, SimVel, WalkSpeed)
            local force  = FrictionDeceleration * math.abs(comp) * Interval
            local newVel
            if     curVel > Goal then newVel = math.max(curVel - force, Goal)
            elseif curVel < Goal then newVel = math.min(curVel + force, Goal)
            else                      newVel = Goal
            end
            if Axis == "X" then SimVel = Vector3.new(newVel, SimVel.Y, SimVel.Z)
            else                 SimVel = Vector3.new(SimVel.X, SimVel.Y, newVel) end
        end

        -- STEP 3: Gravity on Y (photo line ~1461)
        if not isGrounded then
            SimVel = Vector3.new(SimVel.X, SimVel.Y - GRAVITY * Interval, SimVel.Z)
        else
            SimVel = Vector3.new(SimVel.X, math.max(SimVel.Y, 0), SimVel.Z)
        end

        -- STEP 4: Floor clamp (photo ~1462-1465)
        local floorHit = castFloor(SimPos)
        if floorHit then
            local floorLevel = floorHit.Position.Y + rootH
            if SimPos.Y <= floorLevel + 0.06 then
                SimPos = Vector3.new(SimPos.X, floorLevel, SimPos.Z)
                SimVel = Vector3.new(SimVel.X, 0, SimVel.Z)
                isGrounded = true; jumpState = "none"
            else
                isGrounded = false
            end
        else
        ...
        -- STEP 5: Ceiling clamp (photo ~1466-1468)
        local ceilHit = workspace:Raycast(
            SimPos + Vector3.new(0, 0.8, 0), Vector3.new(0, 4.0, 0), rpGnd
        )
        if ceilHit and ceilHit.Position.Y <= SimPos.Y + rootH + 0.5 then
            SimPos = Vector3.new(SimPos.X, ceilHit.Position.Y - rootH, SimPos.Z)
            SimVel = Vector3.new(SimVel.X, math.min(SimVel.Y, 0), SimVel.Z)
        end
    end

    local head   = char:FindFirstChild("Head")
    local headCY = head and head.Position.Y or (root.Position.Y + 2.9)
    local bodyH  = math.max(headCY - root.Position.Y, 1.5)
    local aimY   = SimPos.Y + bodyH * 0.65
    return Vector3.new(SimPos.X, aimY, SimPos.Z), SimPos, bodyH
end

-- Intercept solver WITH LOS verification
local function solvePremiumIntercept(targetPlayer, fromPos)
    local ok, result = pcall(function()
        ...
        for _ = 1, 8 do phantomUpdate(targetPlayer) end
        local rawPing   = getPing()
        local extraLead = premiumSilentAim._clickLatencyOffset or 0
        local totalLead = rawPing + (1 / 60) + extraLead
        local predicted, simPos, bodyH = physicsSimPredict(char, root, hum, targetPlayer, fromPos, totalLead)
        if not predicted then return nil end
        if fromPos then
            local clearPos, wasLOS = bestClearPoint(fromPos, predicted, char)
            if wasLOS then return clearPos end
            if simPos and bodyH then
                for _, frac in ipairs({0.90, 0.65, 0.50, 0.35, 0.20}) do
                    local cand = Vector3.new(simPos.X, simPos.Y + bodyH * frac, simPos.Z)
                    if hasLOS(fromPos, cand, char) then return cand end
                end
            end
            return clearPos
        end
...
-- Piercing intercept: NO LOS check, returns 3 body points (head/chest/gut)
local function solvePiercingIntercept(targetPlayer)
    local ok, result = pcall(function()
        local char = targetPlayer.Character;                  if not char then return nil end
        local root = char:FindFirstChild("HumanoidRootPart"); if not root then return nil end
        local hum  = char:FindFirstChildOfClass("Humanoid");  if not hum  then return nil end
        for _ = 1, 8 do phantomUpdate(targetPlayer) end
        local rawPing   = getPing()
        local totalLead = rawPing + (1 / 60)
        local _, simPos, bodyH = physicsSimPredict(char, root, hum, targetPlayer, nil, totalLead)
        if simPos and bodyH then
            return {
                Vector3.new(simPos.X, simPos.Y + bodyH * 0.90, simPos.Z),
                Vector3.new(simPos.X, simPos.Y + bodyH * 0.65, simPos.Z),
                Vector3.new(simPos.X, simPos.Y + bodyH * 0.35, simPos.Z),
            }
        end
        local head = char:FindFirstChild("Head")
        return { (head and head.Position) or root.Position + Vector3.new(0, 2, 0) }
    end)
    if ok and result then return result end
    local char  = targetPlayer.Character
    local mroot = char and char:FindFirstChild("HumanoidRootPart")
    return { mroot and mroot.Position + Vector3.new(0, 1.5, 0) or Vector3.new() }
end
...
local function doPremiumShoot(targetPlayer)
    ...
    for _ = 1, 6 do phantomUpdate(targetPlayer) end

    local gun = myChar:FindFirstChild("Gun") or myChar:FindFirstChild("Revolver")
             or LocalPlayer.Backpack:FindFirstChild("Gun")
             or LocalPlayer.Backpack:FindFirstChild("Revolver")
    if not gun then
        Fluent:Notify({ Title = "⭐ Freemium Aim", Content = "No gun found.", Duration = 3 })
        return
    end
    ...
    local function getArg1()
        return (rayAtt and rayAtt.WorldCFrame)
            or (myRoot and CFrame.new(myRoot.Position)) or CFrame.new()
    end
    ...
    if not predicted then
        predicted = (mHead and mHead.Position)
                 or (mRoot.Position + Vector3.new(0, 1.5, 0))
    end

    pcall(function() shootRemote:FireServer(getArg1(), CFrame.new(predicted)) end)
    premiumSilentAim.lastShot = tick()
end

-- ── Piercing Bullet: fires 3 shots (head, chest, gut) ignoring walls ─
local piercingCooldown = 0
local function doPiercingShoot(targetPlayer)
    if tick() - piercingCooldown < 0.40 then return end
    local char  = targetPlayer.Character; if not char then return end
    local myChar   = LocalPlayer.Character; if not myChar then return end
    local humanoid = myChar:FindFirstChild("Humanoid"); if not humanoid then return end
    ...
    local gun = myChar:FindFirstChild("Gun") or myChar:FindFirstChild("Revolver")
             ...
             or LocalPlayer.Backpack:FindFirstChild("Revolver")
    if not gun then
        Fluent:Notify({ Title = "🔮 Piercing Bullet", Content = "No gun found.", Duration = 3 })
        return
    end
    if LocalPlayer.Backpack:FindFirstChild(gun.Name) then
        humanoid:EquipTool(gun)
    ...
    -- Get 3 predicted body points: head, chest, gut (no wall check)
    local points = solvePiercingIntercept(targetPlayer)
    for _, pt in ipairs(points) do
        pcall(function() shootRemote:FireServer(getArg1(), CFrame.new(pt)) end)
    end
    piercingCooldown = tick()
end
...
local Tabs = {
 ...
 Premium = Window:AddTab({ Title = "Freemium", Icon = "star" }),
...
Tabs.Main:AddParagraph({
 ...
 Content = "Every single feature in OmniHub — including Freemium Silent Aim, ESP, Auto Farm, God Mode, and more — is completely free. I'll keep adding more features soon. Stay tuned and join the Discord for updates!"
})
...
-- FIX: freeSilentAimMode was never declared — set default here
...
-- ═══════════════════════════════════════════════════════════════════
-- PIERCING BULLET — shoots through all players in line of sight
-- Uses the game's own CreateBeam module for visual tracer effect
-- ═══════════════════════════════════════════════════════════════════
local piercingBullet = {
    enabled  = false,
    cooldown = 0.6,
    lastShot = 0,
}

-- Recreate CreateBeam logic from the game's own module
-- (reverse-engineered from 142823291 ModuleScript CreateBeam)
local TweenService = game:GetService("TweenService")
local twInfo_fast  = TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
local twInfo_slow  = TweenInfo.new(0.2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)

local function MakeTempPart(pos)
...
local function CreateTracerBeam(fromPart, toPos)
    -- fromPart must have the gun's position, toPos is the predicted hit
    local p1 = MakeTempPart(fromPart)
    local p2 = MakeTempPart(toPos)

    -- Try to use the game's CustomBeam if it exists on the gun,
    -- otherwise create our own golden tracer beam
    ...
    beam.Transparency = NumberSequence.new({
        ...
        NumberSequenceKeypoint.new(0.202, 0.2),
        NumberSequenceKeypoint.new(1,    0.6 ),
    })
    beam.Attachment0 = p1.Attachment
    beam.Attachment1 = p2.Attachment
    beam.FaceCamera  = true
    beam.Parent      = p1

    -- Fade out (matches the game module exactly)
    task.wait(0.03)
    TweenService:Create(beam, twInfo_fast, { Width0 = 0 }):Play()
    TweenService:Create(beam, twInfo_slow, { Width1 = 0 }):Play()
end

-- (getPlayersInLine removed — piercing bullet targets murderer only, ignores walls)

local function doPiercingShot()
    if tick() - piercingBullet.lastShot < piercingBullet.cooldown then return end
    local myChar = LocalPlayer.Character; if not myChar then return end
    local myHum  = myChar:FindFirstChildOfClass("Humanoid"); if not myHum then return end

    local murderer = GetMurderer()
    if not murderer then
        Fluent:Notify({ Title = "🔫 Piercing Bullet", Content = "No murderer detected.", Duration = 3 })
        return
    end
    ...
    if not mRoot then
        Fluent:Notify({ Title = "🔫 Piercing Bullet", Content = "Murderer character not found.", Duration = 3 })
        return
    end
    ...
    local gun = myChar:FindFirstChild("Gun")
             ...
             or LocalPlayer.Backpack:FindFirstChild("Revolver")
    if not gun then
        Fluent:Notify({ Title = "🔫 Piercing Bullet", Content = "No gun in hand or backpack.", Duration = 3 })
        return
    end
    if LocalPlayer.Backpack:FindFirstChild(gun.Name) then
        myHum:EquipTool(gun)
    ...
    -- Predict murderer position (through walls — no LOS check needed)
    local mHum = mChar:FindFirstChildOfClass("Humanoid")
    local predicted
    if mHum then
        for _ = 1, 4 do phantomUpdate(murderer) end
        local rawP = getPing()
        -- Pass nil as fromPos so physicsSimPredict skips LOS entirely
        predicted = physicsSimPredict(mChar, mRoot, mHum, murderer, nil, rawP + 1/60)
    end
    -- Fallback: aim at head/chest directly (still no LOS check — pierces walls)
    predicted = predicted
        or (mHead and mHead.Position)
        or (mRoot.Position + Vector3.new(0, 1.5, 0))

    -- Fire the shot — server registers the hit even through walls
    pcall(function()
        shootRemote:FireServer(getArg1(), CFrame.new(predicted))
    end)

    -- Draw tracer beam through the wall
    pcall(function()
        CreateTracerBeam(barrelPos, predicted)
    end)

    piercingBullet.lastShot = tick()
    Fluent:Notify({
        Title   = "🔫 Piercing Bullet",
        Content = string.format("Bullet pierced through walls → %s!", murderer.Name),
        Duration = 3
    })
end
...
Tabs.Premium:AddParagraph({
    Title = "What Is Freemium Silent Aim?",
    Content = "All features on this tab are completely FREE — enjoy! More features coming soon."
})
Tabs.Premium:AddParagraph({
    Title = "How To Use",
...
Tabs.Premium:AddToggle("PremiumSilentAimToggle", {
    Title = "Freemium Silent Aim",
    Description = "Physics-simulation predictor — aims where the target will be based on real movement physics.",
    Default = false,
    Callback = function(v)
        ...
        Fluent:Notify({
            ...
            Content = v
                ...
                or  "Freemium Silent Aim DISABLED.",
            Duration = 3
        })
    end
})
Tabs.Premium:AddButton({
    ...
    Callback = function()
        if not premiumSilentAim.enabled then
            Fluent:Notify({
                Title = " Freemium Aim",
                Content = "Enable 'Freemium Silent Aim' toggle first!",
                Duration = 3
            })
            return
        end
...
-- ── Piercing Bullet Section ───────────────────────────────────────
Tabs.Premium:AddSection("Piercing Bullet")
Tabs.Premium:AddParagraph({
    Title = "What Is Piercing Bullet?",
    Content = "Fires a bullet that pierces through walls and obstacles to hit the Murderer — no matter where they are hiding. Uses physics prediction + golden tracer beam."
})
Tabs.Premium:AddToggle("PiercingBulletToggle", {
    Title   = "Enable Piercing Bullet",
    Description = "Must be toggled ON before using the button below.",
    Default = false,
    Callback = function(v)
        piercingBullet.enabled = v
        Fluent:Notify({
            Title   = "🔫 Piercing Bullet",
            Content = v and "ON — press 'Fire Piercing Bullet' to use." or "OFF.",
            Duration = 3
        })
    end
})
Tabs.Premium:AddSlider("PiercingCooldownSlider", {
    Title       = "Piercing Cooldown (ms)",
    Description = "Cooldown between piercing shots.",
    Default     = 600,
    Min         = 200,
    Max         = 2000,
    Rounding    = 0,
    Callback    = function(v)
        piercingBullet.cooldown = v / 1000
    end
})
Tabs.Premium:AddButton({
    Title = "🔫 Fire Piercing Bullet",
    Name  = "PiercingBulletBtn",
    Callback = function()
        if not piercingBullet.enabled then
            Fluent:Notify({ Title = "🔫 Piercing Bullet", Content = "Enable 'Piercing Bullet' toggle first!", Duration = 3 })
            return
        end
        doPiercingShot()
    end
})
-- FIX: speedGlitch was declared AFTER the toggles that use it — moved here so callbacks don't index nil
local speedGlitch = {
    enabled   = false,
    speed     = 60,
    decayTime = 0.35,
...
-- sgBuild and sgDestroy defined HERE so toggle callbacks below can call them
...
Tabs.Premium:AddParagraph({
    Title = "Speed Glitch",
    Content = "Activates extra speed when strafing sideways while jumping. Adjust the sliders below to tune the boost."
})
