...
------------------------------------------------------------
-- DISABLE SHERIFF SHOOT  (inserts CantShoot into GunClient)
------------------------------------------------------------
local DisableSheriff = { Enabled = false }

local function GetSheriffGun()
    if not Sheriff then return nil end
    local sheriffPlayer = Players:FindFirstChild(Sheriff)
    if not sheriffPlayer then return nil end
    return sheriffPlayer.Backpack:FindFirstChild("Gun")
        or (sheriffPlayer.Character and sheriffPlayer.Character:FindFirstChild("Gun"))
end

local function SetSheriffCantShoot(state)
    local gun = GetSheriffGun()
    if not gun then return end
    local gunClient = gun:FindFirstChild("GunClient")
    if not gunClient then return end
    local cantShoot = gunClient:FindFirstChild("CantShoot")
    if not cantShoot then
        cantShoot = Instance.new("BoolValue")
        cantShoot.Name = "CantShoot"
        cantShoot.Parent = gunClient
    end
    cantShoot.Value = state
end

-- Heartbeat keeps CantShoot enforced even if sheriff re-equips
local DisableSheriffConn = nil
local function StartDisableSheriff()
    if DisableSheriffConn then DisableSheriffConn:Disconnect() end
    DisableSheriffConn = RunService.Heartbeat:Connect(function()
        if DisableSheriff.Enabled then pcall(SetSheriffCantShoot, true) end
    end)
end

local function StopDisableSheriff()
    if DisableSheriffConn then DisableSheriffConn:Disconnect(); DisableSheriffConn = nil end
    pcall(SetSheriffCantShoot, false)   -- restore sheriff on toggle off
end

------------------------------------------------------------
-- AIM BUTTON — SHOOT  (fixed: remote is inside GunServer)
------------------------------------------------------------
AimButton.MouseButton1Click:Connect(function()
    -- Gun can be equipped in character OR sitting in backpack
    ...
    -- Equip so the server accepts the shot
    ...
    -- Remote lives inside Gun.GunServer
    local gunServer = gun:FindFirstChild("GunServer")
    if not gunServer then return end

    local remote = gunServer:FindFirstChildWhichIsA("RemoteEvent")
               or gunServer:FindFirstChildWhichIsA("RemoteFunction")
    if not remote then return end
...
Tabs.Combat:AddSection("Sheriff Control")

Tabs.Combat:AddToggle("DisableSheriffToggle", {
    Title       = "Disable Sheriff Shoot",
    Description = "Inserts CantShoot into the sheriff GunClient so they cannot fire",
    Default     = false,
    Callback    = function(v)
        DisableSheriff.Enabled = v
        if v then
            StartDisableSheriff()
        else
            StopDisableSheriff()
