...
-- Drag system: original simple pattern (most compatible) with three key
-- mobile improvements:
--   1. AbsolutePosition snapshot on drag-start so scale-based Position
--      (AnchorPoint 0.5,0.5) never snaps the window to the top-left corner.
--   2. UserInputService.InputEnded safety release so a fast tap-and-lift
--      on mobile never leaves drag=true stuck.
--   3. TouchMoved signal in addition to InputChanged for consistent
--      sub-frame position updates on low-end mobile devices.
local function Draggable(handle, target)
    local drag = false
    local st   = Vector3.zero          -- starting input position
    local sp   = UDim2.fromOffset(0,0) -- starting target position (pure offsets)

    local function beginDrag(i)
        drag = true
        st   = i.Position
        -- Convert current position to pure pixel offsets so that a
        -- scale-based position like UDim2.new(0.5,0,0.5,0) doesn't make
        -- sp.X.Offset == 0 and snap the window to (deltaX, deltaY).
    ...
    -- Primary movement: works for both mouse and touch
    UserInputService.InputChanged:Connect(function(i)
        if not drag then return end
        if i.UserInputType == Enum.UserInputType.MouseMovement
        or i.UserInputType == Enum.UserInputType.Touch then
            local d = i.Position - st
            target.Position = UDim2.fromOffset(sp.X.Offset + d.X, sp.Y.Offset + d.Y)
        end
    end)

    -- Safety: always release on InputEnded so a fast lift on mobile never
    -- leaves drag=true stuck even if i.Changed doesn't fire
...
function Library:CreateWindow(cfg)
    ...
    -- ── Drag zones ─────────────────────────────────────────
    -- TitleBar is the primary handle. We also attach invisible drag zones
    -- to the left side (TabPanel), right edge, and bottom edge so the user
    -- can grab and move the window from any edge on mobile.
    local function EdgeZone(size, pos)
        return New("TextButton",{
            Size=size, Position=pos,
            BackgroundTransparency=1,
            Text="", AutoButtonColor=false,
            ZIndex=20, Parent=Win,
        })
    end
    -- Left side: covers the entire tab panel area below the title bar
    local LeftZone   = EdgeZone(
        UDim2.new(0, tabW, 1, -44),
        UDim2.new(0, 0,    0,  44))
    -- Right edge: 18px strip on the right
    local RightZone  = EdgeZone(
        UDim2.new(0, 18, 1, 0),
        UDim2.new(1,-18, 0, 0))
    -- Bottom edge: 18px strip at the bottom
    local BottomZone = EdgeZone(
        UDim2.new(1, 0, 0, 18),
        UDim2.new(0, 0, 1,-18))

    -- Drag: title bar + all edge zones move the same Win target
    Draggable(TBar,        Win)
    Draggable(LeftZone,    Win)
    Draggable(RightZone,   Win)
    Draggable(BottomZone,  Win)
