...
local CONFIG = {
    ...
    HWID_FILE       = "Cache.dat",   -- persisted via writefile
...
local function verify(onSuccess)
    ...
    elseif status == "ratelimited" then
        setStatus("Rate limited — wait " .. (response.remaining or 60) .. "s")
        setProgress(0.5)
        LockIcon.Text = "⏳"
    ...
    task.spawn(function()
        while true do
            ...
            elseif pStatus == "maintenance" then
                -- Show maintenance overlay without destroying main UI
                if UILibRef and UILibRef.Notify then
                    UILibRef:Notify({
                        title   = "Maintenance",
                        message = data.reason or "Server entering maintenance.",
                        type    = "warning",
                        duration= 6,
                    })
                end
            ...
            if data.message and data.message ~= "" and UILibRef and UILibRef._lastAnnouncement ~= data.message then
                UILibRef._lastAnnouncement = data.message
                if UILibRef.Notify then
                    UILibRef:Notify({
                        title   = "Announcement",
