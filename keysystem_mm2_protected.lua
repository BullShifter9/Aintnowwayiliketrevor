-- ╔═════════════════════════════════════════════════════════════════════════════╗
-- ║       ADVANCED PROTECTION SYSTEM  v3.0  — Loader Edition                  ║
-- ║  Anti HTTP Spy · Anti Remote Spy · Anti Dex · Hook Detection               ║
-- ║  Continuous Runtime Monitor · Randomised Intervals · Multi-Signal Gate     ║
-- ╠═════════════════════════════════════════════════════════════════════════════╣
-- ║  DESIGN PRINCIPLES (False-Positive-Free)                                   ║
-- ║  · NEVER flags executor-provided globals (syn, request, http_request …)    ║
-- ║  · NEVER flags Lua closures on executor functions (they're legit here)     ║
-- ║  · NEVER flags hookfunction/hookmetamethod/getrawmetatable presence alone  ║
-- ║  · ONLY targets spy-specific behavioural signatures and known spy globals  ║
-- ║  · Requires ≥2 independent positive signals before stopping execution      ║
-- ║  · Randomised check intervals to resist timed bypasses                     ║
-- ╠═════════════════════════════════════════════════════════════════════════════╣
-- ║  Research basis                                                             ║
-- ║  · SimpleSpy v3 source   github.com/78n/SimpleSpy                          ║
-- ║    → sets _G.SimpleSpyExecuted, _G.OriginalNamecall                        ║
-- ║    → hookmetamethod(game,"__namecall",…) or hookfunction(__namecall,…)     ║
-- ║    → UI named "SimpleSpy3"/"SimpleSpy2" parented to CoreGui                ║
-- ║  · Hydroxide source      github.com/Upbolt/Hydroxide                       ║
-- ║    → sets _G.HydroxideRunning, _G.HydroxideConnected                      ║
-- ║  · devforum __namecall identity technique  (July 2025)                     ║
-- ║  · devforum getfenv environment-leak technique  (July 2025)                ║
-- ║  · Timing-ratio side-channel technique  (July 2025)                        ║
-- ╚═════════════════════════════════════════════════════════════════════════════╝

do -- ── Protection scope (all locals sealed inside; original script unaffected) ──

    -- =========================================================================
    -- §0  SECURE FUNCTION CACHE
    --     Capture every standard function we need at the very TOP of execution,
    --     before any spy tool running concurrently can swap globals out.
    --     All protection code below uses ONLY these cached references.
    -- =========================================================================
    local _pcall       = pcall
    ...
    local _string_low  = string.lower
    local _string_fmt  = string.format
    local _table_ins   = table.insert
    local _task_wait   = task.wait
    local _task_spawn  = task.spawn

    -- Executor / debug APIs (may be nil; always accessed via pcall-safe wrappers)
    local _getfenv     = getfenv
    local _getgenv     = getgenv
    local _debug_info  = debug and debug.info

    -- =========================================================================
    -- §1  KILL SWITCH  &  SIGNAL ACCUMULATOR
    --
    --     killScript(reason) — hard-stops the script immediately (synchronous).
    --     runtimeKill(reason) — kicks from background thread (can't cross-thread
    --     error, so we kick and set a flag to stop all background loops).
    --
    --     MULTI-SIGNAL GATE: We require at least KILL_THRESHOLD independent
    --     positive signals before calling killScript().  Each detection category
    --     contributes at most 1 signal so a single false-positive can never
    --     trigger execution stop.
    -- =========================================================================
    local KILL_THRESHOLD  = 3     -- minimum positive signals required to kill
    local _signals        = {}    -- { category = true } accumulator

    local function _addSignal(category)
        _signals[category] = true
    end

    local function _signalCount()
        local n = 0
        for _ in _pairs(_signals) do n = n + 1 end
        return n
    end

    -- Build a compact reason string from all triggered categories.
    local function _reasonStr()
        local parts = {}
        for k in _pairs(_signals) do _table_ins(parts, k) end
        local out = ""
        for i, p in _ipairs(parts) do
            out = out .. (i > 1 and " | " or "") .. p
        end
        return out
    end

    -- Evaluate accumulated signals and kill if threshold is met.
    -- Returns true if execution was halted (never actually returns in that case).
    local function _evaluateAndKill(extraCategory)
        if extraCategory then _addSignal(extraCategory) end
        if _signalCount() >= KILL_THRESHOLD then
            local reason = _reasonStr()
            _warn("[PROTECTION] Stopping execution — " .. reason)
            _pcall(function()
                game:GetService("Players").LocalPlayer:Kick(
                    "[Protection] Spy/Explorer detected:\n" .. reason
                )
            end)
            -- Level-0 error unwinds the entire call stack and prevents all code
            -- below this do-block from ever executing.
            _error("[PROTECTION] " .. reason, 0)
        end
    end

    -- Runtime kill (called from background threads — cannot use error() across threads)
    local _runtimeAlive = true
    local function _runtimeKill(reason)
        if not _runtimeAlive then return end
        _runtimeAlive = false
        _warn("[PROTECTION/RUNTIME] " .. _tostring(reason))
        _pcall(function()
            game:GetService("Players").LocalPlayer:Kick(
    ...
    -- Lowercase a value for keyword matching
    local function _low(v)
        return _string_low(_tostring(v))
    end

    -- Check if string s contains keyword kw (plain search, lowercase)
    local function _has(s, kw)
        return _string_find(_low(s), kw, 1, true) ~= nil
    end

    -- Randomised wait: waits between lo and hi seconds
    local function _randWait(lo, hi)
        _task_wait(lo + _math_random() * (hi - lo))
    end

    -- =========================================================================
    -- §3  SPY-SPECIFIC GLOBAL REGISTRIES
    --
    --  IMPORTANT: This list deliberately EXCLUDES normal executor globals
    --  such as syn, KRNL_LOADED, fluxus, request, http_request, hookfunction,
    --  hookmetamethod, getrawmetatable — those are present in any executor and
    --  must NOT be treated as spy indicators.
    --
    --  The globals here are SET BY THE SPY TOOLS THEMSELVES as part of their
    --  own logic (e.g. SimpleSpy sets _G.SimpleSpyExecuted = true).
    -- =========================================================================

    -- § 3a  Remote-spy specific globals (set by the spy tools themselves)
    local SPY_GLOBALS = {
        -- SimpleSpy (v2 / v3 / exxtremestuffs fork)
        "SimpleSpyExecuted",    -- set to true by SimpleSpy on launch
        "SIMPLESPYCONFIG_MaxRemotes", -- SimpleSpy config key
        "OriginalNamecall",     -- SimpleSpy stores the original namecall here
        "SimpleSpyV3",
        "SimpleSpy3",
        "SimpleSpy2",
        -- Hydroxide
    ...
    -- § 3b  HTTP-spy specific globals
    local HTTP_SPY_GLOBALS = {
        "HttpSpyActive",
        "HttpSpyEnabled",
        "HTTP_SPY_LOADED",
        "HttpInterceptActive",
    ...
    -- § 3c  Dex-specific globals (set by Dex/DarkDex on launch)
    local DEX_GLOBALS = {
        "DexExplorer",
        "DexLoaded",
        "DexRunning",
    ...
    -- § 3d  Spy-specific UI names in CoreGui (exact known names from source)
    local SPY_UI_NAMES = {
        -- SimpleSpy exact UI names from source code
        "SimpleSpy3", "SimpleSpy2", "SimpleSpy",
        -- Hydroxide
        "Hydroxide", "HydroxideUI",
        -- Generic spy UIs
        "RemoteSpyGui", "SpyGui", "ClosureSpyGui",
        "SigmaSpyUI", "RSpy",
        -- HTTP spy tools
        "HttpSpyGui", "HttpSpy",
    }

    -- § 3e  Dex UI exact names from source / known variants
    local DEX_UI_NAMES = {
        "DexExplorer", "Dex", "DarkDex", "Dex Explorer",
        "DexV3", "DexV4", "DEX",
        "InfiniteYield", "IY",
    }

    -- § 3f  Dex structural keyword pairs (require BOTH to match one ScreenGui)
    --       Much safer than single-keyword matching.
    local DEX_STRUCT_PAIRS = {
        {"propert", "explorer"},
        {"dex",     "properties"},
        {"dex",     "explorer"},
        {"dex",     "script"},
    ...
    -- =========================================================================
    -- §4  __NAMECALL IDENTITY CAPTURE
    --     Capture the real __namecall function pointer (and its caller) by
    --     triggering an intentional error through the namecall path and
    --     reading debug.info at the exact stack level where __namecall lives.
    --     When SimpleSpy/Hydroxide hook __namecall, the pointer at that level
    --     changes — we catch the change during runtime checks.
    -- =========================================================================
    local _nc_baseline   = nil   -- captured __namecall function ref
    local _nc_caller     = nil   -- captured caller above __namecall
    local _nc_captured   = false

    local function _captureNamecall()
        if not _debug_info then return end
        _xpcall(function()
            -- Guaranteed error through __namecall: method doesn't exist
            return (game):__PROT_NC_BASELINE_CAPTURE_999()
        end, function()
            local ok1, f1 = _safe(_debug_info, 2, "f")
            local ok2, f2 = _safe(_debug_info, 3, "f")
            if ok1 and f1 then _nc_baseline = f1 end
            if ok2 and f2 then _nc_caller   = f2 end
            _nc_captured = true
        end)
    end
    _captureNamecall()

    -- Returns true if __namecall identity has changed since baseline capture.
    -- Safe to call at any time; returns false if debug.info unavailable.
    local function _namecallChanged()
        if not _nc_captured or not _debug_info then return false end
        local currentNC, currentCaller
        _xpcall(function()
            return (game):__PROT_NC_VERIFY_999()
        end, function()
            local ok1, f1 = _safe(_debug_info, 2, "f")
            local ok2, f2 = _safe(_debug_info, 3, "f")
            if ok1 then currentNC     = f1 end
            if ok2 then currentCaller = f2 end
        end)
        if currentNC ~= nil and _nc_baseline ~= nil then
            if not _rawequal(currentNC, _nc_baseline) then return true end
        end
        if currentCaller ~= nil and _nc_caller ~= nil then
            if not _rawequal(currentCaller, _nc_caller) then return true end
        end
        return false
    end

    -- =========================================================================
    -- §5  ENVIRONMENT LEAK DETECTOR  (Spy-Key-Filtered)
    --     Triggers an error through the __namecall dispatch path and walks
    --     up to 20 stack levels via getfenv(level).  Spy tools (SimpleSpy,
    --     Hydroxide) run Lua functions inside the namecall hook chain; their
    --     environment leaks into the error handler's call stack.
    --
    --     CRITICAL FALSE-POSITIVE GUARD:
    --     We do NOT flag a populated environment generically — we only flag
    --     if the environment at any level contains a known spy-specific key.
    --     Clean executor hooks are C-level and have no Lua env to leak.
    -- =========================================================================
    local ENV_SPY_KEYS = {
        -- SimpleSpy internal locals that appear in leaked env
        "SimpleSpyExecuted", "OriginalNamecall", "SIMPLESPYCONFIG_MaxRemotes",
        "SimpleSpy3", "SimpleSpy2",
        -- Hydroxide
        "HydroxideRunning", "HydroxideConnected",
        -- Sigma / Generic spy
        "SigmaSpyActive", "RemoteSpyActive",
        -- Common spy toggle vars that appear in their environments
        "toggleSpy", "spyActive", "spyToggle",
        "remoteHooks", "connectedRemotes", "hookRemote",
    }

    local function _envHasSpyKey(env)
        for _, key in _ipairs(ENV_SPY_KEYS) do
            if _rawget(env, key) ~= nil then return true, key end
        end
        return false, nil
    end

    -- Returns: detected (bool), found_key (string or nil)
    local function _envLeakCheck()
        if not _getfenv then return false, nil end
        local detected, foundKey = false, nil
        _xpcall(function()
            -- Route through __namecall to trigger spy hook chain
            return workspace:__PROT_ENV_LEAK_CHECK_999()
        end, function()
            for level = 0, 20 do
                local ok, env = _safe(_getfenv, level)
                if ok and env and _type(env) == "table" then
                    local hit, key = _envHasSpyKey(env)
                    if hit then
                        detected = true
                        foundKey = key
                        break
        ...
        return detected, foundKey
    end

    -- =========================================================================
    -- §6  TIMING-RATIO SIDE-CHANNEL
    --     HTTP spy tools intercept game:HttpGet() to log URLs and responses.
    --     This adds measurable overhead to the namecall dispatch.
    --     We measure game:IsA() (lightweight namecall) vs an empty loop.
    --     Uses a rolling violation counter with a high threshold to prevent
    --     false positives from normal system load variance.
    -- =========================================================================
    local _timingViolations = 0
    local _TIMING_N         = 4000   -- iteration count (kept modest for startup)
    local _RATIO_MIN        = 10     -- below this → near-zero overhead (suspicious hook)
    local _RATIO_MAX        = 85     -- above this → enormous overhead (logging spy)
    local _VIOLATION_LIMIT  = 7      -- need 7 consecutive bad readings to signal

    local function _timingRatioCheck()
        local t0 = _tick()
        for _ = 1, _TIMING_N do game:IsA("Model") end
        local t1 = _tick()
        for _ = 1, _TIMING_N do end
        local t2 = _tick()

        local empty = t2 - t1
        if empty < 1e-9 then return false end  -- guard div/0

        local ratio = (t1 - t0) / empty
        if ratio < _RATIO_MIN or ratio > _RATIO_MAX then
            _timingViolations = _timingViolations + 1
        else
            _timingViolations = 0
        end
        return _timingViolations >= _VIOLATION_LIMIT
    end

    -- =========================================================================
    -- §7  GLOBAL SCANNER  (checks _G and getgenv for known spy globals)
    --     Uses the registries from §3.  Returns: found (bool), key (string).
    --     Safe: executor globals are never in those registries.
    -- =========================================================================
    local function _scanGlobals(registry)
        -- Check _G
        for _, key in _ipairs(registry) do
            if _rawget(_G, key) ~= nil then return true, "_G." .. key end
        end
        -- Check getgenv()
        if _getgenv then
            local ok, genv = _safe(_getgenv)
            if ok and genv and genv ~= _G then
                for _, key in _ipairs(registry) do
                    if _rawget(genv, key) ~= nil then
                        return true, "genv." .. key
                    end
                end
            end
        end
    ...
    -- =========================================================================
    -- §8  UI SCANNER  (CoreGui + PlayerGui)
    --     Exact-name scan uses the known UI name lists from §3.
    --     Structural scan for Dex requires matching BOTH keywords of a pair
    --     within the same ScreenGui — much more reliable than single keywords.
    -- =========================================================================

    -- Exact-name check against a list
    local function _nameInList(name, list)
        local ln = _low(name)
        for _, entry in _ipairs(list) do
            if ln == _low(entry) then return true end
        end
        return false
    end

    -- Scan a root for exact-name UI matches
    local function _scanUIExact(root, nameList)
        if not root then return false, nil end
        local ok, children = _safe(function() return root:GetChildren() end)
        if not ok or not children then return false, nil end
        for _, child in _ipairs(children) do
            if _nameInList(child.Name, nameList) then
                return true, child.Name
            end
            -- One level deeper (spy UIs sometimes nest under a Folder)
            local ok2, gc = _safe(function() return child:GetChildren() end)
            if ok2 and gc then
                for _, g in _ipairs(gc) do
                    if _nameInList(g.Name, nameList) then
                        return true, g.Name
                    end
                end
            end
    ...
    -- Structural Dex detection: within a single ScreenGui, collect all child
    -- names and check if ANY structural pair from DEX_STRUCT_PAIRS has BOTH
    -- keywords present.
    local function _dexStructuralCheck(screenGui)
        local ok, all = _safe(function() return screenGui:GetDescendants() end)
        if not ok or not all then return false end
        local names = {}
        for _, d in _ipairs(all) do
            _table_ins(names, _low(d.Name))
        end
        for _, pair in _ipairs(DEX_STRUCT_PAIRS) do
            local hasA, hasB = false, false
            for _, n in _ipairs(names) do
                if _has(n, pair[1]) then hasA = true end
                if _has(n, pair[2]) then hasB = true end
            end
    ...
    -- Full Dex UI scan
    local function _scanDexUI(root)
        if not root then return false end
        local ok, children = _safe(function() return root:GetChildren() end)
        if not ok then return false end
        for _, child in _ipairs(children) do
            if _nameInList(child.Name, DEX_UI_NAMES) then return true end
            if child.ClassName == "ScreenGui" then
                if _dexStructuralCheck(child) then return true end
            end
        end
        return false
    end
    ...
    -- =========================================================================
    -- §9  INDIVIDUAL PROTECTION CHECKS
    --     Each function adds signals; evaluateAndKill() is called afterwards.
    --     No single check alone stops execution (multi-signal gate).
    -- =========================================================================

    -- ── §9a  Anti HTTP Spy ───────────────────────────────────────────────────
    -- NOTE: We deliberately do NOT check if request/http_request/syn.request
    -- are Lua closures — they are, in every executor, and that is normal.
    -- We do NOT re-scan SPY_GLOBALS here because _checkRemoteSpy() already does
    -- that scan.  Scanning the same list in two functions with different signal
    -- category names would double-count one underlying match.
    local function _checkHTTPSpy()
        -- Signal A: known HTTP-spy-specific globals only
        local found, key = _scanGlobals(HTTP_SPY_GLOBALS)
        if found then
            _addSignal("http_spy_global:" .. (key or "?"))
        end

        -- Signal B: timing ratio anomaly (catches active HTTP logging overhead)
        -- Only fires after 7 consecutive bad readings to avoid hardware variance.
        if _timingRatioCheck() then
            _addSignal("http_timing_anomaly")
        end
    end

    -- ── §9b  Anti Remote Spy ────────────────────────────────────────────────
    local function _checkRemoteSpy()
        -- Signal A: spy-specific globals in _G / getgenv
        local found, key = _scanGlobals(SPY_GLOBALS)
        if found then
            _addSignal("remote_spy_global:" .. (key or "?"))
        end

        -- Signal B: spy UI in CoreGui (exact name match)
        _pcall(function()
            local hit, name = _scanUIExact(game:GetService("CoreGui"), SPY_UI_NAMES)
            if hit then
                _addSignal("remote_spy_ui:" .. (name or "?"))
            end
        end)

        -- Signal C: environment leak with spy-specific key filter
        local leaked, leakKey = _envLeakCheck()
        if leaked then
            _addSignal("remote_spy_env_leak:" .. (leakKey or "?"))
        end

        -- Signal D: check PlayerGui too
        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if pg then
                local hit, name = _scanUIExact(pg, SPY_UI_NAMES)
                if hit then
                    _addSignal("remote_spy_playerGui:" .. (name or "?"))
                end
            end
        end)
    end

    -- ── §9c  Anti Dex Explorer ───────────────────────────────────────────────
    local function _checkDexExplorer()
        -- Signal A: Dex-specific globals
        local found, key = _scanGlobals(DEX_GLOBALS)
        if found then
            _addSignal("dex_global:" .. (key or "?"))
        end

        -- Signal B: exact UI name match in CoreGui
        _pcall(function()
            local hit, name = _scanUIExact(game:GetService("CoreGui"), DEX_UI_NAMES)
            if hit then
                _addSignal("dex_ui_coregui:" .. (name or "?"))
            end
        end)

        -- Signal C: structural pattern match inside ScreenGuis
        _pcall(function()
            local cg = game:GetService("CoreGui")
            if _scanDexUI(cg) then
                _addSignal("dex_structural_pattern")
            end
        end)

        -- Signal D: PlayerGui scan
        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if pg and _scanDexUI(pg) then
                _addSignal("dex_playerGui_structural")
            end
        end)
    end

    -- ── §9d  Advanced Hook Detection ────────────────────────────────────────
    -- NOTE: _envLeakCheck() is intentionally NOT called here.
    --   It is already called inside _checkRemoteSpy(). Calling it twice with
    --   different signal category names would double-count one underlying check
    --   and hit KILL_THRESHOLD from a single false-positive source.
    --   _namecallChanged() is NOT used at startup because Delta and other
    --   executors legitimately hook __namecall for their own internals.
    --   It runs only in the background thread where the spy-global check
    --   provides corroborating evidence before a kill is triggered.
    local function _checkHooks()
        -- Intentionally minimal at startup to avoid false positives.
        -- Background threads handle ongoing hook monitoring.
    end

    -- =========================================================================
    -- §10  INITIAL SYNCHRONOUS CHECKS
    --      These run immediately and block execution.
    --      Each check adds signals; evaluateAndKill() fires the gate at the end.
    -- =========================================================================
    _checkHTTPSpy()
    _checkRemoteSpy()
    _checkDexExplorer()
    _checkHooks()
    _evaluateAndKill()   -- only kills if ≥ KILL_THRESHOLD signals accumulated

    -- =========================================================================
    -- §11  CONTINUOUS RUNTIME MONITOR
    --      Four independent background threads run periodic re-checks with
    --      randomised intervals so the check timing is unpredictable.
    --
    --      Runtime kill accumulates into a separate signal store and calls
    --      _runtimeKill() (kick) rather than error(), because error() cannot
    --      cross Lua thread boundaries in Roblox.
    -- =========================================================================

    -- Runtime signal store (independent from startup signals)
    local _rtSignals = {}
    local function _rtSignal(cat)
        if _rtSignals[cat] then return end  -- deduplicate
        _rtSignals[cat] = true
        local n = 0
        for _ in _pairs(_rtSignals) do n = n + 1 end
        if n >= KILL_THRESHOLD then
            local parts = {}
            for k in _pairs(_rtSignals) do _table_ins(parts, k) end
            local reason = ""
            for i, p in _ipairs(parts) do
                reason = reason .. (i > 1 and " | " or "") .. p
            end
            _runtimeKill("Runtime — " .. reason)
        end
    end

    -- ── Thread 1: Remote spy + HTTP spy globals & UI  (fast interval) ────────
    _task_spawn(function()
        _randWait(3, 6)   -- initial randomised delay
        while _runtimeAlive do

            -- Spy globals
            _pcall(function()
                local found, key = _scanGlobals(SPY_GLOBALS)
                if found then _rtSignal("rt_spy_global:" .. (key or "?")) end
            end)

            -- HTTP spy globals
            _pcall(function()
                local found, key = _scanGlobals(HTTP_SPY_GLOBALS)
                if found then _rtSignal("rt_http_spy_global:" .. (key or "?")) end
            end)

            -- Spy UI in CoreGui
            _pcall(function()
                local hit, name = _scanUIExact(
                    game:GetService("CoreGui"), SPY_UI_NAMES)
                if hit then _rtSignal("rt_spy_ui:" .. (name or "?")) end
            end)

            _randWait(2, 5)   -- randomised interval
        end
    end)

    -- ── Thread 2: Dex globals & structural UI scan  (medium interval) ────────
    _task_spawn(function()
        _randWait(4, 8)   -- staggered start
        while _runtimeAlive do

            _pcall(function()
                local found, key = _scanGlobals(DEX_GLOBALS)
                if found then _rtSignal("rt_dex_global:" .. (key or "?")) end
            end)

            _pcall(function()
                local cg = game:GetService("CoreGui")
                if _scanDexUI(cg) then _rtSignal("rt_dex_ui_coregui") end
            end)
    ...
    -- ── Thread 3: __namecall identity  (fast interval, high priority) ────────
    _task_spawn(function()
        _randWait(2, 4)
        while _runtimeAlive do

            _pcall(function()
                if _namecallChanged() then
                    _rtSignal("rt_namecall_identity_changed")
                end
            end)
    ...
    -- ── Thread 4: Environment leak + timing ratio  (slow/expensive) ──────────
    _task_spawn(function()
        _randWait(8, 14)   -- longer initial delay; these checks are heavier
        while _runtimeAlive do

            _pcall(function()
                local leaked, key = _envLeakCheck()
                if leaked then _rtSignal("rt_env_leak:" .. (key or "?")) end
            end)

            -- Timing ratio (heavier — runs on a slower sub-cycle)
            _pcall(function()
                if _timingRatioCheck() then
                    _rtSignal("rt_timing_anomaly")
                end
            end)

            _randWait(10, 20)   -- slow randomised cycle
        end
    end)

    -- =========================================================================
    -- §12  FINAL GATE
    --      If execution reaches here all synchronous checks passed.
    --      The four background threads continue monitoring.
    -- =========================================================================

end -- ── End of protection do-block ──────────────────────────────────────────

-- ╔═════════════════════════════════════════════════════════════════════════════╗
-- ║         KEYSYSTEM MM2 SCRIPT — UNCHANGED BELOW THIS LINE                  ║
-- ╚═════════════════════════════════════════════════════════════════════════════╝
