...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- Add optional click-latency offset (set by doPremiumShoot for button-click shots)
    ...
    -- Ground aim: stable platform height + body centre ratio
    local groundAimY   = platformBase + bodyH * 0.60
    -- Airborne aim: use the REAL current rootY (not platform) so arc displacement is correct
    local airborneAimY = rootY + bodyH * 0.60

    local finalY
    if isAirborne then
        -- ── Ascending / at peak of jump ──────────────────────────────────────────
        if tr and (tr.jumpState == "ascending" or tr.jumpState == "peak") then
            local elapsed  = tick() - tr.jumpStartT
            -- clamp elapsed so we never evaluate behind jump start
            elapsed = math.max(elapsed, 0)
            local tTotal   = elapsed + leadTime
            -- Parabolic arc relative to jump-start position
            ...
            -- Apply displacement to current airborne body-centre (NOT platform base)
            finalY = airborneAimY + futureDisp

        -- ── Descending ──────────────────────────────────────────────────────────
        elseif tr and tr.jumpState == "descending" then
            -- Use Kalman-filtered vertical velocity + full gravity integration
            local gravDrop    = 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
            local predRootYd  = rootY + kalmanVelY * leadTime - gravDrop
            finalY = predRootYd + bodyH * 0.60

        -- ── Generic airborne (no jump state tracked) ─────────────────────────
        else
            local gravDrop   = 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
            local predRootYg = rootY + kalmanVelY * leadTime - gravDrop
            finalY = predRootYg + bodyH * 0.60
        end
        ...
        -- ── Grounded ───────────────────────────────────────────────────────────
        -- Small vertical nudge for sloped surfaces or crouch-walk
...
local function doPremiumShoot(targetPlayer)
    ...
    -- Prime the Kalman tracker with multiple samples before solving intercept.
    -- More samples = tighter velocity estimate, especially critical during jumps.
    for _ = 1, 10 do phantomUpdate(targetPlayer) end

    -- Determine if target is currently airborne so we can widen the burst window
    local tr = pTrackers[targetPlayer]
    ...
    -- Account for GUI button-click input latency (~1-2 frames ≈ 33ms on 30fps servers).
    -- We inject a tiny extra lead into the solve by temporarily nudging baseLead.
    -- We store a flag that solvePremiumIntercept checks.
    ...
    -- Burst count: more shots when target is mid-air (larger uncertainty window)
    ...
    -- Airborne targets get extra burst shots (arc changes frame-to-frame)
    ...
    burstConn = RunService.Heartbeat:Connect(function()
        ...
        -- Re-prime tracker and re-solve each burst frame
        phantomUpdate(targetPlayer)
