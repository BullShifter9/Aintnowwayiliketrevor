...
-- ================================================================
-- VALUE DATABASE — loaded dynamically from GitHub
--
-- Host your mm2_sv_full_database.json on GitHub and paste the
-- raw URL below. The JSON format matches supremevalues.com:
--   { "item name lowercase": { name, value, demand, rarity, ... } }
--
-- How to set up:
--   1. Upload mm2_sv_full_database.json to your GitHub repo
--   2. Open the file on GitHub → click "Raw" → copy the URL
--   3. Paste it as GITHUB_DB_URL below
-- ================================================================

local GITHUB_DB_URL = "https://raw.githubusercontent.com/BullShifter9/Aintnowwayiliketrevor/refs/heads/main/Xkosoeoficjd_database.json"

local SV_DATABASE = {}

do
    -- Fetch the JSON database from GitHub using _fetchRaw (supports all executors)
    local dbStatus = "Loading values database..."
    local res = _fetchRaw(GITHUB_DB_URL)

    if res and type(res.Body) == "string" and #res.Body > 10 then
        local ok, decoded = pcall(HttpService.JSONDecode, HttpService, res.Body)
        if ok and type(decoded) == "table" then
            -- Normalise each entry to match the format expected by the rest of the script
            for key, entry in pairs(decoded) do
                if type(entry) == "table" then
                    -- Ensure required fields exist with safe defaults
                    SV_DATABASE[key] = {
                        name   = entry.name   or entry.Name   or key,
                        value  = tonumber(entry.value  or entry.Value)  or 0,
                        demand = tonumber(entry.demand or entry.Demand) or 0,
                        rarity = (entry.rarity or entry.Rarity or "misc"):lower(),
                        trend  = entry.stability or entry.trend or "stable",
                        origin = entry.origin or "",
                    }
                end
            end
            dbStatus = string.format("✓ Loaded %d items from GitHub database", (function()
                local n = 0; for _ in pairs(SV_DATABASE) do n = n + 1 end; return n
            end)())
        else
            warn("[OmniHub] Failed to parse database JSON: " .. tostring(decoded))
            dbStatus = "⚠ Database parse error — values unavailable"
        end
    else
        warn("[OmniHub] Could not fetch database from GitHub. Check GITHUB_DB_URL.")
        dbStatus = "⚠ Database fetch failed — check GitHub URL"
    end

    -- Print status to console for debugging
    print("[OmniHub] " .. dbStatus)
end

-- ValueChecker wrapper (keeps rest of script compatible) 
local ValueChecker = {
 cache = SV_DATABASE,
 loaded = next(SV_DATABASE) ~= nil, -- true if GitHub fetch succeeded
 loading = false,
 autoTradeVal = true,
}
-- All entries already have .trend from the GitHub fetch normalisation above
