...
Tabs.World:AddToggle("KnifeAuraToggle", {
    ...
    Callback = function(v)
        knifeAura.enabled = v
        if not v then knifeAuraRestore() end
        Fluent:Notify({
            Title = "Knife Aura",
...
GrabGunFrame.Size                    = UDim2.new(0, 90, 0, 90)
GrabGunFrame.Position                = UDim2.new(0.01, 0, 0.45, 0)
GrabGunFrame.BackgroundColor3        = Color3.fromRGB(0, 140, 220)
GrabGunFrame.BackgroundTransparency  = 0.15
...
GrabGunStroke.Thickness   = 3
GrabGunStroke.Transparency = 0.20
...
GrabGunFrame.MouseButton1Click:Connect(function()
    manualGrabGun()
    GrabGunFrame.BackgroundColor3 = Color3.fromRGB(0, 255, 120)
    task.wait(0.18)
    GrabGunFrame.BackgroundColor3 = Color3.fromRGB(0, 140, 220)
end)
...
-- Finds the equipped knife by structure (any Tool with Handle +
-- KnifeServer or Events child) so renamed skins work correctly.
-- Expands Handle.Size client-side → server sees touch → kill.
-- All paths fully nil-guarded; original size restored on toggle off.
-- ================================================================

local knifeAura            = { enabled = false, size = 10 }
local knifeHandleRef       = nil   -- the Handle BasePart we expanded
local knifeHandleOrigSize  = nil   -- its original Vector3 size

-- Find the equipped knife tool by structure, not name
local function findEquippedKnife()
    local myChar = LocalPlayer.Character
    if not myChar then return nil end
    for _, tool in ipairs(myChar:GetChildren()) do
        if tool:IsA("Tool") then
            local handle = tool:FindFirstChild("Handle")
            -- MM2 knife always has KnifeServer or Events.KnifeStabbed
            local isKnife = tool:FindFirstChild("KnifeServer")
                         or (tool:FindFirstChild("Events")
                             and tool.Events:FindFirstChild("KnifeStabbed"))
            if handle and isKnife then
                return handle
            end
        end
    end
...
local function knifeAuraRestore()
    -- Guard: only restore if we have both a valid ref and a saved size
    if knifeHandleRef and knifeHandleOrigSize then
        pcall(function()
            -- Re-check the ref is still a valid instance before writing
            local _ = knifeHandleRef.Size  -- errors if destroyed → caught by pcall
            knifeHandleRef.Size = knifeHandleOrigSize
        end)
    end
    knifeHandleRef      = nil
    knifeHandleOrigSize = nil
end

RunService.Heartbeat:Connect(function()
    if not knifeAura.enabled then
        if knifeHandleRef then knifeAuraRestore() end
        return
    end
    ...
    local handle = findEquippedKnife()

    if not handle then
        -- Knife unequipped or not found — restore and wait
        if knifeHandleRef then knifeAuraRestore() end
        return
    end

    -- New knife equipped: cache original size first
    if handle ~= knifeHandleRef then
        if knifeHandleRef then knifeAuraRestore() end
        knifeHandleRef = handle
        -- Read size safely; if it errors the ref is bad → skip
        local ok, sz = pcall(function() return handle.Size end)
        if not ok or not sz then
            knifeHandleRef = nil
            return
        end
        knifeHandleOrigSize = sz
    end
