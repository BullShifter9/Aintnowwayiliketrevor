...
function BreakGun:BreakSheriffGun()
    ...
    -- Find gun tool in inventory
    local gun = nil
    for _, tool in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
        if tool:IsA("Tool") and (tool.Name == "Gun" or tool.Name:find("Gun") or tool.Name:find("Revolver")) then
            gun = tool
            break
        end
    end
    ...
    -- If gun found, proceed with breaking it
    if gun then
...
function AutoNotifyMurderersPerk:ScanForMurderers()
    ...
    -- Try GetData2 first
    pcall(function()
        if game:GetService("ReplicatedStorage"):FindFirstChild("GetData2", true) then
            playerData = game:GetService("ReplicatedStorage"):FindFirstChild("GetData2", true):InvokeServer()
        end
    end)
    
    -- If GetData2 failed, try GetPlayerData
    if not playerData then
        pcall(function()
            if game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true) then
                playerData = game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true):InvokeServer()
            end
        end)
    end
    
    -- Process the data if we got it from either method
    if playerData then
...
if Tabs and Tabs.Character then
    Tabs.Character:AddToggle("BreakGunToggle", {
...
if Tabs and Tabs.Visuals then
    Tabs.Visuals:AddToggle("MurdererPerkNotifyToggle", {
...
return AutoNotifyMurderersPerk
