...
local GunDropESP = {
    enabled = true,
    highlightColor = Color3.fromRGB(0, 255, 255), -- Cyan
...
-- Reference to GunDrop variable (assuming it's in the same script)
-- If not, replace with proper reference to your GunDropTracker

-- Initialize ESP components
function GunDropESP:Initialize()
    -- Create BillboardGui for the label
    self.labelTemplate = Instance.new("BillboardGui")
    self.labelTemplate.Name = "GunDropLabel"
    self.labelTemplate.Size = UDim2.new(0, 100, 0, 40)
    self.labelTemplate.StudsOffset = Vector3.new(0, 2, 0)
    self.labelTemplate.AlwaysOnTop = true
    self.labelTemplate.LightInfluence = 0
    ...
    textLabel.Parent = self.labelTemplate
    ...
    -- Check for existing GunDrop
    self:CheckExistingGunDrop()
end

-- Apply ESP to the gun drop
function GunDropESP:ApplyESP(gunDrop)
    ...
    label.Adornee = gunDrop
    label.Parent = gunDrop
...
-- Remove ESP from the gun drop
function GunDropESP:RemoveESP()
    -- Remove highlight if it exists
    if self.currentHighlight and self.currentHighlight.Parent then
        self.currentHighlight:Destroy()
        self.currentHighlight = nil
    end
    
    -- Remove label if it exists
    if self.currentLabel and self.currentLabel.Parent then
...
-- Check for existing gun drop
function GunDropESP:CheckExistingGunDrop()
    local existingGunDrop = workspace:FindFirstChild("GunDrop", true)
    if existingGunDrop then
        self:ApplyESP(existingGunDrop)
    end
end

-- Toggle the ESP system
function GunDropESP:SetEnabled(state)
    self.enabled = state
...
-- Extreme prediction method - highest accuracy with advanced trajectory calculation
local function predictMurderExtreme(murderer)
    local character = murderer.Character
    if not character then return nil end
    ...
    -- Enhanced physics constants for extreme accuracy
    local PHYSICS = {
        MICRO_TICK = 1/480,       -- Higher simulation resolution
        MACRO_TICK = 1/60,
        GRAVITY = workspace.Gravity * 1.02, -- Slight adjustment for prediction bias
        TERMINAL_VELOCITY = -198.5,
        PREDICTION_WINDOW = 2.8,   -- Extended prediction window
        SAMPLE_COUNT = 60,         -- More samples for pattern analysis
        PATTERN_DEPTH = 7,         -- Deeper pattern recognition
        GROUND_OFFSET = 5.2,
        MAX_SPEED_MULTIPLIER = 1.65,
        ACCELERATION_FACTOR = 1.15
    }

    -- Advanced probability metrics for behavioral prediction
    local PROBABILITY = {
        VELOCITY_WEIGHT = 0.95,
        PATTERN_WEIGHT = 0.92,
        MOMENTUM_WEIGHT = 0.88,
        DIRECTION_WEIGHT = 0.94,
        GROUND_WEIGHT = 0.97,
        AIR_WEIGHT = 0.86,
        CONFIDENCE_DECAY = 0.985,
        PIVOT_PROBABILITY = 0.78,
        DIRECTION_CHANGE_THRESHOLD = 0.35,
        MIN_CONFIDENCE = 0.82
    }

    -- Advanced trajectory state tracking
    local state = {
        position = rootPart.Position,
        velocity = rootPart.AssemblyLinearVelocity,
        velocityHistory = table.create(PHYSICS.SAMPLE_COUNT),
        positionHistory = table.create(PHYSICS.SAMPLE_COUNT),
        accelerationHistory = table.create(PHYSICS.SAMPLE_COUNT),
        lastMoveDirection = rootPart.CFrame.LookVector,
        movementState = humanoid:GetState(),
        patterns = {},
        groundContact = humanoid:GetState() ~= Enum.HumanoidStateType.Freefall,
        jumpPrediction = humanoid:GetState() == Enum.HumanoidStateType.Jumping,
        lastJumpTime = 0,
        confidenceScore = 1.0,
        predictionAccuracy = 1.0,
        lastCalculationTime = tick(),
        accelerationFactor = Vector3.new()
    }

    -- Initialize with more granular historical data
    for i = 1, PHYSICS.SAMPLE_COUNT do
        state.velocityHistory[i] = state.velocity
        state.positionHistory[i] = state.position
        state.accelerationHistory[i] = Vector3.new()
    end

    -- Calculate acceleration from historical velocity
    local function calculateAcceleration()
        if #state.velocityHistory < 2 then return Vector3.new() end
        
        local recentAccel = Vector3.new()
        local totalWeight = 0
        
        for i = 2, math.min(8, #state.velocityHistory) do
            local velDiff = state.velocityHistory[#state.velocityHistory - i + 1] - 
                          state.velocityHistory[#state.velocityHistory - i]
            local weight = 1 / i
            recentAccel = recentAccel + (velDiff * weight)
            totalWeight = totalWeight + weight
        end
        
        return recentAccel / (totalWeight * PHYSICS.MACRO_TICK) * PHYSICS.ACCELERATION_FACTOR
    end

    -- Enhanced movement pattern analysis with momentum prediction
    local function analyzeAdvancedMovementPatterns()
        local patterns = {}
        local totalWeight = 0
        local directionChangeIndices = {}
        
        -- Detect pivot points where direction changes significantly
        for i = 3, #state.positionHistory do
            local prevDir = (state.positionHistory[i-1] - state.positionHistory[i-2]).Unit
            local currDir = (state.positionHistory[i] - state.positionHistory[i-1]).Unit
            
            if prevDir:Dot(currDir) < PROBABILITY.DIRECTION_CHANGE_THRESHOLD then
                table.insert(directionChangeIndices, i)
            end
        end
        
        -- Generate patterns with emphasis on recent movement
        for depth = 1, PHYSICS.PATTERN_DEPTH do
            local pattern = Vector3.new()
            local weight = 1 / (depth * 0.8) -- Higher weight for smaller depths
            
            -- Recent history gets higher impact
            local recentEnd = #state.positionHistory
            local recentStart = math.max(1, recentEnd - math.floor(PHYSICS.SAMPLE_COUNT * 0.4))
            
            for i = recentStart + depth, recentEnd do
                local delta = state.positionHistory[i] - state.positionHistory[i - depth]
                pattern = pattern:Lerp(delta.Unit, 0.3 * weight)
            end
            
            -- Apply direction change probability
            local changeProb = #directionChangeIndices > 0 and 
                              math.min(1, #directionChangeIndices / (PHYSICS.SAMPLE_COUNT * 0.2)) or 0
            
            table.insert(patterns, {
                direction = pattern.Unit,
                weight = weight,
                confidence = math.exp(-depth * 0.16) * (1 - changeProb * PROBABILITY.PIVOT_PROBABILITY),
                pivotProbability = changeProb
            })
            
            totalWeight = totalWeight + weight
        end
        
        return patterns, totalWeight, #directionChangeIndices > 0
    end

    -- Enhanced velocity prediction with state-based adjustments
    local function predictAdvancedVelocityVector()
        local patterns, totalWeight, potentialPivot = analyzeAdvancedMovementPatterns()
        local acceleration = calculateAcceleration()
        local predictedVel = state.velocity
        
        -- Apply acceleration influence
        predictedVel = predictedVel + (acceleration * PHYSICS.MACRO_TICK * 2)
        
        -- Apply pattern influence with potential direction change awareness
        local patternInfluence = Vector3.new()
        for _, pattern in ipairs(patterns) do
            patternInfluence = patternInfluence + 
                (pattern.direction * pattern.weight * pattern.confidence)
        ...
        -- Factor in current movement state
        local stateMultiplier = 1.0
        if state.movementState == Enum.HumanoidStateType.Jumping then
            stateMultiplier = 1.2
        elseif state.movementState == Enum.HumanoidStateType.Freefall then
            stateMultiplier = 1.3
        elseif state.movementState == Enum.HumanoidStateType.Running then
            stateMultiplier = 1.15
        end
        
        local speedFactor = math.min(
            (predictedVel.Magnitude / humanoid.WalkSpeed) * stateMultiplier,
            PHYSICS.MAX_SPEED_MULTIPLIER
        )
        
        -- Calculate final velocity with pivot probability adjustment
        predictedVel = predictedVel:Lerp(
            patternInfluence * humanoid.WalkSpeed * speedFactor,
            potentialPivot and PROBABILITY.PATTERN_WEIGHT * 0.85 or PROBABILITY.PATTERN_WEIGHT
        )
        
        return predictedVel
    end

    -- Enhanced physics simulation with environment interaction
    local function simulateAdvancedPhysics(startPos, startVel, duration)
        local pos = startPos
        local vel = startVel
        local time = 0
        local confidence = 1.0
        ...
        -- Apply current movement state to initial conditions
        local movementState = humanoid:GetState()
        if movementState == Enum.HumanoidStateType.Jumping then
            vel = vel + Vector3.new(0, humanoid.JumpHeight * 0.35, 0)
        end
        
        while time < duration do
            -- Higher resolution micro-step simulation
            for _ = 1, PHYSICS.MACRO_TICK / PHYSICS.MICRO_TICK do
                -- Check for ground and obstacles with more complex raycasting
                local groundData = {}
                local directions = {
                    Vector3.new(0, -PHYSICS.GROUND_OFFSET, 0),
                    Vector3.new(0.8, -PHYSICS.GROUND_OFFSET, 0.8),
                    Vector3.new(-0.8, -PHYSICS.GROUND_OFFSET, -0.8),
                    Vector3.new(0.8, -PHYSICS.GROUND_OFFSET, -0.8),
                    Vector3.new(-0.8, -PHYSICS.GROUND_OFFSET, 0.8),
                }
                ...
                for _, dir in ipairs(directions) do
                    ...
                    if result then
                        table.insert(groundData, result)
                    end
                end
                
                local isGrounded = #groundData > 0
                
                -- Apply physically accurate simulation
                if isGrounded then
                    ...
                    vel = vel + Vector3.new(
                        acceleration.X * PHYSICS.MICRO_TICK, 
                        0, 
                        acceleration.Z * PHYSICS.MICRO_TICK
                    )
                else
                    -- Air movement with less control
                    vel = Vector3.new(
                        vel.X * 0.99,
                        vel.Y + (PHYSICS.GRAVITY * PHYSICS.MICRO_TICK),
                        vel.Z * 0.99
                    )
                    
                    -- Apply limited air acceleration
                    vel = vel + Vector3.new(
                        acceleration.X * PHYSICS.MICRO_TICK * 0.6, 
                        0, 
                        acceleration.Z * PHYSICS.MICRO_TICK * 0.6
                    )
                    
                    -- Cap terminal velocity
                    if vel.Y < PHYSICS.TERMINAL_VELOCITY then
                        vel = Vector3.new(vel.X, PHYSICS.TERMINAL_VELOCITY, vel.Z)
                    end
                end
    ...
    local acceleration = calculateAcceleration()
    local predictedVel = predictAdvancedVelocityVector()
    local finalPos, finalVel, confidence = simulateAdvancedPhysics(
        state.position,
        predictedVel,
        PHYSICS.PREDICTION_WINDOW
    )
    
    -- Apply additional contextual adjustment based on movement state
    local movementState = humanoid:GetState()
    if movementState == Enum.HumanoidStateType.Jumping then
        finalPos = finalPos + Vector3.new(0, humanoid.JumpHeight * 0.25, 0)
    elseif movementState == Enum.HumanoidStateType.Freefall then
        local fallDirection = rootPart.AssemblyLinearVelocity.Unit
        finalPos = finalPos + (fallDirection * rootPart.AssemblyLinearVelocity.Magnitude * 0.12)
    end
    ...
    -- Fall back to simpler prediction if confidence is low
    return state.position + (predictedVel * (PHYSICS.PREDICTION_WINDOW * 0.5))
end

-- Chilled prediction method - more conservative to appear natural and avoid detection
local function predictMurderChilled(murderer)
   local character = murderer.Character
   if not character then return nil end
   ...
   -- Reduced physics parameters for more conservative prediction
   local PHYSICS = {
       MICRO_TICK = 1/240,
       MACRO_TICK = 1/60,
       GRAVITY = workspace.Gravity * 0.96, -- Slightly reduced for conservative prediction
       PREDICTION_WINDOW = 1.6,    -- Shorter window for more natural aiming
       SAMPLE_COUNT = 30,          -- Fewer samples to reduce aggressive prediction
       PATTERN_DEPTH = 4,
       GROUND_OFFSET = 4.5,
       MAX_SPEED_MULTIPLIER = 1.25 -- Lower speed cap for natural movement
   }

   -- Conservative probability settings
   local PROBABILITY = {
       VELOCITY_WEIGHT = 0.85,
       PATTERN_WEIGHT = 0.75,      -- Reduced pattern influence
       MOMENTUM_WEIGHT = 0.82,
       DIRECTION_WEIGHT = 0.87,
       NATURAL_RANDOMNESS = 0.08,  -- Add slight randomness for human-like aiming
       MIN_CONFIDENCE = 0.90       -- Higher confidence threshold
   }

   -- Basic state tracking (simplified from original)
   local state = {
       position = rootPart.Position,
       velocity = rootPart.AssemblyLinearVelocity,
       velocityHistory = table.create(PHYSICS.SAMPLE_COUNT),
       positionHistory = table.create(PHYSICS.SAMPLE_COUNT),
       humanLikeness = math.random() * PROBABILITY.NATURAL_RANDOMNESS -- Random factor for human-like prediction
   }
   ...
   -- Simplified pattern analysis focused on immediate movement
   local function analyzeBasicMovement()
       local recentVelocity = Vector3.new()
       local totalWeight = 0
       
       -- Focus on recent movement only
       for i = 1, math.min(8, #state.velocityHistory) do
           local weight = 1 / i
           recentVelocity = recentVelocity + (state.velocityHistory[#state.velocityHistory - i + 1] * weight)
           totalWeight = totalWeight + weight
       end
       
       return recentVelocity / totalWeight
   end

   -- Conservative velocity prediction with intentional limitations
   local function predictConservativeVelocity()
       local recentVelocity = analyzeBasicMovement()
       local baseVelocity = state.velocity
       
       -- Intentionally limit prediction to appear more human-like
       local walkSpeedRatio = math.min(baseVelocity.Magnitude / humanoid.WalkSpeed, 0.95)
       local naturalFactor = 1 - state.humanLikeness
       
       -- Blend current velocity with recent movement
       local predictedVel = baseVelocity:Lerp(
           recentVelocity,
           PROBABILITY.VELOCITY_WEIGHT * naturalFactor
       )
       
       -- Intentionally scale down prediction for natural appearance
       return predictedVel * walkSpeedRatio
   end

   -- Simplified physics simulation with reduced complexity
   local function simulateBasicPhysics(startPos, startVel, duration)
       local pos = startPos
       local vel = startVel
       local time = 0
       
       -- Add slight variation for human-like aiming
       local randomOffset = Vector3.new(
           (math.random() - 0.5) * state.humanLikeness,
           (math.random() - 0.5) * state.humanLikeness,
           (math.random() - 0.5) * state.humanLikeness
       ) * 0.5
       
       -- Perform simplified simulation for naturalness
       while time < duration do
           local isGrounded = pos.Y <= (workspace.FallenPartsDestroyHeight + 100)
           ...
           if isGrounded then
               ...
               vel = Vector3.new(
                   vel.X * 0.98,
                   vel.Y + (PHYSICS.GRAVITY * PHYSICS.MACRO_TICK),
                   vel.Z * 0.98
               )
           end
           
           pos = pos + (vel * PHYSICS.MACRO_TICK)
           time = time + PHYSICS.MACRO_TICK
       end
       
       -- Apply conservative offset for natural aiming
       return pos + randomOffset
   end

   -- Execute the prediction with conservative approach
   local predictedVel = predictConservativeVelocity()
   local finalPos = simulateBasicPhysics(
       state.position,
       predictedVel,
       PHYSICS.PREDICTION_WINDOW
   )
   
   -- Apply contextual blending for more human-like prediction
   local movementState = humanoid:GetState()
   local blendFactor = 0.75 -- Conservative blend with current position
   
   if movementState == Enum.HumanoidStateType.Jumping or 
      movementState == Enum.HumanoidStateType.Freefall then
       blendFactor = 0.65 -- Even more conservative for aerial movements
   end
   
   -- Final blend with current position for natural appearance
   return rootPart.Position:Lerp(finalPos, blendFactor)
end
...
local function predictMurderChilled(murderer)
   ...
   local PHYSICS = {
       MICRO_TICK = 1/240,
       MACRO_TICK = 1/60,
       GRAVITY = workspace.Gravity * 0.96,
       PREDICTION_WINDOW = 1.6,
       SAMPLE_COUNT = 30,
       PATTERN_DEPTH = 4,
       GROUND_OFFSET = 4.5,
   ...
   local state = {
       ...
       velocityHistory = table.create(PHYSICS.SAMPLE_COUNT),
       positionHistory = table.create(PHYSICS.SAMPLE_COUNT),
       humanLikeness = math.random() * PROBABILITY.NATURAL_RANDOMNESS
   }
   ...
   local function simulateBasicPhysics(startPos, startVel, duration)
       ...
       while time < duration do
           ...
           local rayResult = workspace:Raycast(pos, Vector3.new(0, -PHYSICS.GROUND_OFFSET, 0), rayParams)
   ...
   -- Execute prediction with conservative approach
   local predictedVel = predictConservativeVelocity()
   local finalPos = simulateBasicPhysics(
       state.position,
       predictedVel,
   ...
   if movementState == Enum.HumanoidStateType.Jumping or 
      movementState == Enum.HumanoidStateType.Freefall then
       blendFactor = 0.65 -- More conservative for aerial movements
   end
...
-- Function to select prediction method based on the current setting
local function predictMurderer(murderer)
   if _G.SilentAimMethod == "Simple" then
       return predictMurderV2(murderer)
   elseif _G.SilentAimMethod == "Extreme" then
       return predictMurderExtreme(murderer)
   elseif _G.SilentAimMethod == "Chilled" then
       return predictMurderChilled(murderer)
   end
...
Icon.Image = "rbxassetid://122380482857500" -- Hub/menu icon
...
if Tabs and Tabs.Visuals then
    Tabs.Visuals:AddToggle("GunESPToggle", {
        Title = "Gun Drop ESP",
        Default = false,
        Callback = function(Value)
            GunDropESP:SetEnabled(Value)
        end
    })
end
...
local SilentAimToggle = Tabs.Combat:AddToggle("SilentAimToggle", {
   ...
   Callback = function(toggle)
       ...
       if toggle then
           ...
           -- Function to check for gun and shoot murderer immediately
           local function autoShootMurderer()
               ...
               if gun then
                   ...
                   if murderer then
                       ...
                       local predictedPos = predictMurderer(murderer)
                       
                       if predictedPos then
                           -- Shoot at predicted position immediately when gun is equipped
           ...
           _G.SilentAimConnection = RunService.Heartbeat:Connect(autoShootMurderer)
       else
...
-- Add dropdown for prediction methods
local SilentAimMethodDropdown = Tabs.Combat:AddDropdown("SilentAimMethod", {
   Title = "Aim Method",
   Values = {"Simple", "Extreme", "Chilled"},
   Default = "Simple",
   Multi = false,
   Callback = function(option)
       _G.SilentAimMethod = option
   end
})

-- Link the dropdown to be enabled/disabled with the toggle
SilentAimToggle:OnChanged(function(toggle)
   SilentAimMethodDropdown:SetEnabled(toggle)
end)

-- Initialize method
_G.SilentAimMethod = "Simple"
