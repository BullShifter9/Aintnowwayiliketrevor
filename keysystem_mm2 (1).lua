...
-- Core intercept: Kalman vel + ping lead (no deep simulation)
local function solvePremiumIntercept(targetPlayer)
    ...
    -- Warm up the Kalman tracker for this target
    phantomUpdate(targetPlayer)
    local smoothVel = getSmoothedVel(targetPlayer)

    -- Lead time = measured ping + one server physics tick (16 ms)
    -- Clamped: never under 20 ms (not useful) nor over 250 ms (over-predicts)
    local leadTime = math.clamp(getPing() + 0.016, 0.020, 0.250)

    -- Blend physics velocity (Kalman) with current input direction
    -- 70 / 30 split handles the gap between velocity and where player is pressing
    local vel = smoothVel
    if hum then
        local inputVel = hum.MoveDirection * (hum.WalkSpeed or 16)
        vel = smoothVel * 0.70 + inputVel * 0.30
    end

    -- Hard-cap lateral speed to 1.2× walkSpeed — prevents over-shooting
    -- when the murderer is strafing or just changed direction
    local walkSpeed   = (hum and hum.WalkSpeed) or 16
    local lateralVel  = Vector3.new(vel.X, 0, vel.Z)
    if lateralVel.Magnitude > walkSpeed * 1.2 then
        lateralVel = lateralVel.Unit * (walkSpeed * 1.2)
    end
    vel = Vector3.new(lateralVel.X, vel.Y, lateralVel.Z)

    -- Predicted XZ position
    local predicted = root.Position + vel * leadTime

    -- Aim at chest-to-neck band (≈60% up between root and head)
    local rootY  = root.Position.Y
    local headY  = head and head.Position.Y or (rootY + 1.5)
    local aimY   = rootY + (headY - rootY) * 0.60
    -- Apply vertical lead only partially (jumping targets) to stay stable
    local vertLead = vel.Y * leadTime * 0.5
    predicted = Vector3.new(predicted.X, aimY + vertLead, predicted.Z)

    -- Floor / ceiling safety — keeps aim inside the map
    local rp = RaycastParams.new()
    rp.FilterType = Enum.RaycastFilterType.Blacklist
    rp.FilterDescendantsInstances = {char}
    local fR = workspace:Raycast(predicted, Vector3.new(0, -5, 0), rp)
    local cR = workspace:Raycast(predicted, Vector3.new(0,  5, 0), rp)
...
-- Premium shoot: frame-0 + 3-frame Heartbeat burst
-- Triple-verified: (1) prediction re-computed each burst, (2) cooldown guard,
-- (3) pcall isolation so one failed fire never breaks the burst chain.
local function doPremiumShoot(targetPlayer)
    ...
    -- Frames 1-3: burst across 3 Heartbeat frames
    -- Re-solving each frame so a fast-moving target is tracked continuously.
    -- 3 frames (vs Fusion's 2) gives one extra server-tick window for high-ping.
    local burst = 0
    local burstConn
    burstConn = RunService.Heartbeat:Connect(function()
        burst = burst + 1
        if burst > 3 then burstConn:Disconnect(); return end
...
Tabs.World:AddToggle("FullbrightToggle", {
    Title = "Fullbright",
    Description = "Maximises ambient and outdoor brightness so dark maps are fully visible.",
    Default = false,
    Callback = function(v)
        ...
        Fluent:Notify({ Title = "Fullbright", Content = v and "Fullbright ON." or "Fullbright OFF.", Duration = 2 })
    end
})
...
Tabs.World:AddParagraph({
    ...
    Content =
        "• Infinite Jump — jump again mid-air by re-triggering jump state\n" ..
        "• Murderer Chance % — reads GetChance remote and shows % in notify\n" ..
        "• See Dead Chat — relay dead players' messages to you as system chat\n" ..
        "• TP To Murderer — instantly teleport behind the murderer\n" ..
        "• TP To Gun Drop — teleport directly to gun drop location\n" ..
        "• Anti-AFK — periodically sends virtual input to stop the AFK kick\n" ..
        "• Fog Remover — sets FogEnd to max on foggy maps"
})
...
Tabs.Premium:AddParagraph({
    ...
    Content = "A cleaner, more accurate silent aim engine built on Kalman-filtered velocity + ping-lead compensation. Designed to hit at 90-100% accuracy even at 180ms+ ping. No deep physics simulation = less error, more stability."
})
