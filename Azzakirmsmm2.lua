...
local function collectNearbyCoins()
   if state.coinAuraActive then return end
   
   local character = Players.LocalPlayer.Character
   if not character or not character:FindFirstChild("HumanoidRootPart") then return end
   
   local hrp = character.HumanoidRootPart
   local nearbyParts = workspace:GetPartBoundsInRadius(hrp.Position, state.coinAuraRange)
   
   state.coinAuraActive = true
   
   for _, part in ipairs(nearbyParts) do
       if VALID_COIN_TYPES[part.Name] and part.Size.magnitude <= state.smallCoinSize.magnitude then
...
local function checkAndNotifyRole()
   ...
   if not roles or not roles[localPlayer.Name] then return end
   
   local myRole = roles[localPlayer.Name].Role
   ...
   if myRole == "Murderer" then
       notificationConfig.Name = "Role: Murderer"
       notificationConfig.Content = "You are the Murderer! Eliminate all players."
   elseif myRole == "Sheriff" then
       notificationConfig.Name = "Role: Sheriff"
       notificationConfig.Content = "You are the Sheriff! Protect the innocents."
   else
       notificationConfig.Name = "Role: Innocent"
       notificationConfig.Content = "You are Innocent! Try to survive."
   end
   
   OrionLib:MakeNotification(notificationConfig)
end
...
coinSection:AddToggle({
   ...
   Callback = function(Value)
       state.coinAura = Value
   end
})

coinSection:AddSlider({
   Name = "Coin Aura Range",
   Min = 5,
   Max = 15,
   Default = 8,
...
for _, v in ipairs(getconnections(ReplicatedStorage.Remotes.Gameplay.RoundStart.OnClientEvent)) do
   local oldCallback = v.Function
   v.Function = function(...)
       if state.roleNotifications then
           task.delay(1, checkAndNotifyRole)
       end
       return oldCallback(...)
   end
end
