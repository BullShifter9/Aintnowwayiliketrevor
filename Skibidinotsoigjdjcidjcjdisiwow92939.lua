...
-- Role colors matching ESP configuration
local RoleColors = {
   Murderer = Color3.fromRGB(255, 0, 0),
   Sheriff = Color3.fromRGB(0, 100, 255),
   Hero = Color3.fromRGB(255, 215, 0),
...
-- Role icons (Roblox Asset IDs)
local RoleIcons = {
   Murderer = "rbxassetid://7733658271", -- Knife icon
   Sheriff = "rbxassetid://7733715400",  -- Gun icon
   Hero = "rbxassetid://7734034303",     -- Shield/star icon
   Innocent = "rbxassetid://7734030470"  -- Person icon
}

-- Create notification UI framework
function RoleNotifier:CreateUI()
   if self.UI then return end
   
   -- Create container ScreenGui
   local notifierGui = Instance.new("ScreenGui")
   notifierGui.Name = "RoleNotifierGui"
   notifierGui.ResetOnSpawn = false
   notifierGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
   
   -- Apply protection if available
   pcall(function()
       if syn and syn.protect_gui then
           syn.protect_gui(notifierGui)
       elseif get_hidden_gui then
           notifierGui = get_hidden_gui()
       end
   ...
   container.Position = UDim2.new(0, -270, 0, 0)
   container.BackgroundTransparency = 1
   container.Parent = notifierGui
   
   -- Add UIListLayout for multiple notifications
...
function RoleNotifier:CreateNotification(playerName, roleName)
   ...
   -- Role color and icon
   local roleColor = RoleColors[roleName] or Color3.fromRGB(200, 200, 200)
   local roleIcon = RoleIcons[roleName] or ""
   
   -- Create notification frame
   local notification = Instance.new("Frame")
   notification.Name = "Notification_" .. playerName
   notification.Size = UDim2.new(0, 250, 0, 70)
   notification.BackgroundColor3 = Color3.fromRGB(25, 25, 30)
   notification.BackgroundTransparency = 0.1
   notification.BorderSizePixel = 0
   notification.LayoutOrder = #self.UI.Container:GetChildren()
   notification.Parent = self.UI.Container
   ...
   accentBar.BackgroundColor3 = roleColor
   accentBar.BorderSizePixel = 0
   accentBar.Parent = notification
   
   -- Round just the left corners of accent bar
   local accentCorner = Instance.new("UICorner")
   accentCorner.CornerRadius = UDim.new(0, 4)
   accentCorner.Parent = accentBar
   
   -- Clip the accent bar corners on the right side
   local accentClip = Instance.new("Frame")
   accentClip.Name = "AccentClip"
   accentClip.Size = UDim2.new(0, 3, 1, 0)
   accentClip.Position = UDim2.new(1, -3, 0, 0)
   accentClip.BackgroundColor3 = roleColor
   accentClip.BorderSizePixel = 0
   accentClip.Parent = accentBar
   ...
   avatarFrame.Parent = notification
   
   -- Round avatar frame
   local avatarCorner = Instance.new("UICorner")
   avatarCorner.CornerRadius = UDim.new(1, 0) -- Make circular
   ...
   -- Role icon overlay
   local roleIconLabel = Instance.new("ImageLabel")
   roleIconLabel.Name = "RoleIcon"
   roleIconLabel.Size = UDim2.new(0, 26, 0, 26)
   roleIconLabel.Position = UDim2.new(1, -8, 1, -8)
   roleIconLabel.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
   roleIconLabel.BorderSizePixel = 0
   roleIconLabel.Image = roleIcon
   roleIconLabel.ImageColor3 = Color3.fromRGB(255, 255, 255)
   roleIconLabel.ScaleType = Enum.ScaleType.Fit
   roleIconLabel.Parent = avatarFrame
   
   -- Round role icon
   local roleIconCorner = Instance.new("UICorner")
   roleIconCorner.CornerRadius = UDim.new(1, 0)
   roleIconCorner.Parent = roleIconLabel
   
   -- Role label
   local roleLabel = Instance.new("TextLabel")
   roleLabel.Name = "RoleLabel"
   roleLabel.Size = UDim2.new(0, 150, 0, 20)
   roleLabel.Position = UDim2.new(0, 76, 0, 14)
   roleLabel.BackgroundTransparency = 1
   roleLabel.Text = roleName:upper()
   ...
   nameLabel.Position = UDim2.new(0, 76, 0, 36)
   ...
   notification.Position = UDim2.new(0, -260, 0, 20 + (80 * (#self.UI.Container:GetChildren() - 1)))
   
   -- Slide-in animation
   local tweenInfo = TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
   local slideTween = TweenService:Create(notification, tweenInfo, {Position = UDim2.new(0, 10, 0, notification.Position.Y.Offset)})
   slideTween:Play()
   
   -- Auto-dismiss after 5 seconds
   task.delay(5, function()
       -- Slide-out animation
       local dismissTween = TweenService:Create(notification, tweenInfo, {Position = UDim2.new(0, -260, 0, notification.Position.Y.Offset)})
...
function RoleNotifier:ProcessQueue()
   if self.ProcessingNotification or #self.NotifyQueue == 0 then return end
   
   self.ProcessingNotification = true
   local notification = table.remove(self.NotifyQueue, 1)
   
   self:CreateNotification(notification.Player, notification.Role)
...
function RoleNotifier:QueueNotification(playerName, roleName)
   if not self.Enabled then return end
...
-- Setup role detection system
function RoleNotifier:SetupRoleDetection()
   ...
   -- Connect to role update heartbeat
   self.Connections.RoleUpdate = RunService.Heartbeat:Connect(function()
       if not self.Enabled then return end
       
       -- Cache current roles data
       local currentRoles = {}
           ...
           if ReplicatedStorage:FindFirstChild("GetPlayerData", true) then
               ...
               for playerName, data in pairs(rolesData) do
                   if data and data.Role then
                       currentRoles[playerName] = data.Role
                       
                       -- Check if role is new or changed
                       if lastRoles[playerName] ~= data.Role and data.Role ~= "None" then
                           -- Don't notify for local player
                           if playerName ~= LocalPlayer.Name then
                               self:QueueNotification(playerName, data.Role)
                           end
                       end
                   end
               end
       ...
       -- Update last roles for next comparison
       lastRoles = currentRoles
   end)
   
   -- Also listen for murderer kills for additional notifications
   self.Connections.MurderEvent = RunService.Heartbeat:Connect(function()
       pcall(function()
           if ReplicatedStorage:FindFirstChild("GetPlayerData", true) then
               local rolesData = ReplicatedStorage:FindFirstChild("GetPlayerData", true):InvokeServer()
               
               for playerName, data in pairs(rolesData) do
                   -- Check for players who were just killed
                   if data and data.Killed and not data.Dead and not data.PreviouslyKilled then
                       data.PreviouslyKilled = true
                       
                       -- Find the murderer
                       local murderer = nil
                       for pName, pData in pairs(rolesData) do
                       ...
                       if murderer and murderer ~= LocalPlayer.Name then
                           self:QueueNotification(murderer, "Murderer")
                       end
                   end
               end
           end
...
-- Toggle role notifications
function RoleNotifier:Toggle(state)
   self.Enabled = state
   
   if state then
       self:CreateUI()
       self:SetupRoleDetection()
   else
...
Tabs.Visuals:AddToggle("RoleNotifierToggle", {
   Title = "Instant Role Notifications",
   Default = RoleNotifier.Enabled,
   Callback = function(Value)
       RoleNotifier:Toggle(Value)
   end
})

-- Initialize on script load
RoleNotifier:Toggle(true)


-- Gun Drop ESP Toggler Implementation
Tabs.Visuals:AddToggle("GunDropESPToggle", {
   Title = "Gun Drop ESP",
   Default = true,
   Callback = function(Value)
       -- Store configuration state
       if not ESP.GunDropConfig then
           ESP.GunDropConfig = {Enabled = true}
       end
       ESP.GunDropConfig.Enabled = Value
       
       -- Handle existing Gun Drop ESP elements
       if GunDrop and GunDrop.Parent then
           ...
           if not Highlights.GunDrop then
               ...
               billboardGui.Name = "GunDropLabel"
               billboardGui.AlwaysOnTop = true
               billboardGui.Size = UDim2.new(0, 200, 0, 50)
               billboardGui.StudsOffset = Vector3.new(0, 2, 0)
               billboardGui.MaxDistance = ESP.MaxRenderDistance
               ...
               textLabel.TextColor3 = ESP.Colors.GunDrop
               textLabel.TextStrokeTransparency = 0.3
               textLabel.TextStrokeColor3 = Color3.new(0, 0, 0)
               textLabel.TextSize = 18
               textLabel.Parent = billboardGui
               
               billboardGui.Parent = HighlightFolder
           ...
           Highlights.GunDrop.Highlight.Enabled = Value and ESP.Enabled
           Highlights.GunDrop.Label.Enabled = Value and ESP.Enabled
           
           -- Update adorning
           Highlights.GunDrop.Highlight.Adornee = GunDrop
           Highlights.GunDrop.Label.Adornee = GunDrop
       end
       
       -- Force immediate update if enabled
...
-- Enhance UpdateESP to handle Gun Drop ESP
local originalUpdateESP = UpdateESP
UpdateESP = function()
   -- Run the original player ESP update
   originalUpdateESP()
   
   -- Handle Gun Drop ESP
   if GunDrop and GunDrop.Parent then
       -- Check if Gun Drop ESP is enabled in config
       local gunDropEnabled = ESP.GunDropConfig and ESP.GunDropConfig.Enabled
       if not gunDropEnabled then
           -- Disable existing elements if toggle is off
           if Highlights.GunDrop then
               Highlights.GunDrop.Highlight.Enabled = false
               Highlights.GunDrop.Label.Enabled = false
           end
           return
       end
       ...
       if not Highlights.GunDrop then
           ...
           -- Create pulsing effects for better visibility
           local highlightTween = TweenService:Create(
               highlight, 
               TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
               {OutlineTransparency = 0.5}
       ...
       Highlights.GunDrop.Highlight.Enabled = ESP.Enabled
       Highlights.GunDrop.Label.Enabled = ESP.Enabled
       
       -- Update adorning
       Highlights.GunDrop.Highlight.Adornee = GunDrop
       Highlights.GunDrop.Label.Adornee = GunDrop
   elseif Highlights.GunDrop then
       -- Clean up if gun drop doesn't exist anymore
       Highlights.GunDrop.Highlight:Destroy()
       Highlights.GunDrop.Label:Destroy()
       Highlights.GunDrop = nil
   end
end

-- Enhance the gun tracking connections to ensure ESP updates
local enhancedSetupGunTracking = SetupGunTracking
SetupGunTracking = function()
   -- Run original setup
   enhancedSetupGunTracking()
   
   -- Replace connections with improved ones that update ESP immediately
   for i, conn in ipairs(GunTrackingConnections) do
       conn:Disconnect()
   end
   table.clear(GunTrackingConnections)
   ...
   GunTrackingConnections[1] = workspace.ChildAdded:Connect(function(child)
       if child.Name == "GunDrop" then 
           GunDrop = child
           task.spawn(UpdateESP) -- Update ESP immediately
       end
   end)
   
   GunTrackingConnections[2] = workspace.ChildRemoved:Connect(function(child)
       if child == GunDrop then 
           GunDrop = nil
           task.spawn(UpdateESP) -- Update ESP immediately
       end
   end)
...
-- Initialize Gun Drop ESP by calling the enhanced setup
SetupGunTracking()
