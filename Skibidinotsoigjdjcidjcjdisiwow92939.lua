...
-- Precise hook into game's knife system
local function hookKnifeSystem()
   if SlashConfig.connection then 
       SlashConfig.connection:Disconnect()
       SlashConfig.connection = nil
   end
   
   if not SlashConfig.enabled then return end
   
   -- Efficient metatable hook to intercept slash events
   SlashConfig.connection = RunService.Heartbeat:Connect(function()
       ...
       -- Original method reference for validation
       local originalStab = knife.Stab
       if not originalStab or typeof(originalStab) ~= "Instance" or not originalStab:IsA("RemoteEvent") then return end
       
       -- Monitor knife animation state to detect active attacks
       local animator = character:FindFirstChildOfClass("Animator")
       if not animator then return end
       
       for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
           if track.Animation and track.Animation.AnimationId:match("slash") or track.Animation.AnimationId:match("stab") then
               -- Execute multiple slashes during a single animation
               for i = 1, SlashConfig.attacksPerCycle do
                   task.spawn(function()
                       pcall(function()
                           originalStab:FireServer("Slash")
                       end)
                   end)
                   task.wait(SlashConfig.cycleDelay)
               end
               break
           end
       end
