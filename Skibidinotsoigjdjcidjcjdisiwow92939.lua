...
local MAX_PARTICLES = 40 -- Increased for more water droplets
local PARTICLES_PER_BATCH = 5
local WAVE_SPEED = 0.3 -- Slowed down for more realistic water movement
local RIPPLE_FREQUENCY = 0.05 -- How often ripples appear
...
-- Water effect container with mask for proper clipping
...
waterLevel.BackgroundColor3 = Color3.fromRGB(65, 140, 255) -- Slightly adjusted for better water look
waterLevel.BackgroundTransparency = 0.15 -- More visible water
waterLevel.BorderSizePixel = 0
waterLevel.Parent = waterContainer

-- Enhanced water gradient for depth effect
...
waterWave1.Parent = waterSurface

local waterWave2 = Instance.new("ImageLabel")
waterWave2.Name = "WaterWave2"
waterWave2.Size = UDim2.new(2, 0, 1, 0)
waterWave2.Position = UDim2.new(-0.5, 0, 0.2, 0)
...
waterWave2.Parent = waterSurface

local waterWave3 = Instance.new("ImageLabel")
waterWave3.Name = "WaterWave3" 
waterWave3.Size = UDim2.new(2, 0, 0.6, 0)
waterWave3.Position = UDim2.new(-0.3, 0, 0.1, 0)
waterWave3.BackgroundTransparency = 1
waterWave3.Image = "rbxassetid://6764361046"
waterWave3.ImageTransparency = 0.8
waterWave3.ImageColor3 = Color3.fromRGB(210, 235, 255)
waterWave3.Parent = waterSurface

-- Water caustics effect (light refraction)
local caustics = Instance.new("ImageLabel")
caustics.Name = "Caustics"
caustics.Size = UDim2.new(2, 0, 2, 0)
caustics.Position = UDim2.new(0, 0, 0, 0)
caustics.BackgroundTransparency = 1
caustics.Image = "rbxassetid://8577379354" -- Use a caustics/water light texture
caustics.ImageTransparency = 0.8
caustics.ImageColor3 = Color3.fromRGB(160, 215, 255)
caustics.Parent = waterLevel

-- Ripple container for dynamic water effects
...
rippleContainer.Parent = waterLevel
...
-- Optimized particle system with physics properties
local particlePool = {}
local activeParticles = {}
local ripplePool = {}
local activeRipples = {}

-- Pre-create particle objects for water droplets
local function initializeParticlePool()
    for i = 1, MAX_PARTICLES do
        ...
        droplet.BackgroundTransparency = 0.2
        ...
        -- Store physics properties with each particle
        droplet:SetAttribute("VelocityX", 0)
        droplet:SetAttribute("VelocityY", 0)
        droplet:SetAttribute("Mass", math.random(7, 15) / 10)
        
        table.insert(particlePool, droplet)
    end
    
    -- Pre-create ripple effects
    for i = 1, 20 do
        local ripple = Instance.new("ImageLabel")
        ripple.BackgroundTransparency = 1
        ripple.Image = "rbxassetid://2825586705" -- Circle image
...
local function recycleParticle(particle)
    for i, p in ipairs(activeParticles) do
        if p == particle then
            ...
            particle:SetAttribute("VelocityX", 0)
            particle:SetAttribute("VelocityY", 0)
...
-- Get ripple from pool
local function getRipple()
    if #ripplePool > 0 then
        local ripple = table.remove(ripplePool)
        table.insert(activeRipples, ripple)
        return ripple
    elseif #activeRipples > 0 then
        local oldest = table.remove(activeRipples, 1)
        table.insert(activeRipples, oldest)
        return oldest
    end
    return nil
end

-- Return ripple to pool
local function recycleRipple(ripple)
...
local function createRipple(posX, size)
    ...
    ripple.Position = UDim2.new(0, posX, 0, 5)
    ...
    local expandTween = TweenService:Create(
        ...
        {Size = UDim2.new(0, size, 0, size/2), ImageTransparency = 1}
    )
...
-- Create and animate water particles with physics
local function createWaterParticles(count, startYRange, endYOffset, speedRange)
    ...
    for batch = 1, batchCount do
        ...
        for i = 1, particlesInBatch do
            ...
            -- Configure particle appearance and physics properties
            ...
            -- Randomize initial velocities for natural movement
            local initialVelocityX = math.random(-20, 20)
            local initialVelocityY = math.random(speedRange[1] * 100, speedRange[2] * 100)
            
            particle:SetAttribute("VelocityX", initialVelocityX)
            particle:SetAttribute("VelocityY", initialVelocityY)
            ...
            -- Create physics simulation for this particle
            local gravity = 980 -- Gravity constant
            local airResistance = 0.97 -- Damping factor
            local surfaceY = waterLevel.AbsolutePosition.Y
            local floorY = surfaceY + waterLevel.AbsoluteSize.Y
            local terminalVelocity = 800
            
            -- Track whether particle has hit water surface
            local hasHitSurface = false
            local startTime = tick()
            
            -- Physics update connection for this particle
            local connection = RunService.Heartbeat:Connect(function(deltaTime)
                ...
                local velocityX = particle:GetAttribute("VelocityX")
                local velocityY = particle:GetAttribute("VelocityY")
                
                -- Apply gravity and air resistance
                velocityY = velocityY + (gravity * deltaTime * mass)
                velocityY = math.min(velocityY, terminalVelocity)
                
                velocityX = velocityX * airResistance
                
                -- Check for collision with water surface
                if pos.Y + velocityY * deltaTime >= surfaceY and not hasHitSurface and waterLevel.Size.Y.Scale > 0.05 then
                    -- Create ripple effect at impact point
                    createRipple(pos.X - waterContainer.AbsolutePosition.X, size * 4)
                    
                    -- Reduce velocity on impact (water resistance)
                    velocityY = -velocityY * 0.4
                    velocityX = velocityX * 0.7
                    
                    -- Mark as having hit surface
                    hasHitSurface = true
                end
                
                -- Update position
                local newX = pos.X - waterContainer.AbsolutePosition.X + velocityX * deltaTime
                local newY = pos.Y - waterContainer.AbsolutePosition.Y + velocityY * deltaTime
                
                -- Keep particle within bounds
                newX = math.max(0, math.min(newX, waterContainer.AbsoluteSize.X))
                
                -- Update particle position
                particle.Position = UDim2.new(0, newX, 0, newY)
                
                -- Store updated velocity
                particle:SetAttribute("VelocityX", velocityX)
                particle:SetAttribute("VelocityY", velocityY)
                
                -- Recycle particle if it's been active too long or has fallen below container
                if tick() - startTime > 3 or newY > waterContainer.AbsoluteSize.Y + 50 then
                    connection:Disconnect()
...
local causticsConnection = nil
local function startCausticsAnimation()
    local offset = 0
    
    causticsConnection = RunService.Heartbeat:Connect(function(deltaTime)
        offset = (offset + deltaTime * 0.2) % 1
        caustics.Position = UDim2.new(-offset, 0, -offset, 0)
        caustics.Rotation = math.sin(tick() * 0.5) * 5
    end)
end

-- Enhanced wave animation with variable speeds and natural movement
local waveConnection = nil
local function startWaveAnimation()
    local wave1Offset = 0
    local wave2Offset = 0.3
    local wave3Offset = 0.6
    local timeElapsed = 0
    
    waveConnection = RunService.Heartbeat:Connect(function(deltaTime)
        timeElapsed = timeElapsed + deltaTime
        
        -- Variable wave speeds with natural oscillation
        local baseSpeed = WAVE_SPEED * (1 + 0.1 * math.sin(timeElapsed * 0.7))
        
        wave1Offset = (wave1Offset + deltaTime * baseSpeed * 1.0) % 1
        wave2Offset = (wave2Offset + deltaTime * baseSpeed * 0.7) % 1
        wave3Offset = (wave3Offset + deltaTime * baseSpeed * 1.2) % 1
        
        -- Animate waves with slight vertical movement for bobbing effect
        waterWave1.Position = UDim2.new(-wave1Offset, 0, 0, math.sin(timeElapsed * 1.5) * 2)
        waterWave2.Position = UDim2.new(-wave2Offset, 0, 0.2, math.sin(timeElapsed * 1.2 + 1) * 1.5)
        waterWave3.Position = UDim2.new(-wave3Offset, 0, 0.1, math.sin(timeElapsed * 1.8 + 2) * 2)
        
        -- Occasionally create small random ripples for ambient water movement
        if math.random() < RIPPLE_FREQUENCY * deltaTime then
            createRipple(math.random(50, 400), math.random(10, 30))
        end
    end)
end
...
local function stopAnimations()
    ...
    if causticsConnection then causticsConnection:Disconnect() end
end
...
local function updateLoadingProgress(startProgress, endProgress, duration)
   -- Create a smoother easing curve with tiny fluctuations
...
-- Enhance water level with dynamic interactive fill
local function enhanceWaterLevel(percentage, duration)
   local targetSize = UDim2.new(1, 0, percentage, 0)
   local currentSize = waterLevel.Size
   
   -- Create a natural water fill with spring-like movement
   local tween = TweenService:Create(
       waterLevel,
       TweenInfo.new(duration, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out, 0, false, 0),
       {Size = targetSize}
   )
   
   -- Create particles as water rises or falls
   local heightDiff = percentage - currentSize.Y.Scale
   local particleY = waterContainer.AbsoluteSize.Y * (1 - percentage)
   
   if heightDiff > 0 then
       -- Water is rising - create bubbles from bottom
       createWaterParticles(math.ceil(heightDiff * 30), 
           {particleY + 20, particleY + 50}, -50, {0.5, 1.0})
   elseif heightDiff < 0 then
       -- Water is falling - create droplets from surface
       createWaterParticles(math.ceil(-heightDiff * 30), 
           {particleY - 10, particleY + 10}, 100, {0.8, 1.2})
   end
...
local function startLoader()
   ...
   -- Initial water rise animation with dynamic filling
   enhanceWaterLevel(1, 2.5)
   
   -- Start with water droplets coming from top with physics
   ...
   for i, step in ipairs(loadingSteps) do
       ...
       -- Create ambient water particles during loading with varied effects for each stage
       if i == 1 then
           -- First batch: bubbles rising from bottom
           createWaterParticles(5, {200, 230}, -150, {0.5, 0.8})
       elseif i == 2 then 
           -- Second batch: droplets from top hitting surface
           createWaterParticles(8, {-10, 10}, 200, {0.8, 1.2})
       elseif i == 3 then
           -- Third batch: random movement in water
           createWaterParticles(6, {100, 150}, 50, {0.4, 0.7})
       else
           -- Final batch: more vigorous movement
           createWaterParticles(10, {50, 200}, 100, {0.6, 1.0})
       end
       
       -- Simulate small water movements during loading
       if math.random() < 0.7 then
           -- Create small ripples at random positions
           local rippleX = math.random(50, 400)
           local rippleSize = math.random(15, 30)
           createRipple(rippleX, rippleSize)
       end
   ...
   -- Water drain animation with enhanced splash effect
   createWaterParticles(MAX_PARTICLES, {50, 200}, 250, {0.8, 1.5})
   
   -- Add splash effect before draining
   for i = 1, 5 do
       createRipple(math.random(50, 400), math.random(30, 60))
       wait(0.1)
   end
   
   -- Drain water with elastic effect for natural movement
   enhanceWaterLevel(0, 1.8)
   ...
   for _, ripple in ipairs(ripplePool) do
       if ripple then
           ripple:Destroy()
       end
   end
