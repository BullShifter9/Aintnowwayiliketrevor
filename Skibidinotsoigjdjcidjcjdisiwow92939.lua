...
local RoleNotifyToggle = Tabs.Visuals:AddToggle("InstantRoleNotify", {
    Title = "Instant Role Notify",
    Default = false,
    Callback = function(toggle)
        _G.InstantRoleNotifyEnabled = toggle
        
        if toggle then
            local Players = game:GetService("Players")
            local localPlayer = Players.LocalPlayer
            local ReplicatedStorage = game:GetService("ReplicatedStorage")
            local RoleSelect = ReplicatedStorage.Extras.RoleSelect
            
            -- Function to create and display role notifications
            local function createRoleNotification(title, playerName, iconID)
            ...
            -- Function to identify players with roles
            local function identifyRoles()
                if not _G.InstantRoleNotifyEnabled then return end
                
                for _, player in pairs(Players:GetPlayers()) do
                    if player.Character then
                        -- Check for Murderer
                        if player.Character:FindFirstChild("Knife") then
                            createRoleNotification("Murderer Found", player.Name, "rbxassetid://6031075931")
                        end
                        
                        -- Check for Sheriff
                        if player.Character:FindFirstChild("Gun") or 
                           (player.Backpack and player.Backpack:FindFirstChild("Gun")) then
                            createRoleNotification("Sheriff Found", player.Name, "rbxassetid://6031075925")
                        end
                    end
                end
            end
            
            -- Monitor role selection event
            _G.RoleSelectConnection = RoleSelect.ChildAdded:Connect(function(child)
                -- Wait a short time for roles to be fully assigned
                task.wait(0.5)
                identifyRoles()
            end)
            
            -- Additional periodic check for roles
            _G.RoleCheckConnection = game:GetService("RunService").Heartbeat:Connect(function()
                -- Only run the check every second to avoid spamming
                if tick() % 1 < 0.1 then
                    identifyRoles()
                end
            end)
            ...
            if _G.RoleSelectConnection then
                _G.RoleSelectConnection:Disconnect()
                _G.RoleSelectConnection = nil
            end
            
            if _G.RoleCheckConnection then
                _G.RoleCheckConnection:Disconnect()
                _G.RoleCheckConnection = nil
            end
        end
    end
})
