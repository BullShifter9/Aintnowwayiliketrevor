...
Tabs.Main:AddToggle("MurdererDetect", {
   Title = "Notify Murderer",
   Default = true,
   Callback = function(Value)
       -- Update setting for persistence
       getgenv().settings.murdererDetect = Value
       
       -- Implement murderer detection logic
       if Value then
           -- Set up remote hook if not already established
           if not getgenv().murdererDetectionInitialized then
               local NotifyRemote = game:GetService("ReplicatedStorage").Remotes.Misc.GetPlayerProfile
               local oldNamecall
               ...
               oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                   ...
                   -- Intercept profile data calls to detect murderer
                   if method == "InvokeServer" and self == NotifyRemote and typeof(result) == "table" then
                       if result.EquippedPerk == "Murderer" and typeof(args[1]) == "Instance" then
                           -- Notify player of murderer detection
                           Fluent:Notify({
                               Title = "Murderer Detected!",
                               Content = args[1].Name .. " is the murderer",
                               Duration = 5
                           })
                       end
                   end
               ...
               getgenv().murdererDetectionInitialized = true
           end
       end
   end
})
