...
local CoinFarmer = {
    enabled = false,
    farmMethod = "TeleportTo", -- "TeleportTo" or "WalkTo"
    farmSpeed = 2, -- Collection speed (1-5 scale)
    safeMode = true, -- Uses smoother collection methods
    lastCoinTime = 0, -- For anti-detection cooldown
    targetCoin = nil,
    farmingActive = false
}
...
-- Reinitialize character references when player respawns
LocalPlayer.CharacterAdded:Connect(function(newCharacter)
    ...
    if CoinFarmer.enabled then
        CoinFarmer:StartFarming()
    end
end)

-- MM2-specific coin detection
...
function CoinFarmer:StartFarming()
    if self.farmingActive then return end
    self.farmingActive = true
    
    -- Initialize the farming loop
    self.farmingConnection = RunService.Heartbeat:Connect(function()
        if not self.enabled or not Character or not Character:FindFirstChild("HumanoidRootPart") then
            self:StopFarming()
            return
        end
        
        -- Respect cooldown for anti-detection
        local currentTime = tick()
        if currentTime - self.lastCoinTime < (6 - self.farmSpeed) * (self.safeMode and 0.5 or 0.2) then
            return
        end
        ...
        if coin then
            -- Collect the coin using selected method
            if self.farmMethod == "TeleportTo" then
                self:TeleportToCoin(coin)
            else
                self:WalkToCoin(coin)
            end
...
function CoinFarmer:StopFarming()
    ...
    if self.farmingConnection then
        self.farmingConnection:Disconnect()
        self.farmingConnection = nil
    end
    self.targetCoin = nil
end

-- Teleport directly to coin (faster but more detectable)
function CoinFarmer:TeleportToCoin(coin)
    -- Add slight randomization to position for anti-detection
    ...
    -- Teleport to coin
    HumanoidRootPart.CFrame = CFrame.new(coin.Position + offset)
    
    -- Small wait to ensure coin gets collected
    task.wait(0.1)
end

-- Walk to coin using Humanoid (slower but more natural)
function CoinFarmer:WalkToCoin(coin)
    if self.safeMode then
        -- Use tweening for smoother movement
        local distance = (HumanoidRootPart.Position - coin.Position).Magnitude
        local speed = math.min(distance / 5, self.farmSpeed)
        ...
        connection = RunService.Stepped:Connect(function()
            if (HumanoidRootPart.Position - coin.Position).Magnitude < 2 then
                arrived = true
        ...
        while not arrived and tick() - startTime < 3 do
            task.wait()
        end
...
-- Toggle auto farming
function CoinFarmer:Toggle(enabled)
...
function CoinFarmer:SetFarmMethod(method)
    self.farmMethod = method
end

-- Set safety level
function CoinFarmer:SetSafeMode(enabled)
    self.safeMode = enabled
end

-- Set collection speed
function CoinFarmer:SetFarmSpeed(speed)
    self.farmSpeed = math.clamp(speed, 1, 5)
end
...
if Tabs and Tabs.Main then
    -- Main toggle for auto farm
    Tabs.Main:AddToggle("CoinFarmToggle", {
        Title = "Auto Collect Coins",
        Default = false,
        Callback = function(Value)
            CoinFarmer:Toggle(Value)
        end
    })
    
    -- Collection method dropdown
    Tabs.Main:AddDropdown("FarmMethodDropdown", {
        Title = "Collection Method",
        Default = "TeleportTo",
        Values = {"TeleportTo", "WalkTo"},
        Callback = function(Value)
            CoinFarmer:SetFarmMethod(Value)
        end
    })
    
    -- Safe mode toggle
    Tabs.Main:AddToggle("SafeModeToggle", {
        Title = "Safe Mode",
        Default = true,
        Callback = function(Value)
            CoinFarmer:SetSafeMode(Value)
        end
    })
