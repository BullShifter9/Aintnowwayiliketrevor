...
local function breakGun()
   ...
   for _, player in pairs(players:GetPlayers()) do
       if player ~= localPlayer then
           ...
           -- Search for gun in both backpack and character
           ...
           if gunTool then
               -- Use the confirmed working remote path
               local success = pcall(function()
                   local args = {
                       [1] = 1,
                       [2] = Vector3.new(math.huge, math.huge, math.huge), -- Use extreme values to cause calculation errors
                       [3] = "AH2"
                   }
                   
                   -- Target the specific remote function that's vulnerable
                   local remoteFunction = gunTool:FindFirstChild("KnifeLocal")
                   if remoteFunction and remoteFunction:FindFirstChild("CreateBeam") then
                       remoteFunction.CreateBeam.RemoteFunction:InvokeServer(unpack(args))
                       
                       -- Secondary attack to ensure effectiveness
               ...
               if success then
                   ...
                   Fluent:Notify({
                       Title = "Gun Breaker",
                       Content = "Successfully targeted " .. player.Name .. "'s gun",
                       Duration = 2
                   })
               end
           end
   ...
   if successCount > 0 then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Applied exploits to " .. successCount .. " guns",
           Duration = 3
       })
   else
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "No targetable guns found",
           Duration = 2
       })
   end
end
...
local BreakGunToggle = Tabs.Main:AddToggle("BreakGunToggle", {
   ...
   Callback = function(toggle)
       if toggle then
           ...
           -- Setup continuous breaking with tactical timing
           ...
           breakGunConnection = game:GetService("RunService").Heartbeat:Connect(function()
               if not getgenv().breakGunDebounce then
                   ...
                   -- Optimal timing to avoid detection while maintaining effectiveness
