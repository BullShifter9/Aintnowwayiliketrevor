...
Tabs.Visuals:AddToggle("InstantRoleNotify", {
    ...
    Callback = function(toggle)
        ...
        if toggle then
            ...
            -- Function to determine player roles
            local function identifyRoles()
                if not _G.InstantRoleNotifyEnabled then return end
                
                local murderer, sheriff = nil, nil
                
                -- Identify murderer and sheriff by checking for tools
                for _, player in pairs(Players:GetPlayers()) do
                    if player.Character then
                ...
                -- Create notification when roles are found
                if murderer then
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Murderer Found",
                        Text = murderer.Name,
                        Icon = "rbxassetid://6031075931", -- Knife icon
                        Duration = 5
                    })
                end
                ...
                if sheriff then
                    game.StarterGui:SetCore("SendNotification", {
                        Title = "Sheriff Found",
                        Text = sheriff.Name,
                        Icon = "rbxassetid://6031075925", -- Gun icon
                        Duration = 5
                    })
                end
            end
            
            -- Monitor for role assignments
            _G.RoleCheckConnection = game:GetService("RunService").Heartbeat:Connect(function()
                identifyRoles()
            end)
            
            -- Additional check for when the round starts
            _G.RoundStartConnection = ReplicatedStorage.ChildAdded:Connect(function(child)
            ...
            if _G.RoundStartConnection then
                _G.RoundStartConnection:Disconnect()
                _G.RoundStartConnection = nil
            end
        end
    end
})
