-- HWID System with Robust Error Handling
-- Comprehensive executor detection and device fingerprinting solution

-- Initialize with defensive programming approach to prevent nil function calls
local function initialize_http_request()
    -- Systematically check all known HTTP request implementations
    if syn and type(syn.request) == "function" then 
        return syn.request
    elseif SENTINEL_V2 and type(request) == "function" then
        return function(tb)
    ...
    elseif type(request) == "function" then
        return request
    elseif httpservice and type(httpservice.request) == "function" then
        return httpservice.request
    else
        -- Fallback implementation that won't error but indicates HTTP is unavailable
        return function() 
            warn("No HTTP request functionality available")
            return {StatusCode = 404, Body = "{}"} 
        end
    end
end

-- Safely initialize the HTTP request function
local http_request = initialize_http_request()
...
-- Utility function for safe JSON operations
local function safe_json_encode(data)
    ...
    if not success then
        warn("JSON encode failed: " .. tostring(result))
        return "{}"
    end
...
local function safe_json_decode(json_string)
    ...
    local success, result = pcall(function()
        return game:GetService("HttpService"):JSONDecode(json_string)
    end)
...
-- Generate a consistent device HWID with error resilience
function generate_device_hwid()
    local system_info = {}
    local hwid_components = {}
    
    -- Collect screen resolution with proper error handling
    pcall(function()
        if game:GetService("GuiService") then
            local resolution = game:GetService("GuiService"):GetScreenResolution()
            if resolution then
                system_info.screen_resolution = resolution.X .. "x" .. resolution.Y
            end
        end
    end)
    
    -- Collect hardware information via HTTP with comprehensive error handling
    pcall(function()
        local request_result = http_request({
            Url = "https://httpbin.org/get",
            Method = "GET"
        })
        
        if request_result and type(request_result) == "table" and request_result.StatusCode == 200 and type(request_result.Body) == "string" then
            local json_data = safe_json_decode(request_result.Body)
            
            if type(json_data) == "table" and json_data.headers then
                -- Extract browser and hardware info from headers
    ...
    -- Collect DPI scale factor with error handling
    pcall(function()
        if workspace and workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize and 
           game:GetService("GuiService") and game:GetService("GuiService"):GetScreenResolution() then
            system_info.dpi_scale = string.format("%.3f", 
                workspace.CurrentCamera.ViewportSize.X / game:GetService("GuiService"):GetScreenResolution().X)
        end
    end)
    
    -- Collect graphics quality settings with error handling
    pcall(function()
        local user_settings = UserSettings()
        if user_settings then
            local game_settings = user_settings:GetService("UserGameSettings")
            if game_settings then
                system_info.graphics_quality = game_settings.SavedQualityLevel
                system_info.fullscreen_mode = tostring(game_settings.FullscreenMode)
            end
        end
    end)
    
    -- Performance benchmark with error handling
    pcall(function()
    ...
    pcall(function()
        ...
        if stats then
            ...
            if stats.GetCPUMemoryUsageMb then
                system_info.cpu_usage = string.format("%.2f", stats:GetCPUMemoryUsageMb())
            end
        end
    end)
    
    -- Generate HWID from collected components
    ...
    -- Ensure we have at least one component for HWID generation
    if #hwid_components == 0 then
        -- Fallback to time-based identifier with random salt
        local random_seed = math.random(1, 1000000)
        table.insert(hwid_components, "fallback:" .. os.time() .. "-" .. random_seed)
    end
    ...
    -- Create a hash of the combined string (FNV-1a hash algorithm variant)
    local hash = 0
    for i = 1, #combined_hwid do
        hash = (hash * 31 + string.byte(combined_hwid, i)) % 2147483647
    end
    
    -- Format final HWID with hex hash prefix and alphanumeric suffix
    local hex_hash = string.format("%x", hash)
    local cleaned_suffix = combined_hwid:gsub("[^a-zA-Z0-9]", "")
    local final_hwid = hex_hash .. "-" .. string.sub(cleaned_suffix, 1, 16)
    
    return final_hwid, system_info
end

-- Robust executor identification with feature detection
function identify_executor()
    -- Initialize with comprehensive detection patterns
    local executor_patterns = {
        {name = "Synapse X", check = function() return syn ~= nil and type(syn) == "table" and syn.protect_gui ~= nil end},
        {name = "ScriptWare", check = function() 
            return identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("ScriptWare") ~= nil 
        end},
        {name = "KRNL", check = function() 
            return KRNL_LOADED ~= nil or 
                  (identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("KRNL") ~= nil)
        end},
        {name = "Fluxus", check = function() 
            return fluxus ~= nil or 
                  (identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Fluxus") ~= nil)
        end},
        {name = "Oxygen U", check = function() 
            return identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Oxygen") ~= nil
        end},
        {name = "Electron", check = function() 
            return identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Electron") ~= nil
        end},
        {name = "Arceus X", check = function() 
            return identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Arceus") ~= nil
        end},
        {name = "Delta", check = function() 
            return delta ~= nil or 
                  (identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Delta") ~= nil)
        end},
        {name = "Hydrogen", check = function() 
            return hydrogen ~= nil or 
                  (identifyexecutor ~= nil and type(identifyexecutor) == "function" and 
                   identifyexecutor():find("Hydrogen") ~= nil)
        end}
    }
    
    -- Try to identify executor through safe feature detection
    ...
    -- Fallback detection based on capability hierarchy
    if hookfunction ~= nil or hookfunc ~= nil then return "Level 4 Executor" end
    if getgenv ~= nil or getgc ~= nil then return "Level 3 Executor" end
    if loadstring ~= nil or is_synapse_function ~= nil then return "Level 2 Executor" end
    if http_request ~= nil and type(http_request) == "function" then return "Level 1 Executor" end
    
    return "Unknown Executor"
end

-- Safe HTTP request with comprehensive error handling
function safe_http_request(request_data)
...
-- Send HWID data to Render.com Python API with robust error handling
function send_to_render(device_hwid, executor, player_info, system_info)
    -- Generate timestamp with proper formatting
    local timestamp = os.time()
    local formatted_time = os.date("%Y-%m-%d %H:%M:%S", timestamp)
    
    -- Construct payload with defensive null checks
    local payload = {
        hwid = device_hwid or "unknown",
        executor = executor or "unknown",
        player = {
            username = player_info and player_info.Username or "unknown",
            userId = player_info and player_info.UserId or 0,
            accountAge = player_info and player_info.AccountAge or 0
        },
        game = {
            placeId = game and game.PlaceId or 0,
            placeName = "unknown", -- Will be set below if available
    ...
    -- Safely get place name with error handling
    pcall(function()
        if game and game:GetService("MarketplaceService") then
            local info = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
            if info and info.Name then
                payload.game.placeName = info.Name
            end
        end
    end)
    
    -- Convert to JSON with error handling
    local json_payload = safe_json_encode(payload)
    
    -- Send to Render.com Python API with error handling
    ...
    if response and response.StatusCode and response.StatusCode >= 200 and response.StatusCode < 300 then
        local parsed_response = safe_json_decode(response.Body)
        
        if parsed_response and parsed_response.success then
            return true, parsed_response
        else
            return true, response.Body
        end
    else
        local status_code = response and response.StatusCode or "unknown"
        return false, "HTTP Error: " .. tostring(status_code)
    end
end

-- Check if HWID is allowed (whitelist check) with comprehensive error handling
function is_hwid_allowed(device_hwid)
    -- Validate input
    if not device_hwid or type(device_hwid) ~= "string" then
    ...
    if response and response.StatusCode == 200 and response.Body then
        local result = safe_json_decode(response.Body)
...
-- Ensures the player is fully loaded before execution
local function wait_for_player()
    -- Wait for game to load if not already loaded
    ...
    if not Players.LocalPlayer then
        ...
        -- If player is already added but reference isn't set yet
...
local function main()
    local success, result = pcall(function()
        ...
        -- Collect player information with null checks
        local player_info = {
        ...
        if send_success then
            ...
            -- Implement retry logic with proper task scheduling
            task.spawn(function()
                wait(5)
        ...
        return {
            ...
            is_whitelisted = is_allowed or false
        }
    end)
    ...
    if not success then
        ...
        -- Return fallback data that won't cause errors
        return {
...
-- Execute initialization with pcall to ensure no errors propagate
local success, hwid_data = pcall(main)

-- Provide safe default if main execution fails
if not success then
    warn("Critical HWID system failure: " .. tostring(hwid_data))
    hwid_data = {
        device_hwid = "critical-error-" .. tostring(os.time()),
        executor = "Unknown",
        system_info = {},
        is_whitelisted = false
    }
...
-- Store functions globally with safety checks
_G.get_device_hwid = function() 
    return hwid_data and hwid_data.device_hwid or "error" 
end

_G.get_executor = function() 
    return hwid_data and hwid_data.executor or "Unknown" 
end

_G.is_whitelisted = function() 
    if not hwid_data or not hwid_data.device_hwid then
        return false
    end
    local success, result = pcall(function()
...
-- Function to verify script access permission with error handling
_G.verify_access = function()
    if not hwid_data or not hwid_data.device_hwid then
        return false
    end
    
    local success, is_allowed, info = pcall(function()
        local allowed, info = is_hwid_allowed(hwid_data.device_hwid)
        return allowed, info
    end)
    
    if not success or not is_allowed then
        pcall(function()
...
-- Return HWID data for module usage
return hwid_data
