...
local function findSheriffGun()
   ...
   -- Get player roles using game's system
   local sheriffPlayer, gunTool
   ...
   if getPlayerDataRemote and getPlayerDataRemote:IsA("RemoteFunction") then
       ...
       if success and type(playerData) == "table" then
           -- Search for the Sheriff in the role data
   ...
   -- If sheriff player found, locate their gun
   if sheriffPlayer then
       -- Check backpack first
       for _, item in pairs(sheriffPlayer.Backpack:GetChildren()) do
       ...
       -- If not in backpack, check character
       if not gunTool and sheriffPlayer.Character then
           for _, item in pairs(sheriffPlayer.Character:GetChildren()) do
               if item.Name == "Gun" and item:IsA("Tool") then
                   gunTool = item
                   break
               end
...
-- Gun breaker function with precise exploitation techniques
local function breakGun()
   local sheriffPlayer, gunTool = findSheriffGun()
   ...
   if not gunTool then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Sheriff found but couldn't locate their gun",
           Duration = 2
       })
       return false
   end
   
   -- Exploit the gun using the known remote path
   local success = pcall(function()
       -- Target the specific RemoteFunction with precisely crafted arguments
       if gunTool:FindFirstChild("KnifeLocal") and 
          gunTool.KnifeLocal:FindFirstChild("CreateBeam") and
          gunTool.KnifeLocal.CreateBeam:FindFirstChild("RemoteFunction") then
           
           -- Use only the Vector3 argument for targeting
           gunTool.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(1, Vector3.new(math.huge, math.huge, math.huge), "AH2")
           
           -- Secondary exploit attempt with different parameters
   ...
   if success then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Successfully targeted " .. sheriffPlayer.Name .. "'s gun",
           Duration = 2
       })
       return true
   else
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Exploit attempt failed - gun may be protected",
           Duration = 2
       })
       return false
   end
...
local BreakGunToggle = Tabs.Main:AddToggle("BreakGunToggle", {
   ...
   Callback = function(toggle)
       if toggle then
           ...
           -- Setup recurring exploitation attempts with proper debounce
           breakGunConnection = game:GetService("RunService").Heartbeat:Connect(function()
               if not getgenv().breakGunDebounce then
                   getgenv().breakGunDebounce = true
                   
                   breakGun()
                   
                   -- Set appropriate delay to avoid detection systems
                   task.delay(2, function()
                       getgenv().breakGunDebounce = false
                   end)
               end
           end)
       else
