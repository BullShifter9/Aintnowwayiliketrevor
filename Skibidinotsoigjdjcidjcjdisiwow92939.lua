...
local COLLECTION_DISTANCE = 20        -- Maximum distance to detect gun drops
local MURDERER_AVOIDANCE_RANGE = 15   -- Distance to avoid murderers
local TELEPORT_DELAY = 0.1            -- Teleport back delay (reduced for speed)
local COLLECTION_ATTEMPTS = 3         -- Max attempts if collection fails
local BYPASS_METHOD = 2               -- Collection method (1=Standard, 2=NetworkOwnership, 3=SimRadius)

-- Core gun collection function with instant pickup optimization
function collectGunDrop()
   -- Find gun drop in workspace with distance verification
   ...
   -- Check if murderer is nearby (safety feature)
   if isMurdererNearby(MURDERER_AVOIDANCE_RANGE) then
       return false -- Skip collection when murderer is close
   end
   
   -- Store original position and camera state
   local originalPosition = playerChar.HumanoidRootPart.CFrame
   local originalCameraSubject = workspace.CurrentCamera.CameraSubject
   
   -- Apply pickup optimization based on selected method
   if BYPASS_METHOD == 2 then
       -- Network ownership bypass for faster collection
       local success = pcall(function()
           for _, v in pairs(getconnections(GunDrop.ChildAdded)) do v:Disable() end
           for _, v in pairs(getconnections(GunDrop:GetPropertyChangedSignal("CFrame"))) do v:Disable() end
       end)
   elseif BYPASS_METHOD == 3 then
       -- Simulation radius method for improved success rate
       local success = pcall(function()
   ...
   -- Execute precise teleportation sequence
   playerChar.HumanoidRootPart.CFrame = GunDrop.CFrame * CFrame.new(0, 1.5, 0)
   
   -- Wait minimal time for collection to register
   task.wait(TELEPORT_DELAY)
   
   -- Return to original position with error handling
   pcall(function()
       if playerChar and playerChar:FindFirstChild("HumanoidRootPart") then
           playerChar.HumanoidRootPart.CFrame = originalPosition
...
-- Murderer detection function for safety
function isMurdererNearby(range)
   ...
   if playerRoot and murdererRoot then
       return (playerRoot.Position - murdererRoot.Position).Magnitude <= range
   end
   
   return false
end

-- Utility function to identify the murderer in-game
function getMurderer()
   for _, player in pairs(game.Players:GetPlayers()) do
       if player.Character and player.Backpack then
           -- Check for knife in character or backpack
           if player.Character:FindFirstChild("Knife") or player.Backpack:FindFirstChild("Knife") then
               return player
           end
           
           -- Secondary check for murderer role
           if player:FindFirstChild("PlayerData") and 
              player.PlayerData:FindFirstChild("Role") and 
              player.PlayerData.Role.Value == "Murderer" then
               return player
           end
       end
   end
...
-- Continuous monitoring system for gun drops
local gunCollectConnection = nil
function enableInstantGunCollect(enabled)
   if enabled then
       if gunCollectConnection then gunCollectConnection:Disconnect() end
       
       gunCollectConnection = game:GetService("RunService").Heartbeat:Connect(function()
           local GunDrop = workspace:FindFirstChild("GunDrop")
           if GunDrop then
               local playerChar = game.Players.LocalPlayer.Character
               if not playerChar or not playerChar:FindFirstChild("HumanoidRootPart") then return end
               
               local distance = (playerChar.HumanoidRootPart.Position - GunDrop.Position).Magnitude
       ...
       if gunCollectConnection then
           gunCollectConnection:Disconnect()
           gunCollectConnection = nil
       end
   end
end
...
Tabs.Main:AddToggle("InstantGunCollect", {
   Title = "Instant Gun Collect",
   Default = false,
   Callback = function(Value)
       enableInstantGunCollect(Value)
   end
})

-- Add manual collection with safety checks
Tabs.Main:AddButton({
   Title = "Collect Gun (Safe)",
   Callback = function()
       if isMurdererNearby(MURDERER_AVOIDANCE_RANGE) then
           Fluent:Notify({
               Title = "Safety Alert",
               Content = "Murderer nearby - collection aborted",
               Duration = 3
           })
           return
       end
...
Tabs.Settings:AddSlider("GunCollectRange", {
   Title = "Gun Collection Range",
   Default = 20,
   Min = 5,
   Max = 50,
   Rounding = 0,
   Callback = function(Value)
       COLLECTION_DISTANCE = Value
   end
})

Tabs.Settings:AddSlider("MurdererAvoidance", {
   Title = "Murderer Avoidance Range",
   Default = 15,
   Min = 5,
   Max = 30,
   Rounding = 0,
   Callback = function(Value)
       MURDERER_AVOIDANCE_RANGE = Value
   end
})
