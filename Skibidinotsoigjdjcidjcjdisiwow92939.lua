...
local function breakGun()
   ...
   -- Find sheriff with gun
   for _, player in pairs(players:GetPlayers()) do
       if player ~= localPlayer then
           local gunTool
           
           -- Search backpack and character
           ...
           -- Execute server-side exploits if gun found
           if gunTool then
               pcall(function()
                   -- Primary exploit vector
                   if gunTool:FindFirstChild("KnifeServer") and gunTool.KnifeServer:FindFirstChild("ShootGun") then
                       -- Send invalid parameters to corrupt server-side state
                       gunTool.KnifeServer.ShootGun:InvokeServer(0/0, CFrame.new(0/0, 0/0, 0/0))
                       
                       -- Secondary exploit with extreme values
                       task.spawn(function()
                           gunTool.KnifeServer.ShootGun:InvokeServer(-1, CFrame.new(2e9, 2e9, 2e9))
                       end)
                       
                       -- Rapid fire exploit to overwhelm validation
                       task.spawn(function()
                           for i = 1, 8 do
                               gunTool.KnifeServer.ShootGun:InvokeServer(i, CFrame.new())
                               task.wait(0.01)
                           end
                       end)
                   end
               ...
               Fluent:Notify({
                   Title = "Gun Breaker",
                   Content = "Successfully broke gun",
                   Duration = 2
               })
           end
       end
...
local BreakGunToggle = Tabs.Main:AddToggle("BreakGunToggle", {
   ...
   Callback = function(toggle)
       if toggle then
           ...
           if gunBreakerConnection then
               if typeof(gunBreakerConnection) == "thread" then
                   task.cancel(gunBreakerConnection)
               else
                   gunBreakerConnection:Disconnect()
               end
           end
           
           gunBreakerConnection = task.spawn(function()
