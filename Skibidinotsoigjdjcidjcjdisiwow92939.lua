...
local RENDER_API_URL = "https://hwid-api-rubliexxx-2.onrender.com/api/hwid" -- Replace with your actual Render service URL
local API_KEY = "Supernaturalshithappenonearthniggers90908080" -- Must match the API_KEY in your Python backend

-- Generate a consistent device HWID that persists across accounts
function generate_device_hwid()
    -- Track collection of hardware identifiers
    local system_info = {}
    local hwid_components = {}
    
    -- Collect screen resolution (consistent per device)
    pcall(function()
        local gui_service = game:GetService("GuiService")
        system_info.screen_resolution = gui_service:GetScreenResolution().X .. "x" .. gui_service:GetScreenResolution().Y
    end)
    
    -- Collect hardware information via HTTP headers
    pcall(function()
        ...
        if request_result and request_result.StatusCode == 200 then
            local json_data = game:GetService("HttpService"):JSONDecode(request_result.Body)
            
            if json_data.headers then
            ...
            -- Network identifiers (useful but not primary identifier)
    ...
    -- Collect DPI scale factor (consistent per device setup)
    pcall(function()
    ...
    -- Collect graphics quality settings (consistent per device)
    pcall(function()
    ...
    -- Measure hardware performance (consistent on same hardware)
    pcall(function()
    ...
    -- Combine all collected info into consistent components
    ...
    -- Generate final HWID with unique identifier
    local final_hwid = hex_hash .. "-" .. string.sub(combined_hwid:gsub("[^a-zA-Z0-9]", ""), 1, 16)
    
    return final_hwid, system_info
end

-- Identify the executor without using specific fingerprints
function identify_executor()
    -- Feature detection for different executors
    ...
    -- Try to identify executor through feature detection
    ...
    -- Fallback detection based on available functions
    ...
    if http_request ~= nil then return "Level 1 Executor" end
    
    return "Unknown Executor"
end

-- Send HWID data to Render.com Python API
...
-- Check if HWID is allowed (whitelist check)
...
local function main()
    ...
    -- Generate device HWID (consistent across accounts)
    local device_hwid, system_info = generate_device_hwid()
    
    -- Identify executor separately
    ...
    -- Send data to Render.com Python API
    local success, result = send_to_render(device_hwid, executor, player_info, system_info)
    ...
    if success then
        ...
        warn("❌ Failed to store HWID:", result)
