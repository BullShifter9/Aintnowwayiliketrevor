...
-- Configuration
local API_URL = "https://hwid-api-rubliexxx-2.onrender.com/verify"
local REQUEST_TIMEOUT = 10  -- seconds
local CACHE_DURATION = 300  -- seconds

-- Private utilities
local function generateHash(input: string): string
    -- Jenkins hash implementation (optimized)
    local hash = 0
...
function HWIDSystem.generateHWID(): string
    local components = {
        RbxAnalytics:GetClientId(),
        tostring(Players.LocalPlayer.UserId),
        string.format("%.6f", GuiService:GetGuiInset().Y),
        string.format("%.6f", workspace.CurrentCamera.ViewportSize.X)
    }
    
    local rawHWID = table.concat(components, "-")
    return generateHash(rawHWID)
end

-- Verify with timeout handling
function HWIDSystem.verify()
    local hwid = HWIDSystem.generateHWID()
    local payload = HttpService:JSONEncode({hwid = hwid})
    
    -- Initialize result variables
    local success, authorized, isAdmin = false, false, false
    local errorMessage = nil
    
    -- Create a thread for the verification
    local verifyThread = coroutine.create(function()
        ...
        if requestSuccess and response.Success then
            ...
            errorMessage = requestSuccess and "Server error" or tostring(response)
        end
    end)
    
    -- Start verification thread
    coroutine.resume(verifyThread)
    
    -- Wait with timeout
    local startTime = os.clock()
    while coroutine.status(verifyThread) ~= "dead" do
        if os.clock() - startTime > REQUEST_TIMEOUT then
...
Tabs.Combat:AddToggle("FastSlashToggle", {
   Title = "Fast Slash",
   Default = false,
   Callback = function(enabled)
       SlashConfig.enabled = enabled
       if enabled then
           hookKnifeSystem()
       else
           if SlashConfig.connection then
...
-- Robust character lifecycle integration
LocalPlayer.CharacterAdded:Connect(function()
   if SlashConfig.enabled then
       task.wait(0.5)
       hookKnifeSystem()
   end
end)

-- Clean disconnection on player removal
Players.PlayerRemoving:Connect(function(player)
   if player == LocalPlayer and SlashConfig.connection then
       SlashConfig.connection:Disconnect()
       SlashConfig.connection = nil
   end
end)
