...
-- Create a draggable toggle button that works on PC and mobile
local ToggleGui = Instance.new("ScreenGui")
ToggleGui.Name = "OmniHubToggle"
ToggleGui.ResetOnSpawn = false
ToggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

-- Use proper parent based on exploit capabilities
if syn and syn.protect_gui then
...
-- Text indicator to show LeftControl shortcut
local KeyHint = Instance.new("TextLabel")
KeyHint.Name = "KeyHint"
KeyHint.Size = UDim2.new(0, 120, 0, 20)
KeyHint.Position = UDim2.new(0.5, -60, 0, -25)
...
KeyHint.Text = "Press LeftCtrl"
KeyHint.Font = Enum.Font.GothamSemibold
KeyHint.TextSize = 12
KeyHint.Visible = false
KeyHint.Parent = ToggleButton

-- Add corner to the KeyHint
...
local function simulateLeftControl()
    -- This simulates the same behavior as pressing LeftControl
    -- by utilizing Fluent UI's built-in minimize key
    local simulatedInput = {
        KeyCode = Enum.KeyCode.LeftControl,
        UserInputState = Enum.UserInputState.Begin
    }
    
    -- We call Window:Minimize() directly as this is equivalent to pressing LeftControl
    Window:Minimize()
    
    -- Update our tracking state
    isMinimized = not isMinimized
    
    -- Show/hide the key hint based on minimized state
    KeyHint.Visible = isMinimized
    
    -- Animate the key hint
    if isMinimized then
        KeyHint.Position = UDim2.new(0.5, -60, 0, -15)
        KeyHint.BackgroundTransparency = 1
        
        game:GetService("TweenService"):Create(KeyHint, 
            TweenInfo.new(0.3, Enum.EasingStyle.Quad), 
            {Position = UDim2.new(0.5, -60, 0, -25), BackgroundTransparency = 0.5}):Play()
    end
end
...
-- Listen for actual LeftControl presses to update our state tracking
