...
-- Perk Detection Configuration
local PerkDetection = {
    Enabled = true,
    NotifyOnMurdererPerk = true,
    CooldownBetweenChecks = 5, -- Seconds between profile checks to prevent spam
    LastCheckTime = 0
}
...
local function CreateNotification(title, text, duration)
    pcall(function()
        ...
        notification.Name = "PerkNotification"
        ...
        -- Use syn.protect_gui if available for better security
        if syn and syn.protect_gui then
        ...
        local tweenIn = TweenService:Create(frame, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {
        ...
        task.delay(duration, function()
            local tweenOut = TweenService:Create(frame, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {
...
-- Murderer Perk Detection
local MurdererPerkInfo = {
    LastKnownPerk = nil,
    HasNotified = false
}

-- Profile fetching function with rate limiting
local function CheckMurdererPerk()
    if not PerkDetection.Enabled or not Murder then return end
    
    -- Implement rate limiting to prevent spam
    local currentTime = tick()
    if currentTime - PerkDetection.LastCheckTime < PerkDetection.CooldownBetweenChecks then
        return
    end
    PerkDetection.LastCheckTime = currentTime
    
    -- Attempt to fetch the murderer's profile
    local success, profileData = pcall(function()
        return ReplicatedStorage.Remotes.Misc.GetPlayerProfile:InvokeServer(Murder)
    end)
    
    if not success or not profileData then return end
    
    -- Extract perk information
    local murdererPerk = nil
    if typeof(profileData) == "table" and profileData.EquippedPerk then
        murdererPerk = profileData.EquippedPerk
    end
    
    -- Only notify if the perk changed and notification hasn't been sent yet
    if murdererPerk and murdererPerk ~= MurdererPerkInfo.LastKnownPerk and not MurdererPerkInfo.HasNotified then
        MurdererPerkInfo.LastKnownPerk = murdererPerk
        MurdererPerkInfo.HasNotified = true
        
        CreateNotification("MURDERER PERK DETECTED", 
            "The murderer is using: " .. murdererPerk, 5)
    end
end

-- Add perk detection to role update logic
local function SetupRoleTracking()
    -- Clear previous connection if it exists
    if RoleUpdateConnection then
        RoleUpdateConnection:Disconnect()
    ...
    -- Reset perk tracking state when roles change
    MurdererPerkInfo.HasNotified = false
    MurdererPerkInfo.LastKnownPerk = nil
    
    -- Create new connection with proper error handling
    RoleUpdateConnection = RunService.Heartbeat:Connect(function()
        pcall(function()
            local oldMurderer = Murder
            ...
            -- If murderer has changed, reset perk notification state
            if oldMurderer ~= Murder then
                MurdererPerkInfo.HasNotified = false
                MurdererPerkInfo.LastKnownPerk = nil
            end
            
            -- Check murderer perk if enabled
            if PerkDetection.NotifyOnMurdererPerk and Murder then
                CheckMurdererPerk()
            end
        end)
    end)
...
-- Add toggle for perk detection in the UI
Tabs.Visuals:AddToggle("PerkDetectionToggle", {
   Title = "Notify Murderer Perk",
   Default = PerkDetection.NotifyOnMurdererPerk,
   Callback = function(Value)
       PerkDetection.NotifyOnMurdererPerk = Value
       -- Reset notification state when toggled on
       if Value then
           MurdererPerkInfo.HasNotified = false
       end
   end
