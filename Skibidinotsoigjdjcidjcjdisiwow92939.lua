...
-- Re-get character references when character changes
LocalPlayer.CharacterAdded:Connect(function(newCharacter)
    Character = newCharacter
    Humanoid = newCharacter:WaitForChild("Humanoid")
    HRP = newCharacter:WaitForChild("HumanoidRootPart")
    
    -- Reapply settings to new character
    applyCharacterSettings()
end)

-- Apply all active settings to character
function applyCharacterSettings()
    if PlayerMods.walkspeedEnabled then
        Humanoid.WalkSpeed = PlayerMods.walkspeedValue
    end
    
    if PlayerMods.jumppowerEnabled then
        Humanoid.JumpPower = PlayerMods.jumppowerValue
    end
    
    if PlayerMods.noclipEnabled then
        PlayerMods:SetNoclip(true)
    end
    
    if PlayerMods.floatEnabled then
        PlayerMods:SetFloat(true)
    end
end

-- WalkSpeed Functions
function PlayerMods:SetWalkspeed(enabled, value)
    self.walkspeedEnabled = enabled
    if value then self.walkspeedValue = value end
...
-- JumpPower Functions
function PlayerMods:SetJumpPower(enabled, value)
    self.jumppowerEnabled = enabled
    if value then self.jumppowerValue = value end
...
-- FOV Functions
function PlayerMods:SetFOV(enabled, value)
    self.fovEnabled = enabled
    if value then self.fovValue = value end
...
function PlayerMods:SetXRay(enabled, value)
    self.xrayEnabled = enabled
    if value then self.xrayValue = value end
    ...
    if enabled then
        -- Apply X-Ray effect to all parts
        for _, part in pairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") and not part:IsDescendantOf(Character) then
                part.LocalTransparencyModifier = self.xrayValue
            end
        end
        
        -- Keep X-Ray updated with new parts
...
-- Touch Teleport Functions (Mobile version of Click TP)
local touchTpEnabled = false
local touchTpButton

function PlayerMods:SetTouchTp(enabled)
    self.touchTpEnabled = enabled
    
    if enabled then
        -- Create a teleport button for mobile
        if not touchTpButton then
            touchTpButton = Instance.new("TextButton")
            touchTpButton.Name = "TpButton"
            touchTpButton.Size = UDim2.new(0, 60, 0, 60)
            touchTpButton.Position = UDim2.new(0.9, -30, 0.6, -30)
            touchTpButton.AnchorPoint = Vector2.new(0.5, 0.5)
            touchTpButton.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
            touchTpButton.BackgroundTransparency = 0.3
            touchTpButton.Text = "TP"
            touchTpButton.TextColor3 = Color3.fromRGB(255, 255, 255)
            touchTpButton.Font = Enum.Font.GothamBold
            touchTpButton.TextSize = 20
            touchTpButton.BorderSizePixel = 0
            
            -- Add corner rounding
            local corner = Instance.new("UICorner")
            corner.CornerRadius = UDim.new(1, 0) -- Make it circular
            corner.Parent = touchTpButton
            
            touchTpButton.Parent = LocalPlayer.PlayerGui:WaitForChild("ScreenGui") or
                                    LocalPlayer.PlayerGui:FindFirstChildOfClass("ScreenGui") or
                                    (function()
            ...
            -- Touch activated teleport
            touchTpButton.TouchTap:Connect(function()
                -- Enter teleport mode
                touchTpButton.BackgroundColor3 = Color3.fromRGB(255, 0, 0) -- Red during active TP mode
                
                -- Set up a one-time tap handler for destination selection
                local tapConn
                tapConn = UserInputService.TouchTap:Connect(function(_, gameProcessed)
                    if not gameProcessed then
                        -- Get tap position and raycast to find destination
                        local touchPosition = UserInputService:GetLastTouchPosition()
                        ...
                        -- End teleport mode
                        touchTpButton.BackgroundColor3 = Color3.fromRGB(255, 165, 0) -- Reset button color
                        tapConn:Disconnect()
                    end
                end)
            end)
        else
            touchTpButton.Visible = true
        end
    else
        -- Hide the teleport button
        if touchTpButton then
            touchTpButton.Visible = false
        end
    end
end
...
local jumpButton
local originalJumpFunction
function PlayerMods:SetInfiniteJump(enabled)
    self.infiniteJumpEnabled = enabled
    
    -- For mobile we need to override the jump button
    if enabled then
        -- Find the jump button in PlayerGui
        local jumpButton = LocalPlayer.PlayerGui:FindFirstChild("TouchGui", true)
        if jumpButton then
            local jumpControl = jumpButton:FindFirstChild("TouchControlFrame"):FindFirstChild("JumpButton")
            
            if jumpControl and not originalJumpFunction then
                -- Store original jump function
                originalJumpFunction = jumpControl.TouchEnded:Connect(function()
                    if Character and Humanoid then
        ...
        -- Connect to TouchTap as fallback
        UserInputService.TouchTap:Connect(function()
            if self.infiniteJumpEnabled and Humanoid and UserInputService:IsKeyDown(Enum.KeyCode.Space) then
                Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
            end
        end)
    end
...
function PlayerMods:SetFloat(enabled)
    self.floatEnabled = enabled
    ...
    if enabled then
        ...
        floatConnection = RunService.Heartbeat:Connect(function()
            if Character and HRP then
                floatPart.CFrame = CFrame.new(HRP.Position.X, HRP.Position.Y - 3.5, HRP.Position.Z)
            end
        end)
    end
end
...
local function breakGun()
   ...
   for _, player in pairs(players:GetPlayers()) do
       if player ~= localPlayer then
           ...
           -- Optimized search pattern for gun across player containers
           for _, container in ipairs({player.Backpack, player.Character}) do
               if container then
                   gunTool = container:FindFirstChild("Gun")
                   if gunTool and gunTool:IsA("Tool") then break end
               end
           end
           
           -- Attempt to break the gun using the server endpoint
           if gunTool then
               local success = pcall(function()
                   gunTool.KnifeServer.ShootGun:InvokeServer(0, CFrame.new(), "AH")
               end)
   ...
   -- Summary notification if multiple guns broken
   if successCount > 1 then
       Fluent:Notify({
           Title = "Gun Breaker Summary",
           Content = "Broke " .. successCount .. " guns in total",
           Duration = 3
       })
   elseif successCount == 0 then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "No guns found to break",
           Duration = 2
       })
   end
end
...
if Tabs and Tabs.Main then
    ...
    Tabs.Main:AddSlider("WalkspeedSlider", {
        ...
        Rounding = 0,
        Callback = function(Value)
            PlayerMods.walkspeedValue = Value
            if PlayerMods.walkspeedEnabled then
                PlayerMods:SetWalkspeed(true)
            end
        end
    })
    ...
    Tabs.Main:AddSlider("JumppowerSlider", {
        ...
        Rounding = 0,
        Callback = function(Value)
            PlayerMods.jumppowerValue = Value
            if PlayerMods.jumppowerEnabled then
                PlayerMods:SetJumpPower(true)
            end
        end
    })
    ...
    Tabs.Main:AddSlider("FOVSlider", {
        ...
        Rounding = 0,
        Callback = function(Value)
            PlayerMods.fovValue = Value
            if PlayerMods.fovEnabled then
                PlayerMods:SetFOV(true)
            end
        end
    })
    ...
    Tabs.Main:AddToggle("XRayToggle", {
        ...
        Default = false,
        Callback = function(Value)
            PlayerMods:SetXRay(Value)
        end
    })
    
    Tabs.Main:AddSlider("XRaySlider", {
        ...
        Rounding = 2,
        Callback = function(Value)
            PlayerMods.xrayValue = Value
            if PlayerMods.xrayEnabled then
                PlayerMods:SetXRay(true)
            end
        end
    })
    
    -- Mobile Movement Modifications
    Tabs.Main:AddToggle("NoclipToggle", {
        Title = "Noclip",
        Default = false,
        Callback = function(Value)
            PlayerMods:SetNoclip(Value)
        end
    })
    
    Tabs.Main:AddToggle("TouchTpToggle", {
        Title = "Click Tp",
        Default = false,
        Callback = function(Value)
            PlayerMods:SetTouchTp(Value)
        end
    })
    ...
    Tabs.Main:AddToggle("InfiniteJumpToggle", {
        ...
        Default = false,
        Callback = function(Value)
            PlayerMods:SetInfiniteJump(Value)
        end
    })
...
---Break Gun
Tabs.Combat:AddButton("BreakGunToggle", {
   Title = "Break Gun",
   Default = false,
