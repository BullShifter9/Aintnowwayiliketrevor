...
-- Author: Your Name
-- Description: System to verify authorized users connecting to your game
...
-- Configuration
local API_BASE_URL = "https://hwid-api-rubliexxx-2.onrender.com"
local ADMIN_KEY = "Supernaturalshithappenonearthniggers90908080"
local KICK_DELAY = 1.5 -- Time in seconds before kicking unauthorized players

-- Cache system to reduce API calls
local HWIDCache = {}

--[[
    HWIDSystem - Main module for HWID verification
]]
local HWIDSystem = {}

-- Generates a hardware ID for the player
function HWIDSystem.GetPlayerHWID(player)
    if not player then return nil end
    
    -- Create a unique identifier using available player data
    -- This is a simple implementation - enhance as needed for security
    local hwid = tostring(player.UserId) .. "_" .. player.Name
    return hwid
end

-- Verifies if a player is authorized to join
function HWIDSystem.VerifyPlayer(player)
    local hwid = HWIDSystem.GetPlayerHWID(player)
    
    -- Check cache first to minimize API calls
    if HWIDCache[hwid] then
        return HWIDCache[hwid].authorized, HWIDCache[hwid].admin
    end
    
    -- Build verification URL with query parameters
    local verifyURL = API_BASE_URL .. "/verify?hwid=" .. HttpService:UrlEncode(hwid)
    
    -- Send API request
    local success, result = pcall(function()
        local response = HttpService:GetAsync(verifyURL)
    ...
    if success then
        ...
        return result.success, result.admin
    else
        warn("[HWID System] Verification failed for player: " .. player.Name)
        return false, false
    end
end

-- Admin functions for managing HWIDs
function HWIDSystem.AddHWID(hwid, isAdmin)
    -- Build admin action URL with query parameters
    local adminURL = API_BASE_URL .. "/admin?admin_key=" .. HttpService:UrlEncode(ADMIN_KEY) 
                  .. "&action=add_hwid&hwid=" .. HttpService:UrlEncode(hwid)
                  .. "&is_admin=" .. tostring(isAdmin or false)
    
    -- Send API request
    local success, result = pcall(function()
        local response = HttpService:GetAsync(adminURL)
        return HttpService:JSONDecode(response)
    end)
    
    if success then
        return result.success, result.message or "Operation completed"
    else
        return false, "API request failed"
    end
end

function HWIDSystem.RemoveHWID(hwid)
    -- Build admin action URL with query parameters
    local adminURL = API_BASE_URL .. "/admin?admin_key=" .. HttpService:UrlEncode(ADMIN_KEY)
                  .. "&action=remove_hwid&hwid=" .. HttpService:UrlEncode(hwid)
...
function HWIDSystem.ListHWIDs()
    ...
    if success and result.success then
        return true, result.hwids or {}, result.admin_hwids or {}
    else
        return false, "Failed to retrieve HWID list"
    end
end

-- Handle player join events
local function onPlayerJoin(player)
    -- Verify the player's HWID
    local isAuthorized, isAdmin = HWIDSystem.VerifyPlayer(player)
    
    if not isAuthorized then
        -- Kick unauthorized players after a short delay (allows kick message to be seen)
        task.delay(KICK_DELAY, function()
            if player and player:IsDescendantOf(game) then
                player:Kick("Unauthorized access. Please contact an administrator.")
            end
        end)
        
        -- Log unauthorized access attempt
        warn("[HWID System] Unauthorized access attempt: " .. player.Name)
        return
    end
    
    -- Set up player attributes based on authorization level
    player:SetAttribute("Authorized", true)
    player:SetAttribute("Admin", isAdmin)
    
    -- Create authorization marker
    local authMarker = Instance.new("StringValue")
    authMarker.Name = "AuthorizationStatus"
    authMarker.Value = isAdmin and "Admin" or "User"
    authMarker.Parent = player
    
    -- Log successful authorization
    if isAdmin then
        print("[HWID System] Admin authorized: " .. player.Name)
    else
        print("[HWID System] User authorized: " .. player.Name)
    end
    
    -- Initialize any game-specific features for authorized players
    -- You can add your game-specific code here
end

-- Set up admin command handling
local function setupAdminCommands()
    -- Create RemoteFunction for secure admin operations
    local adminRemote = Instance.new("RemoteFunction")
    adminRemote.Name = "AdminCommandHandler"
    ...
    adminRemote.OnServerInvoke = function(player, command, ...)
        -- Verify admin status
        local isAuthorized, isAdmin = HWIDSystem.VerifyPlayer(player)
        if not (isAuthorized and isAdmin) then
            return false, "Unauthorized: Admin permissions required"
        end
        ...
        if command == "add_user" then
            local targetHWID = args[1]
            local makeAdmin = args[2] == true
            
            if not targetHWID or targetHWID == "" then
                return false, "Invalid HWID provided"
            end
            
            return HWIDSystem.AddHWID(targetHWID, makeAdmin)
            
        elseif command == "remove_user" then
            local targetHWID = args[1]
            ...
            return HWIDSystem.RemoveHWID(targetHWID)
            
        elseif command == "list_users" then
            return HWIDSystem.ListHWIDs()
            
        elseif command == "get_hwid" then
            local targetPlayer = args[1]
            if typeof(targetPlayer) == "Instance" and targetPlayer:IsA("Player") then
                return true, HWIDSystem.GetPlayerHWID(targetPlayer)
            else
...
local function init()
    ...
    -- Set up admin command system
    setupAdminCommands()
    
    -- Check any players who might already be in the game
    for _, player in ipairs(Players:GetPlayers()) do
        task.spawn(onPlayerJoin, player)
    end
    
    print("[HWID System] Initialized successfully")
end

-- Start the system
init()

-- Return the module for use by other scripts
return HWIDSystem
...
-- Improved gun detection function using direct tool reference matching
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local RunService = game:GetService("RunService")
...
-- Core variable setup
local LocalPlayer = Players.LocalPlayer
local breakGunConnection = nil
local lastBreakAttempt = 0
local breakCooldown = 0.5 -- Half-second cooldown between break attempts

-- Function to find the sheriff and their gun with enhanced detection
local function findSheriffAndGun()
   local sheriffPlayer, gunTool = nil, nil
   local getPlayerDataRemote = ReplicatedStorage:FindFirstChild("GetPlayerData", true)
   
   -- Method 1: Use MM2's built-in role system
   ...
   -- Method 2: Fallback to manual inspection of player inventories
   if not sheriffPlayer then
       for _, player in pairs(Players:GetPlayers()) do
           if player ~= LocalPlayer then
               -- Check player character and backpack
   ...
   -- If we found sheriff through Method 1 but not their gun, locate it now
   if sheriffPlayer and not gunTool then
       for _, container in pairs({sheriffPlayer.Character, sheriffPlayer.Backpack}) do
           if container then
               for _, item in pairs(container:GetChildren()) do
                   if item.Name == "Gun" and item:IsA("Tool") then
                       gunTool = item
   ...
   return sheriffPlayer, gunTool
end

-- Focused gun breaking function with two primary exploit vectors
local function breakGun()
   if tick() - lastBreakAttempt < breakCooldown then return false end
   lastBreakAttempt = tick()
   
   local sheriffPlayer, gunTool = findSheriffAndGun()
   
   if not sheriffPlayer then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "No sheriff detected in this round",
           Duration = 1.5
       })
       return false
   end
   ...
   if not gunTool then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Sheriff found but gun not accessible",
           Duration = 1.5
       })
       return false
   end
   
   -- Multiple exploit vectors to maximize chances of success
   local exploitSucceeded = false
   
   -- Vector 1: Target the CreateBeam RemoteFunction
   -- This remote is used to create the visual beam when firing, corrupting it breaks the gun
   if gunTool:FindFirstChild("KnifeLocal") and gunTool.KnifeLocal:FindFirstChild("CreateBeam") and 
      gunTool.KnifeLocal.CreateBeam:FindFirstChild("RemoteFunction") then
       
       pcall(function()
           -- We use InvokeServer because the gun expects a return value from this remote
           -- Using extreme Vector3 values causes calculation errors in the gun's logic
           gunTool.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(1, Vector3.new(math.huge, math.huge, math.huge), "AH2")
           
           -- Follow up with a secondary attack to ensure the gun state is completely corrupted
           task.delay(0.1, function()
               gunTool.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(0, Vector3.new(-math.huge, -math.huge, -math.huge), "AH2")
           end)
       end)
   ...
   -- Vector 2: Target the ShootGun RemoteFunction
   -- This is the main firing mechanism, corrupting it directly prevents shooting
   if not exploitSucceeded and gunTool:FindFirstChild("KnifeServer") and gunTool.KnifeServer:FindFirstChild("ShootGun") then
       pcall(function()
           -- InvokeServer is required as the gun expects a response to validate the shot
           gunTool.KnifeServer.ShootGun:InvokeServer(math.huge, CFrame.new(0, math.huge, 0))
           
           -- Secondary attack with different parameters to ensure corruption
           task.delay(0.1, function()
               gunTool.KnifeServer.ShootGun:InvokeServer(0, CFrame.new(math.huge, 0, math.huge))
           end)
       end)
   ...
   -- Vector 4: Brute force attack on all gun remotes
   if not exploitSucceeded then
       local remoteCount = 0
       pcall(function()
           for _, obj in pairs(gunTool:GetDescendants()) do
               if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                   -- RemoteFunctions require InvokeServer because they expect a return value
                   -- Using incorrect argument types or extreme values causes server-side errors
                   if obj:IsA("RemoteFunction") then
                       obj:InvokeServer(math.huge, Vector3.new(math.huge, math.huge, math.huge))
                   else
                       obj:FireServer(math.huge, Vector3.new(math.huge, math.huge, math.huge))
                   end
                   remoteCount += 1
               end
           end
       ...
       exploitSucceeded = remoteCount > 0
   end
   
   if exploitSucceeded then
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "Exploited " .. sheriffPlayer.Name .. "'s gun",
           Duration = 1.5
       })
       return true
   else
       Fluent:Notify({
           Title = "Gun Breaker",
           Content = "All exploit vectors failed",
           Duration = 1.5
       })
       return false
   end
...
---Break Gun
local BreakGunToggle = Tabs.Main:AddToggle("BreakGunToggle", {
   Title = "Break Sheriff Gun",
   Default = false,
   Callback = function(toggle)
       if toggle then
           -- Perform initial break attempt
           breakGun()
           
           -- Clean up existing connection if present
           if breakGunConnection then
               breakGunConnection:Disconnect()
               breakGunConnection = nil
           end
           
           -- Set up automatic breaking on interval
           breakGunConnection = RunService.Heartbeat:Connect(function()
               local currentTime = tick()
               
               -- Attempt to break the gun every 2 seconds
               if currentTime - lastBreakAttempt >= 2 then
                   breakGun()
               end
           end)
       else
           -- Clean disconnection when toggled off
           if breakGunConnection then
               breakGunConnection:Disconnect()
               breakGunConnection = nil
           end
...
-- Button for manual breaking
Tabs.Main:AddButton({
   Title = "Force Break Gun (Manual)",
   Callback = function()
       breakGun()
   end
})
