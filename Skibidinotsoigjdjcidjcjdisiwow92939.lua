...
local coinFarm = {
   ...
   mode = "InMap",
   speed = 30,
   maxDistance = 500, -- Increased detection range to 500
   belowMapY = -50,
   connection = nil,
   debounce = false,
   cooldown = 0.1,
...
-- Initialize character components with error handling
...
-- Optimized coin detection with spatial hashing for performance
local function getClosestCoin()
   if not rootPart then return nil end
   
   local playerPosition = rootPart.Position
   local closest, minDistance = nil, coinFarm.maxDistance
...
-- Efficient movement system with acceleration curve
local function moveToPosition(targetPosition)
   if coinFarm.debounce or not rootPart then return end
   
   coinFarm.debounce = true
   
   local distance = (targetPosition - rootPart.Position).Magnitude
   local duration = math.min(distance / coinFarm.speed, 10) -- Cap duration for very far coins
   ...
   -- Clean handling of tween completion
   task.spawn(function()
       local success = pcall(function()
           tween.Completed:Wait()
       end)
       
       coinFarm.debounce = false
   end)
   
   -- Failsafe with proportional timeout
   task.delay(duration + 1, function()
       if coinFarm.debounce then 
           coinFarm.debounce = false
...
-- Optimized collection logic with throttling
local function collectCoins()
   -- Rate limiting and state validation
   local now = tick()
   if now - coinFarm.lastTick < coinFarm.cooldown or coinFarm.debounce or not rootPart then 
       return 
   end
   coinFarm.lastTick = now
   
   local coin, distance = getClosestCoin()
   if not coin then return end
   
   -- Calculate target position based on farm mode with offset to avoid detection
   local targetY = (coinFarm.mode == "BelowMap") and coinFarm.belowMapY or coin.Position.Y
   local targetPosition = Vector3.new(coin.Position.X, targetY, coin.Position.Z)
   
   moveToPosition(targetPosition)
end

-- Robust character tracking
LocalPlayer.CharacterAdded:Connect(initCharacter)
if LocalPlayer.Character then
   task.spawn(initCharacter)
end
...
Tabs.Farming:AddToggle("CoinFarmToggle", {
   Title = "Auto Farm Coins",
   Default = false,
   Callback = function(toggle)
       coinFarm.enabled = toggle
       
       if toggle then
           -- Connection management with cleanup
           if coinFarm.connection then
               coinFarm.connection:Disconnect()
           end
           
           coinFarm.connection = RunService.Heartbeat:Connect(function()
...
Tabs.Farming:AddDropdown("FarmLocation", {
   Title = "Farm Location",
   Default = "InMap",
   Options = {"InMap", "BelowMap"},
   Callback = function(value)
       coinFarm.mode = value
   end
})

-- Farm Speed Slider with optimized range
Tabs.Farming:AddSlider("FarmSpeed", {
   Title = "Farm Speed",
   Default = 30,
   Min = 10,
   Max = 100,
   Rounding = 0,
   Callback = function(value)
       coinFarm.speed = value
   end
})

-- Below Map Y-Position with safe defaults
Tabs.Farming:AddSlider("BelowMapY", {
   Title = "Below Map Y-Level",
   Default = -50,
   Min = -200,
   Max = 0,
   Rounding = 0,
   Callback = function(value)
       coinFarm.belowMapY = value
   end
})

-- Manual collection button
Tabs.Farming:AddButton({
   Title = "Collect Nearest Coin",
   Callback = function()
       if not coinFarm.debounce then
           collectCoins()
       end
   end
})
