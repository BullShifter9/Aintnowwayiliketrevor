...
-- Verifies players connecting to your game against an authorization API
local HttpService = game:GetService("HttpService")
local Players = game:GetService("Players")

-- Configuration
local API_URL = "https://your-api-domain.com/verify"
...
local function GetPlayerHWID(player)
    ...
    -- Simple HWID generation - modify as needed for your security requirements
    return tostring(player.UserId) .. "_" .. player.Name
end

-- Verifies if a player is authorized
local function VerifyPlayer(player)
    local hwid = GetPlayerHWID(player)
    
    -- Check cache first to reduce API calls
    if verifiedPlayers[hwid] then
        return verifiedPlayers[hwid].authorized, verifiedPlayers[hwid].admin
    end
    
    -- Build the verification URL with query parameter
    local verifyURL = API_URL .. "?hwid=" .. HttpService:UrlEncode(hwid)
    DebugLog("Verifying player: " .. player.Name)
    
    -- Make the API request
    local success, response = pcall(function()
        return HttpService:GetAsync(verifyURL)
    end)
    
    -- Handle potential errors
    if not success then
        DebugLog("API request failed: " .. tostring(response))
        return false, false
    end
    ...
    if not success then
        DebugLog("Failed to parse response: " .. tostring(data))
        return false, false
    end
    
    -- Cache the verification result
    verifiedPlayers[hwid] = {
        authorized = data.success,
        admin = data.admin
    }
    
    DebugLog("Player " .. player.Name .. " authorization: " .. tostring(data.success))
    return data.success, data.admin
end
...
local function OnPlayerJoin(player)
    ...
    -- Give the player a moment to fully connect before verification
    task.wait(1)
    
    -- Verify the player's authorization
    local isAuthorized, isAdmin = VerifyPlayer(player)
    
    -- Handle unauthorized players
    if not isAuthorized then
        DebugLog("Unauthorized player: " .. player.Name)
        player:Kick("Unauthorized access. Please contact an administrator.")
        return
    end
    
    -- Set up player attributes based on authorization
    player:SetAttribute("Authorized", true)
    player:SetAttribute("Admin", isAdmin)
    
    -- Create authorization marker object (useful for other scripts)
    ...
    if isAdmin then
        DebugLog("Admin authorized: " .. player.Name)
    else
        DebugLog("User authorized: " .. player.Name)
    end
end
...
-- Handle existing players (in case script runs after players have joined)
...
local function TestAPIConnection()
    ...
    local success, response = pcall(function()
        return HttpService:GetAsync(API_URL .. "?hwid=connection_test")
    end)
    ...
    if success then
        ...
        warn("HWID verification may not work correctly")
    end
end

-- Initialize system
TestAPIConnection()
DebugLog("HWID Verification System initialized")

-- Return API for other scripts to use if needed
return {
    VerifyPlayer = VerifyPlayer,
    GetPlayerHWID = GetPlayerHWID
}
...
function BreakGun:IsSheriff()
    ...
    pcall(function()
        if game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true) then
            ...
            -- Check if local player is Sheriff
            local localPlayerName = localPlayer.Name
            if playerData[localPlayerName] and playerData[localPlayerName].Role == "Sheriff" then
                return true
            end
        end
    end)
...
-- Function to break the gun (disable shooting)
function BreakGun:BreakSheriffGun()
    if self.Breaking or not self.Enabled then return end
    self.Breaking = true
    
    -- Only proceed if player is Sheriff
    if not self:IsSheriff() then
        self.Breaking = false
        return
    end
    
    -- Find ShootGun remote event
    local shootGunRemote = game:GetService("ReplicatedStorage"):FindFirstChild("ShootGun", true)
    
    if shootGunRemote and shootGunRemote:IsA("RemoteEvent") then
        -- Hook the remote event to prevent shooting
        local oldNamecall
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if method == "FireServer" and self == shootGunRemote then
                -- Block the ShootGun FireServer call
                return nil
            end
        ...
        game.StarterGui:SetCore("SendNotification", {
            Title = "Gun Disabled",
            Text = "Sheriff's gun has been broken",
            Duration = 3
        })
    end
...
-- Setup gun break on round start
local roundStartedConnection = nil
if game:GetService("ReplicatedStorage"):FindFirstChild("RoundStarted", true) then
    roundStartedConnection = game:GetService("ReplicatedStorage"):FindFirstChild("RoundStarted", true).Event:Connect(function()
        -- Wait a moment for roles to be assigned
        task.wait(1)
        if BreakGun.Enabled then
            BreakGun:BreakSheriffGun()
        end
    end)
end

-- Auto Notify Murderers Perk Feature
local AutoNotifyMurderersPerk = {
    Enabled = true,
    NotificationDuration = 4,
    DetectedMurderers = {}
}
...
function AutoNotifyMurderersPerk:ShowMurdererPerkNotification(playerName, perkName)
    -- Skip if disabled or already detected this murderer
    if not self.Enabled or self.DetectedMurderers[playerName] then return end
    
    -- Mark as detected
    self.DetectedMurderers[playerName] = true
    
    -- Use existing notification system or create a simple one
    game.StarterGui:SetCore("SendNotification", {
        Title = "MURDERER DETECTED",
        Text = playerName .. " using " .. perkName .. " perk!",
        Icon = "rbxassetid://6031071053", -- Warning icon
        Duration = self.NotificationDuration
    })
end

-- Function to scan for murderer perks
function AutoNotifyMurderersPerk:ScanForMurderers()
    -- Attempt to get player data
    pcall(function()
        if game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true) then
            local playerData = game:GetService("ReplicatedStorage"):FindFirstChild("GetPlayerData", true):InvokeServer()
            
            -- Check all players for Murderer role
            for playerName, data in pairs(playerData) do
                if data.Role == "Murderer" then
                    -- Check if we can access the player's equipped perk
                    local player = game.Players:FindFirstChild(playerName)
                    if player and player:FindFirstChild("EquippedPerk") then
                        local perkName = player.EquippedPerk.Value or "Unknown"
                        self:ShowMurdererPerkNotification(playerName, perkName)
                    else
                        -- If we can't directly access the perk, check if it's in the player data
                        local perkName = data.EquippedPerk or "Unknown"
                        self:ShowMurdererPerkNotification(playerName, perkName)
                    end
                end
            end
...
-- Reset detected murderers when round restarts
local function ResetDetectedMurderers()
    AutoNotifyMurderersPerk.DetectedMurderers = {}
end

-- Setup round events
local function SetupRoundEvents()
    -- Look for common round indicators
    local roundStarted = game:GetService("ReplicatedStorage"):FindFirstChild("RoundStarted", true)
    local roundEnded = game:GetService("ReplicatedStorage"):FindFirstChild("RoundEnded", true)
    
    if roundStarted then
        roundStarted.Event:Connect(function()
            ResetDetectedMurderers()
        end)
    end
...
pcall(SetupRoundEvents)

-- Start scanning loop
local scanConnection = game:GetService("RunService").Heartbeat:Connect(function()
    if AutoNotifyMurderersPerk.Enabled then
        AutoNotifyMurderersPerk:ScanForMurderers()
    end
end)
...
if Tabs and Tabs.Main then
    Tabs.Main:AddToggle("BreakGunToggle", {
        Title = "Break Sheriff Gun",
        Default = false,
        Callback = function(Value)
            BreakGun.Enabled = Value
...
if Tabs and Tabs.Main then
    Tabs.Main:AddToggle("MurdererPerkNotifyToggle", {
        Title = "Auto Notify Murderers Perk",
        Default = true,
        Callback = function(Value)
            AutoNotifyMurderersPerk.Enabled = Value
            
            -- Reset detections when re-enabling
            if Value then
                ResetDetectedMurderers()
            end
        end
