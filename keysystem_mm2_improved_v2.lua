...
GameplayEvents.RoundEndFade.OnClientEvent:Connect(function()
 ...
 state.sheriff = nil
 state.hero = false
 -- Clear gun pickup debounce so next round works fresh
 gunPickupDebounce = {}
end)
...
-- Per-player debounce for gun-pickup notification.
-- Prevents multiple ChildAdded listeners (backpack + character + CharacterAdded
-- accumulating over rounds) from all firing at once and spamming the notify.
...
local function watchPlayerForGunPickup(player)
 ...
 -- Debounce: suppress duplicate fires within 3 seconds for this player.
 -- Multiple ChildAdded listeners (backpack + char + per-round hooks) would
 -- otherwise all fire simultaneously and send several notifications at once.
 local now = tick()
 ...
 -- Immediately update role state so ESP reflects Hero colour right away
 state.sheriff = player.Name
 state.hero = true
 if player.Name == LocalPlayer.Name then
 localRole = "Hero"
 end
 -- Update the cached R table entry immediately (don't wait for zK)
 ...
 Fluent:Notify({
 Title = "🔫 Gun Grabbed!",
 Content = player.Name .. " picked up the gun — role updated to Hero!",
 Duration = 5
 })
 end)
 -- Sync with server shortly after so all role data is fresh
 task.delay(0.3, zK)
 end
 end
 end
...
-- Main intercept solver — v3.0
-- Key improvements over v2:
--   • Wall-slide detection: measures velocity deficit vs expected speed;
--     if player is pressing into geometry, projects MoveDirection onto the
--     wall surface plane and reduces input-bias contribution.
--   • Long-range compensation: at >50 studs, input blend is reduced so we
--     rely more on Kalman physics velocity (more stable at distance).
--   • Wall-occlusion fallback: when bestClearPoint finds NO clear LOS,
--     tries lateral/vertical micro-offsets around the blocked geometry
--     to find exposed body-part edges (corner-peek optimisation).
--   • Body-relative Y clamp: uses rootY→headY+0.3 instead of spatial
--     raycasts from predicted position (those misfired inside geometry).
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- Warm up Kalman tracker for this player
    ...
    local baseLead = rawPing + 0.0167   -- +1 server tick (16.7ms)

    -- ── Range-based lead micro-correction ───────────────────────────
    -- Hitscan is instant, but long-range shots are processed later in the
    -- server tick queue. +1ms per 10 studs, capped at +20ms.
    local shooterPos = fromPos or root.Position
    local rangeDist  = (root.Position - shooterPos).Magnitude
    local rangeLead  = math.min(rangeDist * 0.0001, 0.020)   -- max 20ms extra
    ...
    local leadTime = math.clamp(baseLead * jitterPad + rangeLead, 0.018, 0.500)

    -- ── Wall-slide detection ─────────────────────────────────────────
    -- Compare actual lateral speed vs expected (MoveDir * WalkSpeed).
    -- wallFactor ≈ 1 → free movement;  wallFactor ≈ 0 → pressed into wall.
    ...
    local actualLateral   = Vector3.new(smoothVel.X, 0, smoothVel.Z).Magnitude
    local wallFactor = (expectedLateral > 1)
        and math.clamp(actualLateral / expectedLateral, 0, 1)
        or  1.0

    -- Project MoveDirection onto the wall surface when wall-hugging
    local effectiveMoveDir = moveDir
    if wallFactor < 0.60 and moveDir.Magnitude > 0.1 then
        ...
        if wallRay then
            -- Project move direction onto the wall plane
            local n = wallRay.Normal
            effectiveMoveDir = moveDir - n * moveDir:Dot(n)
        end
    end

    -- ── Long-range input-blend reduction ────────────────────────────
    -- Past 60 studs, trust physics velocity more and input direction less.
    -- The player's intent is harder to predict at distance; Kalman is stable.
    local rangeBlend = math.clamp(1 - (rangeDist - 60) / 180, 0.50, 1.0)
    ...
    if hum then
        ...
        local agreement = (smoothVel.Magnitude > 0.5 and inputVel.Magnitude > 0.5)
                          and smoothVel.Unit:Dot(inputVel.Unit) or 1.0
        -- Base blend: less in air, less at long range, less when wall-hugging
        local baseBlend = isAirborne and 0.20 or 0.40
        baseBlend = baseBlend * rangeBlend * math.max(wallFactor, 0.10)
        local inputBlend = math.clamp(baseBlend + (1 - agreement) * 0.12, 0.08, 0.50)
        vel = smoothVel * (1 - inputBlend) + inputVel * inputBlend
    end

    -- Lateral speed cap (prevents over-prediction on sharp direction changes)
    ...
    -- Scale by ping (high ping → more accel matters).
    -- Reduce when airborne (gravity dominates), wall-hugging (accel unreliable),
    -- or at long range (accel errors compound with distance).
    local accelScale = math.clamp(rawPing / 0.200, 0, 1)
        * (isAirborne and 0.45 or 1.0)
        * math.max(wallFactor, 0.15)
        * rangeBlend
    local accelContrib = smoothAccel * accelScale
    -- Hard cap: never more than 2.5× walkSpeed acceleration contribution
    ...
    -- SAFETY: headY MUST be above rootY so that clamp(min, max) never has max < min.
    -- Dead / ragdoll characters can place Head below HRP, which triggered the error:
    -- "invalid argument #3 to 'clamp' (max must be greater than or equal to min)"
    local rawHeadY = head and head.Position.Y or (rootY + 2.9)
    local headY    = math.max(rawHeadY, rootY + 1.0)  -- guaranteed headY > rootY always
    local aimY     = rootY + (headY - rootY) * 0.58   -- chest / neck band

    local finalY
    if isAirborne then
        -- Apply gravity drop, capped at 1.8 studs max so high-ping arcs
        -- don't over-drop and tunnel through the floor.
        local gravDrop = math.min(0.5 * ROBLOX_GRAVITY * (leadTime * leadTime), 1.8)
        finalY = aimY + vel.Y * leadTime - gravDrop
    else
        -- Grounded: small slope compensation only (vel.Y on flat ground ≈ 0)
        finalY = aimY + vel.Y * leadTime * 0.06
    end

    -- Anatomically-safe clamp: rootY+0.1 → headY+0.3
    -- headY >= rootY+1.0 is guaranteed above, so max > min is always true.
    finalY    = math.clamp(finalY, rootY + 0.1, headY + 0.3)
    predicted = Vector3.new(predicted.X, finalY, predicted.Z)

    -- ── Wall / LOS optimisation ──────────────────────────────────────
    -- bestClearPoint walks a priority chain (predicted → head → chest → root+1.5 → root).
    -- If NONE of those have clear LOS (target fully behind wall), we try
    -- small lateral and vertical offsets to find an exposed edge (corner peek).
    if fromPos then
        local clearPos, wasLOS = bestClearPoint(fromPos, predicted, char)
        if wasLOS then
            predicted = clearPos
        else
            -- Wall-occlusion recovery: probe 8 candidate micro-offsets
            -- around the target's body to find any exposed surface.
            local rootPos = root.Position
            local offsets = {
                -- Horizontal probes (left/right/forward/back ± 1 stud)
                Vector3.new( 1.0, headY - rootY - 0.4,  0),
                Vector3.new(-1.0, headY - rootY - 0.4,  0),
                Vector3.new( 0,   headY - rootY - 0.4,  1.0),
                Vector3.new( 0,   headY - rootY - 0.4, -1.0),
                -- Head-level probes
                Vector3.new( 0.6, headY - rootY + 0.1,  0),
                Vector3.new(-0.6, headY - rootY + 0.1,  0),
                -- Wider horizontal probes
                Vector3.new( 1.6, headY - rootY - 0.4,  0),
                Vector3.new(-1.6, headY - rootY - 0.4,  0),
            }
            local found = false
            ...
            -- Last resort: use the best-candidate from original chain
            if not found then
                predicted = clearPos
            end
        end
