...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- getVelAtTime extrapolates velocity via weighted least-squares on
    -- the ring buffer. At high ping (200ms+) the murderer may have
    -- changed direction by the time our shot registers — this compensates.
    -- We project to leadTime/2 (midpoint of flight window) for best average.
    local trendVel = tr and getVelAtTime(tr, leadTime * 0.5) or smoothVel
    ...
    -- Long-range blend: at >50 studs trust physics velocity more than input
    local rangeBlend = math.clamp(1 - (rangeDist - 50) / 160, 0.45, 1.0)

    -- ── 4. Velocity blend (trend + input direction) ──────────────────
    local vel = trendVel
    if hum then
        local inputVel  = effectiveMoveDir * walkSpeed
        -- Agreement: how well does input direction align with measured velocity?
        local agreement = (trendVel.Magnitude > 0.5 and inputVel.Magnitude > 0.5)
                          and trendVel.Unit:Dot(inputVel.Unit) or 1.0
        -- Higher ping → rely more on trend velocity (more accurate at distance)
        -- Lower ping  → can afford more input-direction blend
        local baseBlend = isAirborne and 0.18 or 0.35
        baseBlend = baseBlend * rangeBlend * math.max(wallFactor, 0.10)
        -- Reduce input blend further when disagreement is high (direction change)
        local inputBlend = math.clamp(baseBlend - (1 - agreement) * 0.10, 0.05, 0.45)
        vel = trendVel * (1 - inputBlend) + inputVel * inputBlend
    end

    -- Lateral speed cap: prevents over-prediction on sharp turns
    local lateralCap = isAirborne and (walkSpeed * 1.15) or (walkSpeed * 1.02)
    ...
    -- Scale: more at high ping (accel matters more over longer lead times),
    -- less when airborne (gravity already handled separately) or wall-hugging.
    local accelVec = tr and (getVelAtTime(tr, leadTime) - trendVel) or Vector3.new()
    -- Clamp acceleration contribution to avoid runaway extrapolation
    if accelVec.Magnitude > walkSpeed * 2.0 then
        accelVec = accelVec.Unit * (walkSpeed * 2.0)
    end
    local accelScale = math.clamp(rawPing / 0.150, 0, 1.2)  -- allow >1 at high ping
