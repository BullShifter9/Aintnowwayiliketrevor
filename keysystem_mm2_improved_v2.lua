...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    local aimY = platformBase + bodyH * 0.60
...
local SharpShooterToggle = Tabs.Combat:AddToggle("SharpShooterToggle", {
 Title = "Sharp Shooter",
 Description = "A Classic Sharp ShoeTer",
 Default = false,
 Callback = function(toggle)
 SharpShooterEnabled = toggle
 Fluent:Notify({
 Title = "Sharp Shooter",
 Content = toggle and "Sharp On." or "Sharp Off.",
 Duration = 3
 })
 end
})
...
local PredictionPingToggle = Tabs.Combat:AddToggle("PredictionPingToggle", {
 ...
 Content = string.format("ping locked to %dms", predictionState.pingValue),
 Duration = 3
 })
 else
 Fluent:Notify({
 Title = "Prediction Ping",
 Content = string.format("currently measuring ~%dms", math.floor(_ping * 1000)),
 Duration = 3
 })
 end
 end
