...
local function watchPlayerForGunPickup(player)
 ...
 -- Notify that an innocent picked up the gun
 pcall(function()
 Fluent:Notify({
 Title = "🔫 Gun Grabbed!",
 Content = player.Name .. " (Innocent) picked up the gun — now Hero!",
 Duration = 5
 })
 end)
 task.delay(0.3, zK)
...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- Anatomically-safe body-relative clamp: root bottom → slightly above head
