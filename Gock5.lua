...
local _pingWindow = {}  -- rolling window for spike detection
    ...
    while true do
        ...
        pcall(function()
            ...
            local raw   = math.clamp(
                stats.Network.ServerStatsItem["Data Ping"]:GetValue(), 5, 600) / 1000
            _pingRaw = raw
            table.insert(_pingWindow, raw)
            if #_pingWindow > 4 then table.remove(_pingWindow, 1) end
            local peak = 0
            for _, v in ipairs(_pingWindow) do if v > peak then peak = v end end
            local smoothed = _ping * 0.40 + raw * 0.60
            _ping = math.max(smoothed, peak * 0.80)
        end)
    end
end)
...
-- Lower KR_V: trust AssemblyLinearVelocity more (it's a physics engine value, not a
-- networked estimate). Faster velocity tracking = better slight-movement prediction.
local KR_P, KR_V = 0.08, 0.25
...
local function getVelAtTime(tr, dt_ahead)
    ...
    -- Increased decay constant (8 → faster recency weighting vs old 6)
    for i = math.max(1, n - 15), n do
        local age   = now - h[i].t
        local w     = math.exp(-age * 8)
    ...
    if math.abs(denom) > 1e-6 then
        ...
        if trend.Magnitude > 180 then trend = trend.Unit * 180 end
    else
        trend = tr.accel
    end

    -- Jump state override for vertical trend
    if tr.jumpState == "ascending" or tr.jumpState == "peak" then
        ...
        trend = Vector3.new(trend.X,
            (projVelY - baseVel.Y) / math.max(dt_ahead, 0.001), trend.Z)
    elseif tr.jumpState == "descending" then
        trend = Vector3.new(trend.X, -ROBLOX_GRAVITY * 0.75, trend.Z)
    end

    -- Speed-aware lateral trend scale:
    -- At low lateral speed, noise is large relative to signal → dampen trend heavily.
    -- At high lateral speed, trend is reliable → use it fully.
    local lateralBase = Vector3.new(baseVel.X, 0, baseVel.Z)
    local speedRatio  = math.clamp(lateralBase.Magnitude / 16.0, 0, 1.0)
    -- Nonlinear: squares to bias aggressively toward 0 at low speed
    local trendScale  = speedRatio * speedRatio   -- 0 → 0, 0.5 → 0.25, 1.0 → 1.0
    -- Allow minimum of 0.15 so there's always a trace of correction
    trendScale = math.max(trendScale, 0.15)

    local scaledTrend = Vector3.new(
        trend.X * trendScale,
        trend.Y,            -- Y (vertical): always full, it's deterministic physics
        trend.Z * trendScale
    )

    -- BUG FIX: removed the `* 0.55` that was silently dampening ALL predictions by 45%
    return baseVel + scaledTrend * dt_ahead
end
...
local premiumSilentAim = {
    ...
    autoAim   = false,
    lastShot  = 0,
    cooldown  = 0.05,   -- 50ms: fast tap support, still prevents accidental double-fire
}
...
local function hasLOS(fromPos, toPos, targetChar)
    ...
    rp.FilterType = Enum.RaycastFilterType.Blacklist
    local bl = {targetChar}
    if myChar then table.insert(bl, myChar) end
    rp.FilterDescendantsInstances = bl
    return workspace:Raycast(fromPos, dir.Unit * dist, rp) == nil
end

-- ── True bounding box: works for any avatar scale ─────────────────────────────
local function measureCharBounds(char)
    ...
    for _, p in ipairs(char:GetDescendants()) do
        if p:IsA("BasePart") and p.Name ~= "HumanoidRootPart" then
            ...
            local by, ty = p.Position.Y - half, p.Position.Y + half
...
-- ── LOS-aware fallback (body-height-fraction grid) ────────────────────────────
local function bestClearPoint(fromPos, predictedPos, targetChar, bodyH)
    if hasLOS(fromPos, predictedPos, targetChar) then return predictedPos, true end
    local root = targetChar:FindFirstChild("HumanoidRootPart")
    local rp   = root and root.Position or predictedPos
    local bH   = bodyH or 3.0
    for _, frac in ipairs({0.50, 0.75, 0.25, 0.90, 0.10}) do
        for _, lat in ipairs({
            Vector3.new(0,0,0), Vector3.new(0.8,0,0), Vector3.new(-0.8,0,0),
            Vector3.new(0,0,0.8), Vector3.new(0,0,-0.8),
        }) do
            local cand = rp + Vector3.new(lat.X, frac * bH, lat.Z)
            if hasLOS(fromPos, cand, targetChar) then return cand, true end
        end
    end
    local head = targetChar:FindFirstChild("Head")
    if head and hasLOS(fromPos, head.Position, targetChar) then return head.Position, true end
    return predictedPos, false
end

-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--  PREMIUM INTERCEPT SOLVER  (v6 — physics-correct lead formula)
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--
--  LEAD TIME ROOT-CAUSE FIX:
--    MM2 hit-detection uses the SERVER's copy of the target's position when your
--    FireServer packet arrives.  The server's copy of a target with ping T_p lags
--    their real position by T_p/2.  Your packet arrives T_s/2 after you sent it.
--    Combined: you must aim at pos + vel × (T_s/2 + T_p/2) = (T_s + T_p) / 2.
--
--    KC8WAJ6 "Seismic" uses vel/16.5 = vel×0.0606s ≈ vel×60ms for a 60ms shooter.
--    That is exactly (60ms + 60ms)/2 = 60ms.  ✓
--
--    So the correct formula is: lead = (YOUR_ping + TARGET_ping) / 2.
--    Since we don't know the target's ping, we use 60ms as the median estimate
--    (same calibration point as KC8WAJ6).
--
--    Old code used rawPing + 18ms ≈ 198ms at 180ms ping → 65% over-prediction.
--    New code uses (180ms + 60ms)/2 = 120ms → correct for average target.
--
--  VELOCITY ROOT-CAUSE FIX:
--    The old multi-tier confidence model with lateralInput×0.65 was producing
--    wildly wrong predictions for slight movement.  Replaced with a three-branch
--    model that matches how Roblox character physics actually works:
--      • No input → player is stopping; decay Kalman to avoid stale-velocity lead
--      • Low speed + input → player accelerating; use input direction × observed mag
--      • Normal speed → Kalman is stable; use it with minor input blend
--
--  leadScale parameter: burst shots call this with different scales
--  (0.7–1.4×) to create a spatial spread covering the ping uncertainty window.
local function solvePremiumIntercept(targetPlayer, fromPos, leadScale)
    leadScale = leadScale or 1.0
    ...
    -- ── 1. True body bounds ───────────────────────────────────────────────────
    local bMinY, bMaxY = measureCharBounds(char)
    local rootY = root.Position.Y
    local bodyH, bodyCentreY

    if bMinY and bMaxY and (bMaxY - bMinY) > 0.05 then
        bodyH       = bMaxY - bMinY
        bodyCentreY = (bMinY + bMaxY) * 0.5
    else
        local rawHeadY = head and head.Position.Y or (rootY + 2.9)
        bodyH       = math.max(rawHeadY - rootY, 0.5)
        bodyCentreY = rootY + bodyH * 0.52
    end
    local bodyTop    = (bMaxY or (rootY + bodyH)) - 0.04
    local bodyBottom = (bMinY or rootY) + 0.04

    -- ── 2. State ──────────────────────────────────────────────────────────────
    local rawVel    = root.AssemblyLinearVelocity
    local rawVelY   = rawVel.Y
    ...
    do
        local rpG = RaycastParams.new()
        rpG.FilterType = Enum.RaycastFilterType.Blacklist
        rpG.FilterDescendantsInstances = {char}
        local function cg(o)
            return workspace:Raycast(root.Position + o, Vector3.new(0,-5.5,0), rpG) ~= nil
        end
        isOnGround = cg(Vector3.new(0,0,0)) or cg(Vector3.new(0.6,0.8,0))
                  or cg(Vector3.new(-0.6,0.8,0)) or cg(Vector3.new(0,0.8,0.6))
                  or cg(Vector3.new(0,0.8,-0.6))
    ...
    local isAirborne = not isOnGround and (isJumping or isFalling
                                           or (tr and tr.jumpState ~= "none"))
    if tr then
    ...
    -- ── 3. Lead time (PHYSICS-CORRECT) ────────────────────────────────────────
    -- See header comment for derivation. TARGET_PING_ESTIMATE = 60ms.
    local rawPing     = getPing()
    local clickOffset = premiumSilentAim._clickLatencyOffset or 0
    local TARGET_PING_ESTIMATE = 0.060  -- 60ms: KC8WAJ6 calibration / typical player

    local baseLead = (rawPing + TARGET_PING_ESTIMATE) / 2 + clickOffset
    -- Slight ascending-arc pad only (arc integration is approximate).
    -- Ground: exactly 1.00 — any multiplier causes systematic over-prediction.
    local airPad  = (tr and tr.jumpState == "ascending") and 1.08 or 1.00
    local leadTime = math.clamp(baseLead * airPad * leadScale, 0.008, 0.480)

    -- ── 4. Lateral (XZ) velocity — clean 3-branch model ──────────────────────
    local kVel    = Vector3.new(smoothVel.X, 0, smoothVel.Z)   -- Kalman lateral
    local iVel    = moveDir * walkSpeed                          -- movement intent
    local kSpeed  = kVel.Magnitude

    local predictVel
    if moveDir.Magnitude < 0.05 then
        -- ❶ No input: player stopping or turning. Kalman still shows old velocity.
        --    Halve it — they'll shed most speed in the next lead-time window.
        predictVel = kVel * 0.50

    elseif kSpeed < walkSpeed * 0.25 then
        -- ❷ Input active but low speed: player just started moving (acceleration
        --    phase). Roblox acceleration is fast (~1 frame); trust INPUT direction
        --    but only at the OBSERVED magnitude so we don't overshoot.
        local mag = math.max(kSpeed, 0.5)
        predictVel = iVel.Magnitude > 0.1 and (iVel.Unit * mag) or kVel

    else
        -- ❸ Normal motion: Kalman is reliable. Blend 85% Kalman + 15% input to
        --    stay responsive when direction changes without over-trusting input.
        predictVel = kVel * 0.85 + iVel * 0.15
        -- Hard cap: character cannot exceed walkSpeed on ground, +10% for slopes
        local cap = walkSpeed * (isAirborne and 1.30 or 1.10)
        if predictVel.Magnitude > cap then
            predictVel = predictVel.Unit * cap
        end
    end

    -- Wall-hug: if player input says move but speed is low, deflect off wall normal
    if moveDir.Magnitude > 0.1 and not isAirborne
       and kSpeed < moveDir.Magnitude * walkSpeed * 0.45 then
        local rpW = RaycastParams.new()
        rpW.FilterType = Enum.RaycastFilterType.Blacklist
        rpW.FilterDescendantsInstances = {char}
        local wHit = workspace:Raycast(root.Position, moveDir * 3.0, rpW)
        if wHit then
            local n   = wHit.Normal
            local def = moveDir - n * moveDir:Dot(n)
            if def.Magnitude > 0.01 then
                predictVel = def.Unit * kSpeed
            end
        end
    end

    -- XZ prediction
    local predictedXZ = root.Position + predictVel * leadTime
    ...
    if isAirborne then
        if tr and (tr.jumpState == "ascending" or tr.jumpState == "peak") then
            ...
            local arcNow    = tr.jumpStartVelY * elapsed - 0.5 * ROBLOX_GRAVITY * elapsed * elapsed
            local arcFuture = tr.jumpStartVelY * tTotal  - 0.5 * ROBLOX_GRAVITY * tTotal  * tTotal
            finalY = (rootY + bodyH * 0.50) + (arcFuture - arcNow)
        else
            local gravDrop  = 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
            local predRootY = rootY + kalmanVelY * leadTime - gravDrop
            finalY = predRootY + bodyH * 0.50
        end
        local pRoot = rootY + kalmanVelY * leadTime - 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
        finalY = math.clamp(finalY, pRoot - 0.2, pRoot + bodyH + 0.5)
    else
        local slopeNudge = math.abs(rawVelY) >= 1.2 and (rawVelY * leadTime * 0.07) or 0
        finalY = bodyCentreY + slopeNudge
        finalY = math.clamp(finalY, bodyBottom, bodyTop)
    end

    local predicted = Vector3.new(predictedXZ.X, finalY, predictedXZ.Z)

    -- ── 6. LOS ────────────────────────────────────────────────────────────────
    if fromPos then
        local clearPos, _ = bestClearPoint(fromPos, predicted, char, bodyH)
        predicted = clearPos
    end
...
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
--
--  Burst strategy: each shot uses a DIFFERENT lead scale covering the target-ping
--  uncertainty range.  For 180ms shooter vs 40–180ms target:
--    Correct lead window: (180+40)/2 to (180+180)/2 = 110ms–180ms
--    Our base estimate (60ms target): 120ms
--    Scales 0.7–1.5 of 120ms cover: 84ms–180ms → hits the full realistic range.
local _burstLeadScales = {1.00, 1.25, 0.85, 1.10, 0.90, 1.50, 0.75, 1.35, 0.95}

local function doPremiumShoot(targetPlayer)
    local now = tick()
    if now - premiumSilentAim.lastShot < premiumSilentAim.cooldown then return end
    premiumSilentAim.lastShot = now
    ...
    -- NOTE: removed the 12×phantomUpdate loop — all had dt≈0µs and were
    -- skipped by the guard `if dt <= 0 then return end`. The RenderStepped
    -- loop already keeps the tracker continuously warm.
    local tr             = pTrackers[targetPlayer]
    local targetAirborne = tr and (tr.jumpState ~= "none")
    ...
    local gun = LocalPlayer.Backpack:FindFirstChild("Gun")
             ...
             or myChar:FindFirstChild("Revolver")
    if not gun then
        Fluent:Notify({ Title="⭐ Premium Aim", Content="No gun found.", Duration=3 })
        return
    end
    if LocalPlayer.Backpack:FindFirstChild(gun.Name) then
        humanoid:EquipTool(gun)
    ...
    -- ── FIRST SHOT: fires this exact frame, no task.spawn overhead ────────────
    local barrelPos = getBarrelPos()
    premiumSilentAim._clickLatencyOffset = 0.025

    local primary = solvePremiumIntercept(targetPlayer, barrelPos, 1.00)
    premiumSilentAim._clickLatencyOffset = nil

    if not primary then
        local _, bMaxY = measureCharBounds(char)
        primary = mRoot.Position + Vector3.new(0, bMaxY and (bMaxY - mRoot.Position.Y)*0.5 or 1.5, 0)
    end

    pcall(function()
        shootRemote:FireServer(getArg1(), CFrame.new(primary))
    end)

    -- Burst count: base by ping, +2 if airborne
    local pingMs    = getPing() * 1000
    local burstBase = pingMs <= 60  and 3
                   or pingMs <= 120 and 4
                   or pingMs <= 200 and 5
                   or pingMs <= 300 and 6
                   or pingMs <= 400 and 7 or 8
    local burstMax = targetAirborne and (burstBase + 2) or burstBase
    ...
    burstConn = RunService.Heartbeat:Connect(function()
        ...
        phantomUpdate(targetPlayer)
        local fb    = getBarrelPos()
        local scale = _burstLeadScales[(burst % #_burstLeadScales) + 1]
        premiumSilentAim._clickLatencyOffset = 0.025
        local fp = solvePremiumIntercept(targetPlayer, fb, scale)
        premiumSilentAim._clickLatencyOffset = nil

        if not fp then
            fp = mRoot.Position + Vector3.new(0, 1.5, 0)
        end

        -- BUG FIX: was `if fromPos and ...` — fromPos is nil here; use fb
        if not hasLOS(fb, fp, char) then
            fp = primary  -- fallback to first-shot position if LOS blocked
        end

        pcall(function()
            shootRemote:FireServer(getArg1(), CFrame.new(fp))
        end)
    end)
end
