...
-- ═══════════════════════════════════════════════════════════════════
-- PHYSICS SIMULATION PREDICTOR (matches real Roblox character physics)
-- Steps through friction deceleration + gravity + floor/ceiling clamp
-- This replaces the old Kalman/ensemble system for intercept solving
-- ═══════════════════════════════════════════════════════════════════
local SIM_STEPS    = 120      -- steps to simulate ahead
local SIM_INTERVAL = 1 / 60  -- step size (60 Hz)
local SIM_GRAVITY  = workspace.Gravity or 196.2
local SIM_FRICTION = 16       -- Roblox default friction deceleration (studs/s²)

local function calculateMoveComponent(moveDir, axis, simVel)
    -- Returns signed unit contribution of this axis toward the walk goal
    local axisVal = (axis == "X") and moveDir.X or moveDir.Z
    local velVal  = (axis == "X") and simVel.X  or simVel.Z
    -- If moving in this axis, return the direction; otherwise dampen toward 0
    if math.abs(axisVal) > 0.01 then
        return math.sign(axisVal)
    else
        return -math.sign(velVal)
    end
end
...
local function physicsSimPredict(char, root, hum, fromPos, ping)
    ...
    local GRAVITY    = SIM_GRAVITY
    local FRICTION   = SIM_FRICTION
    local Interval   = SIM_INTERVAL
    local Steps      = SIM_STEPS

    -- Snapshot current state
    local vel        = root.AssemblyLinearVelocity
    local walkSpeed  = hum.WalkSpeed or 16
    local moveDir    = hum.MoveDirection  -- world-space unit vector (XZ only)

    local SimVel = Vector3.new(vel.X, vel.Y, vel.Z)
    local SimPos = Vector3.new(root.Position.X, root.Position.Y, root.Position.Z)

    -- Raycast params (ignore the target character)
    ...
    -- How many steps ahead we need to cover ping + 1 frame
    local targetTime    = math.clamp(ping + 0.0167, 0.016, 0.500)
    local stepsNeeded   = math.max(1, math.ceil(targetTime / Interval))
    stepsNeeded = math.min(stepsNeeded, Steps)

    for _ = 1, stepsNeeded do
        local PreviousSimPos = SimPos

        -- ── XZ friction + move integration ──────────────────────────
        local FrictionDeceleration = FRICTION * walkSpeed * 0.10

        -- Advance position
        SimPos = Vector3.new(
            SimPos.X + SimVel.X * Interval + FrictionDeceleration * moveDir.X * Interval^2 / 2,
            SimPos.Y + SimVel.Y * Interval + (-GRAVITY) * Interval^2 / 2,
            SimPos.Z + SimVel.Z * Interval + FrictionDeceleration * moveDir.Z * Interval^2 / 2
        )

        -- ── Per-axis friction / acceleration toward walk goal ───────
        for _, Axis in ipairs(Couple) do
            local Goal = (Axis == "X" and moveDir.X or moveDir.Z) * walkSpeed * 16.001 / 16

            if (Axis == "X" and SimVel.X > Goal) or (Axis == "Z" and SimVel.Z > Goal) then
                -- Decelerating
                local comp = calculateMoveComponent(moveDir, Axis, SimVel)
                if Axis == "X" then
                    SimVel = SimVel - Vector3.new(FrictionDeceleration * math.abs(comp) * Interval, 0, 0)
                else
                    SimVel = SimVel - Vector3.new(0, 0, FrictionDeceleration * math.abs(comp) * Interval)
                end
                -- Clamp to goal so we don't overshoot
                if (Axis == "X" and SimVel.X < Goal) then
                    SimVel = Vector3.new(Goal, SimVel.Y, SimVel.Z)
                elseif (Axis == "Z" and SimVel.Z < Goal) then
                    SimVel = Vector3.new(SimVel.X, SimVel.Y, Goal)
                end

            elseif (Axis == "X" and SimVel.X < Goal) or (Axis == "Z" and SimVel.Z < Goal) then
                -- Accelerating
                local comp = calculateMoveComponent(moveDir, Axis, SimVel)
                if Axis == "X" then
                    SimVel = SimVel + Vector3.new(FrictionDeceleration * math.abs(comp) * Interval, 0, 0)
        ...
        -- ── Gravity on Y ─────────────────────────────────────────────
        SimVel = SimVel - Vector3.new(0, GRAVITY * Interval, 0)

        -- ── Floor intercept (prevent going through ground) ───────────
        local CheckFloorIntercept = workspace:Blockcast(
            CFrame.new(PreviousSimPos - Vector3.new(0, 1.001, 0)) * CFrame.Angles(0, math.rad(root.Orientation.Y), 0),
            Vector3.new(2, 0.002, 1),
            (PreviousSimPos - Vector3.new(0, 1.001, 0)) - (SimPos - Vector3.new(0, 1.001, 0)),
            RaycastParams
        )
        if CheckFloorIntercept and CheckFloorIntercept.Position and
           CheckFloorIntercept.Position.Y <= PreviousSimPos.Y then
            SimPos = Vector3.new(SimPos.X, CheckFloorIntercept.Position.Y + 3, SimPos.Z)
            SimVel = Vector3.new(SimVel.X, 0, SimVel.Z)  -- kill Y velocity on landing
        end

        -- ── Ceiling intercept (prevent going through ceiling) ────────
        local CheckCeilIntercept = workspace:Blockcast(
            CFrame.new(PreviousSimPos + Vector3.new(0, 1.001, 0)) * CFrame.Angles(0, math.rad(root.Orientation.Y), 0),
            Vector3.new(2, 0.002, 1),
            (PreviousSimPos + Vector3.new(0, 1.001, 0)) - (SimPos + Vector3.new(0, 1.001, 0)),
            RaycastParams
        )
        if CheckCeilIntercept and CheckCeilIntercept.Position and
           CheckCeilIntercept.Position.Y >= PreviousSimPos.Y then
            SimPos = Vector3.new(SimPos.X, CheckCeilIntercept.Position.Y - 2, SimPos.Z)
            SimVel = Vector3.new(SimVel.X, 0, SimVel.Z)
        end
    end

    -- Aim at chest height (60% up the body)
    local head   = char:FindFirstChild("Head")
    local rootY  = root.Position.Y
    local headY  = head and head.Position.Y or (rootY + 2.9)
    local bodyH  = math.max(headY - rootY, 1.0)
    local aimY   = SimPos.Y + bodyH * 0.60

    local predicted = Vector3.new(SimPos.X, aimY, SimPos.Z)

    -- LOS fallback sweep if wall is in the way
    if fromPos then
...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- Warm up the Kalman tracker so velocity estimate is fresh
    for _ = 1, 5 do phantomUpdate(targetPlayer) end

    local rawPing     = getPing()
    local clickOffset = premiumSilentAim._clickLatencyOffset or 0
    local totalPing   = rawPing + clickOffset

    return physicsSimPredict(char, root, hum, fromPos, totalPing)
    end)
