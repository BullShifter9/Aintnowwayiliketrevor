...
function ExtraLib:CreateWindow(cfg)
    ...
    local TABBAR_H = math.floor((IsMobile and 62 or 56) * S)
    ...
    -- ── Content area (between title and bottom tab bar) ───────────
    local ContentClip = Make("Frame", {
        Name             = "ContentClip",
        Size             = UDim2.new(1, 0, 1, -(TITLE_H + TABBAR_H)),
    ...
    -- Tab indicator pill — sits at the TOP edge of the selected tab
    ...
    function WinObj:CreateTab(name, icon)
        -- Dynamic equal-width tab buttons: recalculate all when a new tab is added
        local function ResizeAllTabs()
            local n = #self._tabs
            if n == 0 then return end
            local btnW = math.floor(W / n)
            for i, t in ipairs(self._tabs) do
                t._btn.Size     = UDim2.new(0, btnW, 1, 0)
                t._btn.Position = UDim2.new(0, (i - 1) * btnW, 0, 0)
            end
        end

        local btnW = W  -- temporary; will be resized
        ...
        -- Icon label (large, centered upper area)
        local IconLbl = Make("TextLabel", {
            ...
            TextColor3       = Theme.TextMuted,
            TextSize         = math.floor((IsMobile and 20 or 18) * S),
            Font             = Enum.Font.GothamBold,
            ZIndex           = 8,
            Parent           = TabBtn,
        })
        ...
        local NameLbl = Make("TextLabel", {
            ...
            Position         = UDim2.new(0, 2, 0.62, 0),
            BackgroundTransparency = 1,
            Text             = name,
            TextColor3       = Theme.TextMuted,
            TextSize         = math.floor((IsMobile and 9 or 9) * S),
        ...
        local function ActivateTab()
            for i, t in ipairs(self._tabs) do
                if t ~= TabObj then
                    ...
                    Tween(t._iconLbl, {TextColor3 = Theme.TextMuted}, TI.Fast)
                    Tween(t._nameLbl, {TextColor3 = Theme.TextMuted}, TI.Fast)
                    Tween(t._btn,     {BackgroundTransparency = 1},   TI.Fast)
                end
            end
            ...
            Tween(IconLbl,  {TextColor3 = Theme.Accent},         TI.Fast)
            Tween(NameLbl,  {TextColor3 = Theme.Accent},         TI.Fast)
            Tween(TabBtn,   {BackgroundTransparency = 0.88},     TI.Fast)

            task.defer(function()
                local bPos  = TabBtn.AbsolutePosition
                local bSize = TabBtn.AbsoluteSize
                local barX  = bPos.X - TabBar.AbsolutePosition.X
                Tween(TabIndicator, {
                    Position = UDim2.new(0, barX + math.floor(bSize.X * 0.15), 0, 0),
                    Size     = UDim2.new(0, math.floor(bSize.X * 0.7), 0, 3),
                }, TI.Smooth)
            end)
        ...
        TabBtn.MouseEnter:Connect(function()
            if self._active ~= TabObj then
                Tween(IconLbl, {TextColor3 = Theme.Text}, TI.Fast)
            end
        end)
        TabBtn.MouseLeave:Connect(function()
            if self._active ~= TabObj then
                Tween(IconLbl, {TextColor3 = Theme.TextMuted}, TI.Fast)
            end
        end)
        ...
        if #self._tabs == 1 then
            ...
            task.defer(function()
                ...
                local bSize = TabBtn.AbsoluteSize
                local barX  = bPos.X - TabBar.AbsolutePosition.X
                TabIndicator.Position = UDim2.new(0, barX + math.floor(bSize.X * 0.15), 0, 0)
                TabIndicator.Size     = UDim2.new(0, math.floor(bSize.X * 0.7), 0, 3)
            end)
            self._active = TabObj
        else
