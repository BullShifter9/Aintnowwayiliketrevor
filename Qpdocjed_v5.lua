...
-- ESP
Tabs.Visuals:AddSection("ESP (Player Tracking)")
...
Tabs.Visuals:AddSection("HUD & World")

-- GunDrop HighlightTabs.Visuals:AddToggle("GunDropESPToggle", {
...
-- Timer update loop: runs every 1 second via task.wait (NOT Heartbeat)
-- This prevents the 60x/sec InvokeServer throttle that killed the old version.
-- Time formatting uses math.floor — no O(n) while loop.
task.spawn(function()
    while true do
        task.wait(1)
        if not _timerOn then continue end
        local ok, o = pcall(function()
            return timerRemote:InvokeServer()
        end)
        if not ok or type(o) ~= "number" then continue end
        if o < 1 then
            RoundTimerLabel.Visible = false
            RoundTimerLabel.Active  = false
        else
            local mins = math.floor(o / 60)
            local secs = math.floor(o % 60)
            RoundTimerLabel.Visible = true
            RoundTimerLabel.Active  = true
            if mins > 0 then
                RoundTimerLabel.Text       = mins .. "m " .. secs .. "s"
                RoundTimerLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            else
                RoundTimerLabel.Text       = secs .. "s"
                RoundTimerLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
            end
        end
    end
end)
...
-- FARMING TAB — UI
-- ================================================================
Tabs.Farming:AddSection("Auto Coin Farm")

Tabs.Farming:AddParagraph({
    Title = "Auto Coin Farm",
    Content = "Automatically navigates your character to the nearest coin on the map and collects it. Works every Heartbeat. Stops and restores movement when toggled off."
})
