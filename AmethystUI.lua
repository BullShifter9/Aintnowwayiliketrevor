...
local function Draggable(handle, target)
    local drag,st,sp = false
    handle.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1
        or i.UserInputType==Enum.UserInputType.Touch then
            drag=true; st=i.Position; sp=target.Position
...
function Library:CreateWindow(cfg)
    ...
    local UIScl=New("UIScale",{Scale=1,Parent=Root})
    ...
    local wx=math.floor(vp.X/2-winW/2)
    local wy=math.floor(vp.Y/2-winH/2)
    ...
    local function DoMin()
        ...
        MinOrb.Position=UDim2.fromOffset(p.X.Offset+4,p.Y.Offset+4)
    ...
    BtnClose.MouseButton1Click:Connect(function() SGui:Destroy() end)
    ...
    function Window:_AnimSel(btn)
        local relY=btn.AbsolutePosition.Y-TabPanel.AbsolutePosition.Y
            +TabScroll.CanvasPosition.Y-8
        local h=math.max(btn.AbsoluteSize.Y-10,14)
        Tween(TabSel,TI_MED,{
            Position=UDim2.fromOffset(0,relY+5),
    ...
    function Window:AddTab(tabCfg)
        ...
        function Tab:AddToggle(id, cfg2)
            ...
            F.MouseButton1Click:Connect(function() Tog:SetValue(not Tog.Value) end)
    ...
    function Window:AddPremiumTab(cfg)
        ...
        local Gate=New("Frame",{
            ...
            ZIndex=30, Parent=PTab.Container,
        },{Corner(8)})
        New("UIGradient",{
            Color=ColorSequence.new({
    ...
    function Window:Destroy() SGui:Destroy() end
