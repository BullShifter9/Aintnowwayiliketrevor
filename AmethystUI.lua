...
-- Fluent-style drag: the handle's InputChanged tracks the active drag input
-- object; the global UIS.InputChanged only fires when Input == DragInput.
-- This eliminates the jitter caused by processing every global mouse event.
local function Draggable(handle, target)
    ...
    handle.InputBegan:Connect(function(i)
        ...
        or i.UserInputType == Enum.UserInputType.Touch then
            Dragging = true
            MousePos = i.Position
            -- Snapshot position as pure pixel offsets so AnchorPoint scale
            -- values don't corrupt the arithmetic when we add deltas.
    ...
    -- Track which input is the active drag (Fluent pattern)
    ...
    -- Only move when the event matches the last drag input (no jitter)
...
function Library:CreateWindow(cfg)
    ...
    function Window:AddTab(tabCfg)
        ...
        -- Always defer canvas measurement one frame so Roblox's layout engine
        -- has finished computing AbsoluteSize for all children first.
        -- UIPadding is Pad(10,10,10,10) → +20 for top and bottom padding.
