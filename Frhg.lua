...
-- ═══════════════════════════════════════════════════════════════════
-- PHYSICS SIMULATION PREDICTOR — Roblox Character Controller Replica
-- Exactly mirrors Roblox's per-axis goal-seeking friction model:
--   for each axis in {X, Z}:
--     Goal = MoveDirection[Axis] * WalkSpeed
--     if Vel[Axis] > Goal  → decelerate (apply -FrictionDecel * dt)
--     if Vel[Axis] < Goal  → accelerate (apply +FrictionDecel * dt)
--     else                 → snap to Goal
--     clamp vel to goal to prevent overshoot
--   Gravity: SimVel.Y -= G * dt  (only when airborne)
--   Floor:   Raycast down, snap root back to floor + rootHeight
-- ═══════════════════════════════════════════════════════════════════

-- CalculateMoveDirection: returns the friction-axis contribution
-- exactly as the Roblox character controller computes it.
-- If moving toward goal  → return axis component of MoveDirection (positive force)
-- If moving away / idle  → return 1 (full deceleration force)
local function CalcMoveDir(MoveDirection, Axis, SimVel)
    local axisGoal = (Axis == "X") and MoveDirection.X or MoveDirection.Z
    local axisVel  = (Axis == "X") and SimVel.X        or SimVel.Z
    -- No input on this axis → pure friction to zero
    if math.abs(axisGoal) < 0.01 then
        return 1.0
    end
    -- Input exists → drive toward goal
    -- Return 1 when vel is opposite to goal (overshoot) so friction is full
    if (axisGoal > 0 and axisVel > axisGoal) or
       (axisGoal < 0 and axisVel < axisGoal) then
        return 1.0
    end
    return math.abs(axisGoal)
end

local function physicsSimPredict(char, root, hum, fromPos, ping)
    if not char or not root or not hum then return nil end

    local G         = workspace.Gravity or 196.2
    local Interval  = 1 / 60                  -- 60 Hz simulation steps
    local WalkSpeed = hum.WalkSpeed or 16

    -- Roblox character friction deceleration rate (studs/s²).
    -- Measured from real character: reaches WalkSpeed in ~0.1s from rest.
    -- FrictionDecel × dt per step nudges vel toward goal.
    local FrictionDecel = WalkSpeed * 10.0

    -- Snapshot live state
    local MoveDirection = hum.MoveDirection  -- world-space, already normalized
    local rawVel        = root.AssemblyLinearVelocity
    local SimPos        = Vector3.new(root.Position.X, root.Position.Y, root.Position.Z)
    local SimVel        = Vector3.new(rawVel.X, rawVel.Y, rawVel.Z)

    -- Raycast filter: ignore target character
    local myChar  = LocalPlayer.Character
    ...
    -- Measure root-to-floor height RIGHT NOW so every step snaps accurately
    local function castFloor(pos)
        return workspace:Raycast(pos + Vector3.new(0, 1.2, 0), Vector3.new(0, -7.0, 0), rpGnd)
    end
    local initHit  = castFloor(SimPos)
    local rootH    = initHit and math.clamp(SimPos.Y - initHit.Position.Y, 1.5, 5.0) or 2.9
    local isGrounded = (initHit ~= nil) and (rawVel.Y < 4.0)

    -- Steps = how many 60Hz ticks cover the full ping lead time
    local steps = math.clamp(math.ceil(ping / Interval), 1, 60)

    -- Axes to simulate (matches the photo's `Couple = {"X","Z"}`)
    local Couple = {"X", "Z"}

    for _ = 1, steps do
        local PrevSimPos = SimPos

        -- ── 1. Per-axis XZ friction toward walk goal ─────────────────
        --   Mirrors exactly: for _ = 1, 2 do  Axis = Couple[_]  ...  end
        for _, Axis in ipairs(Couple) do
            local Goal = (Axis == "X" and MoveDirection.X or MoveDirection.Z) * WalkSpeed

            local curVel = (Axis == "X") and SimVel.X or SimVel.Z
            local delta

            if curVel > Goal then
                -- Decelerating: subtract friction scaled by move contribution
                local comp = CalcMoveDir(MoveDirection, Axis, SimVel)
                delta = -FrictionDecel * math.abs(comp) * Interval
                local newVel = curVel + delta
                -- Clamp: don't overshoot past Goal
                newVel = math.max(newVel, Goal)
                if Axis == "X" then SimVel = Vector3.new(newVel, SimVel.Y, SimVel.Z)
                else                 SimVel = Vector3.new(SimVel.X, SimVel.Y, newVel) end

            elseif curVel < Goal then
                -- Accelerating: add friction scaled by move contribution
                local comp = CalcMoveDir(MoveDirection, Axis, SimVel)
                delta = FrictionDecel * math.abs(comp) * Interval
                local newVel = curVel + delta
                -- Clamp: don't overshoot past Goal
                newVel = math.min(newVel, Goal)
                if Axis == "X" then SimVel = Vector3.new(newVel, SimVel.Y, SimVel.Z)
                else                 SimVel = Vector3.new(SimVel.X, SimVel.Y, newVel) end

            else
                -- Exactly at Goal: snap and hold
                if Axis == "X" then SimVel = Vector3.new(Goal, SimVel.Y, SimVel.Z)
                else                 SimVel = Vector3.new(SimVel.X, SimVel.Y, Goal) end
            end
        end

        -- ── 2. Gravity on Y (only when airborne) ─────────────────────
        if not isGrounded then
            SimVel = Vector3.new(SimVel.X, SimVel.Y - G * Interval, SimVel.Z)
        else
            -- Grounded: kill downward velocity so we don't sink
            SimVel = Vector3.new(SimVel.X, math.max(SimVel.Y, 0), SimVel.Z)
        end

        -- ── 3. Integrate position (Euler) ─────────────────────────────
        SimPos = SimPos + SimVel * Interval

        -- ── 4. Floor clamp (Raycast — works on all executors) ─────────
        local floorHit = castFloor(SimPos)
        if floorHit then
            local floorLevel = floorHit.Position.Y + rootH
            if SimPos.Y <= floorLevel + 0.08 then
            ...
            -- Off a ledge / in the air over a void
            isGrounded = false
        end

        -- ── 5. Ceiling clamp ──────────────────────────────────────────
        local ceilHit = workspace:Raycast(
            SimPos + Vector3.new(0, 0.5, 0),
            Vector3.new(0, 3.5, 0),
            rpGnd
        )
        if ceilHit and ceilHit.Position.Y < SimPos.Y + 2.5 then
            SimPos = Vector3.new(SimPos.X, ceilHit.Position.Y - 2.5, SimPos.Z)
            SimVel = Vector3.new(SimVel.X, 0, SimVel.Z)
        end
    end

    -- ── Aim point: 65% up from root to head ───────────────────────────
    local head  = char:FindFirstChild("Head")
    local headCurY = head and head.Position.Y or (root.Position.Y + 2.9)
    local bodyH    = math.max(headCurY - root.Position.Y, 1.5)
    local aimY     = SimPos.Y + bodyH * 0.65

    local predicted = Vector3.new(SimPos.X, aimY, SimPos.Z)

    -- ── LOS check at all predicted body heights ───────────────────────
    if fromPos then
        local clearPos, wasLOS = bestClearPoint(fromPos, predicted, char)
        if wasLOS then return clearPos end
        -- Try head → chest → gut at predicted XZ
        local alts = {
            Vector3.new(SimPos.X, SimPos.Y + bodyH * 0.88, SimPos.Z),
            Vector3.new(SimPos.X, SimPos.Y + bodyH * 0.65, SimPos.Z),
            Vector3.new(SimPos.X, SimPos.Y + bodyH * 0.45, SimPos.Z),
            Vector3.new(SimPos.X, SimPos.Y + bodyH * 0.28, SimPos.Z),
        }
        for _, cand in ipairs(alts) do
...
local function solvePremiumIntercept(targetPlayer, fromPos)
    local ok, result = pcall(function()
        ...
        -- Update Kalman tracker so velocity snapshot is fresh
        ...
        -- Full round-trip ping + 1 server tick + click latency offset
        local totalLead = rawPing + (1 / 60) + extraLead

        return physicsSimPredict(char, root, hum, fromPos, totalLead)
    end)
...
local function doPremiumShoot(targetPlayer)
    ...
    if not gun then
        Fluent:Notify({ Title = "⭐ Freemium Aim", Content = "No gun found in backpack or hand.", Duration = 3 })
        return
    end
    ...
    -- Solve intercept once with fresh data
    ...
    -- Fallback: if prediction failed aim at head/chest
    if not predicted then
        predicted = (mHead and mHead.Position)
                 or (mRoot.Position + Vector3.new(0, 1.5, 0))
    end

    -- Fire ONE clean shot at the predicted position
    pcall(function()
