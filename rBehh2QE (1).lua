...
local function httpGet(url)
    -- Collect every possible HTTP function this executor might expose
    local fn = nil
    for _, candidate in ipairs({
        rawget(getfenv and getfenv(0) or {}, "http_request"),
        rawget(getfenv and getfenv(0) or {}, "request"),
        syn and rawget(syn, "request"),
        http and rawget(http, "request"),
        rawget(getfenv and getfenv(0) or {}, "fetchremote"),
    }) do
    ...
    if not fn then
        -- Last-ditch: pcall common names directly
        for _, name in ipairs({"http_request","request","fetchremote"}) do
            local ok2, f2 = pcall(function() return getfenv(0)[name] end)
            if ok2 and type(f2) == "function" then fn = f2; break end
        end
    end
...
-- ════════════════════════════════════════════════════════════════
--  POST-AUTH FEATURES — Announce, Maintenance, Freeze, Poll
--  All run live after the script loads via startStatusPoll()
-- ════════════════════════════════════════════════════════════════

-- ── GUI Helpers ──────────────────────────────────────────────────
local function _destroyOld(name)
    pcall(function()
...
local function _newGui(name, order)
    _destroyOld(name)
...
local function _newFrame(parent, size, pos, color, radius)
...
local function _newLabel(parent, size, pos, text, color, font, wrap)
...
local function showAnnounce(message)
    _destroyOld("AnnounceGUI")
    local gui  = _newGui("AnnounceGUI", 998)
    local card = _newFrame(gui, UDim2.new(0,420,0,88), UDim2.new(0.5,-210,0,14), Color3.fromRGB(18,18,18), 12)
    local top  = _newFrame(card, UDim2.new(1,0,0,38), UDim2.new(0,0,0,0), Color3.fromRGB(26,115,232), 12)
    _newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(26,115,232), 0)
    _newLabel(top,  UDim2.new(1,0,1,0),    UDim2.new(0,0,0,0),  "Message from Owner", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    _newLabel(card, UDim2.new(1,-16,0,36), UDim2.new(0,8,0,42), message, Color3.fromRGB(210,210,210), Enum.Font.Gotham, true)
    _makeDraggable(top, card)
...
local function showMaintenanceGUI(reason)
    _destroyOld("MaintenanceGUI")
    local gui  = _newGui("MaintenanceGUI", 999)
    local card = _newFrame(gui, UDim2.new(0,460,0,210), UDim2.new(0.5,-230,0.5,-105), Color3.fromRGB(18,18,18), 12)
    local top  = _newFrame(card, UDim2.new(1,0,0,55), UDim2.new(0,0,0,0), Color3.fromRGB(50,80,200), 12)
    _newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(50,80,200), 0)
    _newLabel(top,  UDim2.new(1,0,1,0),     UDim2.new(0,0,0,0),    "Maintenance",                                    Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    _newLabel(card, UDim2.new(1,-20,0,105), UDim2.new(0,10,0,62),  reason or "Script is under maintenance. Please wait.", Color3.fromRGB(220,220,220), Enum.Font.Gotham, true)
    _newLabel(card, UDim2.new(1,-20,0,26),  UDim2.new(0,10,0,176), "Script will resume when maintenance ends.",      Color3.fromRGB(70,70,70),   Enum.Font.Gotham)
    _makeDraggable(top, card)
end

-- ── Ban Card (mid-session) ───────────────────────────────────────
local function showBanCard(reason)
    _destroyOld("BanGUI")
    local gui  = _newGui("BanGUI", 999)
    local card = _newFrame(gui, UDim2.new(0,460,0,210), UDim2.new(0.5,-230,0.5,-105), Color3.fromRGB(18,18,18), 12)
    local top  = _newFrame(card, UDim2.new(1,0,0,55), UDim2.new(0,0,0,0), Color3.fromRGB(180,20,20), 12)
    _newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(180,20,20), 0)
    _newLabel(top,  UDim2.new(1,0,1,0),     UDim2.new(0,0,0,0),    "Banned",                                           Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    _newLabel(card, UDim2.new(1,-20,0,105), UDim2.new(0,10,0,62),  "Reason: " .. (reason or "No reason given.") .. " Contact the owner to appeal.", Color3.fromRGB(220,220,220), Enum.Font.Gotham, true)
    _newLabel(card, UDim2.new(1,-20,0,26),  UDim2.new(0,10,0,176), "HWID: " .. _authHWID, Color3.fromRGB(70,70,70), Enum.Font.Code)
    _makeDraggable(top, card)
end
...
local function unfreezePlayer()
    ...
    pcall(function()
        for _, v in pairs(_frozenParts) do
...
-- ── Real-Time Status Poll (every 5s) ─────────────────────────────
local function startStatusPoll()
    task.spawn(function()
        local inMaintenance = false

        while task.wait(5) do
            local res = httpGet(AUTH_API .. "/poll/" .. _authHWID)
            if res and res.StatusCode == 200 then
                local ok, pd = pcall(_HTTP.JSONDecode, _HTTP, res.Body)
                if ok and pd then

                    -- ── Banned mid-session ──────────────────────
                    if pd.status == "blacklisted" then
                        _destroyOld("MaintenanceGUI")
                        showBanCard(pd.reason)
                        task.wait(3)
                        doKick("You have been banned.")
                        break
                    end

                    -- ── Maintenance turned ON ───────────────────
                    if pd.status == "maintenance" and not inMaintenance then
                        inMaintenance = true
                        showMaintenanceGUI(pd.reason)
                        freezePlayer()
                        doKick("Script under maintenance.")
                    end

                    -- ── Maintenance turned OFF ──────────────────
                    if pd.status == "ok" and inMaintenance then
                        inMaintenance = false
                        _destroyOld("MaintenanceGUI")
                        unfreezePlayer()
                        -- "Back online" toast
                        local g2 = _newGui("ResumeGUI", 997)
                        local c2 = _newFrame(g2, UDim2.new(0,360,0,80), UDim2.new(0.5,-180,0,20), Color3.fromRGB(18,18,18), 12)
                        local t2 = _newFrame(c2, UDim2.new(1,0,0,40), UDim2.new(0,0,0,0), Color3.fromRGB(30,160,80), 12)
                        _newFrame(t2, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(30,160,80), 0)
                        _newLabel(t2, UDim2.new(1,0,1,0),       UDim2.new(0,0,0,0),  "Back Online!",                        Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
                        _newLabel(c2, UDim2.new(1,-12,0,28),    UDim2.new(0,6,0,44), "Maintenance ended. Script is running.", Color3.fromRGB(200,200,200), Enum.Font.Gotham, true)
                        _makeDraggable(t2, c2)
                        task.delay(5, function() if g2 and g2.Parent then g2:Destroy() end end)
                    end

                    -- ── Owner Announcement ──────────────────────
                    if not inMaintenance and pd.status == "ok" then
                        if type(pd.message) == "string" and pd.message ~= "" and pd.message ~= _lastAnnounce then
                            _lastAnnounce = pd.message
                            showAnnounce(pd.message)
                        end
                    end
...
-- Store initial announcement so poll doesn't immediately re-show it
if type(d.announcement) == "string" and d.announcement ~= "" then
    _lastAnnounce = d.announcement
end
...
-- Safe loader — wraps every HttpGet+loadstring so a network hiccup doesn't crash the whole script
local function safeLoad(url, label)
    local src, err1 = pcall(function() return game:HttpGet(url, true) end)
    if not src or type(err1) ~= "string" or #err1 == 0 then
        error("[OmniHub] Failed to fetch " .. (label or url) .. ": " .. tostring(err1), 2)
    end
    local chunk, err2 = loadstring(err1)
    if not chunk then
        error("[OmniHub] Failed to parse " .. (label or url) .. ": " .. tostring(err2), 2)
    end
    return chunk()
end
