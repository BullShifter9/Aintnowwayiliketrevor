...
local function fetchHttp(url)
    -- Try game:HttpGet first (Synapse X, some others)
    local ok, body = pcall(function() return game:HttpGet(url) end)
    if ok and body and #body > 10 then return body end
    -- Fallback: executor request function (Delta, KRNL, etc.)
    ...
    if fn then
        ...
        if ok2 and res and res.Body and #res.Body > 10 then return res.Body end
    end
    return nil
end

local _fluentCode = fetchHttp("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua")
assert(_fluentCode, "Failed to load Fluent — check your internet or executor HTTP permissions")
local Fluent = loadstring(_fluentCode)()
...
local SaveManager = loadstring(_saveCode)()
...
local InterfaceManager = loadstring(_ifaceCode)()
