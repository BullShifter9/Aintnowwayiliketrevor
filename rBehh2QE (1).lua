...
if data.status == "allowed" then
    ...
    -- ✅ Auth passed. Load Fluent FIRST so all MM2 functions can access it.

    -- ── Fluent loader (right after auth, before any MM2 code) ─────────────
    local function fetchHttp(url)
        local function isValidLua(body)
            if not body or #body < 50 then return false end
            local p = body:sub(1,200):lower()
            return not (p:find("<!doctype") or p:find("<html") or p:find("<head"))
        end
        local fn = http_request or request or (syn and syn.request) or (http and http.request)
        if fn then
            local ok, res = pcall(fn, { Url = url, Method = "GET" })
            if ok and res and isValidLua(res.Body) then return res.Body end
        end
        local ok2, body = pcall(function() return game:HttpGet(url, true) end)
        if ok2 and isValidLua(body) then return body end
        return nil
    end

    local _fc = fetchHttp("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua")
    assert(_fc, "[OmniHub] Failed to fetch Fluent — check internet / executor HTTP")
    local _ff = loadstring(_fc); assert(_ff, "[OmniHub] Failed to parse Fluent")
    local Fluent = _ff()

    local _sc = fetchHttp("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/SaveManager.lua")
    assert(_sc, "[OmniHub] Failed to fetch SaveManager")
    local _sf = loadstring(_sc); assert(_sf, "[OmniHub] Failed to parse SaveManager")
    local SaveManager = _sf()

    local _ic = fetchHttp("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/InterfaceManager.lua")
    assert(_ic, "[OmniHub] Failed to fetch InterfaceManager")
    local _iif = loadstring(_ic); assert(_iif, "[OmniHub] Failed to parse InterfaceManager")
    local InterfaceManager = _iif()

    -- ── Services (auth already defined Players, CoreGui, UIS, HttpService) ──
