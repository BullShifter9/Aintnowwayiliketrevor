...
-- ════════════════════════════════════════════════════════════════
--  HWID AUTH CONFIG  ─ change these two values
-- ════════════════════════════════════════════════════════════════
...
local _lp_auth  = Players.LocalPlayer
...
local function getHWID()
    ...
    local ok3, plat = pcall(function()
        return tostring(_UIS_auth.TouchEnabled)..tostring(_UIS_auth.KeyboardEnabled)..tostring(_UIS_auth.GamepadEnabled)
    end)
...
local _authUsername = tostring(_lp_auth.Name)
-- ════════════════════════════════════════════════════════════════
...
-- ── Loader sequence (with HWID auth on stage 1) ───────────────
local STAGES = {
    { "Verifying HWID...",               0.15 },
...
-- Shows an error inside the loader card (red accent + message) then kicks.
local function loaderFail(titleText, bodyText)
    breathActive = false
    -- Turn accent + fill red
    ease(Accent, { BackgroundColor3 = Color3.fromRGB(180, 30, 30) }, 0.3)
    ease(Fill,   { BackgroundColor3 = Color3.fromRGB(200, 40, 40), Size = UDim2.new(1, 0, 1, 0) }, 0.5)
    AccentGrad.Enabled = false
    ...
    Version.Text       = "HWID: " .. _authHWID
    -- Show HWID in hint for easy copy
    Hint.Text          = "Contact the script owner to appeal."
    Hint.TextColor3    = Color3.fromRGB(180, 80, 80)
    task.wait(3.5)
    doKick(titleText .. " — " .. bodyText)
    task.wait(1)
    OmniLoader:Destroy()
end
...
local function animateOmniLoader()
    ...
    for i, stage in ipairs(STAGES) do
        ...
        -- ── Stage 1: HWID auth check ─────────────────────────
        if i == 1 then
            -- Run check while the fill bar is animating
            ...
            if not result then
                loaderFail("Connection Error", "Could not reach auth server. Try again later.")
                return false
            end

            local ok, data = pcall(_HTTP.JSONDecode, _HTTP, result.Body)

            if not ok or not data then
                loaderFail("Server Error", "Invalid response from auth server.")
                return false
            end

            -- Rate limited
            if data.status == "ratelimited" then
                local secs = tostring(data.remaining or 60)
                loaderFail("Rate Limited", "Too many attempts. Wait " .. secs .. "s and try again.")
                return false
            end

            -- Maintenance
            if data.status == "maintenance" then
                loaderFail("Maintenance", data.reason or "Script is under maintenance. Please wait.")
                return false
            end

            -- Banned
            if data.status == "blacklisted" then
                loaderFail("Banned", "Reason: " .. (data.reason or "No reason given.") .. " Contact the owner.")
                return false
            end

            -- Not in whitelist / any other non-allowed status
            if data.status ~= "allowed" then
                loaderFail("Unauthorized", "You are not authorized to use this script.")
                return false
            end

            -- ✅ Allowed — lock HWID if server returned one
            if data.hwid_lock and data.hwid_lock ~= "" then
                _authHWID = data.hwid_lock
            end
        end
        -- ─────────────────────────────────────────────────────
...
-- Gate: if auth failed inside the loader, stop here — nothing else runs
if not animateOmniLoader() then return end
