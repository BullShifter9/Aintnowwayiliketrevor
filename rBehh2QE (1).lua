...
-- ════════════════════════════════════════════════════════════════
--  POST-AUTH FEATURES
--  All variables used here (_HTTP, _authHWID, AUTH_API, httpGet,
--  doKick, Players, CoreGui) are top-level and guaranteed in scope.
-- ════════════════════════════════════════════════════════════════
...
-- Store initial announcement from /check so poll doesn't re-show it immediately
local _d = _authResult
if type(_d) == "table" and type(_d.announcement) == "string" and _d.announcement ~= "" then
    _lastAnnounce = _d.announcement
end
...
task.spawn(function()
    ...
    while task.wait(5) do
        ...
        if not res or res.StatusCode ~= 200 then continue end

        local ok, pd = pcall(_HTTP.JSONDecode, _HTTP, res.Body)
        if not ok or type(pd) ~= "table" then continue end
