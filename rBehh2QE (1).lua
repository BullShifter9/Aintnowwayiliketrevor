-- ── CONFIG ────────────────────────────────────────────────────────────────
local API = "https://hwid-drz0.onrender.com"
local KEY = "FUCKNIGGERS"
...
local function getHWID()
    ...
    local ok3, plat = pcall(function()
        return tostring(UIS.TouchEnabled) .. tostring(UIS.KeyboardEnabled) .. tostring(UIS.GamepadEnabled)
    end)
...
local function newLabel(parent, size, pos, text, color, font, wrap)
...
-- ── SHOW CARD — ban / error / maintenance (blocking, no auto close) ───────
local function showCard(title, body, barColor)
    local gui  = newGui("AuthGUI", 999)
    local card = newFrame(gui, UDim2.new(0,460,0,210), UDim2.new(0.5,-230,0.5,-105), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,55), UDim2.new(0,0,0,0), barColor, 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), barColor, 0)
    newLabel(top,  UDim2.new(1,0,1,0),     UDim2.new(0,0,0,0),    title, Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    newLabel(card, UDim2.new(1,-20,0,105), UDim2.new(0,10,0,62),  body,  Color3.fromRGB(220,220,220), Enum.Font.Gotham, true)
    newLabel(card, UDim2.new(1,-20,0,26),  UDim2.new(0,10,0,176), "HWID: " .. getHWID(), Color3.fromRGB(70,70,70), Enum.Font.Code)
    makeDraggable(top, card)
end

-- ── LOADING ───────────────────────────────────────────────────────────────
local TIPS = {
    "Do not share your HWID with anyone.",
    "Keep your executor updated for best compatibility.",
    "If you get banned by mistake, contact the owner.",
    "Your HWID is unique to this device.",
    "New features are added regularly, stay tuned.",
    "Report any bugs to the script owner.",
    "Respect the rules to keep your access.",
    "Thanks for using the script!",
    "Stay updated, check for new versions regularly.",
    "Hi! Hope you enjoy the script.",
}

local function showLoading()
    local gui  = newGui("LoadingGUI", 1000)
    local card = newFrame(gui, UDim2.new(0,400,0,118), UDim2.new(0.5,-200,0.5,-59), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,50), UDim2.new(0,0,0,0), Color3.fromRGB(26,115,232), 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(26,115,232), 0)
    newLabel(top, UDim2.new(1,0,1,0), UDim2.new(0,0,0,0), "Verifying HWID...", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    local dots = newLabel(card, UDim2.new(1,0,0,34), UDim2.new(0,0,0,52), "Please wait...", Color3.fromRGB(150,150,150))
    local tip  = TIPS[math.random(1, #TIPS)]
    newLabel(card, UDim2.new(1,-16,0,22), UDim2.new(0,8,0,90), "Tip: " .. tip, Color3.fromRGB(65,65,80), Enum.Font.Gotham, true)
    task.spawn(function()
        local t = {"Please wait.", "Please wait..", "Please wait..."}
        local n = 1
        while gui and gui.Parent do
            if dots and dots.Parent then dots.Text = t[n]; n = n % 3 + 1 end
            task.wait(0.5)
        end
    end)
    makeDraggable(top, card)
...
-- ── WELCOME — auto disappears after 5s ────────────────────────────────────
local function showWelcome(username, isNew)
    local gui  = newGui("WelcomeGUI", 997)
    local card = newFrame(gui, UDim2.new(0,380,0,95), UDim2.new(0.5,-190,0,20), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,45), UDim2.new(0,0,0,0), Color3.fromRGB(30,160,80), 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(30,160,80), 0)
    newLabel(top, UDim2.new(1,0,1,0), UDim2.new(0,0,0,0),
        isNew and "Welcome!" or "Welcome back!", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    newLabel(card, UDim2.new(1,-12,0,36), UDim2.new(0,6,0,50),
        isNew and ("Hi " .. username .. "! Your HWID has been registered.") or ("Good to see you, " .. username .. "!"),
        Color3.fromRGB(200,200,200), Enum.Font.Gotham, true)
    makeDraggable(top, card)
    -- Auto destroy after 5 seconds — use the gui reference directly
...
-- ── COOLDOWN ──────────────────────────────────────────────────────────────
local function showCooldown(seconds)
    local gui  = newGui("CooldownGUI", 999)
    local card = newFrame(gui, UDim2.new(0,380,0,108), UDim2.new(0.5,-190,0.5,-54), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,50), UDim2.new(0,0,0,0), Color3.fromRGB(180,120,0), 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(180,120,0), 0)
    newLabel(top, UDim2.new(1,0,1,0), UDim2.new(0,0,0,0), "Slow Down!", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    local countdown = newLabel(card, UDim2.new(1,0,0,44), UDim2.new(0,0,0,52),
        "Please wait " .. seconds .. " seconds...", Color3.fromRGB(200,200,200))
    makeDraggable(top, card)
    task.spawn(function()
        for i = seconds, 1, -1 do
            if countdown and countdown.Parent then
                countdown.Text = "Please wait " .. i .. " second" .. (i == 1 and "" or "s") .. "..."
            end
            task.wait(1)
        end
...
-- ── ANNOUNCE — message from owner, disappears after 10s ───────────────────
-- lastAnnounce is declared here at file scope so poll can always read/write it
local lastAnnounce = ""

local function showAnnounce(message)
    destroyOld("AnnounceGUI")
    local gui  = newGui("AnnounceGUI", 998)
    local card = newFrame(gui, UDim2.new(0,420,0,88), UDim2.new(0.5,-210,0,14), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,38), UDim2.new(0,0,0,0), Color3.fromRGB(26,115,232), 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(26,115,232), 0)
    newLabel(top,  UDim2.new(1,0,1,0),       UDim2.new(0,0,0,0),  "Message from Owner", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    newLabel(card, UDim2.new(1,-16,0,36),    UDim2.new(0,8,0,42), message, Color3.fromRGB(210,210,210), Enum.Font.Gotham, true)
    makeDraggable(top, card)
...
-- ── MAINTENANCE SCREEN ────────────────────────────────────────────────────
local function showMaintenanceGUI(reason)
    destroyOld("MaintenanceGUI")
    local gui  = newGui("MaintenanceGUI", 999)
    local card = newFrame(gui, UDim2.new(0,460,0,210), UDim2.new(0.5,-230,0.5,-105), Color3.fromRGB(18,18,18), 12)
    local top  = newFrame(card, UDim2.new(1,0,0,55), UDim2.new(0,0,0,0), Color3.fromRGB(50,80,200), 12)
    newFrame(top, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(50,80,200), 0)
    newLabel(top,  UDim2.new(1,0,1,0),     UDim2.new(0,0,0,0),   "Maintenance", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
    newLabel(card, UDim2.new(1,-20,0,105), UDim2.new(0,10,0,62), reason or "Script is under maintenance. Please wait.", Color3.fromRGB(220,220,220), Enum.Font.Gotham, true)
    newLabel(card, UDim2.new(1,-20,0,26),  UDim2.new(0,10,0,176), "Script will resume when maintenance ends.", Color3.fromRGB(70,70,70), Enum.Font.Gotham)
    makeDraggable(top, card)
end
...
local function unfreezePlayer()
    ...
    pcall(function()
        for _, v in pairs(frozenParts) do
            if v and v.Parent then v.Anchored = false end
        end
        frozenParts = {}
        lp:LoadCharacter()
    end)
...
-- ── REAL-TIME POLL — 5 seconds ────────────────────────────────────────────
local function startStatusPoll(hwid)
    task.spawn(function()
        local inMaintenance = false

        while task.wait(5) do
            local res = httpGet(API .. "/poll/" .. hwid)
            if res and res.StatusCode == 200 then
                ...
                if ok and d then
                    ...
                    if d.status == "blacklisted" then
                        destroyOld("MaintenanceGUI")
                        showCard("Banned", "Reason: " .. (d.reason or "No reason given") .. " Contact the owner to appeal.", Color3.fromRGB(180,20,20))
                        doKick("You are banned.")
                        break
                    end

                    -- MAINTENANCE TURNED ON
                    if d.status == "maintenance" and not inMaintenance then
                        inMaintenance = true
                        showMaintenanceGUI(d.reason)
                        freezePlayer()
                        doKick("Script under maintenance.")
                    end

                    -- MAINTENANCE TURNED OFF
                    if d.status == "ok" and inMaintenance then
                        inMaintenance = false
                        destroyOld("MaintenanceGUI")
                        unfreezePlayer()
                        -- Small back online notice
                        local g2   = newGui("ResumeGUI", 997)
                        local c2   = newFrame(g2, UDim2.new(0,360,0,80), UDim2.new(0.5,-180,0,20), Color3.fromRGB(18,18,18), 12)
                        local t2   = newFrame(c2, UDim2.new(1,0,0,40), UDim2.new(0,0,0,0), Color3.fromRGB(30,160,80), 12)
                        newFrame(t2, UDim2.new(1,0,0.5,0), UDim2.new(0,0,0.5,0), Color3.fromRGB(30,160,80), 0)
                        newLabel(t2, UDim2.new(1,0,1,0), UDim2.new(0,0,0,0), "Back Online!", Color3.fromRGB(255,255,255), Enum.Font.GothamBold)
                        newLabel(c2, UDim2.new(1,-12,0,28), UDim2.new(0,6,0,44), "Maintenance ended. Script is running.", Color3.fromRGB(200,200,200), Enum.Font.Gotham, true)
                        makeDraggable(t2, c2)
                        task.delay(5, function() if g2 and g2.Parent then g2:Destroy() end end)
                    end

                    -- ANNOUNCE — only when not in maintenance, only if message is new
                    if not inMaintenance and d.status == "ok" then
                        if type(d.message) == "string" and d.message ~= "" and d.message ~= lastAnnounce then
                            lastAnnounce = d.message
                            showAnnounce(d.message)
                        end
                    end
...
local result = httpGet(
    API .. "/check/" .. hwid .. "?username=" .. HttpService:UrlEncode(username)
)

destroyOld("LoadingGUI")

if not result then
    showCard("Connection Error", "Could not reach auth server. Try again later.", Color3.fromRGB(200,100,0))
    doKick("Could not reach auth server.")
    return
end
...
if not ok or not data then
    showCard("Server Error", "Invalid response from server. Try again later.", Color3.fromRGB(200,100,0))
    doKick("Invalid server response.")
    return
end

-- Account lock
if data.hwid_lock and data.hwid_lock ~= "" then
    hwid = data.hwid_lock
    print("HWID LOCKED TO: " .. hwid)
end

-- Rate limited
if data.status == "ratelimited" then
    showCooldown(data.remaining or 60)
    return
end

-- Maintenance on execute
if data.status == "maintenance" then
    showMaintenanceGUI(data.reason)
    doKick("Script under maintenance.")
    return
end

-- Blacklisted
if data.status == "blacklisted" then
    showCard("Banned", "Reason: " .. (data.reason or "No reason given") .. " Contact the owner to appeal.", Color3.fromRGB(180,20,20))
    doKick("You are banned.")
    return
end

-- Allowed
if data.status == "allowed" then
    -- Store current announcement so poll doesn't immediately re-show it
    if type(data.announcement) == "string" and data.announcement ~= "" then
        lastAnnounce = data.announcement
    end
    showWelcome(username, data.is_new == true)
    startStatusPoll(hwid)

    -- ✅ MM2 SCRIPT BELOW ───────────────────────────────────────────────────
    -- Only runs if HWID is authenticated and allowed

    -- Re-declare services MM2 needs (auth already has Players, CoreGui, UIS)
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local RunService        = game:GetService("RunService")
    local StarterGui        = game:GetService("StarterGui")
    local Workspace         = game:GetService("Workspace")
    local LocalPlayer       = lp  -- lp is already defined by auth

-- local ReplicatedStorage = game:GetService("ReplicatedStorage")  -- (defined by HWID auth)
-- local Players = game:GetService("Players")  -- (defined by HWID auth)
-- local RunService = game:GetService("RunService")  -- (defined above)
-- local CoreGui = game:GetService("CoreGui")  -- (defined by HWID auth)
-- local UserInputService = game:GetService("UserInputService")  -- (defined by HWID auth)
-- local HttpService = game:GetService("HttpService")  -- (defined by HWID auth)
local GetPlayerData = game.ReplicatedStorage:FindFirstChild("GetPlayerData", true)
-- local StarterGui = game:GetService("StarterGui")  -- (defined by HWID auth)
-- local Workspace = game:GetService("Workspace")  -- (defined by HWID auth)
-- local LocalPlayer = Players.LocalPlayer  -- (defined by HWID auth)
...
local function animateOmniLoader()
    ...
    for _, stage in ipairs(STAGES) do
        ...
        -- Glow pulse on fill color briefly
...
-- Executor-safe HTTP fetch (works on Delta, Synapse, KRNL, etc.)
local function isValidLua(body)
    if not body or #body < 50 then return false end
    local preview = body:sub(1, 200):lower()
    -- Reject HTML pages (GitHub redirects, Cloudflare errors, etc.)
    if preview:find("<!doctype") or preview:find("<html") or preview:find("<head") then
        return false
    end
    return true
end
...
local function fetchHttp(url)
    -- Prefer executor request functions (Delta, KRNL, etc.) — more reliable on mobile
    local fn = http_request or request or (syn and syn.request) or (http and http.request)
    if fn then
        local ok2, res = pcall(fn, { Url = url, Method = "GET" })
        if ok2 and res and isValidLua(res.Body) then return res.Body end
    end
    -- Fallback: game:HttpGet (Synapse X, some others)
    local ok, body = pcall(function() return game:HttpGet(url, true) end)
    if ok and isValidLua(body) then return body end
    return nil
end

local _fluentCode = fetchHttp("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua")
assert(_fluentCode, "Failed to load Fluent — check internet or executor HTTP permissions")
local _fluentFn = loadstring(_fluentCode)
assert(_fluentFn, "Failed to parse Fluent code — possible corrupt download, retry")
local Fluent = _fluentFn()

local _saveCode = fetchHttp("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/SaveManager.lua")
assert(_saveCode, "Failed to load SaveManager")
local _saveFn = loadstring(_saveCode)
assert(_saveFn, "Failed to parse SaveManager")
local SaveManager = _saveFn()

local _ifaceCode = fetchHttp("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/InterfaceManager.lua")
assert(_ifaceCode, "Failed to load InterfaceManager")
local _ifaceFn = loadstring(_ifaceCode)
assert(_ifaceFn, "Failed to parse InterfaceManager")
local InterfaceManager = _ifaceFn()
...
-- Fallback
showCard("Unauthorized", "You are not authorized to use this script.", Color3.fromRGB(180,20,20))
doKick("Unauthorized.")