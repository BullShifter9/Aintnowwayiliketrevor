...
local function addTextESP(player)
 ...
 _espTF = _espTF + 1; if _espTF % 2 ~= 0 then return end
...
local function addBoxESP(player)
 ...
 local _boxTF = 0
 local conn = RunService.Heartbeat:Connect(function()
 _boxTF = _boxTF + 1; if _boxTF % 2 ~= 0 then return end
 local root = espRoot(player)
 if root then
...
local function addTracerESP(player)
 ...
 local _tracTF = 0
 local conn = RunService.Heartbeat:Connect(function()
 _tracTF = _tracTF + 1; if _tracTF % 2 ~= 0 then return end
 local root = espRoot(player)
 if root then
 local screenPos, onScreen = workspace.CurrentCamera:WorldToViewportPoint(
...
workspace.ChildRemoved:Connect(function(child)
 ...
 task.spawn(function()
     task.wait(0.3) -- wait for server to update R so we can read who got it
     zK()
     -- Notify who picked up the gun
     local picker = state.sheriff
     local isHero = state.hero
     if picker then
         Fluent:Notify({
             Title = "Gun Picked Up!",
             Content = picker .. " picked up the gun" .. (isHero and " — now Hero!" or " — Sheriff"),
             Duration = 5
         })
     else
         Fluent:Notify({
             Title = "Gun Picked Up!",
             Content = "Someone grabbed the dropped gun.",
             Duration = 4
         })
     end
 end)
...
local function watchPlayerForGunPickup(player)
 ...
 if pd and pd.Role ~= "Murderer" and pd.Role ~= "Sheriff" then
     -- Promote instantly — don't wait for server R update
     ...
     task.delay(0.3, zK) -- sync R in background
 end
 end
 end
...
local function doKnifeThrow(targetPlayer)
    ...
    pcall(function() remote:InvokeServer(0, predicted, "AH") end)
end
...
-- ── Murder Chance compact label — sits ABOVE the role flash label ──
-- No separate panel; embedded in RoleFlashGui so it shows at the same
-- screen position (top-center above role notifier). Auto-hides when the
-- role label is hidden. Updates every 5 s while enabled.
local murderChanceLabel = Instance.new("TextLabel")
murderChanceLabel.Name              = "MurderChanceLabel"
murderChanceLabel.Parent            = RoleFlashGui
murderChanceLabel.BackgroundColor3  = Color3.fromRGB(10, 10, 20)
murderChanceLabel.BackgroundTransparency = 0.35
murderChanceLabel.AnchorPoint       = Vector2.new(0.5, 1)
murderChanceLabel.Position          = UDim2.new(0.5, 0, 0.28, -60)  -- just above role label
murderChanceLabel.Size              = UDim2.new(0, 200, 0, 0)       -- AutomaticSize fills height
murderChanceLabel.AutomaticSize     = Enum.AutomaticSize.Y
murderChanceLabel.Font              = Enum.Font.Gotham
murderChanceLabel.TextSize          = 11
murderChanceLabel.TextColor3        = Color3.fromRGB(230, 230, 230)
murderChanceLabel.TextXAlignment    = Enum.TextXAlignment.Center
murderChanceLabel.TextWrapped       = true
murderChanceLabel.RichText          = true
murderChanceLabel.ZIndex            = 11
murderChanceLabel.Visible           = false

local mcCorner2 = Instance.new("UICorner")
mcCorner2.CornerRadius = UDim.new(0, 6)
mcCorner2.Parent = murderChanceLabel

local mcPad = Instance.new("UIPadding")
mcPad.PaddingLeft   = UDim.new(0, 6)
mcPad.PaddingRight  = UDim.new(0, 6)
mcPad.PaddingTop    = UDim.new(0, 4)
mcPad.PaddingBottom = UDim.new(0, 4)
mcPad.Parent = murderChanceLabel

local murderChanceEnabled = false

local function updateMurderChance()
    if not murderChanceEnabled then return end
    local alivePlayers = {}
    ...
    if count < 2 then
        murderChanceLabel.Text = "<b>MURDER CHANCE</b>\nNot enough players."
        return
    end
    local lines = { "<b><font color='rgb(255,80,80)'>MURDER CHANCE</font></b>" }
    for _, p in ipairs(alivePlayers) do
        local chance
        if p.Name == state.murder then
            chance = "<font color='rgb(255,60,60)'>100%</font>"
        else
            chance = string.format("%.0f%%", (1 / count) * 100)
        end
        local tag = (p.Name == state.murder) and " <font color='rgb(255,60,60)'>[M]</font>"
                 or (p.Name == state.sheriff) and " <font color='rgb(60,130,255)'>[S]</font>"
                 or ""
        table.insert(lines, p.Name .. tag .. ": " .. chance)
    end
    murderChanceLabel.Text = table.concat(lines, "\n")
end
...
-- Update timer — poll server once per second instead of every render frame
-- (calling InvokeServer on RenderStepped = 60 remote calls/sec — major lag source)
task.spawn(function()
    while true do
        task.wait(1)
        if not TimerGui.Enabled then continue end
...
-- Circle button — transparent background, "Grab gun" label
GrabGunFrame.Name                    = "GrabGunFrame"
GrabGunFrame.Size                    = UDim2.new(0, 70, 0, 70)
GrabGunFrame.Position                = UDim2.new(0.01, 0, 0.45, 0)
GrabGunFrame.BackgroundColor3        = Color3.fromRGB(0, 0, 0)
...
-- Flash stroke on tap + grab logic
GrabGunFrame.MouseButton1Click:Connect(function()
    manualGrabGun()
    GrabGunStroke.Color = Color3.fromRGB(0, 255, 120)
    task.wait(0.18)
    GrabGunStroke.Color = Color3.fromRGB(255, 255, 255)
end)
...
local _dotTF = 0
RunService.Heartbeat:Connect(function()
    _dotTF = _dotTF + 1; if _dotTF % 10 ~= 0 then return end
...
-- ================================================================
-- ANTI-AFK ENGINE
-- Simulates a tiny virtual input every 3 minutes so the server
-- never idles the connection. Safe — only fires when enabled.
-- ================================================================
...
-- ================================================================
-- OPTIMIZE COINS
-- Hides all CoinVisual parts in the workspace so the GPU doesn't
-- render hundreds of spinning coin meshes each frame.
-- Coins are still collectible — only their visuals are hidden.
-- ================================================================
local optimizeCoins = { enabled = false, hidden = {} }

local function applyCoinOptimize(enable)
    pcall(function()
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj.Name == "CoinVisual" and obj:IsA("BasePart") then
                if enable then
                    optimizeCoins.hidden[obj] = obj.LocalTransparencyModifier
                    obj.LocalTransparencyModifier = 1
                else
                    obj.LocalTransparencyModifier = optimizeCoins.hidden[obj] or 0
                end
            end
        end
        if not enable then optimizeCoins.hidden = {} end
    end)
end

-- Also hide new coins that spawn mid-round when enabled
workspace.DescendantAdded:Connect(function(obj)
    if not optimizeCoins.enabled then return end
    if obj.Name == "CoinVisual" and obj:IsA("BasePart") then
        pcall(function()
            task.wait()  -- 1 frame so the part is parented fully
            optimizeCoins.hidden[obj] = obj.LocalTransparencyModifier
            obj.LocalTransparencyModifier = 1
        end)
    end
...
-- ================================================================
-- MAIN TAB — PERFORMANCE SECTION UI
-- ================================================================
Tabs.Main:AddSection("Performance")

Tabs.Main:AddToggle("OptimizeCoinsToggle", {
    Title = "Optimize Coins (Hide Visuals)",
    Description = "Hides all coin models visually so the GPU doesn't render them. Coins are still collectible. Big FPS boost on maps with many coins.",
    Default = false,
    Callback = function(v)
        optimizeCoins.enabled = v
        applyCoinOptimize(v)
        Fluent:Notify({
            Title = "Optimize Coins",
            Content = v and "Coin visuals hidden — better FPS." or "Coin visuals restored.",
            Duration = 3
        })
    end
})
...
-- ================================================================
-- MAIN TAB — ANTI-AFK UI
-- ================================================================
Tabs.Main:AddSection("Anti-AFK")

Tabs.Main:AddToggle("AntiAFKToggle", {
    Title = "Anti-AFK",
    Description = "Simulates a virtual input every 3 minutes to prevent the server from kicking you for inactivity. Essential for mobile.",
    Default = false,
    Callback = function(v)
        antiAFK.enabled = v
        Fluent:Notify({
            Title = "Anti-AFK",
            Content = v and "ON — you won't be kicked for inactivity." or "Anti-AFK OFF.",
            Duration = 3
        })
    end
})
...
-- ================================================================
-- MAIN TAB — MURDER CHANCE UI
-- ================================================================
Tabs.Main:AddSection("Murder Chance Display")

Tabs.Main:AddToggle("MurderChanceToggle", {
    Title = "Show Murder Chance",
    Description = "Shows each player's murder probability directly above the Role Notifier. Updates every 5 seconds.",
    Default = false,
    Callback = function(v)
        murderChanceEnabled = v
        murderChanceLabel.Visible = v
        if v then updateMurderChance() end
        Fluent:Notify({
            Title = "Murder Chance",
            Content = v and "ON — shown above role notifier." or "OFF.",
            Duration = 3
        })
    end
})
...
-- ================================================================
-- GOD MODE ENGINE
-- Sets local player health to max every Heartbeat while enabled.
-- ================================================================
...
-- ================================================================
-- TWO LIVES ENGINE
-- Detects when the local player's health drops to 0 (death) and
-- immediately resets the character before the server can confirm
-- the kill. On mobile, firing ReplicatedStorage death remotes is
-- unreliable, so we reset character and re-apply health instantly.
-- ================================================================
local twoLives = { enabled = false, used = false }

local function bindTwoLivesChar(char)
    if not char then return end
    local hum = char:WaitForChild("Humanoid", 5)
    if not hum then return end
    twoLives.used = false  -- reset per life
    hum.HealthChanged:Connect(function(hp)
        if hp <= 0 and twoLives.enabled and not twoLives.used then
            twoLives.used = true
            task.wait(0.05)
            -- Revive: set health back to max before server processes death
            pcall(function()
            ...
            Fluent:Notify({
                Title = "Two Lives",
                Content = "Near-death saved! One life remaining this round.",
                Duration = 4
            })
        end
    end)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    bindTwoLivesChar(char)
    if twoLives.enabled then twoLives.used = false end
end)
if LocalPlayer.Character then bindTwoLivesChar(LocalPlayer.Character) end

-- ================================================================
-- HITBOX EXPANDER ENGINE
-- Inflates the murderer's HumanoidRootPart size client-side so
-- that hitscan bullets are easier to register. Color-coded
-- selection box shows which player has the expanded hitbox.
-- ================================================================
local hitboxExpander = {
    enabled   = false,
    size      = 8,       -- stud radius (default 8)
    color     = Color3.fromRGB(255, 60, 60),  -- red by default
    boxes     = {}       -- { [playerName] = SelectionBox instance }
}
...
local function applyHitbox(player, size, color)
    ...
    if not hitboxExpander.boxes[player.Name] then
        ...
        sb.Parent         = CoreGui
        hitboxExpander.boxes[player.Name] = sb
    end
    hitboxExpander.boxes[player.Name].Adornee = root
    hitboxExpander.boxes[player.Name].Color3  = color
    hitboxExpander.boxes[player.Name].SurfaceColor3 = color
end
...
local function removeHitbox(player)
    ...
    local sb = hitboxExpander.boxes[player.Name]
    if sb then sb:Destroy(); hitboxExpander.boxes[player.Name] = nil end
end
...
local function refreshHitboxes()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            applyHitbox(p, hitboxExpander.size, hitboxExpander.color)
        end
    end
end

-- Keep hitboxes applied each time characters respawn
...
-- ================================================================
-- KNIFE AURA ENGINE (real — firetouchinterest, KC8WAJ6 style)
-- Works the same way as Coin Aura but for the knife:
-- Finds any KnifeVisual / knife BasePart in the murderer's equipped
-- knife and fires firetouchinterest between nearby players' roots and
-- the knife part — server registers hit without movement.
-- Murderer only. Radius capped at 15 studs by default.
-- ================================================================
local knifeAura = {
    enabled = false,
    radius  = 10   -- studs — default safe value
}

local function getKnifePart()
    local myChar = LocalPlayer.Character; if not myChar then return nil end
    -- Check equipped (Character) first, then Backpack
    for _, item in ipairs(myChar:GetChildren()) do
        if item:IsA("Tool") and (item.Name == "Knife" or item.Name:lower():find("knife")) then
...
local function tickKnifeAura()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            local hum  = char and char:FindFirstChild("Humanoid")
            if root and hum and hum.Health > 0 then
                if (root.Position - myPos).Magnitude <= radius then
                    pcall(function()
                        firetouchinterest(root, knifePart, 0)
                        task.wait()
                        firetouchinterest(root, knifePart, 1)
                    end)
                end
            end
        end
...
RunService.Heartbeat:Connect(function()
    if knifeAura.enabled then tickKnifeAura() end
end)

-- ================================================================
-- DEAD PLAYER ESP
-- Keeps showing ESP on players after their Humanoid.Health reaches 0.
-- Uses a grey Drawing text above their fallen body position.
-- ================================================================
local deadESP = { enabled = false, labels = {} }

local function addDeadLabel(player)
    if deadESP.labels[player.Name] then return end
    local d = Drawing.new("Text")
    d.Text     = "[DEAD] " .. player.Name
    d.Size     = 14
    ...
    local conn = RunService.Heartbeat:Connect(function()
        ...
        if root then
            local screenPos, onScreen = workspace.CurrentCamera:WorldToViewportPoint(
                (root.CFrame * CFrame.new(0, 3, 0)).Position
            )
            d.Position = Vector2.new(screenPos.X, screenPos.Y)
            d.Visible  = onScreen
        else
    ...
    deadESP.labels[player.Name] = { drawing = d, conn = conn }
end

local function removeDeadLabel(player)
    local e = deadESP.labels[player.Name]
    if not e then return end
    e.conn:Disconnect()
    e.drawing:Remove()
    deadESP.labels[player.Name] = nil
end

-- Watch each player for death
local function watchForDeath(player)
    player.CharacterAdded:Connect(function(char)
        local hum = char:WaitForChild("Humanoid", 5)
        if not hum then return end
        hum.Died:Connect(function()
            if deadESP.enabled then
                task.wait(0.1)
                addDeadLabel(player)
            end
        end)
    end)
    -- Also hook already-spawned character
...
for _, p in ipairs(Players:GetPlayers()) do
    if p ~= LocalPlayer then watchForDeath(p) end
end
Players.PlayerAdded:Connect(function(p)
    watchForDeath(p)
end)
...
-- ================================================================
-- COMBAT TAB — MURDERER SECTION UI
-- Kill All, Kill Sheriff, Knife Aura, God Mode, Hitbox Expander, Two Lives
-- ================================================================
Tabs.Combat:AddSection("Murderer Tools")

Tabs.Combat:AddButton({
    Title = "Kill All (Knife Throw Everyone)",
    Name = "KillAllBtn",
    Callback = function()
        if localRole ~= "Murderer" then
            Fluent:Notify({ Title = "Kill All", Content = "You are not the Murderer.", Duration = 3 })
            return
        end
        local myChar = LocalPlayer.Character
        local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            ...
            if p ~= LocalPlayer then
                ...
                if hum and hum.Health > 0 then
                    ...
                    task.delay(i * 0.25, function()
                        task.spawn(function() doKnifeThrow(p) end)
                    end)
                end
            end
        end
        Fluent:Notify({ Title = "Kill All", Content = "Throwing at " .. count .. " players...", Duration = 3 })
    end
})
...
Tabs.Combat:AddButton({
    ...
    Callback = function()
        if localRole ~= "Murderer" then
            Fluent:Notify({ Title = "Kill Sheriff", Content = "You are not the Murderer.", Duration = 3 })
            return
        end
        local sheriffName = state.sheriff
        if not sheriffName then
            Fluent:Notify({ Title = "Kill Sheriff", Content = "No Sheriff/Hero detected.", Duration = 3 })
            return
        end
        local target = Players:FindFirstChild(sheriffName)
        if target then
            task.spawn(function() doKnifeThrow(target) end)
            Fluent:Notify({ Title = "Kill Sheriff", Content = "Throwing at " .. sheriffName .. ".", Duration = 3 })
        end
    end
})

Tabs.Combat:AddToggle("KnifeAuraToggle", {
    Title = "Knife Aura",
    Description = "Expands your knife's hit detection using firetouchinterest — kills any player within the radius without needing to touch them. Murderer only.",
    Default = false,
    Callback = function(v)
        knifeAura.enabled = v
        Fluent:Notify({
            Title = "Knife Aura",
            Content = v and ("ON — radius: " .. knifeAura.radius .. " studs") or "OFF.",
            Duration = 3
        })
    end
})

Tabs.Combat:AddSlider("KnifeAuraRadiusSlider", {
    Title = "Knife Aura Radius (studs)",
    Description = "How far the knife hitbox reaches. Keep below 15 to stay safe.",
    Default = 10,
    Min = 3,
    Max = 25,
    Rounding = 0,
    Callback = function(v) knifeAura.radius = v end
})

-- ================================================================
-- COMBAT TAB — SURVIVAL SECTION UI
-- God Mode, Two Lives, Hitbox Expander
-- ================================================================
Tabs.Combat:AddSection("Survival")

Tabs.Combat:AddToggle("GodModeToggle", {
    Title = "God Mode",
    Description = "Sets your health to maximum every frame. You cannot die from knives, traps, or bullets.",
    Default = false,
    Callback = function(v)
        godMode.enabled = v
        Fluent:Notify({
            Title = "God Mode",
            Content = v and "ON — you cannot be killed." or "OFF.",
            Duration = 3
        })
    end
})

Tabs.Combat:AddToggle("TwoLivesToggle", {
    Title = "Two Lives",
    Description = "When your health drops to 0 the first time, you are revived instantly. One save per life.",
    Default = false,
    Callback = function(v)
        twoLives.enabled = v
        twoLives.used    = false
        Fluent:Notify({
            Title = "Two Lives",
            Content = v and "ON — one extra life active." or "OFF.",
            Duration = 3
        })
    end
})
...
Tabs.Combat:AddToggle("HitboxExpanderToggle", {
    ...
    Description = "Inflates all players' hitboxes client-side. Makes every shot easier to register. Selection box shows the expanded area.",
    Default = false,
    Callback = function(v)
        ...
        Fluent:Notify({
            ...
            Content = v and ("ON — size: " .. hitboxExpander.size) or "OFF — hitboxes restored.",
            Duration = 3
        })
    end
})
...
Tabs.Combat:AddSlider("HitboxSizeSlider", {
    ...
    Description = "How large to inflate the hitbox. 8 is safe; 20+ is very obvious.",
    Default = 8,
    Min = 2,
    Max = 25,
    Rounding = 0,
    Callback = function(v)
        ...
        if hitboxExpander.enabled then refreshHitboxes() end
    end
})

Tabs.Combat:AddDropdown("HitboxColorDropdown", {
    Title = "Hitbox Box Color",
    Description = "Color of the selection box drawn around expanded hitboxes.",
    Values = { "Red", "Blue", "Green", "Yellow", "White", "Orange", "Purple" },
    Default = 1,
    Callback = function(v)
        local colorMap = {
            Red    = Color3.fromRGB(255,  60,  60),
            Blue   = Color3.fromRGB( 60, 130, 255),
            Green  = Color3.fromRGB( 60, 220,  60),
            Yellow = Color3.fromRGB(255, 215,   0),
            White  = Color3.fromRGB(255, 255, 255),
            Orange = Color3.fromRGB(255, 140,   0),
            Purple = Color3.fromRGB(180,  60, 255),
        }
        hitboxExpander.color = colorMap[v] or Color3.fromRGB(255, 60, 60)
        if hitboxExpander.enabled then refreshHitboxes() end
    end
})

-- ================================================================
-- VISUALS TAB — DEAD PLAYER ESP UI
-- ================================================================
Tabs.Visuals:AddToggle("DeadESPToggle", {
    Title = "Dead Player ESP",
    Description = "Keeps showing a grey [DEAD] label above players after they die so you know where every body fell.",
    Default = false,
    Callback = function(v)
        deadESP.enabled = v
        if not v then
        ...
        Fluent:Notify({
            Title = "Dead Player ESP",
            Content = v and "ON — dead bodies will be labelled." or "OFF.",
            Duration = 3
        })
    end
})
