-- ╔═════════════════════════════════════════════════════════════════════════════╗
-- ║    ADVANCED PROTECTION SYSTEM  v4.0  — Hardened Edition                    ║
-- ║  Anti HTTP Spy · Anti Remote Spy · Anti Dex · Anti Blatant Spy             ║
-- ║  Anti Octy Spy · Anti ALL Known Remote/Dex Tools · Hard-Kill Gate          ║
-- ║  Broad Keyword Sweep · Continuous Runtime Monitor · Multi-Signal Gate       ║
-- ╠═════════════════════════════════════════════════════════════════════════════╣
...
-- ║  · Definitive single-signal globals trigger instant hard-kill (no gate)    ║
-- ║  · All other detections require ≥2 independent signals before kill         ║
-- ║  · Broad keyword sweep catches unknown/future spy GUIs automatically       ║
...
do -- ── Protection scope (all locals sealed inside; original script unaffected) ──
    ...
    -- =========================================================================
    -- §11  RUNTIME MONITOR — Direct injection detection
    --      Uses getgc() and getscripts() to check if spy/dex code is actually
    --      running. This catches everything regardless of GUI or globals.
    -- =========================================================================
    ...
    -- Keywords found in spy/dex script source or names — if any loaded script
    -- contains these, it's a spy/dex tool. Chosen to never match legit scripts.
    local INJECT_KEYWORDS = {
        "simplespy", "hydroxide", "blatantspy", "blatant_spy",
        "octyspy", "octospy", "sigmaspy", "remotespy", "remote_spy",
        "dexexplorer", "darkdex", "dex_explorer",
        "httpspy", "http_spy",
        "oxsspy", "claraspy", "fluxspy", "neutronspy",
        "elysianspy", "spyx", "closurespy",
        "hookfunction", -- only flagged when found inside an injected script name
    }

    local function _isSpyScript(name, src)
        local ln = _low(_tostring(name or ""))
        local ls = _low(_tostring(src  or ""))
        for _, kw in _ipairs(INJECT_KEYWORDS) do
            if _has(ln, kw) then return true, kw end
            if _has(ls, kw) then return true, kw end
        end
        return false, nil
    end

    -- Check all running scripts via getscripts() — executor API
    local function _checkInjectedScripts()
        local ok, scripts = _pcall(getscripts or function() return {} end)
        if not ok or not scripts then return end
        for _, s in _ipairs(scripts) do
            _pcall(function()
                local name = s.Name or ""
                local src  = ""
                -- Try to get source via decompiler if available
                _pcall(function()
                    if decompile then src = decompile(s) or "" end
                end)
                local hit, kw = _isSpyScript(name, src)
                if hit then
                    _hardKill("Injected spy script detected: " .. name .. " (kw: " .. kw .. ")")
                end
            end)
        end
    end

    -- Check via getgc() — scans all live Lua objects for spy function closures
    local function _checkGC()
        local ok, gc = _pcall(getgc or function() return {} end, true)
            ...
            if _type(obj) == "function" then
                _pcall(function()
                    ...
                    if info and info.source then
                        local hit, kw = _isSpyScript("", info.source)
                        if hit then
                            _hardKill("Spy function in GC: " .. _tostring(info.source):sub(1, 60) .. " (kw: " .. kw .. ")")
                        end
                    end
                end)
            end
    ...
    -- Also keep ChildAdded for instant GUI-based detection
    local function _onChildAdded(child)
        ...
        if _nameInList(name, SPY_UI_NAMES) then _hardKill("Spy UI injected: " .. name) end
        if _nameInList(name, DEX_UI_NAMES) then _hardKill("Dex UI injected: " .. name) end
    end
    ...
    -- Main injection-check loop — runs every 2 seconds, reacts as soon as
    -- any spy/dex script is injected and shows up in getscripts()/getgc().
    _task_spawn(function()
        while _runtimeAlive do
            _pcall(_checkInjectedScripts)
            _pcall(_checkGC)
