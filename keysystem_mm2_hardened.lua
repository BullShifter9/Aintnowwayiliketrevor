...
do -- ── Protection scope (all locals sealed inside; original script unaffected) ──
    ...
    -- =========================================================================
    -- §11  RUNTIME MONITOR — Event-driven (fires only when tools are executed)
    --      Uses ChildAdded listeners on CoreGui/PlayerGui so detection triggers
    --      the instant a spy/dex tool injects its GUI, not on a poll timer.
    --      A lightweight global-watch loop runs in parallel to catch tools that
    --      set globals without creating a GUI (e.g. pure-hook HTTP spies).
    -- =========================================================================
    ...
    -- Evaluate a newly added child the moment it appears
    local function _onChildAdded(child)
        if not _runtimeAlive then return end
        local name = _tostring(child.Name)
        local ln   = _low(name)

        -- Hard-kill: broad spy keyword in name
        for _, kw in _ipairs(BROAD_SPY_KEYWORDS) do
            if _has(ln, kw) then
                _hardKill("Spy GUI injected: " .. name .. " (matched: " .. kw .. ")")
            end
        end
        for _, kw in _ipairs(BROAD_DEX_KEYWORDS) do
            if _has(ln, kw) then
                _hardKill("Dex GUI injected: " .. name .. " (matched: " .. kw .. ")")
            end
        end

        -- Exact spy UI name match
        if _nameInList(name, SPY_UI_NAMES) then
            _hardKill("Spy UI injected: " .. name)
        end

        -- Exact dex UI name match
        if _nameInList(name, DEX_UI_NAMES) then
            _hardKill("Dex UI injected: " .. name)
        end

        -- Structural dex check on the new child
        if child.ClassName == "ScreenGui" then
            _pcall(function()
                if _dexStructuralCheck(child) then
                    _hardKill("Dex structural GUI injected: " .. name)
                end
            end)
        end
    end

    -- Hook ChildAdded on CoreGui — fires the instant ANY GUI is injected
    _pcall(function()
        local cg = game:GetService("CoreGui")
        cg.ChildAdded:Connect(function(child)
            _task_spawn(function()
                _pcall(_onChildAdded, child)
            end)
        end)
    end)

    -- Hook ChildAdded on PlayerGui too
    _pcall(function()
        local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
        if pg then
    ...
    -- Lightweight global-watch loop: only checks globals (no UI scan).
    -- Runs every 3-6s to catch tools that work without injecting a GUI,
    -- like pure-hook HTTP spies or tools that set globals on execution.
    _task_spawn(function()
        _task_wait(5) -- wait a few seconds after load before starting
        while _runtimeAlive do

            -- Hard-kill globals (definitive — instant)
            ...
            -- Environment leak (catches hook-based spies with no globals/GUI)
            _pcall(function()
                local leaked, key = _envLeakCheck()
                if leaked then _rtSignal("rt_env_leak:" .. (key or "?")) end
            end)

            -- __namecall identity change
            _pcall(function()
                if _namecallChanged() then
                    _rtSignal("rt_namecall_changed")
                end
            end)
