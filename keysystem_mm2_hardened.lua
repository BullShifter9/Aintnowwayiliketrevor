...
do -- ── Protection scope (all locals sealed inside; original script unaffected) ──
    ...
    -- § 3e  Dex soft globals
    local DEX_GLOBALS = {
        "Dex", "DexV3", "DexV4", "DEX", "DarkDex",
        "InfiniteYield", "IY", "IYLoaded", "IYRunning",
    ...
    local BROAD_DEX_KEYWORDS = {
        "dexexplorer", "darkdex", "infiniteyield",
        "dex_explorer", "scripteditor",
    }
    ...
    -- Thread 1: Hard-kill globals + spy globals & UI  (fast)
    _task_spawn(function()
        _randWait(3, 6)
        while _runtimeAlive do
            -- Hard-kill globals (instant)
            _pcall(_scanHardKillGlobals)

            -- Broad keyword sweep (instant kill inside)
            _pcall(function()
                local cg = game:GetService("CoreGui")
                _broadKeywordSweep(cg, BROAD_SPY_KEYWORDS, "rt_CoreGui_spy")
                _broadKeywordSweep(cg, BROAD_DEX_KEYWORDS, "rt_CoreGui_dex")
            end)
            _pcall(function()
                local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
                if pg then
                    _broadKeywordSweep(pg, BROAD_SPY_KEYWORDS, "rt_PlayerGui_spy")
                end
            end)
    ...
    -- Thread 2: Dex globals & structural scan  (medium)
    _task_spawn(function()
        _randWait(4, 8)
        while _runtimeAlive do
            _pcall(function()
            ...
            _pcall(function()
                ...
                if pg and _scanDexUI(pg) then _rtSignal("rt_dex_ui_playergui") end
            end)
    ...
    -- Thread 3: __namecall identity  (fast)
    _task_spawn(function()
        _randWait(2, 4)
        while _runtimeAlive do
            _pcall(function()
    ...
    -- Thread 4: Environment leak + timing ratio  (slow — heavier checks)
    _task_spawn(function()
        _randWait(8, 14)
        while _runtimeAlive do
            _pcall(function()
