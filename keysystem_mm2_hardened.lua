-- ╔═════════════════════════════════════════════════════════════════════════════╗
-- ║    ADVANCED PROTECTION SYSTEM  v5.0  — Correct Edition                     ║
-- ║  Anti Remote Spy · Anti Dex · Anti Blatant · Anti OctoSpy                  ║
-- ║  Hook Integrity Check (iscclosure) · ChildAdded GUI Watch · Global Scan    ║
-- ╠═════════════════════════════════════════════════════════════════════════════╣
-- ║  HOW IT WORKS                                                               ║
-- ║  · Captures __namecall + FireServer originals BEFORE any spy can hook them ║
-- ║  · Detects reference change → spy hooked the function                      ║
-- ║  · ChildAdded fires instantly when Dex/spy GUI is injected (zero polling)  ║
-- ║  · Known globals scan (cheap table lookup, every 5s)                       ║
-- ║  · NO getgc() — it freezes the game. Never used here.                      ║
-- ╚═════════════════════════════════════════════════════════════════════════════╝
...
do -- ── Protection scope (all locals sealed inside; original script unaffected) ──
    ...
    -- Executor closure-type checkers (may be nil on some executors)
    local _iscclosure  = iscclosure
    local _islclosure  = islclosure
    local _isexecclosure = isexecutorclosure or isourceclosure or nil

    -- Capture the real __namecall and FireServer BEFORE any spy can hook them
    local _rawmt           = getrawmetatable and getrawmetatable(game) or nil
    local _origNamecall    = _rawmt and _rawmt.__namecall or nil
    local _origFireServer  = nil
    local _origInvoke      = nil
    local _origFireUnrel   = nil
    _pcall(function()
        local re = Instance.new("RemoteEvent")
        _origFireServer = re.FireServer
        re:Destroy()
    end)
    _pcall(function()
        local rf = Instance.new("RemoteFunction")
        _origInvoke = rf.InvokeServer
        rf:Destroy()
    end)
    _pcall(function()
        local ure = Instance.new("UnreliableRemoteEvent")
        _origFireUnrel = ure.FireServer
        ure:Destroy()
    end)
    ...
    -- =========================================================================
    local KILL_THRESHOLD  = 2      -- lowered: 2 signals now enough to kill
    local _signals        = {}
        ...
        if _signalCount() >= KILL_THRESHOLD then
            ...
            _pcall(function()
                game:GetService("Players").LocalPlayer:Kick(
                    "[azzakirms] Spy/Explorer detected. You have been removed.\n" .. reason
                )
            end)
    ...
    -- Hard kill: bypasses the signal threshold entirely.
    -- Used for definitive globals that can ONLY exist if a spy/dex tool is active.
    local function _hardKill(reason)
        _warn("[PROTECTION/HARD] Instant kill — " .. _tostring(reason))
        _pcall(function()
            game:GetService("Players").LocalPlayer:Kick(
                "[azzakirms] Spy tool detected and blocked.\n" .. _tostring(reason)
            )
        end)
        _error("[PROTECTION/HARD] " .. _tostring(reason), 0)
    end
    ...
    local function _runtimeKill(reason)
        ...
        _pcall(function()
            game:GetService("Players").LocalPlayer:Kick(
                "[azzakirms] Runtime spy detected.\n" .. _tostring(reason)
            )
        end)
    end
    ...
    -- =========================================================================
    -- §3  SPY / DEX GLOBAL REGISTRIES  (Massively Expanded v4.0)
    -- =========================================================================

    -- § 3a  Remote-spy specific globals — definitive (hard-kill on any hit)
    --       These keys are SET BY THE SPY TOOL ITSELF and cannot appear otherwise.
    local HARD_KILL_GLOBALS = {
        -- SimpleSpy
        "SimpleSpyExecuted",
        "OriginalNamecall",
        "SIMPLESPYCONFIG_MaxRemotes",
        -- Hydroxide
        "HydroxideRunning",
        "HydroxideConnected",
        -- Blatant Spy
    ...
    -- § 3b  Soft spy globals — contribute 1 signal each
    local SPY_GLOBALS = {
        -- SimpleSpy variants
        "SimpleSpyV3", "SimpleSpy3", "SimpleSpy2", "SimpleSpyLoaded",
        -- Hydroxide variants
        "Hydroxide", "HydroxideLoaded",
        -- Sigma variants
        "SigmaSpyLoaded", "SigmaSpyRunning",
        -- Blatant Spy extra variants
        "BlatantSpyEnabled", "blatantspy", "BLATANT_SPY",
        -- Octy extra variants
        "OctySpyRunning", "octyspy", "OCTY_SPY",
        -- Generic remote spy
        "SpyActive", "ClosureSpyActive",
        "RemoteSpy", "RemoteSpyLoaded", "RemoteSpyRunning", "REMOTE_SPY_LOADED",
        -- Oxs Spy
        "OxsSpy", "oxsspy", "OxsSpyActive", "OxsSpyLoaded", "OXS_SPY_LOADED",
        -- Clara Spy
        "ClaraSpy", "clarasspy", "ClaraSpyActive", "CLARA_SPY_LOADED",
        -- Flux Spy
        "FluxSpy", "fluxSpy", "FLUX_SPY", "FluxSpyActive", "FluxSpyLoaded",
        -- Elysian Spy
        "ElysianSpy", "elysianspy", "ElysianSpyActive", "ELYSIAN_SPY",
        -- Caio Spy
        "CaioSpy", "caiospy", "CaioSpyActive", "CAIO_SPY_LOADED",
        -- Neutron Spy
        "NeutronSpy", "neutronspy", "NeutronSpyActive",
        -- Lightning Spy
        "LightningSpy", "lightningspy", "LightningSpyActive",
        -- Vega Spy
        "VegaSpy", "vegaspy", "VegaSpyActive",
        -- SpyX
        "SpyX", "SpyXActive", "SpyXLoaded", "SPYX_LOADED",
        -- Generic catch-alls
        "spyLoaded", "SpyLoaded", "SPY_LOADED", "remoteLogger", "RemoteLogger",
        "hookLogger", "HookLogger", "remoteHooks", "RemoteHooks",
    }

    -- § 3c  HTTP-spy specific globals
    local HTTP_SPY_GLOBALS = {
        "HttpSpyActive", "HttpSpyEnabled", "HTTP_SPY_LOADED", "HttpInterceptActive",
        "HttpSpy", "HttpSpyLoaded", "HttpSpyRunning", "httpspy", "HTTPSPY",
        "HTTP_INTERCEPT", "HttpHookActive", "HttpLoggerActive", "HttpLogger",
        "httpLogger", "HTTP_LOGGER", "HttpMonitor", "httpMonitor",
    }

    -- § 3d  Dex / Explorer globals — definitive (hard-kill)
    local HARD_KILL_DEX_GLOBALS = {
        "DexExplorer",
        "DexLoaded",
        "DexRunning",
        "DarkDexLoaded",
    ...
    local DEX_GLOBALS = {
        "DexV3", "DexV4", "DarkDex",
        "ExplorerLoaded", "ExplorerRunning", "ExplorerActive",
        "ScriptEditorLoaded", "ScriptEditorActive",
        "dexExplorer", "darkDex", "dex_explorer",
    }

    -- § 3f  Spy UI exact names
    local SPY_UI_NAMES = {
        -- SimpleSpy
        "SimpleSpy3", "SimpleSpy2", "SimpleSpy", "SimpleSpyUI",
        -- Hydroxide
        "Hydroxide", "HydroxideUI",
        -- Blatant Spy
        "BlatantSpy", "BlatantSpyGui", "BlatantSpyUI", "Blatant",
        -- Octy Spy
        "OctySpy", "OctySpyGui", "OctySpyUI", "OctoSpy", "OctoSpyGui",
        -- Sigma Spy
        "SigmaSpyUI", "SigmaSpy", "SigmaSpyGui",
        -- Generic
        "RemoteSpyGui", "SpyGui", "ClosureSpyGui", "RSpy",
        "RemoteSpy", "RemoteSpyUI",
        -- HTTP spy UIs
        "HttpSpyGui", "HttpSpy", "HttpSpyUI",
        -- Oxs, Clara, Flux, others
        "OxsSpy", "ClaraSpy", "FluxSpy", "ElysianSpy",
        "CaioSpy", "NeutronSpy", "LightningSpy",
        -- Generic spy catch-alls
        "SpyMain", "SpyFrame", "SpyWindow",
    }

    -- § 3g  Dex UI exact names
    local DEX_UI_NAMES = {
        "DexExplorer", "DarkDex", "Dex Explorer",
        "DexV3", "DexV4",
        "DEX_EXPLORER", "dex_explorer",
    }

    -- § 3h  Broad keyword sweep — catches unknown/future tools automatically
    --       ANY CoreGui/PlayerGui child whose name contains these keywords is flagged.
    --       Keywords chosen to NEVER match legitimate Roblox CoreGui names.
    local BROAD_SPY_KEYWORDS = {
        "spy", "remotespy", "remotespy", "remote_spy",
        "blatant", "octy", "hydroxide", "simplespy", "sigmaspy",
        "oxsspy", "claraspy", "fluxspy", "neutronspy",
        "httpspy", "http_spy", "closurespy",
    }

    local BROAD_DEX_KEYWORDS = {
        "dexexplorer", "darkdex",
        "dex_explorer",
    }

    -- § 3i  Dex structural keyword pairs
    local DEX_STRUCT_PAIRS = {
        {"propert", "explorer"},
        {"dexexplorer", "properties"},
        {"dexexplorer", "explorer"},
        {"darkdex",     "script"},
        {"remote",  "viewer"},
        {"remote",  "logger"},
        {"hook",    "viewer"},
    ...
    -- § 3j  Environment spy key leak list (expanded)
    local ENV_SPY_KEYS = {
        ...
        "BlatantSpy", "BlatantSpyActive",
        "OctySpy", "OctySpyActive", "OctoSpy",
        "toggleSpy", "spyActive", "spyToggle",
        "remoteHooks", "connectedRemotes", "hookRemote",
        "remoteLogger", "hookLogger", "logRemote",
        "spyRemote", "remoteSpyHook",
    }

    -- =========================================================================
    -- §4  __NAMECALL IDENTITY CAPTURE
    -- =========================================================================
    local _nc_baseline = nil
    local _nc_caller   = nil
    ...
    local _TIMING_N         = 4000
    local _RATIO_MIN        = 10
    local _RATIO_MAX        = 85
    local _VIOLATION_LIMIT  = 7
    ...
    local function _timingRatioCheck()
        ...
        local empty = t2 - t1
        if empty < 1e-9 then return false end
    ...
    -- Hard-kill global scanner: checks definitive globals and kills instantly.
    local function _scanHardKillGlobals()
        for _, key in _ipairs(HARD_KILL_GLOBALS) do
            if _rawget(_G, key) ~= nil then
                _hardKill("Spy global detected: _G." .. key)
            end
        end
        if _getgenv then
            local ok, genv = _safe(_getgenv)
            if ok and genv and genv ~= _G then
                for _, key in _ipairs(HARD_KILL_GLOBALS) do
                    if _rawget(genv, key) ~= nil then
                        _hardKill("Spy global detected: genv." .. key)
                    end
                end
            end
        end
        for _, key in _ipairs(HARD_KILL_DEX_GLOBALS) do
            if _rawget(_G, key) ~= nil then
                _hardKill("Dex global detected: _G." .. key)
            end
        end
        if _getgenv then
            local ok, genv = _safe(_getgenv)
            if ok and genv and genv ~= _G then
                for _, key in _ipairs(HARD_KILL_DEX_GLOBALS) do
                    if _rawget(genv, key) ~= nil then
                        _hardKill("Dex global detected: genv." .. key)
                    end
                end
            end
        end
    ...
    -- Broad keyword sweep: flags ANY GUI whose lowercased name contains a spy/dex keyword.
    -- Hard-kills immediately — these keywords cannot appear in any legitimate Roblox GUI.
    local function _broadKeywordSweep(root, keywordList, label)
        if not root then return end
        local ok, children = _safe(function() return root:GetChildren() end)
        if not ok or not children then return end
        for _, child in _ipairs(children) do
            local ln = _low(_tostring(child.Name))
            for _, kw in _ipairs(keywordList) do
                if _has(ln, kw) then
                    _hardKill(label .. " keyword GUI: " .. child.Name .. " (matched: " .. kw .. ")")
                end
            end
        end
    end
    ...
    -- §9a  Hard-kill globals (instant, no threshold needed)
    local function _checkHardKillGlobals()
        _scanHardKillGlobals()
    end

    -- §9b  Anti HTTP Spy
    local function _checkHTTPSpy()
        local found, key = _scanGlobals(HTTP_SPY_GLOBALS)
        if found then _addSignal("http_spy_global:" .. (key or "?")) end
    ...
    local function _checkRemoteSpy()
        ...
        if found then _addSignal("remote_spy_global:" .. (key or "?")) end

        _pcall(function()
            local cg = game:GetService("CoreGui")
            local hit, name = _scanUIExact(cg, SPY_UI_NAMES)
            if hit then _addSignal("remote_spy_ui:" .. (name or "?")) end
        end)

        local leaked, leakKey = _envLeakCheck()
        if leaked then _addSignal("remote_spy_env_leak:" .. (leakKey or "?")) end

        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if pg then
                local hit, name = _scanUIExact(pg, SPY_UI_NAMES)
                if hit then _addSignal("remote_spy_playerGui:" .. (name or "?")) end
            end
        end)
    end

    -- §9d  Anti Dex Explorer
    local function _checkDexExplorer()
        local found, key = _scanGlobals(DEX_GLOBALS)
        if found then _addSignal("dex_global:" .. (key or "?")) end

        _pcall(function()
            local cg = game:GetService("CoreGui")
            local hit, name = _scanUIExact(cg, DEX_UI_NAMES)
            if hit then _addSignal("dex_ui_coregui:" .. (name or "?")) end
        end)

        _pcall(function()
            local cg = game:GetService("CoreGui")
            if _scanDexUI(cg) then _addSignal("dex_structural_pattern") end
        end)

        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if pg and _scanDexUI(pg) then _addSignal("dex_playerGui_structural") end
        end)
    end

    -- §9e  Broad keyword sweep — catches Blatant, Octy, and any unknown future tool
    local function _checkBroadKeywordSweep()
        _pcall(function()
            local cg = game:GetService("CoreGui")
            _broadKeywordSweep(cg, BROAD_SPY_KEYWORDS, "CoreGui_spy")
            _broadKeywordSweep(cg, BROAD_DEX_KEYWORDS, "CoreGui_dex")
        end)
        _pcall(function()
            local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
            if pg then
                _broadKeywordSweep(pg, BROAD_SPY_KEYWORDS, "PlayerGui_spy")
                _broadKeywordSweep(pg, BROAD_DEX_KEYWORDS, "PlayerGui_dex")
            end
        end)
    end

    local function _checkHooks()
        -- Minimal at startup; runtime threads handle ongoing monitoring.
    end

    -- =========================================================================
    -- §10  INITIAL SYNCHRONOUS CHECKS
    -- =========================================================================
    _checkHardKillGlobals()   -- instant kill for definitive globals (no threshold)
    _checkBroadKeywordSweep() -- instant kill for any keyword-matched GUI
    _checkHTTPSpy()
    _checkRemoteSpy()
    _checkDexExplorer()
    _checkHooks()
    _evaluateAndKill()        -- soft kill if ≥2 signals accumulated

    -- =========================================================================
    -- §11  RUNTIME MONITOR  (Correct approach — no getgc, no freeze)
    --
    --  HOW REMOTE SPIES WORK:
    --   Every remote spy (SimpleSpy, OctoSpy, Hydroxide, Blatant, etc.) MUST
    --   hook __namecall on getrawmetatable(game) and/or FireServer/InvokeServer
    --   to intercept remotes. When they do, those functions change from C closures
    --   to Lua closures (even with newcclosure, iscclosure returns true but the
    --   original reference changes). We detect this with two methods:
    --    1. Reference comparison — is __namecall still the same pointer?
    --    2. iscclosure — is it still a native C function or wrapped in Lua?
    --
    --  HOW DEX WORKS:
    --   Dex always parents a ScreenGui to CoreGui named "DexExplorer".
    --   ChildAdded fires the instant it's injected.
    --
    --  NO getgc() — EVER. It freezes the game. Not used here.
    --  NO getscripts() in a tight loop — unreliable and executor-specific.
    -- =========================================================================

    local _rtSignals = {}
    local function _rtSignal(cat)
        if _rtSignals[cat] then return end
        _rtSignals[cat] = true
        local n = 0
    ...
    -- ── Method 1: Hook detection via reference + iscclosure ──────────────────
    -- Checks if __namecall or FireServer has been replaced since we captured them.
    -- Every remote spy replaces these — this is the definitive detection.
    local function _checkHookIntegrity()
        -- Check __namecall reference changed
        if _rawmt and _origNamecall then
            _pcall(function()
                local current = _rawmt.__namecall
                -- Reference changed = something replaced it
                if current ~= nil and not _rawequal(current, _origNamecall) then
                    _rtSignal("namecall_hook_replaced")
                end
                -- iscclosure = false means it's a plain Lua hook (not newcclosure)
                if _islclosure and _islclosure(current) then
                    _rtSignal("namecall_is_lclosure")
                end
            end)
        end

        -- Check FireServer reference changed
        if _origFireServer then
            _pcall(function()
                local re = Instance.new("RemoteEvent")
                local current = re.FireServer
                re:Destroy()
                if not _rawequal(current, _origFireServer) then
                    _rtSignal("fireserver_hook_replaced")
                end
                if _islclosure and _islclosure(current) then
                    _rtSignal("fireserver_is_lclosure")
                end
            end)
        end

        -- Check InvokeServer reference changed
        if _origInvoke then
            _pcall(function()
                local rf = Instance.new("RemoteFunction")
                local current = rf.InvokeServer
                rf:Destroy()
                if not _rawequal(current, _origInvoke) then
                    _rtSignal("invokeserver_hook_replaced")
                end
            end)
        end
    end

    -- ── Method 2: ChildAdded on CoreGui + PlayerGui ───────────────────────────
    -- Fires the instant any spy/dex GUI is parented. Zero polling overhead.
    local function _onChildAdded(child)
        if not _runtimeAlive then return end
        local name = _tostring(child.Name)
        local ln   = _low(name)

        for _, kw in _ipairs(BROAD_SPY_KEYWORDS) do
            if _has(ln, kw) then _hardKill("Spy GUI injected: " .. name) end
        end
        for _, kw in _ipairs(BROAD_DEX_KEYWORDS) do
            if _has(ln, kw) then _hardKill("Dex GUI injected: " .. name) end
        end
        if _nameInList(name, SPY_UI_NAMES) then _hardKill("Spy UI: " .. name) end
        if _nameInList(name, DEX_UI_NAMES) then _hardKill("Dex UI: " .. name) end
    end

    _pcall(function()
        game:GetService("CoreGui").ChildAdded:Connect(function(child)
            _task_spawn(function() _pcall(_onChildAdded, child) end)
        end)
    end)
    _pcall(function()
        local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
    ...
    -- ── Method 3: Lightweight global poll (every 5s — cheap table lookups only)
    _task_spawn(function()
        while _runtimeAlive do
            -- Hook integrity (the real remote spy detector)
            _pcall(_checkHookIntegrity)

            -- Hard-kill globals (definitive spy globals)
            _pcall(_scanHardKillGlobals)
            ...
            _pcall(function()
                local found, key = _scanGlobals(HTTP_SPY_GLOBALS)
                if found then _rtSignal("rt_http_spy:" .. (key or "?")) end
            end)
            _pcall(function()
                local found, key = _scanGlobals(DEX_GLOBALS)
    ...
    -- =========================================================================
    -- §12  FINAL GATE — all synchronous checks passed; background threads live on.
    -- =========================================================================

end -- ── End of protection do-block ──────────────────────────────────────────
-- ╔═════════════════════════════════════════════════════════════════════════════╗
-- ║               END OF PROTECTION SYSTEM v4.0                               ║
