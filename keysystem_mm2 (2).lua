...
local premiumSilentAim = {
    ...
    lastShot  = 0,
    cooldown  = 0.35,    -- seconds between shots (adjustable via UI)
}

-- Core intercept: Kalman vel + acceleration + ping lead
-- Designed to stay accurate at 200–400 ms ping by using second-order
-- (position + velocity*t + 0.5*accel*t²) prediction and an adaptive
-- lead-time multiplier that stretches with high ping.
local function solvePremiumIntercept(targetPlayer)
    ...
    -- Ensure tracker is up to date
    phantomUpdate(targetPlayer)
    local smoothVel   = getSmoothedVel(targetPlayer)
    local smoothAccel = getSmoothedAccel(targetPlayer)

    -- ── Adaptive lead time ──────────────────────────────────────────
    -- At low ping (≤50 ms): lead = ping + 1 server frame (16.67 ms)
    -- At mid ping (50–200 ms): extra 20% padding for packet jitter
    -- At high ping (200 ms+): extra 35% padding + accel fully applied
    ...
    local leadTime = math.clamp((rawPing + 0.0167) * jitterPad, 0.020, 0.500)

    -- ── Velocity blend: Kalman + player input direction ─────────────
    -- 65/35 at normal speed. Pulls toward input direction more when
    -- the Kalman speed disagrees with walkSpeed (direction just changed).
    local walkSpeed = (hum and hum.WalkSpeed) or 16
    local vel = smoothVel
    if hum then
        local inputVel = hum.MoveDirection * walkSpeed
        -- If Kalman and input disagree a lot, trust input more (player changed dir)
        local agreement = smoothVel.Magnitude > 0.5 and inputVel.Magnitude > 0.5
            and (smoothVel.Unit:Dot(inputVel.Unit))
            or 1
        local inputBlend = math.clamp(0.35 + (1 - agreement) * 0.20, 0.30, 0.55)
        vel = smoothVel * (1 - inputBlend) + inputVel * inputBlend
    end

    -- Hard-cap lateral speed to 1.15× walkSpeed (prevents over-shoot on strafe)
    local lateralVel = Vector3.new(vel.X, 0, vel.Z)
    if lateralVel.Magnitude > walkSpeed * 1.15 then
        lateralVel = lateralVel.Unit * (walkSpeed * 1.15)
    end
    vel = Vector3.new(lateralVel.X, vel.Y, lateralVel.Z)

    -- ── Second-order prediction (pos + v*t + ½*a*t²) ───────────────
    -- Acceleration is EWMA-smoothed so it doesn't over-react to one jump.
    -- At high ping the accel term becomes important because the murderer
    -- may have started turning/slowing *after* our last received packet.
    local accelScale = math.clamp(rawPing / 0.200, 0, 1) -- 0 at ≤0ms, 1 at 200ms+
    local accelContrib = smoothAccel * accelScale

    -- Clamp accel magnitude (wild spikes from jumps shouldn't over-predict)
    if accelContrib.Magnitude > walkSpeed * 3 then
        accelContrib = accelContrib.Unit * (walkSpeed * 3)
    end
    ...
    -- ── Vertical aim: chest-to-neck band ────────────────────────────
    local rootY = root.Position.Y
    local headY = head and head.Position.Y or (rootY + 2.9)
    local aimY  = rootY + (headY - rootY) * 0.58   -- ~chest/neck
    -- Vertical lead: half-weight so we don't wildly over-shoot jumping targets
    local vertLead = vel.Y * leadTime * 0.45
    -- At very high ping, the murderer in the air needs more vertical lead
    if rawPing > 0.200 and math.abs(vel.Y) > 5 then
        vertLead = vel.Y * leadTime * 0.65
    end
    predicted = Vector3.new(predicted.X, aimY + vertLead, predicted.Z)

    -- ── Floor / ceiling safety raycast ──────────────────────────────
    local rp = RaycastParams.new()
    rp.FilterType = Enum.RaycastFilterType.Blacklist
    rp.FilterDescendantsInstances = {char}
    local fR = workspace:Raycast(predicted, Vector3.new(0, -6, 0), rp)
...
local function doPremiumShoot(targetPlayer)
    ...
    -- Equip from backpack if needed (minimum wait for server to ack)
    if LocalPlayer.Backpack:FindFirstChild(gun.Name) then
        humanoid:EquipTool(gun)
        task.wait(0.05)
        gun = myChar:FindFirstChild("Gun") or myChar:FindFirstChild("Revolver")
...
GrabGunFrame.Size                    = UDim2.new(0, 110, 0, 48)
GrabGunFrame.Position                = UDim2.new(0.01, 0, 0.45, 0)
GrabGunFrame.BackgroundColor3        = Color3.fromRGB(18, 18, 28)
GrabGunFrame.BackgroundTransparency  = 0.10
...
GrabGunCorner.CornerRadius = UDim.new(0, 10)
GrabGunCorner.Parent       = GrabGunFrame

GrabGunStroke.Color       = Color3.fromRGB(0, 180, 255)
GrabGunStroke.Thickness   = 2
GrabGunStroke.Transparency = 0.25
GrabGunStroke.Parent       = GrabGunFrame

-- Header label (drag handle feel)
GrabGunLabel.Name                = "Header"
GrabGunLabel.Size                = UDim2.new(1, 0, 0, 16)
GrabGunLabel.Position            = UDim2.new(0, 0, 0, 0)
GrabGunLabel.BackgroundColor3    = Color3.fromRGB(0, 140, 220)
GrabGunLabel.BackgroundTransparency = 0.15
GrabGunLabel.BorderSizePixel     = 0
GrabGunLabel.Font                = Enum.Font.GothamBold
GrabGunLabel.TextSize            = 10
GrabGunLabel.TextColor3          = Color3.fromRGB(255, 255, 255)
GrabGunLabel.Text                = "🔫  GRAB GUN"
GrabGunLabel.Parent              = GrabGunFrame

local GrabLabelCorner = Instance.new("UICorner")
GrabLabelCorner.CornerRadius = UDim.new(0, 10)
GrabLabelCorner.Parent = GrabGunLabel

-- Main tap button
GrabGunBtn.Name                    = "GrabBtn"
GrabGunBtn.Size                    = UDim2.new(1, -8, 0, 26)
GrabGunBtn.Position                = UDim2.new(0, 4, 0, 18)
GrabGunBtn.BackgroundColor3        = Color3.fromRGB(0, 160, 255)
GrabGunBtn.BackgroundTransparency  = 0.20
GrabGunBtn.BorderSizePixel         = 0
GrabGunBtn.Font                    = Enum.Font.GothamBold
GrabGunBtn.TextSize                = 13
GrabGunBtn.TextColor3              = Color3.fromRGB(255, 255, 255)
GrabGunBtn.Text                    = "TAP TO GRAB"
GrabGunBtn.Parent                  = GrabGunFrame

local GrabBtnCorner = Instance.new("UICorner")
GrabBtnCorner.CornerRadius = UDim.new(0, 7)
GrabBtnCorner.Parent = GrabGunBtn

-- Flash effect on tap
GrabGunBtn.MouseButton1Click:Connect(function()
    manualGrabGun()
    -- Brief color flash feedback
    GrabGunBtn.BackgroundColor3 = Color3.fromRGB(0, 255, 120)
    task.wait(0.18)
    GrabGunBtn.BackgroundColor3 = Color3.fromRGB(0, 160, 255)
end)

-- Live dot: turns green when gun drop exists, red when none
local gunDropIndicator = Instance.new("Frame")
gunDropIndicator.Size             = UDim2.new(0, 8, 0, 8)
gunDropIndicator.Position         = UDim2.new(1, -12, 0, 4)
...
Tabs.World:AddSection("🛡️ Anti-Fling")
...
Tabs.World:AddSection("⚠️ Anti-Void")
...
Tabs.Premium:AddParagraph({
    Title = "How To Use",
    Content = "1. Enable 'Premium Silent Aim' toggle below.\n2. A draggable ⭐ button will appear on your screen — tap it to fire at the murderer.\n3. Drag the button anywhere on screen for comfort.\n4. Adjust Shot Cooldown if needed."
})

Tabs.Premium:AddToggle("PremiumSilentAimToggle", {
    Title = "⭐ Premium Silent Aim",
    Description = "Shows the on-screen ⭐ button. Tap it to fire at the murderer with premium prediction.",
    Default = false,
    Callback = function(v)
        ...
        Fluent:Notify({
            Title = "⭐ Premium Silent Aim",
            Content = v
                and "ENABLED — tap the ⭐ button on screen to shoot."
...
Tabs.Premium:AddSlider("PremiumCooldownSlider", {
    Title = "Shot Cooldown (ms)",
    Description = "Minimum time between shots. Lower = faster. Higher = safer against spam detection.",
    Default = 350,
    Min = 100,
    Max = 1500,
    Rounding = 0,
    Callback = function(v)
        premiumSilentAim.cooldown = v / 1000
    end
})
