...
local function bestClearPoint(fromPos, predictedPos, targetChar, bodyH_hint)
    local root = targetChar:FindFirstChild("HumanoidRootPart")
    local head = targetChar:FindFirstChild("Head")
    local bH   = bodyH_hint or 3.0

    -- Direct check first (fast path)
    if hasLOS(fromPos, predictedPos, targetChar) then
        return predictedPos, true
    end

    local rp = root and root.Position or predictedPos
    -- Scaled offset grid covering: chest, head, pelvis, and 4 lateral points per height
    local fracs = {0.50, 0.80, 0.25, 0.65, 0.40, 0.90}
    local laterals = {
        Vector3.new( 0.9, 0, 0), Vector3.new(-0.9, 0, 0),
        Vector3.new( 0,   0, 0.9), Vector3.new(0,   0, -0.9),
    ...
    for _, frac in ipairs(fracs) do
        for _, lat in ipairs(laterals) do
            ...
            if hasLOS(fromPos, cand, targetChar) then
                return cand, true
            end
        end
    end
    -- Head direct fallback
    if head and hasLOS(fromPos, head.Position, targetChar) then
        return head.Position, true
    end
...
-- Full pipeline:
--   1. Measure real body bounds (small-avatar safe)
--   2. Compute lead time from ping + range distance
--   3. Solve lateral XZ via Kalman velocity + acceleration trend
--   4. Solve vertical Y via:
--        ground   → stable aim at true body-centre Y
--        ascending → full parabolic arc integration
--        peak      → minimal displacement
--        descending → gravity integration from current velY
--   5. LOS check → body-grid fallback
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- ── 1. Body Bounds (small-avatar aware) ────────────────────────────────
    local bMinY, bMaxY, _ = measureCharBounds(char)
    local rootY = root.Position.Y
    local bodyH, bodyCentreY

    if bMinY and bMaxY and (bMaxY - bMinY) > 0.1 then
        bodyH      = bMaxY - bMinY
        bodyCentreY = (bMinY + bMaxY) * 0.5  -- true geometric centre
    else
        -- Fallback: head-based estimate
        local rawHeadY = head and head.Position.Y or (rootY + 2.9)
        bodyH      = math.max(rawHeadY - rootY, 0.6)
        bodyCentreY = rootY + bodyH * 0.55
    end
    ...
    -- Base ping + 1 frame + click-tap overhead + slight range lead
    local baseLead  = rawPing + 0.0167 + clickOffset
    local rangeLead = math.clamp(rangeDist * 0.00010, 0.0, 0.030)

    -- Airborne targets need a wider window because arc uncertainty grows with time
    ...
    do
        ...
        local function castGnd(off)
            return workspace:Raycast(root.Position + off, Vector3.new(0, -5.5, 0), rpGnd)
        end
        isOnGround = castGnd(Vector3.new(0,0,0)) ~= nil
            or castGnd(Vector3.new(0.6,0.8,0)) ~= nil
            or castGnd(Vector3.new(-0.6,0.8,0)) ~= nil
            or castGnd(Vector3.new(0,0.8,0.6)) ~= nil
            or castGnd(Vector3.new(0,0.8,-0.6)) ~= nil
    ...
    -- Ping-band jitter multiplier for lead time
    local pingBand = (rawPing <= 0.060) and 0
                  or (rawPing <= 0.120) and 1
                  or (rawPing <= 0.200) and 2
                  or (rawPing <= 0.300) and 3 or 4
    local jMults = {
        [0]={g=1.00, asc=1.16, desc=1.10},
        [1]={g=1.05, asc=1.24, desc=1.18},
        [2]={g=1.12, asc=1.34, desc=1.26},
        [3]={g=1.20, asc=1.46, desc=1.38},
        [4]={g=1.26, asc=1.58, desc=1.50},
    }
    local jm  = jMults[pingBand]
    local jPad = isJumping and jm.asc or (isFalling and jm.desc or jm.g)
    -- Close-range correction: at <8 studs, shot is nearly instant; at >40 studs full lead
    local closeRangeScale = math.clamp(rangeDist / 8.0, 0.08, 1.0)
    local leadTime = math.clamp((baseLead * jPad + rangeLead) * closeRangeScale, 0.010, 0.650)

    -- ── 3. Lateral (XZ) position ────────────────────────────────────────────
    local walkSpeed = (hum and hum.WalkSpeed) or 16
    local moveDir   = (hum and hum.MoveDirection) or Vector3.new()

    -- Dual-pass velocity: blend near-term and far-term Kalman trend
    local pass1Vel = tr and getVelAtTime(tr, leadTime * 0.40) or smoothVel
    local pass2Vel = tr and getVelAtTime(tr, leadTime)         or smoothVel
    local trendMix = math.clamp((rangeDist - 15) / 60, 0, 1)
    local trendVel = pass1Vel:Lerp(pass2Vel, trendMix)

    -- Wall-hug detection: if actual lateral speed is less than expected from input,
    -- character is likely running into geometry; re-project moveDir off wall normal
    local expectedLat = moveDir.Magnitude * walkSpeed
    local actualLat   = Vector3.new(trendVel.X, 0, trendVel.Z).Magnitude
    local wallFactor  = (expectedLat > 0.5) and math.clamp(actualLat / expectedLat, 0, 1) or 1.0
    local effectiveMoveDir = moveDir
    if wallFactor < 0.50 and moveDir.Magnitude > 0.1 then
    ...
    -- Blend Kalman velocity with input direction; weight input more at close range
    local rangeBlend = math.clamp(1 - (rangeDist - 20) / 100, 0.25, 1.0)
    local vel = trendVel
    if hum then
        local inputVel  = effectiveMoveDir * walkSpeed
        local agreement = 1.0
        if trendVel.Magnitude > 0.5 and inputVel.Magnitude > 0.5 then
            agreement = math.clamp(trendVel.Unit:Dot(inputVel.Unit), -1, 1)
        end
        local baseBlend
        if isAirborne then
            baseBlend = 0.10  -- mid-air: trust physics more than input
        elseif agreement > 0.85 then
            baseBlend = 0.52
        elseif agreement < 0.40 then
            baseBlend = 0.04  -- direction changed → low input trust
        else
            baseBlend = 0.52 * ((agreement - 0.40) / 0.45)
        end
        baseBlend = math.clamp(baseBlend * rangeBlend * math.max(wallFactor, 0.08), 0.04, 0.52)
        vel = trendVel * (1 - baseBlend) + inputVel * baseBlend
    end
    -- Speed cap
    local latCap = isAirborne and (walkSpeed * 1.28) or (walkSpeed * 1.08)
    local lv = Vector3.new(vel.X, 0, vel.Z)
    if lv.Magnitude > latCap then lv = lv.Unit * latCap end
    vel = Vector3.new(lv.X, vel.Y, lv.Z)

    -- Acceleration correction (2nd-order kinematic term)
    local accelVec = tr and (getVelAtTime(tr, leadTime) - trendVel) or Vector3.new()
    accelVec = Vector3.new(accelVec.X, 0, accelVec.Z)
    if accelVec.Magnitude > walkSpeed * 1.8 then accelVec = accelVec.Unit * (walkSpeed * 1.8) end
    local accelCap   = math.clamp(1.1 + (rangeDist - 20) / 100, 1.1, 1.5)
    local accelScale = math.clamp(rawPing / 0.150, 0, accelCap)
        * (isAirborne and 0.35 or 1.0) * math.max(wallFactor, 0.12) * rangeBlend

    -- XZ predicted position
    local predictedXZ = root.Position
        + vel * leadTime
        + accelVec * (accelScale * 0.5 * leadTime * leadTime)

    -- ── 4. Vertical (Y) position ────────────────────────────────────────────
    -- Use the TRUE body centre from bounding box, NOT an assumption about avatar scale
    local groundAimY   = bodyCentreY                          -- stable grounded centre
    local airborneAimY = (bMinY or rootY) + bodyH * 0.50     -- mid-air centre

    local finalY
    if isAirborne then
        if tr and (tr.jumpState == "ascending" or tr.jumpState == "peak") then
            -- Parabolic arc: project from jump-start, apply to current airborne centre
            local elapsed    = math.max(tick() - tr.jumpStartT, 0)
            local tTotal     = elapsed + leadTime
            -- Displacement from jump-start Y to future Y
            ...
            local deltaArc  = arcFuture - arcNow
            finalY = airborneAimY + deltaArc

        elseif tr and tr.jumpState == "descending" then
            -- Pure kinematic descent: velY negative, gravity accelerates fall
            ...
            -- Generic airborne (no Kalman jump state): use smoothed velY + gravity
            local gravDrop  = 0.5 * ROBLOX_GRAVITY * leadTime * leadTime
            local predRootY = rootY + kalmanVelY * leadTime - gravDrop
            finalY = predRootY + bodyH * 0.50
        end
        ...
        local pBotY     = predRootY + (bMinY and (bMinY - rootY) or 0)
        local pTopY     = predRootY + bodyH
        local hardMin   = math.min(pBotY, rootY - 5.0)
        local hardMax   = math.max(pTopY + 0.4, (bMaxY or rootY + bodyH) + 5.0)
        finalY = math.clamp(finalY, hardMin, hardMax)
    else
        -- Grounded: tiny slope/crouch nudge
        local velYNudge = (math.abs(rawVelY) >= 1.5) and (rawVelY * leadTime * 0.10) or 0
        finalY = groundAimY + velYNudge
        -- Clamp within body bounds (never below feet, never above head + tiny slack)
        finalY = math.clamp(finalY,
            (bMinY or rootY) + 0.1,
            (bMaxY or (rootY + bodyH)) + 0.1)
    end

    local predicted = Vector3.new(predictedXZ.X, finalY, predictedXZ.Z)

    -- ── 5. LOS check with body-grid fallback ────────────────────────────────
    if fromPos then
        local clearPos, wasLOS = bestClearPoint(fromPos, predicted, char, bodyH)
        predicted = clearPos
        if not wasLOS then
            -- Try raw root as last resort
            if hasLOS(fromPos, root.Position, char) then
                predicted = root.Position + Vector3.new(0, bodyH * 0.50, 0)
            end
        end
    end
...
-- ── Premium Shoot ──────────────────────────────────────────────────────────────
-- Key design goals:
--   • ZERO task.spawn overhead before first FireServer (fires this frame)
--   • Small cooldown (0.05 s) so tapping fast works
--   • Body-bounding-box aware so small avatars are hit correctly
--   • More phantom samples so Kalman is warm before solve
--   • Airborne targets get wider burst window with fresh re-solve each frame
local function doPremiumShoot(targetPlayer)
    local now = tick()
    if now - premiumSilentAim.lastShot < premiumSilentAim.cooldown then return end
    premiumSilentAim.lastShot = now   -- stamp BEFORE any async work
    ...
    -- Warm up Kalman tracker with fresh samples synchronously before solve
    ...
    local gun = LocalPlayer.Backpack:FindFirstChild("Gun")
             ...
             or myChar:FindFirstChild("Revolver")
    if not gun then
        Fluent:Notify({ Title = "⭐ Premium Aim", Content = "No gun in backpack or hand.", Duration = 3 })
        return
    end
    -- Equip if in backpack (happens synchronously via EquipTool)
    if LocalPlayer.Backpack:FindFirstChild(gun.Name) then
        humanoid:EquipTool(gun)
        -- Poll up to 3 frames for equipped tool (no task.wait here — use yield-free check)
        for _ = 1, 6 do
            gun = myChar:FindFirstChild("Gun") or myChar:FindFirstChild("Revolver")
            if gun then break end
            task.wait()
    ...
    premiumSilentAim._clickLatencyOffset = 0.030

    -- ── FIRST SHOT: fires on this exact frame (no task.spawn) ─────────────
    ...
    -- Burst count: more shots when airborne (uncertainty is higher)
    local pingMs   = getPing() * 1000
    local burstBase = pingMs <= 60  and 2
                   or pingMs <= 120 and 3
                   or pingMs <= 200 and 4
                   or pingMs <= 300 and 5
                   or pingMs <= 400 and 6 or 7
    -- Airborne: +3 extra bursts (arc changes every frame)
    local burstMax = targetAirborne and (burstBase + 3) or burstBase
    ...
    burstConn = RunService.Heartbeat:Connect(function()
        ...
        premiumSilentAim._clickLatencyOffset = 0.030
        local fp = solvePremiumIntercept(targetPlayer, fb)
                or (mHead and mHead.Position)
                or mRoot.Position + Vector3.new(0, 1.5, 0)
        premiumSilentAim._clickLatencyOffset = nil
        pcall(function()
