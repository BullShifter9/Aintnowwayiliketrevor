...
-- Per-round gun-grab debounce table.
-- Key = playerName, value = true once notified this round.
-- Cleared at each round start (Fade event).
local gunGrabNotified = {}

-- Clear debounce table at round start so notifications work every new round
GameplayEvents.Fade.OnClientEvent:Connect(function()
    gunGrabNotified = {}
end)
...
local function watchPlayerForGunPickup(player)
    local function onGunAdded(child)
        ...
        -- Guard: only fire for innocents (not Sheriff who starts with a gun,
        -- not Murderer, not someone already flagged Hero this round)
        local pd = R[player.Name]
        local role = pd and pd.Role or "Innocent"
        if role == "Murderer" or role == "Sheriff" then return end

        -- Debounce: only notify once per player per round
        if gunGrabNotified[player.Name] then return end
        gunGrabNotified[player.Name] = true
        ...
        -- Single fire-once notification
        Fluent:Notify({
            Title = "🔫 Gun Grabbed!",
            Content = player.Name .. " picked up the gun and is now the Hero!",
            Duration = 5
        })
    ...
    -- Watch character (for when gun is equipped directly into char)
...
local function getTracker(player)
    ...
    pTrackers[player] = {
        ...
        kz = kNew(pos.Z, vel.Z),
        lastT   = tick(),
        -- Acceleration tracking (EWMA of velocity delta per second)
        accel   = Vector3.new(),
        lastVel = vel,
        -- Recent velocity history for high-ping extrapolation (last 8 samples)
        velHistory = {},
        posHistory = {},
    }
...
local function phantomUpdate(player)
    ...
    -- Track acceleration as EWMA of velocity delta / dt (clamped to avoid spikes)
    local rawAccel = (vel - t.lastVel) / dt
    local accelMag = rawAccel.Magnitude
    if accelMag > 200 then rawAccel = rawAccel.Unit * 200 end -- sanity clamp
    local alpha = math.clamp(dt * 8, 0, 0.4)                 -- EWMA speed
    t.accel   = t.accel:Lerp(rawAccel, alpha)
    t.lastVel = vel

    -- Keep a ring-buffer of (time, pos, vel) — max 8 samples
    local h = t.velHistory
    local ph = t.posHistory
    if #h >= 8 then table.remove(h, 1); table.remove(ph, 1) end
...
-- Get EWMA-smoothed acceleration for a player (zero if tracker not warmed up)
...
-- Main intercept solver — IMPROVED v3
-- ────────────────────────────────────────────────────────────────
-- Key improvements over the original:
--
-- 1. WALL-PROXIMITY DETECTION & LEAD REDUCTION
--    Before predicting, we cast 4 feeler rays from the target's
--    root outward (forward/back/left/right).  If any wall is within
--    WALL_FEELER_DIST studs, we classify the target as "near-wall"
--    and aggressively clamp the lateral lead so we don't predict
--    them *into* geometry that the server never accepted.
--
-- 2. RANGE-ADAPTIVE LEAD SCALING
--    Long-range shots take longer to travel and the server applies
--    additional lag compensation. We now scale leadTime linearly by
--    distance:
--      • 0–30 studs  → leadTime × 1.00  (close — stock behaviour)
--      • 30–80 studs → leadTime × 1.08  (mid   — slight boost)
--      • 80+ studs   → leadTime × 1.20  (far   — meaningful boost)
--    This prevents the consistent "behind the target" misses at
--    long range that the original solver produced.
--
-- 3. MULTI-SAMPLE VELOCITY HISTORY for high-uncertainty situations
--    When the target's Kalman velocity is noisy (changing direction,
--    rapid acceleration) we blend in a short-term average of the
--    last 3 velocity samples to stabilise the prediction.  This
--    greatly reduces the "jitter-miss" when the murderer strafe-hops.
--
-- 4. WALL FEELER-BASED FALLBACK CHAIN
--    The existing bestClearPoint() fallback already existed, but now
--    it also receives a `wallBias` scalar (0-1) so that when the
--    target is against a wall, head/chest are tried with a slightly
--    closer Y to avoid grazing the wall top-edge.
--
-- 5. IMPROVED GRAVITY COMPENSATION (airborne)
--    The cap on gravityDrop was a flat 1.8 studs regardless of range.
--    It is now range-scaled: cap = 0.9 + rangeFactor * 3.2 so that
--    at 80 studs we allow up to ~4 studs of drop compensation, which
--    stops the aim snapping to the feet on long aerial shots.
--
-- 6. BETTER Y CLAMPING with range-aware head offset
--    When the head Part can't be found (unlikely but possible mid-
--    respawn), the fallback headY is now rootY+3.1 at close range
--    and rootY+3.5 at distance (player appears taller from far away
--    due to perspective), giving better hitbox coverage.
-- ────────────────────────────────────────────────────────────────

local ROBLOX_GRAVITY = workspace.Gravity  -- typically 196.2
local WALL_FEELER_DIST = 3.5              -- studs; triggers near-wall mode

-- ── Wall proximity check ─────────────────────────────────────────
-- Cast 4 horizontal rays from rootPos outward. Returns 0..1 where
-- 0 = completely open, 1 = wall on all four sides.
local function wallProximityFactor(rootPos, targetChar)
    local rpWall = RaycastParams.new()
    rpWall.FilterType = Enum.RaycastFilterType.Blacklist
    rpWall.FilterDescendantsInstances = {targetChar}
    local hits = 0
    local dirs = {
        Vector3.new( 1, 0,  0),
        Vector3.new(-1, 0,  0),
    ...
    for _, d in ipairs(dirs) do
        local result = workspace:Raycast(rootPos, d * WALL_FEELER_DIST, rpWall)
        if result then hits = hits + 1 end
    end
    return hits / 4  -- 0 = open space, 1 = boxed in
end
...
-- Improved bestClearPoint: also tries wall-bias offsets when standard
-- candidates are all blocked (near-wall scenario).
local function bestClearPoint(fromPos, predictedPos, targetChar, wallBias)
    local root = targetChar:FindFirstChild("HumanoidRootPart")
    local head = targetChar:FindFirstChild("Head")
    wallBias = wallBias or 0

    local rootY = root and root.Position.Y or 0
    local headY = head and head.Position.Y or (rootY + 3.0)
    local chestY = rootY + (headY - rootY) * 0.58

    -- Near-wall bias: nudge aim candidates inward so they clear the wall
    -- top-edge.  wallBias 0 = no change, 1 = full chest-only targeting.
    local wallChestY = rootY + (headY - rootY) * math.max(0.30, 0.58 - wallBias * 0.18)

    local candidates = {
        predictedPos,
        head  and head.Position,
        root  and Vector3.new(root.Position.X, wallChestY, root.Position.Z),
    ...
    -- Nothing clear — last resort: return raw root position
    return root and root.Position or predictedPos, false
end

-- ── Short-window velocity average from ring-buffer ────────────────
-- Averages the last N velocity samples for stabilisation under noise.
local function getRecentVelAvg(player, nSamples)
    local t = pTrackers[player]
    if not t or not t.velHistory or #t.velHistory == 0 then
        local char = player.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        return root and root.AssemblyLinearVelocity or Vector3.new()
    end
    local h = t.velHistory
    local n = math.min(nSamples or 3, #h)
...
local function solvePremiumIntercept(targetPlayer, fromPos)
    ...
    -- ── Velocity: Kalman-smoothed + short-window average blend ────────
    local smoothVel   = getSmoothedVel(targetPlayer)
    local recentAvg   = getRecentVelAvg(targetPlayer, 3)
    local smoothAccel = getSmoothedAccel(targetPlayer)

    -- Blend Kalman and recent-average based on how much the velocity is
    -- changing.  High acceleration = prefer recent avg; stable = Kalman.
    local velChangeMag = (smoothVel - recentAvg).Magnitude
    local blendToAvg   = math.clamp(velChangeMag / 8.0, 0, 0.45)
    local vel = smoothVel:Lerp(recentAvg, blendToAvg)

    -- ── Distance to target ─────────────────────────────────────────────
    local shooterPos = fromPos or (LocalPlayer.Character and
                       LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and
                       LocalPlayer.Character.HumanoidRootPart.Position) or Vector3.new()
    local dist = (root.Position - shooterPos).Magnitude

    -- Range factor: 0 = close (≤30 st), 1 = far (≥80 st)
    local rangeFactor = math.clamp((dist - 30) / 50, 0, 1)
    ...
    local groundRay  = workspace:Raycast(root.Position, Vector3.new(0, -3.8, 0), rp0)
    local isAirborne = (math.abs(smoothVel.Y) > 3.5) and (groundRay == nil)

    -- ── Adaptive lead jitter (ping-tier) ─────────────────────────────
    local jitterPad
    if isAirborne then
        jitterPad = rawPing <= 0.080 and 1.12
                 or rawPing <= 0.150 and 1.25
                 or rawPing <= 0.250 and 1.38
                 or 1.50
    else
        jitterPad = rawPing <= 0.050 and 1.00
                 or rawPing <= 0.120 and 1.10
                 or rawPing <= 0.200 and 1.18
                 or 1.28
    end

    -- ── Range-adaptive lead scaling ───────────────────────────────────
    -- Close range: no change.  Long range: up to +20% more lead.
    local rangeLead = 1.0 + rangeFactor * 0.20
    local leadTime  = math.clamp(baseLead * jitterPad * rangeLead, 0.018, 0.600)

    -- ── Wall proximity factor ─────────────────────────────────────────
    local wallFactor = wallProximityFactor(root.Position, char)
    -- 0 = open space, up to 1.0 = boxed in
    -- When near wall: reduce horizontal lead to avoid predicting into geometry
    local wallLeadScale = 1.0 - wallFactor * 0.65  -- at wallFactor=1 → 35% of lead

    -- ── Velocity blend (input + Kalman) ──────────────────────────────
    local walkSpeed = (hum and hum.WalkSpeed) or 16

    if hum then
        local inputVel  = hum.MoveDirection * walkSpeed
        -- Near walls the target often has constrained move direction, so we
        -- trust the measured velocity more than their input direction.
        local baseBlend = isAirborne and 0.18 or 0.38
        -- Reduce input blend near walls (movement direction is unreliable)
        baseBlend = baseBlend * (1 - wallFactor * 0.5)
        local agreement = (vel.Magnitude > 0.5 and inputVel.Magnitude > 0.5)
                          and vel.Unit:Dot(inputVel.Unit) or 1
        local inputBlend = math.clamp(baseBlend + (1 - agreement) * 0.12, 0.10, 0.50)
        vel = vel * (1 - inputBlend) + inputVel * inputBlend
    end
    ...
    -- Apply wall lead scale ONLY to horizontal component
    lateralVel = lateralVel * wallLeadScale
    vel = Vector3.new(lateralVel.X, vel.Y, lateralVel.Z)

    -- ── Acceleration contribution (XZ only, range-limited) ───────────
    local accelScale = math.clamp(rawPing / 0.200, 0, 1)
    if isAirborne then accelScale = accelScale * 0.45 end
    -- Reduce accel contribution near walls (bounced/constrained accel is noise)
    accelScale = accelScale * (1 - wallFactor * 0.6)
    local accelContrib = smoothAccel * accelScale
    if accelContrib.Magnitude > walkSpeed * 2.0 then
        accelContrib = accelContrib.Unit * (walkSpeed * 2.0)
    end

    -- ── XZ position prediction ────────────────────────────────────────
    local predicted = root.Position
        + vel          * leadTime
        + accelContrib * (0.5 * leadTime * leadTime)

    -- ── Vertical (Y) prediction ───────────────────────────────────────
    local rootY = root.Position.Y
    -- Range-aware head height: player hitbox looks higher at long range
    local headFallback = rootY + 3.0 + rangeFactor * 0.5
    local headY = head and head.Position.Y or headFallback
    local aimY  = rootY + (headY - rootY) * 0.58  -- chest/neck band

    local finalY
    if isAirborne then
        -- Gravity drop cap is now range-adaptive: more compensation at distance
        local gravCapMax    = 0.9 + rangeFactor * 3.2   -- 0.9 close → 4.1 far
        local gravityDrop   = math.min(0.5 * ROBLOX_GRAVITY * (leadTime * leadTime), gravCapMax)
        finalY = aimY + vel.Y * leadTime - gravityDrop
    else
        -- Grounded: gentle slope compensation only; near walls even less drift
        local groundDrift = 0.06 * (1 - wallFactor * 0.4)
        finalY = aimY + vel.Y * leadTime * groundDrift
    end

    -- ── Body-relative Y clamp ─────────────────────────────────────────
    -- Guarantees aim stays between root top and just above head
    finalY = math.clamp(finalY, rootY + 0.1, headY + 0.35)

    predicted = Vector3.new(predicted.X, finalY, predicted.Z)

    -- ── Wall / LOS check with wall-aware fallback chain ───────────────
    if fromPos then
        local clearPos, wasLOS = bestClearPoint(fromPos, predicted, char, wallFactor)
        predicted = clearPos
    end
...
-- Burst size scales with ping:
--   ≤ 80ms  → 2 extra frames  (fast enough — minimal)
--   ≤ 150ms → 3 extra frames
--   ≤ 250ms → 4 extra frames
--   > 250ms → 5 extra frames  (high-lag — need max coverage)
-- Each burst frame re-solves intercept so the aim tracks the murderer live.
local function doPremiumShoot(targetPlayer)
    ...
    local burstMax
    if     pingMs <= 80  then burstMax = 2
    elseif pingMs <= 150 then burstMax = 3
    elseif pingMs <= 250 then burstMax = 4
    else                      burstMax = 5
    end

    -- Frame 0: fire immediately (no Heartbeat delay)
    pcall(function()
...
-- (Murderer Chance display removed)
...
-- ================================================================
-- ROUND TIMER (KC8WAJ6 style — frameless floating label)
-- No background box, no frame — just a clean text label anchored
-- to the top-centre of the screen, exactly like KC8WAJ6.
-- ================================================================
...
RoundTimerGui.DisplayOrder   = 5
RoundTimerGui.Enabled        = true
RoundTimerGui.Parent         = game.CoreGui

local RoundTimerLabel = Instance.new("TextLabel")
RoundTimerLabel.Name                  = "Round Timer"
RoundTimerLabel.Parent                = RoundTimerGui
RoundTimerLabel.Visible               = false
RoundTimerLabel.Active                = false
RoundTimerLabel.Size                  = UDim2.new(0, 0, 0, 0)       -- auto-size via text
RoundTimerLabel.Position              = UDim2.new(0.5, 0, 0.10, 0)  -- KC8WAJ6 exact
RoundTimerLabel.BackgroundTransparency = 1
RoundTimerLabel.Font                  = Enum.Font.SourceSansItalic   -- KC8WAJ6 exact
RoundTimerLabel.Text                  = "0s"
RoundTimerLabel.TextSize              = 25                           -- KC8WAJ6 exact
RoundTimerLabel.TextColor3            = Color3.fromRGB(255, 255, 255)
RoundTimerLabel.TextStrokeTransparency = 0.5
RoundTimerLabel.AnchorPoint           = Vector2.new(0.5, 0.5)       -- KC8WAJ6 exact

-- Toggle wired into Visuals tab
local TimerToggle = Tabs.Visuals:AddToggle("ShowTimer", {
    ...
    Callback = function(v)
        ...
        -- if true, the update loop below will show it on next tick
    end
})

-- KC8WAJ6-faithful update loop:
-- Runs every Heartbeat only when toggle is ON.
-- Mirrors KC8WAJ6's minutes/seconds decomposition exactly.
-- Shows Xm Ys format for > 1 min, Xs for < 1 min (red colour).
-- Hides label when server time < 1 (round over / lobby).
local _timerToggleOn = true   -- tracks toggle state without a global
local _timerConn = nil

local function startTimerLoop()
    if _timerConn then return end
    _timerConn = RunService.Heartbeat:Connect(function()
        if not _timerToggleOn then return end
        local ok, o = pcall(function()
            return timerRemote:InvokeServer()
        end)
        if not ok or not o then return end
        if o < 1 then
            RoundTimerLabel.Visible = false
            RoundTimerLabel.Active  = false
        else
            -- KC8WAJ6 exact: count down from o filling minutes then seconds
            local mins, secs, remaining = 0, 0, o
            ...
            RoundTimerLabel.Visible = _timerToggleOn
            RoundTimerLabel.Active  = _timerToggleOn
            if mins > 0 then
                RoundTimerLabel.Text       = mins .. "m " .. secs .. "s"
                RoundTimerLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            else
                RoundTimerLabel.Text       = secs .. "s"
                RoundTimerLabel.TextColor3 = Color3.fromRGB(255, 0, 0)  -- red under 1 min
            end
        end
    end)
end

-- Patch toggle callback to also update _timerToggleOn
Tabs.Visuals:GetToggles()  -- no-op; we reconnect logic below
local _origTimerToggle = TimerToggle
pcall(function()
    -- Override via Options table that Fluent exposes
    Options.ShowTimer:OnChanged(function(v)
        _timerToggleOn = v
        if not v then
            RoundTimerLabel.Visible = false
            RoundTimerLabel.Active  = false
...
RunService.Heartbeat:Connect(function()
    ...
    _hbFrame = _hbFrame + 1
    if _hbFrame % 3 ~= 0 then return end
    applyHitboxes()
end)
...
Tabs.Combat:AddParagraph({
    ...
    Content = "Silent Aim shows an on-screen AIM button. When you tap it, it fires at the murderer using the Fusion prediction engine (Seismic + Overflow blend). The button is draggable — move it anywhere on screen."
})

local SilentAimEnabled = false
local SilentAimToggle = Tabs.Combat:AddToggle("SilentAimToggle", {
    Title = "Silent Aim",
    Description = "Shows the draggable AIM button on screen. Tap it to silently fire at the murderer.",
    Default = false,
    Callback = function(toggle)
        ...
        Fluent:Notify({
            Title = "Silent Aim",
            Content = toggle and "Silent Aim ON — tap the AIM button to shoot." or "Silent Aim OFF.",
            Duration = 3
        })
    end
})

-- ── Section: Sharp Shooter ───────────────────────────────────────
Tabs.Combat:AddSection("Sharp Shooter")

Tabs.Combat:AddParagraph({
    Title = "How Sharp Shooter Works",
    Content = "Left-click anywhere to instantly fire at the murderer using the Fusion engine. Does NOT require Silent Aim to be enabled — fully independent."
})

local SharpShooterEnabled = false
Tabs.Combat:AddToggle("SharpShooterToggle", {
    Title = "Sharp Shooter (click to shoot murderer)",
    Description = "Left-click to fire at the murderer. Works independently of Silent Aim.",
    Default = false,
    Callback = function(toggle)
        SharpShooterEnabled = toggle
        Fluent:Notify({
            Title = "Sharp Shooter",
            Content = toggle and "Sharp Shooter ON — left-click to fire at murderer." or "Sharp Shooter OFF.",
            Duration = 3
        })
    end
})
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if murderer then
        ...
        Fluent:Notify({ Title = "Sharp Shooter", Content = "No murderer detected.", Duration = 2 })
    end
end)

-- ── Section: Prediction Settings ────────────────────────────────
Tabs.Combat:AddSection("Prediction Settings")

Tabs.Combat:AddParagraph({
    Title = "Ping Override",
    Content = "By default ping is measured automatically from Stats. If your executor blocks Stats, enable the manual override and set your ping with the slider below."
})

Tabs.Combat:AddToggle("PredictionPingToggle", {
    Title = "Override Prediction Ping (manual)",
    Description = "ON = use slider value below. OFF = auto-measure from game Stats.",
    Default = false,
    Callback = function(toggle)
        ...
        Fluent:Notify({
            ...
            Content = toggle
                and string.format("Manual override ON — locked to %dms", predictionState.pingValue)
                or  string.format("Auto ping active — measuring ~%dms", math.floor(_ping * 1000)),
            Duration = 3
        })
    end
})

Tabs.Combat:AddSlider("PingSlider", {
    Title = "Manual Ping (ms)",
    Description = "Set your ping when using manual override. Higher = more lead.",
    Default = 50,
    Min = 0,
    Max = 300,
    Rounding = 0,
...
Tabs.Combat:AddSection("Role Notifier")

Tabs.Combat:AddParagraph({
    Title = "Auto Notify",
    Content = "Automatically announces the murderer's perk and the sheriff's name each round via on-screen notification."
})

Tabs.Combat:AddToggle("AutoNotifyToggle", {
    Title = "Auto Notify Murderer Perk + Roles",
    Description = "Shows murderer perk and sheriff name as a notification at round start.",
    Default = true,
    Callback = function(toggle)
...
Tabs.Combat:AddToggle("HitboxExpanderToggle", {
    ...
    Description = "Enlarges every player's HumanoidRootPart client-side so shots that pass near them register. Restores on toggle off.",
    Default = false,
    Callback = function(v)
...
Tabs.Combat:AddSlider("HitboxSizeSlider", {
    Title = "Hitbox Size",
    Description = "Client-side HumanoidRootPart size. 8 = balanced, higher = easier to hit.",
    Default = 8,
    Min = 2,
    Max = 30,
    Rounding = 0,
...
-- (GunDrop tracking is handled above — no duplicate needed)
...
-- ================================================================
-- ================================================================
-- VALUES TAB — UI (UNDER MAINTENANCE)
-- ================================================================
Tabs.Values:AddSection("⚠️ Values Section — Under Maintenance")

Tabs.Values:AddParagraph({
    Title = "🚧 This Section Is Currently Under Maintenance",
    Content = "The entire Values section — including the Trade Value Overlay and Item Lookup — is currently being reworked and improved.\n\nPlease check back after the next update. We appreciate your patience!"
})

Tabs.Values:AddParagraph({
    Title = "📋 What's Coming",
    Content = "• Live-synced item value database\n• Improved trade overlay accuracy\n• Auto-detect all items in any trade\n• Full chroma / ancient / pet support"
})
...
-- ================================================================
-- DRAGGABLE GRAB GUN BUTTON (mobile-friendly on-screen widget)
-- ================================================================
...
-- Circle button — small, transparent, draggable
GrabGunFrame.Name                    = "GrabGunFrame"
GrabGunFrame.Size                    = UDim2.new(0, 62, 0, 62)
GrabGunFrame.Position                = UDim2.new(0.01, 0, 0.45, 0)
GrabGunFrame.BackgroundColor3        = Color3.fromRGB(10, 10, 10)
GrabGunFrame.BackgroundTransparency  = 0.45
...
GrabGunStroke.Color       = Color3.fromRGB(255, 255, 255)
GrabGunStroke.Thickness   = 1.5
GrabGunStroke.Transparency = 0.55
...
GrabGunFrame.MouseButton1Click:Connect(function()
    manualGrabGun()
    GrabGunFrame.BackgroundColor3 = Color3.fromRGB(0, 200, 80)
    task.wait(0.18)
    GrabGunFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
end)

-- Live dot indicator: green = gun drop exists, red = no drop
local gunDropIndicator = Instance.new("Frame")
gunDropIndicator.Size             = UDim2.new(0, 12, 0, 12)
gunDropIndicator.Position         = UDim2.new(1, -14, 0, 2)
...
-- Live dot indicator: green = gun drop exists, red = no drop (throttled)
local _gdIndicatorFrame = 0
RunService.Heartbeat:Connect(function()
    _gdIndicatorFrame = _gdIndicatorFrame + 1
    if _gdIndicatorFrame % 6 ~= 0 then return end
...
local autoDodgeMurdererEnabled = false

RunService.Heartbeat:Connect(function()
    if not autoDodgeMurdererEnabled then return end
    local murderer = GetMurderer()
...
Tabs.World:AddToggle("AutoDodgeMurdererToggle", {
    ...
    Default = false,
    Callback = function(v)
        autoDodgeMurdererEnabled = v
        Fluent:Notify({
            Title = "Auto Dodge",
            Content = v and "Auto Dodge ON — you will dodge when murderer gets close." or "Auto Dodge OFF.",
            Duration = 3
        })
    end
})
...
-- (godMode + hitboxExpander engines declared before Combat tab UI — see above)
...
local antiVoid = {
    enabled     = false,
    safePos     = nil,
    VOID_Y      = -100,  -- anything below this = void
    SAFE_Y      = -50    -- must be above this to record safe pos
}
...
RunService.Heartbeat:Connect(function()
    ...
    elseif pos.Y < antiVoid.VOID_Y then
        -- Void detected — snap back
        if antiVoid.safePos then
            myRoot.CFrame = antiVoid.safePos
        else
            -- Fallback: teleport to spawn area
