    ...
    for name, player in pairs(espActivePlayers) do
        ...
        if not root then
            ...
            if td then
                ...
                td.Text     = string.format("%s [%s] %d", player.Name, (espRoleColor(player)), math.floor(dist))
                td.Visible  = onScr
            end
...
task.spawn(function()
    while true do
        ...
        if AutoCoinOperator then continue end

        local character = LocalPlayer.Character
        if not character then continue end
        local root     = character:FindFirstChild("HumanoidRootPart"); if not root then continue end
...
-- (Hitbox loop merged into unified Heartbeat above)
...
-- (AntiFling loop merged into unified Heartbeat above)
...
-- (AntiVoid loop merged into unified Heartbeat above)
